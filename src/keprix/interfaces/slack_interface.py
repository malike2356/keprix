"""Slack interface adapter using slash command execution."""
...
async def handle_slack(*, agent_id: str, trace_id: str, **payload: Any) -> dict[str, Any]:
    ...
    ctx = build_context(
        raw_text=normalized,
        user_id=payload.get("user_id", "slack-user"),
        workspace_id=payload.get("workspace_id", "default"),
        channel="slack",
        channel_user_id=payload.get("channel_id", "slack-channel"),
        role=payload.get("role"),
        request_id=trace_id,
    )
    result = await execute_context(ctx)
    return {**render_slack(result), "trace_id": trace_id, "agent_id": agent_id, "interface": "slack"}
