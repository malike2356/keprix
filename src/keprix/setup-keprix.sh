...
# 5. Symlinks the 'keprix' CLI command into a user-facing bin dir
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/keprix-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink keprix into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up keprix command..."

KEPRIX_BIN="$SCRIPT_DIR/venv/bin/keprix"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$KEPRIX_BIN" "$COMMAND_LINK_DIR/keprix"
echo -e "${GREEN}✓${NC} Symlinked keprix → $COMMAND_LINK_DISPLAY_DIR/keprix"
...
# ============================================================================
# Seed bundled skills into ~/.keprix/skills/
# ============================================================================

KEPRIX_SKILLS_DIR="${KEPRIX_HOME:-$HOME/.keprix}/skills"
mkdir -p "$KEPRIX_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.keprix/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$KEPRIX_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  keprix status        # Check configuration"
if is_termux; then
    echo "  keprix gateway       # Run gateway in foreground"
else
    echo "  keprix gateway install # Install gateway service (messaging + cron)"
fi
echo "  keprix cron list     # View scheduled jobs"
echo "  keprix doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m keprix_cli.main setup
fi
