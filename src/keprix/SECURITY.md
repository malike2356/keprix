# Keprix Security Policy

This document describes Keprix's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/NousResearch/keprix/security/advisories/new)
or **security@nousresearch.com**. Do not open public issues for
security vulnerabilities. **Keprix does not operate a bug
bounty program.**
...
- Environment details (`keprix version`, commit SHA, OS, Python
  version).
...
Keprix supports two OS-level isolation postures. They address
...
- **Keprix's own Docker image and Compose setup.** Lighter-
...
Keprix filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Keprix
itself. Bugs in Keprix's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Keprix does not model per-caller capabilities inside a
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  reviewed, not a vulnerability in Keprix. Bugs in Keprix
...
- Keprix includes supply-chain guards for MCP server
