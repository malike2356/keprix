...
`$KEPRIX_HOME/plugins/model-providers/<name>/` (per-user overrides). The
...
- `keprix_cli/config.py` injects every `env_var` into
  `OPTIONAL_ENV_VARS` so the setup wizard knows about it.
- `keprix_cli/runtime_provider.py` reads `profile.api_mode` as a fallback
