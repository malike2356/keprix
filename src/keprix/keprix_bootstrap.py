"""Windows UTF-8 bootstrap for Keprix entry points.
...
should be imported at the very top of every Keprix entry point
(``keprix``, ``keprix``, ``keprix-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import keprix_bootstrap``
# (or ``from keprix_bootstrap import apply_windows_utf8_bootstrap``) at
