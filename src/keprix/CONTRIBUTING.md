...
Thank you for contributing to Keprix! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
Bundled skills (in `skills/`) ship with every Keprix install. They should be **broadly useful to most users**:
...
**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.keprix/plugins/` (or via a pip entry point).
...
The installer creates the Keprix venv, wires the `keprix` command, stamps the
install method for `keprix update`, and clones the full git project into
`$KEPRIX_HOME/keprix` (usually `~/.keprix/keprix`). That keeps your
...
Use this only if you intentionally do not want Keprix' managed install layout
(for example, a throwaway clone inside a container or CI job). If you install
this way, make sure you run the `keprix` entrypoint from this venv; running the
system `python3 -m keprix_cli.main` can pick up unrelated system Python
packages.
...
```bash
mkdir -p ~/.keprix/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.keprix/config.yaml
touch ~/.keprix/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.keprix/.env
```

### Run

```bash
# The standard installer already put `keprix` on PATH.
keprix doctor
keprix chat -q "Hello"
```

If you used the manual clone fallback, run `./keprix` from the checkout or
symlink this clone's venv explicitly:

```bash
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/keprix" ~/.local/bin/keprix
```
...
│
├── keprix_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.keprix/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Documentation site (keprix.nousresearch.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.keprix/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.keprix/`)

| Path | Purpose |
|------|---------|
| `~/.keprix/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.keprix/.env` | API keys and secrets |
| `~/.keprix/auth.json` | OAuth credentials (Nous Portal) |
| `~/.keprix/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.keprix/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.keprix/state.db` | SQLite session database |
| `~/.keprix/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.keprix/cron/` | Scheduled job data |
| `~/.keprix/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`keprix_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.keprix/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
...
(for example `_KEPRIX_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `keprix setup` or update `~/.keprix/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native Keprix tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
Add to `_BUILTIN_SKINS` dict in `keprix_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `keprix_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Keprix runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the keprix wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `keprix_cli/gateway.py::_scan_gateway_pids` for the pattern.
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `keprix_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `keprix_cli/gateway_windows.py::
...
Keprix has terminal access. Security matters.
...
2. **Test manually**: Run `keprix` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/NousResearch/keprix/issues)
- Include: OS, Python version, Keprix version (`keprix version`), full error traceback
