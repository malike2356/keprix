...
#
# Wired into the image as /etc/cont-init.d/01-keprix-setup by the
...
KEPRIX_HOME="${KEPRIX_HOME:-/opt/data}"
INSTALL_DIR="/opt/keprix"

# Drop to keprix via s6-setuidgid, but skip it when already non-root.
as_keprix() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid keprix "$@"; }
...
# the container starts non-root. The baked image trees (/opt/data, /opt/keprix/
# .venv, ui-tui, node_modules) stay owned by the keprix build UID (10000), so an
...
# into $KEPRIX_HOME). See #34837 for the supervision-tree side of this.
#
# The supported way to match host-side ownership is to start as root (the image
# default) and pass KEPRIX_UID/KEPRIX_GID — or the PUID/PGID aliases — which the
...
# `id -u keprix` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# keprix UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u keprix)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-keprix UID).
...
and the baked image directories are owned by the keprix user (UID $(id -u keprix)),
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e KEPRIX_UID=\$(id -u) -e KEPRIX_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the keprix user to that UID/GID at boot and chowns the data
...
# --- Bootstrap KEPRIX_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid keprix mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom KEPRIX_HOME paths whose parents only
# root can create (e.g. `KEPRIX_HOME=/home/keprix/.keprix` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for KEPRIX_UID/KEPRIX_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  KEPRIX_UID/
# KEPRIX_GID still win when both are set.  See #15290, salvages #25872.
KEPRIX_UID="${KEPRIX_UID:-${PUID:-}}"
KEPRIX_GID="${KEPRIX_GID:-${PGID:-}}"

if [ -n "${KEPRIX_UID:-}" ] && validate_uid_gid "$KEPRIX_UID" && [ "$KEPRIX_UID" != "$(id -u keprix)" ]; then
    echo "[stage2] Changing keprix UID to $KEPRIX_UID"
    usermod -u "$KEPRIX_UID" keprix
fi
if [ -n "${KEPRIX_GID:-}" ] && validate_uid_gid "$KEPRIX_GID" && [ "$KEPRIX_GID" != "$(id -g keprix)" ]; then
    echo "[stage2] Changing keprix GID to $KEPRIX_GID"
...
# host's `docker` group (or root). The supervised keprix user (UID 10000)
...
# keprix process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes keprix. Idempotent across container
...
#   - keprix is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G keprix 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] keprix already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" keprix 2>/dev/null; then
        echo "[stage2] Added keprix to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group keprix failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When KEPRIX_UID is remapped or the top-level $KEPRIX_HOME isn't owned by
# the runtime keprix UID, restore ownership to keprix — but ONLY for the
# directories keprix actually writes to. The full $KEPRIX_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of keprix-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_keprix_uid=$(id -u keprix)
needs_chown=false
if [ "$(stat -c %u "$KEPRIX_HOME" 2>/dev/null)" != "$actual_keprix_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $KEPRIX_HOME (targeted) to keprix ($actual_keprix_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $KEPRIX_HOME: chown the directory itself (not its contents)
    # so keprix can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    chown keprix:keprix "$KEPRIX_HOME" 2>/dev/null || \
        echo "[stage2] Warning: chown $KEPRIX_HOME failed (rootless container?) — continuing"
    # Keprix-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by keprix (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing; do
        if [ -e "$KEPRIX_HOME/$sub" ]; then
            chown -R keprix:keprix "$KEPRIX_HOME/$sub" 2>/dev/null || \
                echo "[stage2] Warning: chown $KEPRIX_HOME/$sub failed (rootless container?) — continuing"
        fi
    done
fi

# --- Fix ownership of build trees under $INSTALL_DIR ---
# Keprix-owned trees under $INSTALL_DIR must be re-chowned whenever the
# runtime keprix UID no longer owns them — otherwise:
...
#     the source mtime is newer than dist/ or when KEPRIX_TUI_FORCE_BUILD
#     is set) and writes to ui-tui/dist/. Without this chown the new
#     keprix UID can't write the build output (#28851).
...
# The set mirrors the build-time `chown -R keprix:keprix` line in the
# Dockerfile — keep them in sync if the Dockerfile chown set changes.
# These are under $INSTALL_DIR (not $KEPRIX_HOME), so the bind-mount
# concern doesn't apply — recursive is fine.
#
# This MUST be gated independently of the $KEPRIX_HOME ownership check
# above. `usermod -u <new> keprix` re-chowns the keprix home dir
# ($KEPRIX_HOME == /opt/data) to the new UID as a side effect, so after a
# KEPRIX_UID/PUID remap `stat $KEPRIX_HOME` always already matches the new
# UID and `needs_chown` is false — but the build trees under /opt/keprix
# are NOT touched by usermod and remain owned by the build-time UID
# (10000). Gating them on $KEPRIX_HOME ownership (as #35027 did) silently
...
# when the venv is not already owned by the runtime keprix UID. Idempotent
...
if [ -n "$venv_owner" ] && [ "$venv_owner" != "$actual_keprix_uid" ]; then
    echo "[stage2] Fixing ownership of build trees under $INSTALL_DIR to keprix ($actual_keprix_uid)"
    chown -R keprix:keprix \
        "$INSTALL_DIR/.venv" \
        "$INSTALL_DIR/ui-tui" \
        "$INSTALL_DIR/gateway" \
...
# Always reset ownership of $KEPRIX_HOME/profiles to keprix on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> keprix …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as keprix and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$KEPRIX_HOME/profiles" ]; then
    chown -R keprix:keprix "$KEPRIX_HOME/profiles" 2>/dev/null || true
fi

# Always reset ownership of $KEPRIX_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged keprix runtime even
# after root-context maintenance commands or scheduler writes.
if [ -d "$KEPRIX_HOME/cron" ]; then
    chown -R keprix:keprix "$KEPRIX_HOME/cron" 2>/dev/null || true
fi

# Reset ownership of keprix-owned top-level state files on every boot.
# The targeted data-volume chown above only covers keprix-owned
# *subdirectories*; loose state files living directly under $KEPRIX_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> keprix …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged keprix runtime then hits
...
# sweep so host-owned files in a bind-mounted $KEPRIX_HOME are never
...
# entries of keprix_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$KEPRIX_HOME/$f" ]; then
        chown keprix:keprix "$KEPRIX_HOME/$f" 2>/dev/null || true
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the keprix runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$KEPRIX_HOME/config.yaml" ]; then
    chown keprix:keprix "$KEPRIX_HOME/config.yaml" 2>/dev/null || true
    chmod 640 "$KEPRIX_HOME/config.yaml" 2>/dev/null || true
fi

# --- Seed directory structure as keprix user ---
# Run as keprix via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $KEPRIX_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_keprix mkdir -p \
    "$KEPRIX_HOME/cron" \
    "$KEPRIX_HOME/sessions" \
    "$KEPRIX_HOME/logs" \
    "$KEPRIX_HOME/logs/gateways" \
    "$KEPRIX_HOME/hooks" \
    "$KEPRIX_HOME/memories" \
    "$KEPRIX_HOME/skills" \
    "$KEPRIX_HOME/skins" \
    "$KEPRIX_HOME/plans" \
    "$KEPRIX_HOME/workspace" \
    "$KEPRIX_HOME/home" \
    "$KEPRIX_HOME/pairing" \
    "$KEPRIX_HOME/platforms/pairing"

# --- Install-method stamp (read by detect_install_method() in keprix status) ---
# Preserved from the tini-era entrypoint (PR #27843). Must be written as
# the keprix user so ownership matches the file's documented owner.
# tee is invoked directly via s6-setuidgid (no `sh -c` wrapper) for the
# same shell-metacharacter safety described above.
printf 'docker\n' | as_keprix tee "$KEPRIX_HOME/.install_method" >/dev/null \
    || true
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$KEPRIX_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        as_keprix cp "$INSTALL_DIR/$src" "$KEPRIX_HOME/$dest"
    fi
}
...
if [ -f "$KEPRIX_HOME/.env" ]; then
    chown keprix:keprix "$KEPRIX_HOME/.env" 2>/dev/null || true
    chmod 600 "$KEPRIX_HOME/.env" 2>/dev/null || true
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $KEPRIX_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `keprix update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set KEPRIX_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$KEPRIX_HOME/config.yaml" ]; then
    s6-setuidgid keprix "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$KEPRIX_HOME/auth.json" ] && [ -n "${KEPRIX_AUTH_JSON_BOOTSTRAP:-}" ]; then
    printf '%s' "$KEPRIX_AUTH_JSON_BOOTSTRAP" > "$KEPRIX_HOME/auth.json"
    chown keprix:keprix "$KEPRIX_HOME/auth.json" 2>/dev/null || true
    chmod 600 "$KEPRIX_HOME/auth.json"
fi
...
# can set KEPRIX_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# KEPRIX_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$KEPRIX_HOME/gateway_state.json" ] && \
        [ "${KEPRIX_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    printf '{"gateway_state":"running"}\n' > "$KEPRIX_HOME/gateway_state.json"
    chown keprix:keprix "$KEPRIX_HOME/gateway_state.json" 2>/dev/null || true
    chmod 644 "$KEPRIX_HOME/gateway_state.json"
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_keprix "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/keprix/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Keprix spawns for the
...
# main-wrapper.sh propagates it into the supervised ``keprix`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-keprix, dashboard, etc.).
