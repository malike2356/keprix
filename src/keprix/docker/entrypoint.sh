...
# by /etc/cont-init.d/01-keprix-setup (installed by the Dockerfile). This
...
# on the pre-s6 contract "entrypoint.sh sets up state then execs keprix"
...
echo "[keprix] WARNING: docker/entrypoint.sh is a deprecated shim under " \
...
exec /opt/keprix/docker/stage2-hook.sh "$@"
