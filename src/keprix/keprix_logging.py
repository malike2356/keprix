"""Centralized logging setup for Keprix.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.keprix/logs/`` (profile-aware via ``get_keprix_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``keprix_cli.web_server``,
    ``keprix_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_keprix_session_injector", False):
    ...
    _session_record_factory._keprix_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``keprix logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway", "keprix_plugins"),
...
def setup_logging(
    *,
    ...
    """Configure the Keprix logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    keprix_home
        Override for the Keprix home directory.  Falls back to
        ``get_keprix_home()`` (profile-aware).
    log_level
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_keprix_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._keprix_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the keprix group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from keprix_cli.config import is_managed
