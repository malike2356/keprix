...
function waitForDashboardPort(child, timeoutMs = 45_000) {
  return new Promise((resolve, reject) => {
    ...
    function onExit(code, signal) {
      cleanup()
      reject(new Error(`Keprix backend: exited before port announcement (${signal || code})`))
    }
    ...
    const timer = setTimeout(() => {
      cleanup()
      reject(new Error(`Timed out waiting for Keprix backend port announcement (${timeoutMs}ms)`))
    }, timeoutMs)
