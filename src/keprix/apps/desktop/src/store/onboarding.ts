...
function notifyReady(provider: string) {
  notify({ kind: 'success', title: 'Keprix is ready', message: `${provider} connected.` })
}
...
function notifyGatewayTools(tools: string[] | undefined) {
  ...
  notify({
    durationMs: 8000,
    kind: 'info',
    message: `${list} now run through your connected provider subscription; no separate API keys needed.`,
...
function providerResolutionFailure(reason: null | string) {
  ...
  return detail
    ? `Connected, but Keprix still cannot resolve a usable provider. ${detail}`
    : 'Connected, but Keprix still cannot resolve a usable provider.'
}
...
export async function recheckExternalSignin(ctx: OnboardingContext) {
  ...
  await completeWithModelConfirm(ctx, provider.name, [provider.id], reason =>
    setFlow({
      ...
      message:
        reason?.trim() ||
        `Keprix still cannot reach ${provider.name}. Run \`${provider.cli_command}\` in a terminal first.`
    })
  )
}
...
export async function saveOnboardingLocalEndpoint(baseUrl: string, apiKey: string, ctx: OnboardingContext) {
  ...
  try {
    ...
    if (!runtime.ready) {
      ...
      return { ok: false, message: detail || `Saved, but Keprix still cannot reach ${url}.` }
    }
