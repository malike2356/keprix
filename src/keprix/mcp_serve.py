"""
Keprix MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Keprix-specific extra)

Usage:
    keprix mcp serve
    keprix mcp serve --verbose
...
logger = logging.getLogger("keprix.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using KEPRIX_HOME."""
    try:
        from keprix_constants import get_keprix_home
        return get_keprix_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("KEPRIX_HOME", Path.home() / ".keprix")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from keprix_constants import get_keprix_home
        directory_file = get_keprix_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("KEPRIX_HOME", Path.home() / ".keprix")
...
class EventBridge:
    ...
    This is the Keprix equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from keprix_constants import get_keprix_home
            db_file = get_keprix_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("KEPRIX_HOME", Path.home() / ".keprix")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Keprix MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "keprix",
        instructions=(
            "Keprix messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Keprix MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
