"""ACP auth helpers — detect and advertise Keprix authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "keprix-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Keprix runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Keprix can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Keprix.
    ...
    not have Keprix provider credentials configured yet, so Keprix always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Keprix using the currently configured "
