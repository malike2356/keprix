"""ACP agent server — exposes Keprix via the Agent Client Protocol."""
...
try:
    from keprix_cli import __version__ as KEPRIX_VERSION
except Exception:
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Keprix running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Keprix cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Keprix/OpenAI-compatible user content payload."""
...
class KeprixACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Keprix AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Keprix version",
        },
    )
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so Keprix maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        try:
            from keprix_cli.models import curated_models_for_provider, normalize_provider, provider_label
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from keprix_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Keprix estimates ``used`` from the same
    ...
    def _provenance_meta(
        self,
        acp_session_id: str,
        current_keprix_session_id: str,
        previous_keprix_session_id: Optional[str] = None,
    ) -> Optional[dict]:
        """Best-effort ``_meta.keprix.sessionProvenance`` for an ACP session."""
        try:
            return session_provenance_meta(
                self.session_manager._get_db(),
                acp_session_id,
                current_keprix_session_id,
                previous_keprix_session_id=previous_keprix_session_id,
            )
        except Exception:
            logger.debug(
    ...
    async def _send_session_info_update(
        ...
        session_id: str,
        *,
        current_keprix_session_id: Optional[str] = None,
        previous_keprix_session_id: Optional[str] = None,
    ) -> None:
        """Send ACP native session metadata after Keprix changes it.

        When the internal Keprix head rotated (e.g. compression-driven session
        split during a turn), pass ``previous_keprix_session_id`` so the
        attached ``_meta.keprix.sessionProvenance`` flags the rotation reason.
        """
        if not self._conn:
            return
        try:
        ...
        # keprix_state.py schema — only started_at/ended_at). Use "now" as
        ...
        meta = self._provenance_meta(
            session_id,
            current_keprix_session_id or session_id,
            previous_keprix_session_id,
        )
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["keprix-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="keprix", version=KEPRIX_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Keprix' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Keprix setup/model selection out-of-band.
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes Keprix
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Keprix on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive Keprix' local kawaii waiting/status
        ...
        # not the executor's. Also set KEPRIX_INTERACTIVE so approval.py
        # takes the CLI-interactive path (which calls the registered
        # callback via prompt_dangerous_approval) instead of the
        # non-interactive auto-approve branch (GHSA-96vc-wcxf-jjff).
        # ACP's conn.request_permission maps cleanly to the interactive
        # callback shape — not the gateway-queue KEPRIX_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind KEPRIX_SESSION_KEY for this session so per-session caches
            ...
            previous_interactive = os.environ.get("KEPRIX_INTERACTIVE")
            ...
            previous_session_id = os.environ.get("KEPRIX_SESSION_ID")
            os.environ["KEPRIX_SESSION_ID"] = session_id
            try:
                ...
                # Restore KEPRIX_INTERACTIVE.
                if previous_interactive is None:
                    os.environ.pop("KEPRIX_INTERACTIVE", None)
                else:
                    os.environ["KEPRIX_INTERACTIVE"] = previous_interactive
                # Restore KEPRIX_SESSION_ID symmetrically.
                if previous_session_id is None:
                    os.environ.pop("KEPRIX_SESSION_ID", None)
                else:
                    os.environ["KEPRIX_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            # Snapshot the internal Keprix DB session id before the turn so we
            ...
            pre_turn_keprix_id = getattr(state.agent, "session_id", None)
            # Wrap the executor call in a fresh copy of the current context so
            # concurrent ACP sessions on the shared ThreadPoolExecutor don't
            # stomp on each other's ContextVar writes (KEPRIX_SESSION_KEY in
        ...
        # _meta.keprix.sessionProvenance so ACP clients can render the boundary
        # and keep old/new ids in lineage. The ACP session_id is unchanged.
        post_turn_keprix_id = getattr(state.agent, "session_id", None)
        if (
            conn
            and post_turn_keprix_id
            and pre_turn_keprix_id
            and post_turn_keprix_id != pre_turn_keprix_id
        ):
            try:
                await self._send_session_info_update(
                    session_id,
                    current_keprix_session_id=post_turn_keprix_id,
                    previous_keprix_session_id=pre_turn_keprix_id,
                )
            except Exception:
                logger.debug(
        ...
        # Keprix' local "waiting for model response" interrupt status is metadata,
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Keprix v{KEPRIX_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Keprix has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
