"""CLI entry point for the keprix ACP adapter.

Loads environment variables from ``~/.keprix/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from KEPRIX_HOME (default ``~/.keprix``)."""
    from keprix_cli.env_loader import load_keprix_dotenv

    keprix_home = get_keprix_home()
    loaded = load_keprix_dotenv(keprix_home=keprix_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", keprix_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="keprix-acp",
        description="Run Keprix as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Keprix version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Keprix provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.keprix/node/ "
...
def _print_version() -> None:
    ...
    print(keprix_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import KeprixACPAgent  # noqa: F401

    print("Keprix ACP check OK")


def _run_setup() -> None:
    from keprix_cli.main import main as keprix_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "keprix", "model"]
        keprix_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``keprix postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting keprix ACP adapter")
    ...
    import acp
    from .server import KeprixACPAgent
