#!/usr/bin/env python3
"""
Keprix CLI - Interactive Terminal Interface

A beautiful command-line interface for the Keprix, inspired by Claude Code.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills keprix-dev,github-auth
    python cli.py --list-tools             # List available tools and exit
"""

# IMPORTANT: keprix_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See keprix_bootstrap.py for full rationale.
try:
    import keprix_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Graceful fallback when keprix_bootstrap isn't registered in the venv
    # yet — happens during partial ``keprix update`` where git-reset landed
...
os.environ["KEPRIX_QUIET"] = "1"  # Our own modules

import yaml

from keprix_cli.fallback_config import get_fallback_chain
from keprix_cli.cli_agent_setup_mixin import CLIAgentSetupMixin
from keprix_cli.cli_commands_mixin import CLICommandsMixin
...
try:
    from keprix_cli.pt_input_extras import (
...
from keprix_cli.banner import _format_context_length, format_banner_version_label
...
from keprix_cli.browser_connect import (
...
_keprix_home = get_keprix_home()
_project_env = Path(__file__).parent / '.env'
load_keprix_dotenv(keprix_home=_keprix_home, project_env=_project_env)
...
def _load_prefill_messages(file_path: str) -> List[Dict[str, Any]]:
    ...
    Relative paths are resolved from ~/.keprix/.
...
def _resolve_prefill_messages_file(config: Dict[str, Any]) -> str:
    ...
    """
    env_path = os.getenv("KEPRIX_PREFILL_MESSAGES_FILE", "").strip()
    if env_path:
        return env_path
...
def _parse_reasoning_config(effort: str) -> dict | None:
    ...
    from keprix_constants import parse_reasoning_effort
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.keprix/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If KEPRIX_IGNORE_USER_CONFIG=1 is set (via ``keprix chat --ignore-user-config``),
    the user config at ``~/.keprix/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({KEPRIX_HOME}/config.yaml)
    user_config_path = _keprix_home / 'config.yaml'
    project_config_path = Path(__file__).parent / 'cli-config.yaml'

    # --ignore-user-config: force-skip the user config.yaml (still honor project
    # config as a fallback so defaults stay sensible).
    ignore_user_config = os.environ.get("KEPRIX_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain Keprix, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me Keprix - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
        ...
        "display": {
            "compact": False,
            "resume_display": "full",
            # Recap tuning for /resume — see keprix_cli/config.py DEFAULT_CONFIG.
            "resume_exchanges": 10,
    ...
    if config_path.exists():
        try:
            with open(config_path, "r", encoding="utf-8") as f:
                from keprix_cli.config import _normalize_root_model_keys
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and KeprixCLI.__init__ checks "default" first.
    ...
    from keprix_cli.config import _expand_env_vars
    defaults = _expand_env_vars(defaults)

    # Apply terminal config to environment variables (so terminal_tool picks them up)
    terminal_config = defaults.get("terminal", {})
    
    # Normalize config key: the new config system (keprix_cli/config.py) and all
    ...
    # Local backend: always os.getcwd(). Use `cd /dir && keprix` to control it.
    ...
    # UNLESS we're inside a gateway process (detected by _KEPRIX_GATEWAY marker)
    # where it was already set correctly by gateway/run.py's config bridge.
    _is_gateway = os.environ.get("_KEPRIX_GATEWAY") == "1"
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            os.environ["KEPRIX_REDACT_SECRETS"] = str(redact).lower()
...
# Initialize centralized logging early — agent.log + errors.log in ~/.keprix/logs/.
# This ensures CLI sessions produce a log trail even before AIAgent is instantiated.
try:
    from keprix_logging import setup_logging
...
try:
    from keprix_cli.config import print_config_warnings
...
try:
    from keprix_cli.skin_engine import init_skin_from_config
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``keprix --help`` paths inside the chat command flow).  See
...
def get_tool_definitions(*args, **kwargs):
    from keprix_cli.mcp_startup import wait_for_mcp_discovery
...
from keprix_cli.banner import build_welcome_banner
from keprix_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from keprix_cli.callbacks import prompt_for_secret
...
def _prepare_deferred_agent_startup() -> None:
    ...
    if _deferred_agent_startup_done:
        return
    if os.environ.get("KEPRIX_DEFER_AGENT_STARTUP") != "1":
        return
    _deferred_agent_startup_done = True
    _accept_hooks = os.environ.get("KEPRIX_ACCEPT_HOOKS", "").lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def _notify_session_finalize(
    *,
    ...
    try:
        from keprix_cli.plugins import invoke_hook as _invoke_hook
        _invoke_hook(
            "on_session_finalize",
...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run keprix -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"keprix-{short_id}"
    branch_name = f"keprix/{wt_name}"
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`keprix_cli.config.load_config` (the authoritative loader that
    ...
    if session_db is None:
        return
    try:
        from keprix_cli.config import load_config as _load_full_config
        from keprix_constants import get_keprix_home as _get_keprix_home
        _keprix_home_maint = _get_keprix_home()
        ...
        try:
            if not session_db.get_meta("ghost_session_prune_v1"):
                pruned = session_db.prune_empty_ghost_sessions(
                    sessions_dir=_keprix_home_maint / "sessions"
                )
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`keprix_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``keprix/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("keprix-"):
            continue
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``keprix/keprix-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``keprix -w`` sessions and PR review
    ...
    orphaned = [
        ...
        and (b.startswith("keprix/keprix-") or b.startswith("pr-"))
    ]
...
# - Bright cyan: #00D9FF / #00E5FF (headers, highlights)
# - Sky blue: #33AAFF (secondary highlights)
# - Deep blue: #0066FF (tertiary elements)
# - Light: #E0F7FF (text)
# - Dim: #1B6FA8 (muted text)
...
#
# Detection priority:
#   1. KEPRIX_LIGHT / KEPRIX_TUI_LIGHT env (true/false) — explicit override
#   2. KEPRIX_TUI_THEME=light|dark — explicit theme
#   3. KEPRIX_TUI_BACKGROUND=#RRGGBB — explicit bg hint
#   4. COLORFGBG env (set by xterm/Konsole/urxvt) — bg slot 7/15 = light
#   5. OSC 11 query (\x1b]11;?\x1b\\) — ask the terminal directly
#   6. Default: assume dark (matches the legacy Keprix assumption)
#
...
def _detect_light_mode() -> bool:
    ...
    try:
        ...
        for var in ("KEPRIX_LIGHT", "KEPRIX_TUI_LIGHT"):
            ...
            if _FALSE_RE.match(v):
                ...
                return result
        # 2. Theme hint
        theme = (os.environ.get("KEPRIX_TUI_THEME") or "").strip().lower()
        if theme == "light":
            result = True
            _LIGHT_MODE_CACHE = result
            return result
        if theme == "dark":
            ...
            return result
        # 3. Explicit bg hex
        bg_hint = os.environ.get("KEPRIX_TUI_BACKGROUND") or ""
...
_LIGHT_MODE_REMAP: dict[str, str] = {
    ...
    "#E0F7FF": "#0D2B36",   # ice-cyan text -> near-black teal
    "#00D9FF": "#00738C",   # bright cyan (title/border) -> darker cyan
    "#00E5FF": "#00738C",   # bright cyan (logo top band)
    "#33AAFF": "#0A5FA8",   # sky blue (accent) -> darker blue
    "#1B6FA8": "#0D4D7A",   # muted blue (dim) -> deeper blue
    "#4FC3F7": "#0277BD",   # light blue (labels) -> darker blue
    "#0066FF": "#0047AB",   # deep blue (border/rule) -> darker cobalt
    "#5C7A94": "#33475B",   # session border (cool grey-blue) -> darker
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        from keprix_cli.skin_engine import SkinConfig  # type: ignore[import]
    except Exception:
        return
    if getattr(SkinConfig, "_keprix_light_mode_hook_installed", False):
        return
    ...
    SkinConfig._keprix_light_mode_hook_installed = True  # type: ignore[attr-defined]
...
def _strip_markdown_syntax(text: str) -> str:
    ...
    # but in Keprix output it's common to display cron schedules verbatim.
    #
...
from keprix_constants import is_termux as _is_termux_environment
...
def _apply_bracketed_paste_timeout_patch() -> None:
    ...
    ``_keprix_bp_timeout_patched`` sentinel on the module.
    """
    try:
        ...
        if getattr(_vt100_mod, "_keprix_bp_timeout_patched", False):
            return
        ...
        def _patched_vt100_feed(self_parser, data: str) -> None:
            if self_parser._in_bracketed_paste:
                ...
                if end_mark in self_parser._paste_buffer:
                    ...
                    self_parser._keprix_bp_start = None
                    if remaining:
                        _patched_vt100_feed(self_parser, remaining)
                else:
                    bp_start = getattr(self_parser, "_keprix_bp_start", None)
                    now = time.monotonic()
                    if bp_start is None:
                        self_parser._keprix_bp_start = now
        ...
        _vt100_mod._keprix_bp_timeout_patched = True
...
class ChatConsole:
    ...
    def status(self, *_args, **_kwargs):
        ...
        ``KeprixCLI._busy_command()``.
        """
        yield self

# ASCII Art - KEPRIX-AGENT logo (full width, single line - requires ~95 char terminal)
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        ...
        agent_name = _skin.get_branding("agent_name", "Keprix") if _skin else "Keprix"
        line1 = f"{agent_name} - AI Agent Framework"
        tiny_line = agent_name

    if os.environ.get("KEPRIX_FAST_STARTUP_BANNER") == "1":
        from keprix_cli import __release_date__ as _release_date
        from keprix_cli import __version__ as _version

        version_line = f"Keprix v{_version} ({_release_date})"
    else:
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from keprix_cli.plugins import get_plugin_commands
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.keprix/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.keprix/config.yaml on first use)
...
class KeprixCLI(CLIAgentSetupMixin, CLICommandsMixin):
    """
    Interactive CLI for the Keprix.
    ...
    def __init__(
        ...
        # Read max_tokens from config (env var override: KEPRIX_MAX_TOKENS)
        _env_mt = os.environ.get("KEPRIX_MAX_TOKENS")
        if _env_mt:
            try:
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from keprix_cli.runtime_provider import _auto_detect_local_model
        ...
        self.requested_provider = (
            ...
            or os.getenv("KEPRIX_INFERENCE_PROVIDER")
            or "auto"
        )
        ...
        elif os.getenv("KEPRIX_MAX_ITERATIONS"):
            try:
                self.max_turns = int(os.getenv("KEPRIX_MAX_ITERATIONS", ""))
            except (TypeError, ValueError):
                self.max_turns = 90
        else:
            self.max_turns = 90
        ...
        # by `keprix chat --ignore-rules` in keprix_cli/main.py. When true we
        # pass skip_context_files=True and skip_memory=True to AIAgent so
        # AGENTS.md/SOUL.md/.cursorrules and persistent memory are not loaded.
        self.ignore_rules = ignore_rules or os.environ.get("KEPRIX_IGNORE_RULES") == "1"
        
        # Ephemeral system prompt: env var takes precedence, then config
        self.system_prompt = (
            os.getenv("KEPRIX_EPHEMERAL_SYSTEM_PROMPT", "")
        ...
        # it's shared across all Keprix processes for this KEPRIX_HOME.
        # Never blocks startup on failure.
        _run_state_db_auto_maintenance(self._session_db)

        # Opportunistic shadow-repo cleanup — deletes orphan/stale
        # checkpoint repos under ~/.keprix/checkpoints/.  Opt-in via
        ...
        self._history_file = _keprix_home / ".keprix_history"
    ...
    def _claim_active_session(self, surface: str = "cli", *, stderr: bool = False) -> bool:
        ...
        if self._active_session_lease is not None:
            return True
        try:
            from keprix_cli.active_sessions import try_acquire_active_session
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from keprix_cli.voice import format_voice_record_key_for_status
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            return f"⬡ {self.model if getattr(self, 'model', None) else 'Keprix'}"
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        try:
            from keprix_cli.model_normalize import (
            ...
            if resolved_provider not in _AGGREGATOR_PROVIDERS:
                ...
                if normalized_model and normalized_model != current_model:
                    if not self._model_is_default:
                        self._console_print(
                            f"[#33AAFF]⚠️  Normalized model '{current_model}' to '{normalized_model}' for {resolved_provider}.[/]"
                        )
        ...
        if resolved_provider == "copilot":
            try:
                from keprix_cli.models import copilot_model_api_mode, normalize_copilot_model_id

                canonical = normalize_copilot_model_id(current_model, api_key=self.api_key)
                if canonical and canonical != current_model:
                    if not self._model_is_default:
                        self._console_print(
                            f"[#33AAFF]⚠️  Normalized Copilot model '{current_model}' to '{canonical}'.[/]"
                        )
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from keprix_cli.models import normalize_opencode_model_id, opencode_model_api_mode

                canonical = normalize_opencode_model_id(resolved_provider, current_model)
                if canonical and canonical != current_model:
                    if not self._model_is_default:
                        self._console_print(
                            f"[#33AAFF]⚠️  Stripped provider prefix from '{current_model}'; using '{canonical}' for {resolved_provider}.[/]"
                        )
                    self.model = canonical
                    current_model = canonical
                    changed = True
        ...
        if "/" in current_model:
            ...
            if not self._model_is_default:
                self._console_print(
                    f"[#33AAFF]⚠️  Stripped provider prefix from '{current_model}'; "
        ...
        if self._model_is_default:
            ...
            try:
                from keprix_cli.codex_models import get_codex_model_ids
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            w = self._scrollback_box_width()
            fill = w - 2 - KeprixCLI._status_bar_display_width(label)
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``keprix doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``keprix doctor`` so the banner stays
        small.
        """
        try:
            from keprix_cli.security_advisories import (
                detect_compromised,
                startup_banner,
            )
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            self._console_print()
            self._console_print(
                f"[#33AAFF]⚠️  Context length is only {ctx_len:,} tokens — "
                f"this is likely too low for agent use with tools.[/]"
            )
            self._console_print(
                f"[dim]   Keprix needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
        ...
        # Warn if the configured model is a Nous Keprix LLM (not agentic)
        from keprix_cli.model_switch import is_nous_hermes_non_agentic

        model_name = getattr(self, "model", "") or ""
        if is_nous_hermes_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold #33AAFF]⚠  Nous Research Keprix 3 & 4 models are NOT agentic and are not "
                "designed for use with Keprix.[/]"
            )
            self._console_print(
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.keprix/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from keprix_cli.clipboard import save_clipboard_image

        img_dir = get_keprix_home() / "images"
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                self._console_print()
                self._console_print("[#33AAFF]⚠️  Some tools disabled (missing API keys):[/]")
                for item in api_key_missing:
                    tools_str = ", ".join(item["tools"][:2])  # Show first 2 tools
                    if len(item["tools"]) > 2:
                        tools_str += f", +{len(item['tools'])-2} more"
                    self._console_print(f"   [dim]• {item['name']}[/] [dim italic]({', '.join(item['missing_vars'])})[/]")
                self._console_print("[dim]   Run 'keprix setup' to configure[/]")
        except Exception:
    ...
    def _fast_command_available(self) -> bool:
        try:
            from keprix_cli.models import model_supports_fast_mode
        except Exception:
            return False
        agent = getattr(self, "agent", None)
    ...
    def show_help(self):
        ...
        from keprix_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from keprix_cli.skin_engine import get_active_help_header
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with Keprix!{_RST}")
    ...
    def _list_recent_sessions(self, limit: int = 10) -> list[dict[str, Any]]:
        ...
        if not self._session_db:
            return []
        try:
            from keprix_cli.session_listing import query_session_listing
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from keprix_cli.main import _relative_time
    ...
    def _discard_session_if_empty(self, session_id: Optional[str]) -> bool:
        ...
        ``/resume`` and ``keprix sessions list``. Delegates the
    ...
    def new_session(self, silent=False, title=None):
        ...
        if self._session_db and old_session_id:
            ...
            # /resume and `keprix sessions list` (gemini-cli#27770 port).
            ...
            if self._session_db:
                try:
                    ...
                    self._session_db.create_session(
                        session_id=self.session_id,
                        source=os.environ.get("KEPRIX_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.keprix/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``keprix --resume <id>``
        ...
        saved_dir = get_keprix_home() / "sessions" / "saved"
        try:
            saved_dir.mkdir(parents=True, exist_ok=True)
        except Exception as e:
            print(f"(x_x) Failed to create save directory {saved_dir}: {e}")
            return
        path = saved_dir / f"keprix_conversation_{timestamp}.json"
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: keprix --resume {self.session_id}")
        except Exception as e:
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from keprix_cli.curses_ui import curses_single_select
    ...
    def _confirm_expensive_model_switch(self, result) -> bool:
        ...
        choices = [
            ("once", "Switch anyway", "Use this model for the current Keprix session."),
    ...
    def _apply_model_switch_result(self, result, persist_global: bool) -> None:
        ...
        try:
            from keprix_cli.model_switch import resolve_display_context_length
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `keprix model` and gateway pickers).
            # Only fall back to the live provider catalog when the curated
            # list is empty (e.g. user-defined endpoints with no curated list).
            model_list = provider_data.get("models", [])
            if not model_list:
                try:
                    from keprix_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from keprix_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from keprix_cli.model_switch import switch_model, parse_model_flags
        from keprix_cli.providers import get_label
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_keprix_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_keprix_home()}/config.yaml")
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in keprix_cli/commands.py instead of touching every dispatch site.
        from keprix_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            if len(parts) > 1:
                ...
                if raw_title:
                    if self._session_db:
                        ...
                        from keprix_state import format_session_db_unavailable
            ...
            from keprix_cli.main import _print_version_info
            ...
            from keprix_cli.config import reload_env
            count = reload_env()
            ...
            try:
                # Discover from disk (bundled + user), matching `keprix plugins
                ...
                # catalog lives behind `keprix plugins list`.
                user_entries = [e for e in entries if e[3] != "bundled"]
                bundled_count = len(entries) - len(user_entries)

                if not user_entries:
                    print("No user plugins installed.")
                    print("  Install one: keprix plugins install owner/repo")
                    print(f"  Or drop a plugin directory into {display_keprix_home()}/plugins/")
                    if bundled_count:
                        print(f"  ({bundled_count} bundled plugins available — see: keprix plugins list)")
                else:
                    ...
                    if bundled_count:
                        print(f"  (+{bundled_count} bundled — see: keprix plugins list)")
                    print("  Enable/disable: keprix plugins enable/disable <name>")
            except Exception as e:
            ...
            # loads multiple skills at once. Rescans cheaply when files change.
                ...
                if bundle_result:
                    ...
                    if missing:
                        ChatConsole().print(
                            f"[#33AAFF]Skipped missing skills: {', '.join(missing)}[/]"
                        )
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from keprix_cli.commands import COMMANDS
    ...
    def _get_goal_manager(self):
        ...
        """
        try:
            from keprix_cli.goals import GoalManager
            from keprix_cli.config import load_config
        except Exception as exc:
    ...
    def _toggle_verbose(self):
        ...
        from keprix_cli.colors import Colors as _Colors
        labels = {
    ...
    def _is_session_yolo_active(self) -> bool:
        ...
        ``KEPRIX_YOLO_MODE`` into ``_YOLO_MODE_FROZEN`` before tool imports
        happen.
        """
        try:
            from tools.approval import (
    ...
    def _toggle_yolo(self):
        ...
        ``KEPRIX_YOLO_MODE`` here — that env var is read once at module import
    ...
    def _manual_compress(self, cmd_original: str = ""):
        ...
        * ``/compress here [N]`` — boundary-aware compression. Summarize
        ...
        from keprix_cli.partial_compress import (
    ...
    def _show_usage(self):
        ...
        if self.verbose:
            ...
            # Console quietness is enforced by keprix_logging not
    ...
    def _print_nous_credits_block(self) -> bool:
        ...
        wall-clock-bounded inside the helper; also honors KEPRIX_DEV_CREDITS_FIXTURE
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from keprix_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.keprix/skills/ and queue a note for the
        next user turn.
                ...
                try:
                    if (
                        ...
                        if not is_seen(CLI_CONFIG, TOOL_PROGRESS_FLAG):
                            ...
                            mark_seen(_keprix_home / "config.yaml", TOOL_PROGRESS_FLAG)
    ...
    def _voice_speak_response(self, text: str):
        ...
        try:
            ...
            os.makedirs(os.path.join(tempfile.gettempdir(), "keprix_voice"), exist_ok=True)
            mp3_path = os.path.join(
                tempfile.gettempdir(), "keprix_voice",
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _keprix_home / "interrupt_debug.log"
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            # profile names use the standard KEPRIX_HOME, so no -p needed.
            try:
            ...
            print(f"  keprix --resume {self.session_id}{profile_flag}")
            if session_title:
                print(f"  keprix -c \"{session_title}\"{profile_flag}")
            print()
            ...
            try:
                from keprix_cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from keprix_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from keprix_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            from keprix_cli.model_switch import prewarm_picker_cache_async
        ...
        try:
            _redact_raw = os.getenv("KEPRIX_REDACT_SECRETS", "true")
            if _redact_raw.lower() not in {"1", "true", "yes", "on"}:
                self._console_print(
                    "[bold red]⚠  Secret redaction is DISABLED[/] "
                    f"(KEPRIX_REDACT_SECRETS={_redact_raw}). "
        ...
        # after an OpenClaw→Keprix migration (especially migrations done by
        ...
        def handle_ignored_terminal_sequence(event):
            ...
            install_ignored_terminal_sequences() in keprix_cli.pt_input_extras
                ...
                if self._agent_running and not (text and _looks_like_slash_command(text)):
                    ...
                    try:
                        ...
                        if not is_seen(CLI_CONFIG, BUSY_INPUT_FLAG):
                            ...
                            mark_seen(_keprix_home / "config.yaml", BUSY_INPUT_FLAG)
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting Keprix binding. See issue #22379.
                """
                event.current_buffer.insert_text('\n')
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from keprix_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Keprix")
        ...
        # Dynamic prompt: shows Keprix symbol when agent is working,
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("Keprix needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'Keprix needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("Keprix needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        def _get_model_picker_display():
            ...
            try:
                ...
                term_rows = shutil.get_terminal_size((100, 24)).lines
            scroll_offset, visible = KeprixCLI._compute_model_picker_viewport(
        ...
        try:
            ...
            if not getattr(_pt_renderer, "_keprix_osd_patched", False):
                ...
                _pt_renderer._keprix_osd_patched = True
        except Exception:
            pass
        ...
        def _signal_handler(signum, frame):
            ...
            Grace window (``KEPRIX_SIGTERM_GRACE``, default 1.5 s) gives
            ...
            try:
                if getattr(self, "agent", None) and getattr(self, "_agent_running", False):
                    ...
                    try:
                        _grace = float(os.getenv("KEPRIX_SIGTERM_GRACE", "1.5"))
                    except (TypeError, ValueError):
                        _grace = 1.5
                    if _grace > 0:
                        time.sleep(_grace)
        ...
        try:
            ...
            # delivers spurious CTRL_C_EVENT to the keprix process when
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: keprix setup"
            )
            _run_cleanup()
        ...
        try:
            ...
            if hasattr(self, '_session_db') and self._session_db and self.agent:
                ...
                # drop the empty row so /resume and `keprix sessions list`
...
def _run_kanban_goal_loop_q(cli: "KeprixCLI", first_response: str) -> None:
    """Drive a kanban goal_mode worker through the Ralph-style goal loop.

    Called from the quiet single-query path AFTER the worker's first turn,
    only when ``KEPRIX_KANBAN_GOAL_MODE`` is set (dispatcher-spawned
    ...
    task_id = (_os.environ.get("KEPRIX_KANBAN_TASK") or "").strip()
    if not task_id:
        return

    from keprix_cli import kanban_db as _kb
    from keprix_cli.goals import run_kanban_goal_loop as _run_loop, DEFAULT_MAX_TURNS as _DEF_TURNS
...
def main(
    ...
    """
    Keprix CLI - Interactive AI Assistant
    ...
    os.environ["KEPRIX_INTERACTIVE"] = "1"
    
    # Handle gateway mode (messaging + cron)
    if gateway:
        import asyncio
        from gateway.run import start_gateway
        print("Starting Keprix Gateway (messaging platforms)...")
    ...
    # Default to keprix-cli toolset which includes cronjob management tools
    toolsets_list = None
    if toolsets:
        ...
        # Coding posture (base Keprix): with no explicit --toolsets, collapse
    ...
    # mode registers its own inside KeprixCLI.run(), but `-q` runs
    ...
    # so main unwinds normally.  KEPRIX_SIGTERM_GRACE overrides the 1.5 s
    ...
    def _signal_handler_q(signum, frame):
        ...
        if os.environ.get("KEPRIX_KANBAN_TASK"):
            try:
    ...
    if query or image:
        ...
        try:
            ...
            # Kanban workers spawn with ``keprix chat -q "work kanban task <id>"``;
            ...
            _kanban_task_id = os.environ.get("KEPRIX_KANBAN_TASK", "").strip()
            if _kanban_task_id:
                try:
                    from keprix_cli import kanban_db as _kb
            ...
            if quiet:
                ...
                if cli._ensure_runtime_credentials():
                    ...
                    if cli._init_agent(
                        ...
                        # machine-readable (no styled "Keprix" box, no tool-gen
                        ...
                        if os.environ.get("KEPRIX_KANBAN_GOAL_MODE") == "1":
                            try:
                        ...
                        if isinstance(result, dict) and result.get("failed"):
                            _exit_code = 1
                            if os.environ.get("KEPRIX_KANBAN_TASK") and result.get(
                                "failure_reason"
                            ) in ("rate_limit", "billing"):
                                try:
                                    from keprix_cli.kanban_db import (
                ...
                # Single-query mode (`keprix chat -q "…"`): skip the welcome
