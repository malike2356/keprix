...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      keprixNpmLib = self'.packages.default.passthru.keprixNpmLib;
      ...
      # Non-npm packages may have their own devShellHook (e.g. keprix-agent
    ...
    in
    {
      devShells.default = pkgs.mkShell {
        ...
        shellHook = ''
          echo "Keprix Agent dev shell"
          ${combinedNonNpm}
          ${keprixNpmLib.mkNpmDevShellHook npmPackageJsonPaths fixLockfilesExe}
          echo "Ready. Run 'keprix' to start."
        '';
      };
    };
}
