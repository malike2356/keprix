...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      keprix-agent = self'.packages.default;
      keprixVenv = keprix-agent.keprixVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "keprix-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${keprixVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "keprix-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "keprix-build-package" { } ''
          echo "PASS: package built at ${keprix-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "keprix-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "keprix-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${keprix-agent}/bin/keprix || (echo "FAIL: keprix binary missing"; exit 1)
          test -x ${keprix-agent}/bin/keprix-agent || (echo "FAIL: keprix-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${keprix-agent}/bin/keprix version 2>&1 | grep -qi "keprix" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "keprix-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in keprix keprix-agent keprix-acp; do
            test -x ${keprix-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "keprix-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking keprix --help ==="
          ${keprix-agent}/bin/keprix --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${keprix-agent}/bin/keprix --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "keprix-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${keprix-agent}/share/keprix-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${keprix-agent}/share/keprix-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "KEPRIX_BUNDLED_SKILLS" ${keprix-agent}/bin/keprix || \
            (echo "FAIL: KEPRIX_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: KEPRIX_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "keprix-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${keprix-agent}/share/keprix-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${keprix-agent}/share/keprix-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "KEPRIX_BUNDLED_PLUGINS" ${keprix-agent}/bin/keprix || \
            (echo "FAIL: KEPRIX_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: KEPRIX_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "keprix-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${keprix-agent}/share/keprix-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${keprix-agent}/share/keprix-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${keprix-agent}/share/keprix-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "KEPRIX_BUNDLED_LOCALES" ${keprix-agent}/bin/keprix || \
            (echo "FAIL: KEPRIX_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: KEPRIX_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (KEPRIX_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && KEPRIX_BUNDLED_LOCALES=${keprix-agent}/share/keprix-agent/locales \
            ${keprixVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with KEPRIX_BUNDLED_LOCALES set"; exit 1)
          ...
          BARE_DIR=$(cd "$HOME" && ${keprixVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${keprixVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without KEPRIX_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "keprix-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${keprix-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${keprix-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "KEPRIX_TUI_DIR" ${keprix-agent}/bin/keprix || \
            (echo "FAIL: KEPRIX_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: KEPRIX_TUI_DIR set in wrapper"
        ...
        # Verify KEPRIX_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        keprix-node = pkgs.runCommand "keprix-node-version" { } ''
          set -e
          echo "=== Checking KEPRIX_NODE in wrapper ==="
          grep -q "KEPRIX_NODE" ${keprix-agent}/bin/keprix || \
            (echo "FAIL: KEPRIX_NODE not set in wrapper"; exit 1)
          echo "PASS: KEPRIX_NODE present in wrapper"

          KEPRIX_NODE=$(sed -n "s/^export KEPRIX_NODE='\(.*\)'/\1/p" ${keprix-agent}/bin/keprix)
          test -x "$KEPRIX_NODE" || (echo "FAIL: KEPRIX_NODE=$KEPRIX_NODE not executable"; exit 1)
          echo "PASS: KEPRIX_NODE executable at $KEPRIX_NODE"

          NODE_MAJOR=$("$KEPRIX_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All KEPRIX_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify KEPRIX_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "keprix-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(KEPRIX_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking KEPRIX_MANAGED guards ==="
          check_blocked "config set" ${keprix-agent}/bin/keprix config set model foo
          check_blocked "config edit" ${keprix-agent}/bin/keprix config edit
        ...
        extra-python-packages = let
          ...
          keprixWithExtra = keprix-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "keprix-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${keprixWithExtra}/bin/keprix || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${keprixWithExtra}/bin/keprix || \
          ...
          if grep -q "PYTHONPATH" ${keprix-agent}/bin/keprix; then
        ...
        extra-dependency-groups = let
          keprixWithGroups = keprix-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "keprix-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${keprixWithGroups}"
          echo "venv: ${keprixWithGroups.keprixVenv}"
        ...
        messaging-variant = pkgs.runCommand "keprix-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.keprixVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "keprix-config-roundtrip" {
          ...
          merge_and_load() {
            local keprix_home="$1"
            export KEPRIX_HOME="$keprix_home"
            ${configMergeScript} ${nixSettings} "$keprix_home/config.yaml"
            ${keprixVenv}/bin/python3 -c '
import json, sys
