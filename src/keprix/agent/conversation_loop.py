...
from keprix_constants import PARTIAL_STREAM_STUB_ID
from keprix_logging import set_session_context
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Keprix tool use: "
...
_CONTENT_POLICY_RECOVERY_HINT = (
    ...
    "adding a fallback provider with `keprix fallback add`."
)
...
def run_conversation(
    ...
    # all run inside Codex). Default Keprix path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for Keprix internals.
        #
        # Keprix invariant: the system prompt is built ONCE per session
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for Keprix tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                try:
                    from keprix_cli.middleware import apply_llm_request_middleware
                ...
                if env_var_enabled("KEPRIX_DUMP_REQUESTS"):
                ...
                from keprix_cli.middleware import run_llm_execution_middleware
                ...
                if finish_reason == "content_filter":
                    ...
                    _refusal_response = (
                        ...
                        "(safety refusal — not a Keprix/gateway failure).\n\n"
                        f"{_refusal_detail}\n\n"
                ...
                if (
                    ...
                    try:
                        ...
                        pass
                    print(f"{agent.log_prefix} Authentication failed (HTTP 401).")
                    if _body_text:
                        print(f"{agent.log_prefix}   Response: {_body_text}")
                    print(f"{agent.log_prefix}   This OAuth provider is no longer offered. Switch to BYOK:")
                    print(f"{agent.log_prefix}     • keprix model")
                    print(f"{agent.log_prefix}     • Switch providers: /model <model> --provider openrouter")
                if (
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `keprix doctor` for credential-chain diagnostics, or")
                    ...
                    from keprix_constants import display_keprix_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Keprix-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: keprix config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: keprix config set ANTHROPIC_API_KEY \"\"")
                ...
                # providers (DeepSeek, Kimi, MiMo, Keprix).  A 429 can
                ...
                # which Keprix' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Keprix' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`keprix",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}      2. Then run `keprix auth` to re-authenticate.", force=True)
                            elif _provider == "xai-oauth":
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `keprix model`.", force=True)
                            else:  # leftover nous oauth
                                agent._vprint(f"{agent.log_prefix}   This provider OAuth token was rejected (HTTP 401).", force=True)
                                agent._vprint(f"{agent.log_prefix}      Nous Portal was removed from Keprix. Switch to a BYOK provider:", force=True)
                                agent._vprint(f"{agent.log_prefix}      keprix model", force=True)
                                if isinstance(_model, str) and _model.endswith(":free"):
                                    agent._vprint(f"{agent.log_prefix}      Note: `{_model}` looks like an OpenRouter slug (`:free` suffix).", force=True)
                                    agent._vprint(f"{agent.log_prefix}         Run `/model openrouter:{_model}` to use OpenRouter.", force=True)
                        else:
                            agent._vprint(f"{agent.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: keprix setup", force=True)
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}        keprix fallback add   (interactive picker — same as `keprix model`)",
                            force=True,
                        )
