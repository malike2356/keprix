...
Storage
-------
Bundles live in ``~/.keprix/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``KEPRIX_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``keprix bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under KEPRIX_HOME.

    Honors ``KEPRIX_BUNDLES_DIR`` for tests; falls back to
    ``<KEPRIX_HOME>/skill-bundles``.
    """
    override = os.environ.get("KEPRIX_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_keprix_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `keprix bundles` CLI subcommand.
# ---------------------------------------------------------------------------
