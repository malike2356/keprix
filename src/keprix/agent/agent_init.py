...
from keprix_cli.timeouts import get_provider_request_timeout
...
def _build_codex_gpt55_autoraise_notice(autoraise: Dict[str, float]) -> str:
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: keprix config set compression.codex_gpt55_autoraise false"
    )
...
def init_agent(
    ...
    Args:
        ...
        load_soul_identity (bool): If True, still use ~/.keprix/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from keprix_cli.config import load_config as _load_pc_cfg
    ...
    # report cumulative micros spent.  Surfaced behind KEPRIX_DEV_CREDITS.
    ...
    # both live under ~/.keprix/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from keprix_logging import setup_logging, setup_verbose_logging
    setup_logging(keprix_home=_ra()._keprix_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that keprix_logging.setup_logging() does not
        ...
        # handler level inside keprix_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from keprix_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from keprix_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `keprix model` to "
                        "select a provider, or run `keprix setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff KEPRIX_KANBAN_TASK is set).
    ...
    try:
        ...
        os.environ["KEPRIX_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.keprix/sessions/ alongside gateway sessions
    keprix_home = get_keprix_home()
    agent.logs_dir = keprix_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.keprix/sessions/session_{sid}.json)
    ...
    try:
        from keprix_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from keprix_cli.config import load_config as _load_agent_config
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "keprix"
                    except Exception:
                        pass
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from keprix_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            try:
                from keprix_cli.plugins import get_plugin_context_engine
