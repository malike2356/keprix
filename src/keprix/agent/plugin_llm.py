...
Plugins built on Keprix often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Keprix
...
:class:`~keprix_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Keprix' main config
...
handles every provider, fallback chain, and per-task override Keprix
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`keprix_cli.plugins.PluginContext`
