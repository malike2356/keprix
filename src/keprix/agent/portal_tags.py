...
Every Keprix request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Keprix and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=keprix",
        "client=keprix-client-v<__version__>",
    ]

The version is sourced live from ``keprix_cli.__version__`` so it auto-aligns
...
``keprix_cli.__version__`` is the canonical source of truth.
"""
...
def _keprix_version() -> str:
    """Return the current Keprix release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``keprix_cli`` cannot be imported (should
...
def keprix_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=keprix-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=keprix-client-v{_keprix_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=keprix", keprix_client_tag()]
