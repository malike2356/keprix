...
Owns one Codex thread per Keprix session. Drives `turn/start`, consumes
...
# Keprix' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_KEPRIX_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Keprix/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Keprix session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _KEPRIX_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("KEPRIX_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="keprix",
            client_title="Keprix",
            client_version=_get_keprix_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Keprix' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Keprix' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own keprix-tools callback we
            # auto-accept — the user already approved Keprix' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Keprix approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Keprix returns 'once', 'session', 'always', or 'deny'.
...
def _get_keprix_version() -> str:
    """Best-effort Keprix version string for codex's userAgent line."""
    try:
