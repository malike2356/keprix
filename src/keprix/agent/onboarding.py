...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Keprix starts and finds ``~/.openclaw/``.

    Points users at ``keprix claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``keprix claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Keprix, run "
        "`keprix claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `keprix claw cleanup` (renames it to ~/.openclaw.pre-migration — "
