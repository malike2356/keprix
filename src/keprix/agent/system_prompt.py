...
upstream prefix cache warm.  See ``keprix-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Keprix never re-renders parts of this string mid-
    ...
    # KEPRIX_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Active-profile hint — names the Keprix profile the agent is running
    # under so it doesn't conflate ~/.keprix/skills/ (default profile) with
    # ~/.keprix/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        ...
        stable_parts.append(
            ...
            f"~/.keprix/cron/, ~/.keprix/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Keprix never rebuilds or reinjects parts of it
