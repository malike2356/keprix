...
``auth.json``.  They intentionally have no dependency on ``keprix_cli.auth`` so
...
# Sources Keprix owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Keprix-owned OAuth/device-code state)
