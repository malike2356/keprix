...
from keprix_constants import get_config_path, get_skills_dir, is_termux
...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``KEPRIX_KANBAN_TASK`` / ``KEPRIX_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("KEPRIX_KANBAN_TASK") or os.getenv("KEPRIX_KANBAN_BOARD"):
            result = True
        else:
            try:
        ...
        # The Keprix Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``keprix_cli.config`` on the
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``KEPRIX_PLATFORM`` or
            ``KEPRIX_SESSION_PLATFORM`` env vars.  Returns the global
...
# which becomes the dominant cost of ``keprix`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.keprix/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``keprix`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against KEPRIX_HOME, not cwd
        if not p.is_absolute():
            p = (keprix_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.keprix/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    keprix = metadata.get("keprix") or {}
    if not isinstance(keprix, dict):
        keprix = {}
    return {
        "fallback_for_toolsets": keprix.get("fallback_for_toolsets", []),
        "requires_toolsets": keprix.get("requires_toolsets", []),
        "fallback_for_tools": keprix.get("fallback_for_tools", []),
        "requires_tools": keprix.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    keprix = metadata.get("keprix")
    if not isinstance(keprix, dict):
        return []
    raw = keprix.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Keprix metadata, VCS, virtualenv/dependency, and cache
