...
def check_compression_model_feasibility(agent: Any) -> None:
    ...
    *after* construction, so :func:`replay_compression_warning` re-sends
    ...
    try:
        ...
        if client is None or not aux_model:
            if _aux_cfg_provider and _aux_cfg_provider != "auto":
                ...
                msg = (
                    ...
                    "compression will drop middle turns without a summary. "
                    "Run `keprix setup` or set OPENROUTER_API_KEY."
                )
        ...
        # the minimum context (64K by default, configurable via
        # KEPRIX_MIN_CONTEXT_LENGTH / agent.min_context_length; 0 disables
        # the check).  The main model is already required to meet this
        # floor (checked earlier in __init__), so the compression model
        # must too — otherwise it cannot summarise a full threshold-sized
        # window of main-model content.  Mirrors the main-model rejection
        # pattern.
        min_ctx = get_minimum_context_length()
        if aux_context and min_ctx and aux_context < min_ctx:
            raise ValueError(
                f"Auxiliary compression model {aux_model} has a context "
                f"window of {aux_context:,} tokens, which is below the "
                f"minimum {min_ctx:,} required by Keprix "
                f"Agent.  Choose a compression model with at least "
                f"{min_ctx // 1000}K context (set "
        ...
        if aux_context < threshold:
            ...
            # above guarantees aux_context >= the minimum context floor,
            # so the new threshold is never below that floor.
            #
...
def compress_context(
    ...
    *,
    ...
    # long-lived ``keprix_state.SessionDB`` class still bound to the
        ...
        try:
            ...
            if getattr(agent, "_last_compression_lock_error_sid", None) != _lock_sid:
                ...
                logger.warning(
                    ...
                    "process (or `keprix update`) to resync.",
    ...
    if agent._session_db:
        try:
            ...
            # from ``keprix_logging._session_context`` (set once per turn in
            ...
            agent._session_db.create_session(
                ...
                source=agent.platform or os.environ.get("KEPRIX_SESSION_SOURCE", "cli"),
                model=agent.model,
    ...
    # compression (not a fresh /new). Plugin engines (e.g. keprix-lcm) use
    # boundary_reason="compression" to preserve DAG lineage across the
    # rollover instead of re-initializing fresh per-session state.
    # See keprix-lcm#68. Built-in ContextCompressor ignores kwargs.
    try:
    ...
    def _shrink_data_url(url: str) -> Optional[str]:
        ...
        try:
            ...
            tmp = tempfile.NamedTemporaryFile(
                prefix="keprix_shrink_", suffix=suffix, delete=False,
            )
            try:
                tmp.write(raw)
                tmp.close()
