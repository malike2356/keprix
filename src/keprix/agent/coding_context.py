"""Coding-context awareness — base Keprix, every interactive surface.

When the user runs Keprix inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), Keprix shifts into a **coding posture**. This module is the
...
logger = logging.getLogger("keprix.coding_context")
...
_EDIT_FORMAT_GUIDANCE: dict[str, tuple[tuple[str, ...], str]] = {
    ...
    "replace": (
        ("claude", "sonnet", "opus", "haiku",
         ...
         "keprix", "llama", "mistral", "devstral", "minimax"),
        "- Edit format: author new files with `write_file`; for edits to "
...
# in _KEPRIX_CORE_TOOLS, so they're present on every surface this fires on.
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        selection (``--toolsets``, ``KEPRIX_TUI_TOOLSETS``, …); they never
...
def is_coding_context(
    *,
    ...
    """Whether Keprix should operate in its coding posture right now."""
