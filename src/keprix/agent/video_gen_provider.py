...
as ``kind: backend``) or ``~/.keprix/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``keprix tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$KEPRIX_HOME/cache/videos/``, creating parents as needed."""
    from keprix_constants import get_keprix_home

    path = get_keprix_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$KEPRIX_HOME/cache/videos/``.
