...
Data resolution order (like TypeScript OpenCode):
  ...
  2. Disk cache (~/.keprix/models_dev_cache.json)
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Keprix ↔ models.dev
# ---------------------------------------------------------------------------

# Keprix provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → Keprix (built lazily)
...
def _get_cache_path() -> Path:
    ...
    return get_keprix_home() / "models_dev_cache.json"
...
def fetch_models_dev(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    When ``force_refresh=True`` (used by ``keprix config refresh``, the
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a Keprix provider ID to its models dict from models.dev.
...
# Gemma models whose TPM quotas are too small for normal Keprix agent traffic.
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a Keprix provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve Keprix ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Keprix or models.dev provider ID.  Tries exact match then
