...
as ``kind: backend``) or ``~/.keprix/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``keprix tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``keprix tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``keprix tools`` picker.
...
def _images_cache_dir() -> Path:
    """Return ``$KEPRIX_HOME/cache/images/``, creating parents as needed."""
    from keprix_constants import get_keprix_home

    path = get_keprix_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$KEPRIX_HOME/cache/images/``.
...
def save_url_image(
    ...
    *,
    ...
    """Download an image URL and write it under ``$KEPRIX_HOME/cache/images/``.
