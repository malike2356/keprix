...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Keprix with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.keprix/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Keprix
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``keprix tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``keprix setup`` for
