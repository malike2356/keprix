...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_KEPRIX_CORE_TOOLS)
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``keprix_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_keprix_home()`` themselves.
        """
        if "keprix_home" not in kwargs:
            from keprix_constants import get_keprix_home
            kwargs["keprix_home"] = str(get_keprix_home())
        for provider in self._providers:
            try:
