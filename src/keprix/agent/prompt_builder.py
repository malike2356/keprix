...
from keprix_constants import get_keprix_home, get_skills_dir, is_wsl
...
_KEPRIX_MD_NAMES = (".keprix.md", "KEPRIX.md")


def _find_keprix_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.keprix.md`` or ``KEPRIX.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are Keprix, an intelligent AI assistant created by Nous Research. "
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.keprix/kanban.db`. Your task id is in "
    "`$KEPRIX_KANBAN_TASK`; your workspace is `$KEPRIX_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $KEPRIX_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `keprix kanban <verb>` for board operations. Use "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Keprix "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Keprix WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Keprix itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Keprix runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Keprix itself is running. The host OS, home, and cwd "
                f"of the Keprix process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Keprix "
    ...
    # Keprix desktop GUI — any agent running under the desktop app should know
    # it. KEPRIX_DESKTOP marks the backend powering the chat; KEPRIX_DESKTOP_TERMINAL
    # marks a keprix launched in the embedded terminal pane. Both set by main.cjs.
    _truthy = ("1", "true", "yes")
    _in_desktop = (os.getenv("KEPRIX_DESKTOP") or "").strip().lower() in _truthy
    _in_desktop_term = (os.getenv("KEPRIX_DESKTOP_TERMINAL") or "").strip().lower() in _truthy
    if _in_desktop or _in_desktop_term:
        _desktop_hint = "Runtime surface: you're running inside the Keprix desktop GUI app."
        if _in_desktop_term:
            _desktop_hint += (
    ...
    # Embedder-supplied environment description. Lets a host that wraps Keprix
    ...
    extra = (os.getenv("KEPRIX_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_keprix_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.keprix/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("KEPRIX_PLATFORM")
        or get_session_env("KEPRIX_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Keprix itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `keprix` skill "
            "first. It has the actual commands (e.g. `keprix config set …`, `keprix tools`, "
            "`keprix setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: keprix setup, keprix setup tools, keprix setup terminal, keprix status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from KEPRIX_HOME and return its content, or None.
    ...
    """
    try:
        from keprix_cli.config import ensure_keprix_home
        ensure_keprix_home()
    except Exception as e:
...
def _load_keprix_md(cwd_path: Path) -> str:
    """.keprix.md / KEPRIX.md — walk to git root."""
    keprix_md_path = _find_keprix_md(cwd_path)
    if not keprix_md_path:
        return ""
    try:
        content = keprix_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = keprix_md_path.name
        try:
            rel = str(keprix_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".keprix.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", keprix_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .keprix.md / KEPRIX.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from KEPRIX_HOME is independent and always included when present.
    ...
    # SOUL.md from KEPRIX_HOME only — skip when already loaded as identity
    if not skip_soul:
