...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - keprix_home (str): The active KEPRIX_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.keprix``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "keprix").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'keprix memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], keprix_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'keprix memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``keprix_home`` is the active KEPRIX_HOME directory path.
