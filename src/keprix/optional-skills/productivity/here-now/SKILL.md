...
- Skill helper paths:
  - `${KEPRIX_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${KEPRIX_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${KEPRIX_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client keprix
```
...
```bash
PUBLISH="${KEPRIX_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client keprix
```
...
```bash
DRIVE="${KEPRIX_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `keprix`)    |
