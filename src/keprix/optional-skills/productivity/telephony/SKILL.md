---
name: telephony
description: Give Keprix phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi.
version: 1.0.0
...
- save provider credentials into `${KEPRIX_HOME:-~/.keprix}/.env`
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Keprix memory
...
### 1) "I want Keprix to own a real phone number"
Use **Twilio**.
...
- pairs well with Keprix `text_to_speech` plus a public file host or tunnel

## Files and persistent state

The skill persists telephony state in two places:

### `${KEPRIX_HOME:-~/.keprix}/.env`
...
```bash
SCRIPT="$(find ~/.keprix/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
keprix skills search telephony
keprix skills install official/productivity/telephony
```
...
Then save credentials into Keprix:
...
3. Buy it and save it into `${KEPRIX_HOME:-~/.keprix}/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python3 "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Keprix calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing Keprix's existing `text_to_speech` support.

Use this when:
- you want the call to use Keprix's configured TTS voice rather than Twilio `<Say>`
...
Recommended Keprix TTS -> Twilio Play workflow:

1. Generate the audio with Keprix `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Keprix to create the message audio with `text_to_speech`
...
Important note:
- Keprix TTS is great for prerecorded outbound messages
...
- Keprix STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Keprix memory.
...
- Do not store arbitrary third-party phone numbers in Keprix memory.
...
3. persist that number to `${KEPRIX_HOME:-~/.keprix}/.env`
