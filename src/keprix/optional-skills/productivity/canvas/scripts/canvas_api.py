...
def _check_config():
    ...
    if missing:
        keprix_env = os.path.join(
            os.environ.get("KEPRIX_HOME", os.path.expanduser("~/.keprix")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {keprix_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Keprix"
    )
