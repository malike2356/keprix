---
name: honcho
description: Configure and use Honcho memory with Keprix -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.
version: 2.0.0
author: Keprix
license: MIT
...
# Honcho Memory for Keprix

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Keprix profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/keprix#running-honcho-locally-with-keprix

### Verify

```bash
keprix honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Keprix profile's AI peer
...
Honcho models conversations as interactions between **peers**. Keprix creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Keprix instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Keprix run |
| `global` | Single session across all directories |

Manual override: `keprix honcho map my-project-name`
...
Each Keprix profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
```bash
keprix profile create coder --clone
# creates host block keprix.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `keprix.coder` host block in `honcho.json`
...
```bash
keprix honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Keprix when Honcho memory is active.
...
Config file: `$KEPRIX_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `keprix honcho setup`. Ensure `memory.provider: honcho` is in `~/.keprix/config.yaml`.

### Memory not persisting across sessions
Check `keprix honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `keprix profile create <name> --clone`. For existing profiles: `keprix honcho sync`.
...
| Command | Description |
|---------|-------------|
| `keprix honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `keprix honcho status` | Show resolved config, connection test, peer info for active profile |
| `keprix honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `keprix honcho disable` | Disable Honcho for the active profile |
| `keprix honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `keprix honcho peers` | Show peer identities across all profiles |
| `keprix honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `keprix honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `keprix honcho sessions` | List known directory-to-session-name mappings |
| `keprix honcho map <name>` | Map current working directory to a Honcho session name |
| `keprix honcho identity` | Seed AI peer identity or show both peer representations |
| `keprix honcho sync` | Create host blocks for all Keprix profiles that don't have one yet |
| `keprix honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Keprix + Honcho |
| `keprix memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `keprix memory status` | Show active memory provider and config |
| `keprix memory off` | Disable external memory provider |
