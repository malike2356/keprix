...
- smoke-test a server with the FastMCP CLI before wiring it into Keprix or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Keprix. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.keprix/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.keprix/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Keprix integration, either:

- configure the server in `~/.keprix/config.yaml` using the `native-mcp` skill, or
...
### Keprix cannot see the deployed server

The server-building part may be correct while the Keprix config is not. Load the `native-mcp` skill and configure the server in `~/.keprix/config.yaml`, then restart Keprix.
