...
Usage (via cron with --no-agent):
    ...
    keprix cron create keprix-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$KEPRIX_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name keprix-issues --repo NousResearch/keprix --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Keprix .env file
(``${KEPRIX_HOME:-~/.keprix}/.env``) to avoid the 60 req/hr
