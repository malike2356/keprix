...
3. **Generates** a setup script that creates Keprix profiles, project workspace, and the initial kanban task
...
3. Creates each Keprix profile via `keprix profile create --clone`
...
7. Fires the initial `keprix kanban create` task assigned to the director
...
```bash
keprix kanban watch --tenant <project-tenant>     # live events
keprix kanban list  --tenant <project-tenant>     # board snapshot
keprix dashboard                                   # visual board UI
```
...
8. **Verify API keys BEFORE firing.** External APIs (TTS, image-gen,
   image-to-video) need keys in `${KEPRIX_HOME:-~/.keprix}/.env` or the user's secret store.
