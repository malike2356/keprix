...
```bash
keprix profile create director --clone 2>/dev/null || true
```
...
p = os.path.expanduser(f"~/.keprix/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Keprix
...
Each profile gets a `SOUL.md` at `~/.keprix/profiles/<name>/SOUL.md` that
...
```bash
keprix kanban create "Direct production of <video title>" \
    --assignee director \
...
the Keprix `.env` (`${KEPRIX_HOME:-$HOME/.keprix}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _keprix_env="${KEPRIX_HOME:-$HOME/.keprix}/.env"
    if grep -q "^${var}=" "$_keprix_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_keprix_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_keprix_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY keprix ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY keprix OPENROUTER_API_KEY || exit 1
# ...
```
