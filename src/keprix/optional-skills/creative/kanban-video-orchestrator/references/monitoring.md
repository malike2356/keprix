...
keprix kanban list --tenant <project-slug>
keprix kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
keprix kanban stats --tenant <project-slug>

# Visual dashboard (browser)
keprix dashboard

# Inspect a specific task (includes comments + events)
keprix kanban show <task-id>
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `keprix profile list` (profile exists?), `keprix status` (gateway/dispatcher up?), and verify tenant. |
...
NEW_TASK_ID=$(keprix kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
keprix kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
keprix kanban block <task-id>
keprix kanban archive <task-id>

# Diagnose what happened
keprix kanban show <task-id>      # task body, comments, recent events
keprix kanban tail <task-id>      # follow the live event stream
keprix kanban log <task-id>       # worker process log
```
...
while true; do
    clear
    keprix kanban list --tenant <slug>
    echo "---"
    keprix kanban stats --tenant <slug>
    sleep 30
done
```
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `keprix profile list` and start
