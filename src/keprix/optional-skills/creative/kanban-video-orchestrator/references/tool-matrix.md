...
Maps each role archetype to the Keprix skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public keprix
...
## Keprix skills relevant to video production

### Visual / rendering skills (`keprix/skills/creative/`)
...
### Design / pre-production skills (`keprix/skills/creative/`)
...
### Audio / media skills (`keprix/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`keprix/skills/devops/`)
...
These are **not** Keprix skills but external CLIs / APIs that profiles invoke.
...
## Built-in Keprix tools for media review

These are native Keprix tools — not invoked via terminal but through their own
...
key is present in `${KEPRIX_HOME:-~/.keprix}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Keprix profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${KEPRIX_HOME:-~/.keprix}/.env` → environment variable.
...
When a new Keprix-public video skill ships:
