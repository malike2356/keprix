...
import upstream classes into Keprix itself.
...
```bash
mkdir -p ~/.keprix/cache/darwinian-evolver && cd ~/.keprix/cache/darwinian-evolver
...
```bash
cd ~/.keprix/cache/darwinian-evolver/darwinian_evolver \
...
SKILL_DIR=~/.keprix/skills/research/darwinian-evolver
DE_DIR=~/.keprix/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Keprix core.
   Custom driver scripts under `~/.keprix/skills/...` are user-side and fine.
