...
version: 1.0.0
author: arceus77-7, enhanced by Keprix
license: MIT
...
- `tmux` available for stable authenticated sessions during Keprix terminal calls (desktop app flow only)
...
### Service Account (recommended for Keprix)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${KEPRIX_HOME:-~/.keprix}/.env` (the skill will prompt for this on first load).
...
## Keprix Execution Pattern (desktop app flow)
...
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/keprix-op.sock"
