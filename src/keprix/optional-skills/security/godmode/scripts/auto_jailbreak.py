...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("KEPRIX_HOME", os.path.expanduser("~/.keprix")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("KEPRIX_HOME", Path.home() / ".keprix")) / "skills" / "red-teaming" / "godmode"
...
KEPRIX_HOME = Path(os.getenv("KEPRIX_HOME", Path.home() / ".keprix"))
CONFIG_PATH = KEPRIX_HOME / "config.yaml"
PREFILL_PATH = KEPRIX_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Keprix models — already uncensored, just needs clean prompt
    "keprix": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "keprix" in model_lower or "nous" in model_lower:
        return "keprix"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Keprix config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.keprix/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Keprix for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Keprix for changes to take effect.")
