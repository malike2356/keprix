...
description: Migrate a user's OpenClaw customization footprint into Keprix. Imports Keprix-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
version: 1.0.0
author: Keprix (Nous Research)
license: MIT
platforms: [linux, macos, windows]
metadata:
  keprix:
    tags: [Migration, OpenClaw, Keprix, Memory, Persona, Import]
    related_skills: [keprix]
---

# OpenClaw -> Keprix Migration

Use this skill when a user wants to move their OpenClaw setup into Keprix with minimal manual cleanup.
...
```bash
keprix claw migrate              # Full interactive migration
...
keprix claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `keprix setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_keprix.py` to:

- import `SOUL.md` into the Keprix home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Keprix memory entries
- merge OpenClaw command approval patterns into Keprix `command_allowlist`
- migrate Keprix-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Keprix working-directory configuration
- copy OpenClaw skills into `~/.keprix/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Keprix workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.keprix/tts/`
- archive non-secret docs that do not have a direct Keprix destination
...
- `scripts/openclaw_to_keprix.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.keprix/skills/migration/openclaw-migration/scripts/openclaw_to_keprix.py`

Do not guess a shorter path like `~/.keprix/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.keprix/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Keprix-compatible secrets, currently:
...
Keprix CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Keprix. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Keprix workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Keprix skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.keprix/skills/migration/openclaw-migration/scripts/openclaw_to_keprix.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.keprix/skills/migration/openclaw-migration/scripts/openclaw_to_keprix.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.keprix/skills/migration/openclaw-migration/scripts/openclaw_to_keprix.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.keprix/skills/migration/openclaw-migration/scripts/openclaw_to_keprix.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.keprix/skills/migration/openclaw-migration/scripts/openclaw_to_keprix.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.keprix/skills/migration/openclaw-migration/scripts/openclaw_to_keprix.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Keprix unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Keprix targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Keprix destination. Unsupported auth blobs must still be reported as skipped.
...
- Keprix persona state imported
- Keprix memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.keprix/skills/openclaw-imports/`
