...
```
link-cli auth login --client-name "Keprix" --interval 5 --timeout 300
```
...
`@stripe/link-cli --mcp` exposes the same commands as MCP tools over stdio. To register it with Keprix' native MCP:
