---
name: agentmail
description: Give the agent its own dedicated email inbox via AgentMail. Send, receive, and manage email autonomously using agent-owned email addresses (e.g. keprix@agentmail.to).
version: 1.0.0
...
1. Create a dedicated inbox:
   - Use `create_inbox` with a username (e.g. `keprix`)
   - The agent gets address: `keprix@agentmail.to`
2. Send an email:
...
```
1. create_inbox (username: "keprix-outreach")
...
```
keprix --toolsets mcp -q "Create an AgentMail inbox called test-agent and tell me its email address"
```
