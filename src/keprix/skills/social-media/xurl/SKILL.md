...
version: 1.1.1
author: xdevplatform + openclaw + Keprix
license: MIT
...
> ```
> If `HOME=/opt/data xurl auth status` succeeds but `HOME=/opt/data/home xurl auth status` shows no apps or tokens, Keprix tool calls will not see the credentials.
...
- **Token storage:** `~/.xurl` is YAML. In Docker, use the Keprix subprocess HOME (`/opt/data/home` in the official image) so tokens land under `/opt/data/home/.xurl`. Never read or send this file to LLM context.
...
- Keprix adaptation: reformatted for Keprix skill conventions; safety guardrails preserved verbatim.
