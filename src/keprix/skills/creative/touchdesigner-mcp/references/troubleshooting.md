...
## 5. Keprix Configuration

### Config location

`$KEPRIX_HOME/config.yaml` (defaults to `~/.keprix/config.yaml` when `KEPRIX_HOME` is unset)
...
Restart the Keprix session for changes to take effect. The MCP connection is
