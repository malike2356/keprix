...
KEPRIX_HOME_DIR="${KEPRIX_HOME:-$HOME/.keprix}"
KEPRIX_CFG="${KEPRIX_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Keprix config has twozero_td MCP entry ──
if [[ ! -f "$KEPRIX_CFG" ]]; then
    echo -e " ${FAIL} Keprix config not found at ${KEPRIX_CFG}"
    manual_steps+=("Create ${KEPRIX_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$KEPRIX_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Keprix config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Keprix config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${KEPRIX_CFG} manually"); }
    manual_steps+=("Restart Keprix session to pick up config change")
fi
