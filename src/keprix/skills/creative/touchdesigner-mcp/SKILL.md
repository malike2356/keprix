...
metadata:
  keprix:
    ...
    related_skills: [native-mcp, ascii-video, manim-video, keprix-video]
...
```
Keprix -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${KEPRIX_HOME:-$HOME/.keprix}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to Keprix config (if missing)
...
3. **Restart Keprix session** to pick up the new MCP server
