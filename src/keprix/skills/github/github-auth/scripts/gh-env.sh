#!/usr/bin/env bash
# GitHub environment detection helper for Keprix skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _keprix_env="${KEPRIX_HOME:-$HOME/.keprix}/.env"; [ -f "$_keprix_env" ] && grep -q "^GITHUB_TOKEN=" "$_keprix_env" 2>/dev/null; then
