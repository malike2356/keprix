...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _keprix_env="${KEPRIX_HOME:-$HOME/.keprix}/.env"; [ -f "$_keprix_env" ] && grep -q "^GITHUB_TOKEN=" "$_keprix_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_keprix_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
...
```bash
source "${KEPRIX_HOME:-$HOME/.keprix}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Keprix. Code looks clean; good test coverage, no security concerns."
...
curl -s -X POST \
  ...
  -d "{
    ...
    \"comments\": [
      ...
      {\"path\": \"src/models.py\", \"line\": 23, \"body\": \"WARNING:  **Warning:** Password stored without hashing.\"},
