#!/usr/bin/env python3
"""Google Workspace API CLI for Keprix.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Keprix-facing JSON contract and falls back to the Python client
...
from _keprix_home import get_keprix_home
...
def _gws_binary() -> str | None:
    override = os.getenv("KEPRIX_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Keprix")
