...
Before using the pipeline, verify these are set in `${KEPRIX_HOME:-~/.keprix}/.env`:
...
keprix teams-pipeline list --status failed  # only failed jobs
keprix teams-pipeline show <job-id>         # full detail of one job
keprix teams-pipeline subscriptions         # current Graph webhook subscriptions
```

### Re-running / debugging

```bash
keprix teams-pipeline run <job-id>          # replay a stored job (re-summarize, re-deliver)
keprix teams-pipeline fetch --meeting-id <id>   # dry-run: resolve meeting + transcript without persisting
keprix teams-pipeline fetch --join-web-url "<url>"   # dry-run by join URL
```

### Subscription management

```bash
keprix teams-pipeline subscribe \
  --resource communications/onlineMeetings/getAllTranscripts \
  --notification-url https://<your-public-host>/msgraph/webhook \
  --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE"

keprix teams-pipeline renew-subscription <sub-id> --expiration <iso-8601>
keprix teams-pipeline delete-subscription <sub-id>
keprix teams-pipeline maintain-subscriptions            # renew near-expiry ones
keprix teams-pipeline maintain-subscriptions --dry-run  # show what would be renewed
```
