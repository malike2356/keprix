...
# Python-driven tool session (e.g. Keprix terminal tool). A pre-set PYTHONPATH
...
if [ -n "${KEPRIX_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$KEPRIX_INSTALL_DIR"
...
#   code at /usr/local/lib/keprix, command at /usr/local/bin/keprix,
#   data still at /root/.keprix (KEPRIX_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$KEPRIX_HOME/.no-bundled-skills so future"
            echo "                   'keprix update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Keprix.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.keprix/keprix"
            echo "                   default (root, Linux): /usr/local/lib/keprix"
            echo "  --keprix-home PATH  Data directory (default: ~/.keprix, or \$KEPRIX_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Keprix installs the code under"
            echo "  /usr/local/lib/keprix and links the command into"
            echo "  /usr/local/bin/keprix (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$KEPRIX_HOME"
            echo "  (default /root/.keprix).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$KEPRIX_HOME/keprix are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs keprix setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
# intentional, but they leave the checkout dirty — which forces `keprix update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Keprix-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Keprix","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install keprix command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `keprix` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $KEPRIX_HOME/keprix
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $KEPRIX_HOME/keprix
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/keprix
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $KEPRIX_HOME/keprix — then preserve it)
#
# Always no-op when the user set --dir or $KEPRIX_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in KEPRIX_HOME.
    if is_termux; then
        INSTALL_DIR="$KEPRIX_HOME/keprix"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$KEPRIX_HOME/keprix/.git" ]; then
            INSTALL_DIR="$KEPRIX_HOME/keprix"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/keprix)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/keprix"
        ...
        # /usr/local/bin/keprix wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/keprix"
        log_info "  Data:    $KEPRIX_HOME (unchanged)"
...
# Point a Keprix-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $KEPRIX_HOME/node/bin — which is NOT on
...
# Keprix's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Keprix-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$KEPRIX_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$KEPRIX_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$KEPRIX_HOME/node/etc/npmrc"
}

get_keprix_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/keprix" ]; then
        echo "$link_dir/keprix"
    else
        echo "keprix"
    fi
}
...
install_uv() {
    ...
    # Keprix owns its own uv at $KEPRIX_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (keprix_cli/managed_uv.py) looks in the same
    # place, so install.sh and `keprix update` stay in sync.
    local _managed_uv="$KEPRIX_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $KEPRIX_HOME/bin ..."
    mkdir -p "$KEPRIX_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/keprix-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/keprix-uv-installer.$$.sh")"
    ...
    # directly into $KEPRIX_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$KEPRIX_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# string clears that floor; anything below it is replaced with the Keprix-
...
check_node() {
    ...
    # Repair pre-existing Keprix-managed installs where `npm install -g` lands
    ...
    # Prefer a Keprix-managed Node from a previous run over a too-old system one.
    if [ -x "$KEPRIX_HOME/node/bin/node" ] && node_satisfies_build "$("$KEPRIX_HOME/node/bin/node" --version)"; then
        export PATH="$KEPRIX_HOME/node/bin:$PATH"
        log_success "Node.js $("$KEPRIX_HOME/node/bin/node" --version) found (Keprix-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing Keprix-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    log_info "Extracting to ~/.keprix/node/..."
    ...
    # Place into ~/.keprix/node/ and symlink binaries into the same bin dir
    # the keprix command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$KEPRIX_HOME/node"
    mkdir -p "$KEPRIX_HOME"
    mv "$extracted_dir" "$KEPRIX_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$KEPRIX_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$KEPRIX_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$KEPRIX_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$KEPRIX_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.keprix/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Keprix install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Keprix itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `keprix update` path
                # (#4735).
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Keprix behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up keprix command..."

    if [ "$USE_VENV" = true ]; then
        KEPRIX_BIN="$INSTALL_DIR/venv/bin/keprix"
    else
        KEPRIX_BIN="$(which keprix 2>/dev/null || echo "")"
        if [ -z "$KEPRIX_BIN" ]; then
            log_warn "keprix not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$KEPRIX_BIN" ]; then
        log_warn "keprix entry point not found at $KEPRIX_BIN"
    ...
    # Create a user-facing shim for the keprix command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $KEPRIX_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$KEPRIX_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/keprix"
    cat > "$command_link_dir/keprix" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$KEPRIX_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/keprix"
    log_success "Installed keprix launcher → $command_link_display_dir/keprix"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "keprix command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v keprix' \
                >/dev/null 2>&1; then
        ...
        log_info "keprix not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Keprix — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Keprix — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Keprix — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so keprix works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.keprix directory structure (config at top level, code in subdir)
    mkdir -p "$KEPRIX_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.keprix/.env (top level, easy to find)
    if [ ! -f "$KEPRIX_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$KEPRIX_HOME/.env"
            log_success "Created ~/.keprix/.env from template"
        else
            touch "$KEPRIX_HOME/.env"
            log_success "Created ~/.keprix/.env"
        fi
    else
        log_info "~/.keprix/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$KEPRIX_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.keprix/config.yaml (top level, easy to find)
    if [ ! -f "$KEPRIX_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$KEPRIX_HOME/config.yaml"
            log_success "Created ~/.keprix/config.yaml from template"
        fi
    else
        log_info "~/.keprix/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$KEPRIX_HOME/SOUL.md" ]; then
        cat > "$KEPRIX_HOME/SOUL.md" << 'SOUL_EOF'
# Keprix Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how Keprix communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.keprix/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.keprix/"

    # Seed bundled skills into ~/.keprix/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `keprix update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'keprix update'." \
            > "$KEPRIX_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $KEPRIX_HOME/.no-bundled-skills"
        log_info "  Future 'keprix update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.keprix/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.keprix/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$KEPRIX_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$KEPRIX_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.keprix/skills/"
            fi
        fi
    fi
}
...
configure_browser_env_from_system_browser() {
    local env_file="$KEPRIX_HOME/.env"
    ...
    {
        echo ""
        echo "# Keprix browser tools — use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; Keprix will use the system browser."
        else
            case "$DISTRO" in
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (keprix --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `keprix update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'keprix setup' after install."
        return 0
    fi
    ...
    # Run keprix setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m keprix_cli.main setup < /dev/tty
    else
        python -m keprix_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$KEPRIX_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Keprix to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$KEPRIX_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'keprix whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                KEPRIX_CMD="$(get_keprix_command_path)"
                $KEPRIX_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'keprix whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'keprix gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $KEPRIX_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $KEPRIX_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: keprix gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: keprix gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $KEPRIX_CMD gateway > "$KEPRIX_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.keprix/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: keprix gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: keprix gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $KEPRIX_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $KEPRIX_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $KEPRIX_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}keprix${NC}              Start chatting"
    echo -e "   ${GREEN}keprix setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}keprix config${NC}       View/edit configuration"
    echo -e "   ${GREEN}keprix config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}keprix gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}keprix update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'keprix' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'keprix' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'keprix' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$KEPRIX_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$KEPRIX_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$KEPRIX_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$KEPRIX_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$KEPRIX_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Keprix for pip install"
    ...
    KEPRIX_CMD="$(command -v keprix 2>/dev/null || echo "")"
    if [ -n "$KEPRIX_CMD" ]; then
        log_info "Running keprix setup..."
        "$KEPRIX_CMD" setup
    else
        log_warn "keprix command not found on PATH"
        log_info "Try: python -m keprix_cli.main setup"
    fi
}
...
# extract a tree MISSING the electron binary, so the `electron`->`Keprix` rename
...
# _purge_electron_build_cache() used by `keprix desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the Keprix-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `keprix desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Keprix" ]; then
            app="$desktop_dir/release/linux-unpacked/Keprix"
        elif [ -x "$desktop_dir/release/linux-unpacked/keprix" ]; then
            app="$desktop_dir/release/linux-unpacked/keprix"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Keprix.app" \
            "$desktop_dir/release/mac/Keprix.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # trips Gatekeeper's tamper check ("Keprix is damaged and can't be opened").
    ...
    # checkout stays clean for the next `keprix update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Keprix-managed Node
            # provisioned during prerequisites/node-deps (at $KEPRIX_HOME/node/bin)
        ...
        complete)
            ...
            resolve_install_layout
            print_success
            echo "git" > "$KEPRIX_HOME/.install_method"
            ;;
        *)
