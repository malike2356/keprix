#!/usr/bin/env python3
"""Live test harness for Keprix's Tool Search feature.
...
# Force-isolate the test environment BEFORE any keprix imports.
ORIGINAL_HOME = os.environ.get("KEPRIX_HOME")
ORIGINAL_AUTH = Path.home() / ".keprix" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.keprix/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.keprix/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="keprix_ts_live_"))
    keprix_home = home_dir / ".keprix"
    keprix_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, keprix_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".keprix" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, keprix_home / ".env")
        ...
        load_keprix_dotenv(keprix_home=str(Path.home() / ".keprix"))
    ...
    (keprix_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return keprix_home
...
def reset_module_state():
    """Drop cached modules so the new KEPRIX_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "keprix_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["KEPRIX_HOME"] = str(home)
...
def main():
    ...
    # Restore original KEPRIX_HOME
    if ORIGINAL_HOME is not None:
        os.environ["KEPRIX_HOME"] = ORIGINAL_HOME
    else:
