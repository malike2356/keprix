...
# Bootstrap Open WebUI against Keprix's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.keprix/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-keprix.sh
...
#   KEPRIX_API_PORT=8642
#   KEPRIX_API_HOST=127.0.0.1
#   KEPRIX_API_MODEL_NAME='Keprix'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Keprix WebUI}"
...
KEPRIX_ENV_FILE="${KEPRIX_ENV_FILE:-$HOME/.keprix/.env}"
KEPRIX_API_PORT="${KEPRIX_API_PORT:-8642}"
KEPRIX_API_HOST="${KEPRIX_API_HOST:-127.0.0.1}"
KEPRIX_API_CONNECT_HOST="${KEPRIX_API_CONNECT_HOST:-127.0.0.1}"
KEPRIX_API_MODEL_NAME="${KEPRIX_API_MODEL_NAME:-Keprix}"
KEPRIX_API_BASE_URL="http://${KEPRIX_API_CONNECT_HOST}:${KEPRIX_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-keprix.sh"
LOG_DIR="$HOME/.keprix/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$KEPRIX_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$KEPRIX_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.keprix.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.keprix</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.keprix"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.keprix"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-keprix.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Keprix
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-keprix.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.keprix/logs/openwebui.log
StandardError=append:%h/.keprix/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-keprix.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$KEPRIX_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Keprix API server is configured...'
  upsert_env API_SERVER_ENABLED true "$KEPRIX_ENV_FILE"
  upsert_env API_SERVER_HOST "$KEPRIX_API_HOST" "$KEPRIX_ENV_FILE"
  upsert_env API_SERVER_PORT "$KEPRIX_API_PORT" "$KEPRIX_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$KEPRIX_API_MODEL_NAME" "$KEPRIX_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$KEPRIX_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Keprix gateway so API server settings take effect...'
  keprix gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${KEPRIX_API_CONNECT_HOST}:${KEPRIX_API_PORT}/health" >/dev/null; then
    log 'Keprix API server did not answer on the first check. Trying to start gateway in the background...'
    nohup keprix gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${KEPRIX_API_CONNECT_HOST}:${KEPRIX_API_PORT}/health" >/dev/null
  ...
  log "Keprix API endpoint: ${KEPRIX_API_BASE_URL}"
