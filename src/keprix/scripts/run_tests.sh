#!/usr/bin/env bash
# Canonical test runner for keprix. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.keprix/...)
#
# Usage:
...
# ── Activate venv ───────────────────────────────────────────────────────────
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.keprix/keprix/venv"; do
...
if [ -f "$HOME/.keprix/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.keprix"
