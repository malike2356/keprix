"""Windows ConPTY bridge for the `keprix dashboard` chat tab.

Drop-in counterpart to ``keprix_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``keprix_cli.web_server`` consumes: ``spawn``, ``read``,
