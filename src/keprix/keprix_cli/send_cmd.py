"""CLI subcommand: ``keprix send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"keprix send: {file_path} is not a text file. --file reads the "
                ...
                f'  keprix send --to telegram "MEDIA:{file_path}"\n'
                f'  keprix send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  keprix send --to telegram "[[as_document]] MEDIA:{file_path}"',
                file=sys.stderr,
            )
            sys.exit(_USAGE_EXIT)
        except OSError as exc:
            print(f"keprix send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"keprix send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"keprix send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"keprix send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"keprix send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `keprix gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.keprix/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_keprix_env() -> None:
    """Populate ``os.environ`` from ``~/.keprix/.env`` AND bridge top-level
    ...
    startup that ``keprix send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.keprix/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.keprix/config.yaml`` into
       ...
       via ``keprix config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``keprix send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.keprix/.env and ~/.keprix/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "keprix send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  keprix send --to telegram \"hello\"\n"
            "  keprix send --to discord:#ops --file report.md\n"
            "  keprix send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "keprix send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `keprix send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Keprix "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.keprix/.env + ~/.keprix/config.yaml) — no LLM, "
            "no agent loop, no running gateway required for bot-token "
            "platforms like Telegram/Discord/Slack/Signal."
        ),
        epilog=(
            "Examples:\n"
            "  keprix send --to telegram \"deploy finished\"\n"
            "  echo \"RAM 92%\" | keprix send --to telegram:-1001234567890\n"
            "  keprix send --to discord:#ops --file /tmp/report.md\n"
            "  keprix send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  keprix send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
            "  keprix send --list                  # all platforms\n"
            "  keprix send --list telegram         # filter by platform\n"
            "\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `keprix send --list telegram`.",
    )
