"""Shell completion script generation for keprix CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_keprix_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Keprix bash completion
# Add to ~/.bashrc:
#   eval "$(keprix completion bash)"

_keprix_profiles() {{
    local profiles_dir="$HOME/.keprix/profiles"
...
_keprix_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_keprix_profiles)" -- "$cur"))
        return
    fi
...
complete -F _keprix_completion keprix
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef keprix
# Keprix zsh completion
# Add to ~/.zshrc:
#   eval "$(keprix completion zsh)"

_keprix_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.keprix/profiles" ]]; then
        profiles+=($HOME/.keprix/profiles/*(N/:t))
    fi
...
_keprix() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_keprix_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'keprix command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Keprix fish completion",
        "# Add to your config:",
        "#   keprix completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __keprix_profiles",
        "    echo default",
        "    if test -d $HOME/.keprix/profiles",
        "        for d in $HOME/.keprix/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c keprix -f -s p -l profile"
        " -d 'Profile name' -xa '(__keprix_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__keprix_profiles)' -d 'Profile name'"
                )
