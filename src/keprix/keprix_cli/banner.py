...
Pure display functions with no KeprixCLI state dependency.
"""
...
def _fetch_pypi_latest(package: str = "keprix") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a Keprix update is available.

    Two paths: if ``KEPRIX_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = keprix_home / ".update_check"
    embedded_rev = os.environ.get("KEPRIX_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # KEPRIX_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `keprix update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/keprix/update/check`
    ...
    if embedded_rev:
        ...
        # $KEPRIX_HOME/keprix/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = keprix_home / "keprix"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Keprix git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$KEPRIX_HOME/keprix/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``keprix_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from keprix_cli.build_info import get_build_sha
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Keprix checkout. Cached per-process. Release URL always points at the
    canonical NousResearch/keprix repo (forks don't get a link).
    """
...
def format_banner_version_label() -> str:
    ...
    base = f"Keprix v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    try:
        from keprix_cli.codex_runtime_switch import get_current_runtime
        from keprix_cli.config import load_config as _load_cfg
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from keprix_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
    ...
    # Pip-install warning — `pip install keprix` is not the supported
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else KEPRIX_AGENT_LOGO
        console.print(_logo)
        console.print()
