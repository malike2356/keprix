"""CLI handlers for the ``keprix proxy`` subcommand."""
...
from keprix_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "keprix proxy requires aiohttp. Install one of:\n"
        "  pip install 'keprix[messaging]'\n"
        "  pip install aiohttp",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"keprix auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Keprix proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Keprix proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: keprix proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``keprix proxy <subcommand>``."""
    ...
    print(
        "keprix proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  keprix proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
        "      Run the proxy in the foreground.\n"
        "  keprix proxy status\n"
        "      Show which upstream adapters are ready.\n"
        "  keprix proxy providers\n"
