...
# xAI's public API is OpenAI-compatible for the endpoints Keprix commonly
# uses. The Responses endpoint is included because Keprix' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Keprix-managed OAuth credentials."""

    auth_hint = "keprix auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`keprix auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`keprix auth reset xai-oauth` or re-authenticate with "
                    "`keprix auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `keprix auth add xai-oauth --type oauth`."
            )
