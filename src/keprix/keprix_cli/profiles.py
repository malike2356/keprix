"""
Profile management for multiple isolated Keprix instances.

Each profile is a fully independent KEPRIX_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.keprix/profiles/<name>/`` by default.

The "default" profile is ``~/.keprix`` itself — backward compatible,
zero migration needed.

Usage::

    keprix profile create coder          # fresh profile + bundled skills
    keprix profile create coder --clone  # also copy config, .env, SOUL.md, skills
    keprix profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    keprix -p coder chat                 # or via flag
    keprix profile use coder             # set as sticky default
    keprix profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See keprix_constants.get_subprocess_home().
    "home",
]
...
#   backups             — `keprix backup` archives
...
# Marker file written by `keprix profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `keprix
...
# `keprix skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Keprix infrastructure directories that only the default profile
         (``~/.keprix``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_keprix_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.keprix) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "keprix",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "keprix", "default", "test", "tmp", "root", "sudo",
})
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the keprix root, NOT to the current KEPRIX_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where KEPRIX_HOME points outside
    ``~/.keprix``, profiles live under ``KEPRIX_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_keprix_home() / "profiles"


def _get_default_keprix_home() -> Path:
    """Return the default (pre-profile) KEPRIX_HOME path.

    In standard deployments this is ``~/.keprix``.
    In Docker/custom deployments where KEPRIX_HOME is outside ``~/.keprix``
    (e.g. ``/opt/data``), returns KEPRIX_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_keprix_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Keprix installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its KEPRIX_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_keprix_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, keprix subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _KEPRIX_SUBCOMMANDS:
        return f"'{canon}' conflicts with a keprix subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            keprix_exe = shutil.which("keprix") or "keprix"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(keprix_exe)} -p {profile} "$@"\n')
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Keprix. If we
    ...
    try:
        ...
        # not be migrated. The next `keprix doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``keprix -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``keprix profile alias
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``keprix profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing Keprix config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_keprix_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``keprix update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.keprix)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.keprix/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Keys pages, `keprix -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Keprix profile.\n"
    ...
    # Write the opt-out marker so seed_profile_skills() and `keprix update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`keprix profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running Keprix (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `keprix profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `keprix -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches KEPRIX_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``keprix profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "KEPRIX_HOME": str(profile_dir)},
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_keprix_home() / ".env"
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.keprix).\n"
            "To remove everything, use: keprix uninstall"
        )
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``KEPRIX_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from keprix_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set KEPRIX_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("KEPRIX_HOME")
    try:
        os.environ["KEPRIX_HOME"] = str(profile_dir)
        from keprix_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["KEPRIX_HOME"] = old_home
        elif "KEPRIX_HOME" in os.environ:
            del os.environ["KEPRIX_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.keprix/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: keprix profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from KEPRIX_HOME.

    Returns ``"default"`` if KEPRIX_HOME is not set or points to ``~/.keprix``.
    Returns the profile name if KEPRIX_HOME points into ``~/.keprix/profiles/<name>``.
    Returns ``"custom"`` if KEPRIX_HOME is set to an unrecognized path.
    """
    from keprix_constants import get_keprix_home
    keprix_home = get_keprix_home()
    resolved = keprix_home.resolve()

    default_resolved = _get_default_keprix_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.keprix itself — its parent is ~/ and its
        # directory name is ".keprix", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: keprix profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.keprix itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.keprix). "
            "Specify a different name: keprix profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="keprix_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"keprix_{old_name}"
    legacy_old_host = f"keprix.{old_name}"
    new_host = f"keprix_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_keprix_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("keprix_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a KEPRIX_HOME path string.

    Called early in the CLI entry point, before any keprix modules
    are imported, to set the KEPRIX_HOME environment variable.
    """
    canon = normalize_profile_name(profile_name)
    validate_profile_name(canon)
    profile_dir = get_profile_dir(canon)
