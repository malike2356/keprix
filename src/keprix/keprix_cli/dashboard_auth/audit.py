...
Profile-aware location: ``$KEPRIX_HOME/logs/dashboard-auth.log``.
...
from ``keprix_constants`` or other keprix_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$KEPRIX_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``keprix_constants.get_keprix_home`` semantics: env var wins,
    else ``~/.keprix``. A local copy avoids an import cycle with the
    middleware which lives below ``keprix_cli``.
    """
