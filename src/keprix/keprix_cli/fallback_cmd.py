"""
keprix fallback — manage the fallback provider chain.
...
Subcommands:
  keprix fallback [list]   Show the current fallback chain (default when no subcommand)
  keprix fallback add      Pick provider + model via the same picker as `keprix model`,
                           then append the selection to the chain
  keprix fallback remove   Pick an entry to delete from the chain
  keprix fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.keprix/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from keprix_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `keprix model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  keprix fallback add")
        print()
        return
    ...
    print("  Docs: https://keprix.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `keprix model`, then append the selection to the chain."""
    from keprix_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `keprix model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `keprix fallback list` to view, or `keprix fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``keprix fallback [subcommand]``."""
