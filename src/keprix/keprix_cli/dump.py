"""
Dump command for keprix CLI.

Outputs a compact, plain-text summary of the user's Keprix setup
...
from keprix_cli.config import get_keprix_home, get_env_path, get_project_root, load_config
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.keprix_build_sha`` by the Dockerfile's
    ``KEPRIX_GIT_SHA`` build-arg (see ``keprix_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``keprix dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from keprix_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(keprix_home: Path) -> int:
...
def _cron_summary(keprix_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = keprix_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"keprix_home:      {display_keprix_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # A credential added via `keprix auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `keprix auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["keprix-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(keprix_home)}")
    lines.append(f"  skills:             {_count_skills(keprix_home)}")
