...
Pure logic: walks a Keprix config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `keprix doctor` and a future `keprix migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Keprix config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Keprix config and return retirement issues.
