"""On-demand supply-chain audit for Keprix installs.

Scans three surfaces a Keprix user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Keprix venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.keprix/plugins``
...
def _discover_plugins(keprix_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.keprix/plugins``.
...
def run_audit(
    *,
    ...
    home = keprix_home or Path(get_keprix_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, keprix_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(keprix_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `keprix security audit`."""
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, keprix_home=home
    )
    if total == 0:
