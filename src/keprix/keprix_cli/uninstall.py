...
- Keep data: Remove code but keep ~/.keprix/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Keprix PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing keprix or keprix PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Keprix" comment and following line
                if '# Keprix' in line or '# keprix' in line:
                    skip_next = True
                    continue
                if skip_next and ('keprix' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing keprix
                if 'keprix' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the keprix wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "keprix",
        Path("/usr/local/bin/keprix"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains keprix_cli reference)
                content = wrapper.read_text()
                if 'keprix_cli' in content or 'keprix' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(keprix_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``keprix`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Keprix home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (keprix_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``keprix gateway run`` processes
    ...
    try:
        from keprix_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from keprix_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``KEPRIX_HOME`` and ``KEPRIX_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\keprix\git\cmd``, ``%LOCALAPPDATA%\keprix\git\bin``,
#      ``%LOCALAPPDATA%\keprix\git\usr\bin``, ``%LOCALAPPDATA%\keprix\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\keprix\git\`` and Node to
#      ``%LOCALAPPDATA%\keprix\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``keprix dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\keprix\gateway-service\`` and sometimes
...
def _keprix_path_markers(keprix_home: Path) -> list[str]:
    """Path-entry substrings that identify Keprix-owned User-PATH entries."""
    root = str(keprix_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare keprix install dir.
    markers = [root + "\\keprix", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if KEPRIX_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "keprix".  We don't want to catch
    # unrelated entries like "ckeprix-foo" or "ephermeral", so we look for
    # backslash-keprix as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(keprix_home: Path) -> list[str]:
    """Strip Keprix-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _keprix_path_markers(keprix_home)
            kept: list[str] = []
...
def remove_keprix_env_vars_windows() -> list[str]:
    """Delete KEPRIX_HOME and KEPRIX_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("KEPRIX_HOME", "KEPRIX_GIT_BASH_PATH"):
                try:
...
def _is_default_keprix_home(keprix_home: Path) -> bool:
    """Return True when ``keprix_home`` points at the default (non-profile) root."""
    try:
        from keprix_constants import get_default_keprix_root
        return keprix_home.resolve() == get_default_keprix_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its KEPRIX_HOME directory.

    We shell out to ``keprix -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current KEPRIX_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m keprix_cli.main` so we don't depend on a `keprix`
    #    wrapper that may be half-removed mid-uninstall.
    keprix_invocation = [_sys.executable, "-m", "keprix_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                keprix_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's KEPRIX_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``keprix uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$KEPRIX_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from keprix_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(keprix_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│         ⚕ Keprix Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Keprix Chat GUI installation was found.")
        print(f"  Checked: {keprix_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Keprix agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(keprix_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Keprix agent at {keprix_home / 'keprix'}")
        print(f"  • Your config, sessions, and secrets under {keprix_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Keprix agent is still installed. Run 'keprix' to use the CLI,")
    print("or 'keprix uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.keprix/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.keprix/ for future reinstall
    """
    project_root = get_project_root()
    keprix_home = get_keprix_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie KEPRIX_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_keprix_home(keprix_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.keprix data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Keprix Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {keprix_home / 'config.yaml'}")
    print(f"  Secrets: {keprix_home / '.env'}")
    print(f"  Data:    {keprix_home / 'cron/'}, {keprix_home / 'sessions/'}, {keprix_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their KEPRIX_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Keprix data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Keprix code but keep your configuration and data.")
...
def _perform_uninstall(
    *,
    ...
    ``keprix`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$KEPRIX_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in keprix_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\keprix\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(keprix_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Keprix-owned PATH entries in User environment")

        log_info("Removing KEPRIX_HOME / KEPRIX_GIT_BASH_PATH User env vars...")
        removed_env = remove_keprix_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Keprix-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing keprix command...")
    ...
    #     (only when they still point into this Keprix home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Keprix-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(keprix_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Keprix-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(keprix_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE KEPRIX_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from keprix_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(keprix_home)
        if not gui_removed:
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.keprix/, just remove the keprix subdir
            if keprix_home in project_root.parents or project_root.parent == keprix_home:
    ...
    #     under KEPRIX_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(keprix_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(keprix_home)
    ...
    # 5. Optionally remove ~/.keprix/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile KEPRIX_HOME dirs live under
        ...
        try:
            if keprix_home.exists():
                shutil.rmtree(keprix_home)
                log_success(f"Removed {keprix_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {keprix_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {keprix_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  iex (irm https://keprix.nousresearch.com/install.ps1)", Colors.DIM))
        else:
            print(color("  curl -fsSL https://keprix.nousresearch.com/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Keprix! ⚕")
    print()
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m keprix_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import keprix_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``keprix_constants`` + ``keprix_cli.colors``
    (and lazily ``keprix_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m keprix_cli.uninstall")
    parser.add_argument(
        "--mode",
