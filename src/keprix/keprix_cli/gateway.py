"""
Gateway subcommand for keprix CLI.

Handles: keprix gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_keprix_home is imported lazily at call sites to avoid ImportError
# when keprix_constants is cached from a pre-update version during `keprix update`.
...
from keprix_cli.colors import Colors, color
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "keprix CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``keprix gateway status`` from falsely counting the ``keprix`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``keprix gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    exclude_pids = exclude_pids | _get_ancestor_pids()
    pids: list[int] = []
    patterns = [
        "keprix_cli.main gateway",
        "keprix_cli.main --profile",
        "keprix_cli.main -p",
        "keprix_cli/main.py gateway",
        "keprix_cli/main.py --profile",
        "keprix_cli/main.py -p",
        "keprix gateway",
        ...
        # ``keprix.exe --profile`` / ``keprix.exe -p`` would also match
        # ``keprix.exe --profile foo dashboard`` and other CLI subcommands,
        # producing false-positive gateway PIDs (Copilot review).
        "keprix.exe gateway",
        "keprix-gateway.exe",
        "gateway/run.py",
    ]
    current_home = str(get_keprix_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"keprix_home={current_home_lc}" in command_lc
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. KEPRIX_HOME
        ...
        # KEPRIX_HOME= in argv is.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "keprix_home=" in command_lc
            and f"keprix_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``keprix update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Keprix profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Keprix profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid
        from keprix_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "keprix_cli.main"]
    if profile != "default":
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # when the user closes the terminal, leaving ``keprix update`` users
    # with no running gateway until they re-invoke ``keprix gateway``
    ...
    from keprix_cli._subprocess_compat import (
...
def _sync_keprix_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``KEPRIX_HOME``.

    Under ``sudo``, ``KEPRIX_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_keprix_home` falls back to ``/root/.keprix`` — the wrong
    profile. The unit file pins ``KEPRIX_HOME`` for the actual gateway
    ...
    unit_home = env.get("KEPRIX_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("KEPRIX_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["KEPRIX_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}keprix gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}keprix gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    from keprix_constants import is_container
    ...
    if is_linux() and is_container():
        ...
        try:
            from keprix_cli.service_manager import detect_service_manager, get_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `keprix gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``keprix gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from keprix_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (KEPRIX_HOME-scoped).
...
from keprix_constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Keprix Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``keprix gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``KEPRIX_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("KEPRIX_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "keprix-gateway"
SERVICE_DESCRIPTION = "Keprix Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current KEPRIX_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (KEPRIX_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    ...
    default = get_default_keprix_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary KEPRIX_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(keprix_home: str | None = None, default_root: str | Path | None = None) -> str:
    """Return ``--profile <name>`` only when KEPRIX_HOME is a named profile.

    For ``~/.keprix/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        keprix_home: Optional explicit KEPRIX_HOME path. Defaults to the current
            ``get_keprix_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
        default_root: Optional Keprix root to compare against. Used when
            generating a system service for another user from a sudo/root
            process, where ``Path.home()`` and ``get_default_keprix_root()``
            refer to root but the target profile lives under the service user.
    """
    import re
    from keprix_constants import get_default_keprix_root

    home = Path(keprix_home or str(get_keprix_home())).resolve()
    default = Path(default_root).resolve() if default_root else get_default_keprix_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
    try:
...
def _profile_arg_for_target_user(keprix_home: str, target_home_dir: str) -> str:
    """Return the profile arg for a system service running as another user."""
    target_root = Path(target_home_dir) / ".keprix"
    try:
        Path(keprix_home).resolve().relative_to(target_root.resolve())
        return _profile_arg(keprix_home, default_root=target_root)
    except ValueError:
        return _profile_arg(keprix_home)


def get_service_name() -> str:
    """Derive a systemd service name scoped to this KEPRIX_HOME.

    Default ``~/.keprix`` returns ``keprix-gateway`` (backward compatible).
    Profile ``~/.keprix/profiles/coder`` returns ``keprix-gateway-coder``.
    Any other KEPRIX_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Keprix installs that predate the
# keprix-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (keprix-gateway-*.service) and unrelated third-party
# "keprix" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("keprix.service",)
...
def _find_legacy_keprix_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Keprix gateway units.

    Detects unit files installed by older Keprix versions that used a
    different service name (e.g. ``keprix.service`` before the rename to
    ``keprix-gateway.service``). When both a legacy unit and the current
    ``keprix-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``keprix-gateway-coder.service`` and unrelated third-party
      ``keprix-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``keprix.service`` running an unrelated
...
def has_legacy_keprix_units() -> bool:
    """Return True when any legacy Keprix gateway unit files exist."""
    return bool(_find_legacy_keprix_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Keprix gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_keprix_units()
    if not legacy:
        return
    print_warning("Legacy Keprix gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current keprix-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    keprix gateway migrate-legacy")


def remove_legacy_keprix_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Keprix gateway unit files.

    Iterates over whatever ``_find_legacy_keprix_units()`` returns — which is
    ...
    legacy = _find_legacy_keprix_units()
    if not legacy:
        print("No legacy Keprix gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Keprix gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: keprix gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo keprix gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    keprix gateway uninstall")
    print_info("    sudo keprix gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning(
                "  System service install requires sudo, so Keprix can't create it from this user session."
            )
            if run_as_user:
                print_info(
                    f"  After setup, run: sudo keprix gateway install --system --run-as-user {run_as_user}"
                )
            else:
                print_info(
                    "  After setup, run: sudo keprix gateway install --system --run-as-user <your-user>"
                )
            print_info("  Then start it with: sudo keprix gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Keprix often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.keprix`` → ``ai.keprix.gateway.plist`` (backward compatible).
    Profile ``~/.keprix/profiles/coder`` → ``ai.keprix.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.keprix.gateway-{suffix}" if suffix else "ai.keprix.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.keprix/keprix  -> /home/alice/.keprix/keprix
      /opt/keprix                 -> /opt/keprix  (kept as-is)
...
def _keprix_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current KEPRIX_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_keprix_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.keprix                    → /home/alice/.keprix
      /root/.keprix/profiles/coder     → /home/alice/.keprix/profiles/coder
      /opt/custom-keprix               → /opt/custom-keprix  (kept as-is)
    """
    current_keprix = get_keprix_home().resolve()
    current_default = (Path.home() / ".keprix").resolve()
    target_default = Path(target_home_dir) / ".keprix"

    # Default ~/.keprix → remap to target user's default
    if current_keprix == current_default:
        return str(target_default)

    # Profile or subdir of ~/.keprix → preserve the relative structure
    try:
        relative = current_keprix.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.keprix) — keep as-is
        return str(current_keprix)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    keprix_node = keprix_home / "node" / "bin"
    if _is_dir(keprix_node):
        candidates.append(str(keprix_node))
    keprix_nm = keprix_home / "node_modules" / ".bin"
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m keprix_cli.main``, so module
    ...
    dir, or a clone that ``keprix update`` later relocates/removes — the path
    ...
    ``KEPRIX_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if KEPRIX_HOME
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        # Anchor cwd to the target user's KEPRIX_HOME (stable, always exists)
        # rather than a remapped source-checkout path that can rot. See
        # _stable_service_working_dir() for the full rationale.
        working_dir = str(keprix_home) if keprix_home else _remap_path_for_user(working_dir, home_dir)
...
User={username}
Group={group_name}
ExecStart={python_path} -m keprix_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir KEPRIX_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved KEPRIX_HOME
    ...
    exported a throwaway ``KEPRIX_HOME=/tmp/...`` — writing it to the real
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``keprix gateway restart``
    with ``KEPRIX_HOME=/tmp/keprix-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="KEPRIX_HOME=..."`` lines and launchd
    ``<key>KEPRIX_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'KEPRIX_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>KEPRIX_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp KEPRIX_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: KEPRIX_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported KEPRIX_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only KEPRIX_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp KEPRIX_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../keprix_test`` path into
    # ``Environment="KEPRIX_HOME=..."``. Writing that to the developer's
    ...
    if not system and (
        ...
        or '/keprix_test"' in new_unit
        or "/keprix_test/" in new_unit
    ):
        return False

    # Structural variant of the same belt: refuse to bake ANY temp-dir
    # KEPRIX_HOME into the unit (manual E2E homes like /tmp/keprix-e2e-NNN
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Keprix install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo keprix gateway uninstall --system")
    print_info("         keprix gateway install")
    print_info("         keprix gateway start")


def _get_restart_drain_timeout() -> float:
    """Return the configured gateway restart drain timeout in seconds."""
    raw = os.getenv("KEPRIX_RESTART_DRAIN_TIMEOUT", "").strip()
    if not raw:
        cfg = read_raw_config()
...
def systemd_install(
    ...
    # Offer to remove legacy units (keprix.service from pre-rename installs)
    # before installing the new keprix-gateway.service. If both remain, they
    ...
    if has_legacy_keprix_units():
        print()
        print_legacy_unit_warning()
        print()
        if prompt_yes_no("Remove the legacy unit(s) before installing?", True):
            remove_legacy_keprix_units(interactive=False)
            print()
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}keprix gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}keprix gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}keprix gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    _sync_keprix_home_from_systemd_unit(system=system)
    try:
    ...
    try:
        ...
        print(
            ...
            "check `keprix gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `keprix gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}keprix gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}keprix gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}keprix gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}keprix gateway start{scope_flag}"
        )
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.keprix.gateway-{suffix}" if suffix else "ai.keprix.gateway"


# Cached launchd domain result — probing is cheap but should only run once per
# process invocation (each ``keprix gateway start/stop/status`` call).
...
# (the documented `nohup keprix gateway run` workaround). See #23387.
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m keprix_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active KEPRIX_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "keprix_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup keprix gateway run --replace`
    ...
    """
    from keprix_cli._subprocess_compat import windows_detach_popen_kwargs
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    if _spawn_detached_gateway():
        ...
        print("  Stop it with: keprix gateway stop")
        return True
    print_error("Failed to start the gateway as a background process.")
    print(
        f"  Try manually: nohup keprix gateway run --replace "
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Keprix install"
    )
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  keprix gateway status             # Check status")
    from keprix_constants import display_keprix_home as _dhh
...
def launchd_stop():
    ...
    # `keprix gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Keprix install")
    else:
        print("⚠ Service definition is stale relative to the current Keprix install")
        print("  Run: keprix gateway start")
    ...
    if deep:
        log_file = get_keprix_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _running_under_gateway_supervisor() -> bool:
    ...
    a reliable marker into the child's environment:
      ...
      - the s6-overlay container longrun exports ``KEPRIX_S6_SUPERVISED_CHILD``.
    """
    if os.environ.get("INVOCATION_ID"):
        return True
    if os.environ.get("KEPRIX_S6_SUPERVISED_CHILD"):
        return True
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``keprix gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``keprix gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other KEPRIX_HOME roots.
    """
    ...
    print("  Use 'keprix gateway restart' to replace it,")
    print("  or 'keprix gateway stop' first.")
    print("  Or use 'keprix gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("KEPRIX_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Keprix gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'keprix' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/keprix/docker/entrypoint.sh before the Keprix command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$KEPRIX_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set KEPRIX_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `keprix gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set KEPRIX_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `keprix gateway restart` which already
    ...
    # `keprix gateway start/restart` — leaving the gateway vulnerable to
    ...
    # KEPRIX_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("KEPRIX_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
            from keprix_constants import get_keprix_home as _ghh
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Keprix will log in directly",
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Full Matrix user ID, e.g. @keprix:matrix.example.org",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. keprix) and copy the bot token",
        ...
        "vars": [
            ...
            },
            {
                ...
                "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)",
                "password": False,
                "help": "Channel ID where Keprix delivers cron results and notifications.",
            },
            {
        ...
        "vars": [
            {
                ...
                "password": False,
                "help": "The email address Keprix will use (e.g., keprix@gmail.com).",
            },
            {
                "name": "EMAIL_PASSWORD",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Keprix connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: keprix pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Keprix will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``keprix setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run keprix under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.keprix/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_keprix_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_standard_platform(platform: dict):
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from keprix_cli.telegram_managed_bot import (
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'keprix pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: keprix pairing approve <platform> <code>"
                    )
                else:
                    print_info(
                        "  Skipped — configure later with 'keprix gateway setup'"
                    )
            continue
...
def _setup_whatsapp():
    ...
    from keprix_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from keprix_cli.dingtalk_auth import dingtalk_qr_auth
...
def _setup_wecom():
    ...
    if allowed:
        ...
        if access_idx == 0:
            ...
            print_info("  Approve with: keprix pairing approve <platform> <code>")
        elif access_idx == 2:
            save_env_value("WECOM_DM_POLICY", "disabled")
            print_warning("  Direct messages disabled.")
        else:
            print_info("  Skipped — configure later with 'keprix gateway setup'")
...
def _is_service_installed() -> bool:
    ...
    if supports_systemd_services():
        ...
        from keprix_cli import gateway_windows
...
def _setup_weixin():
    ...
    print_info(
        "  3. Keprix will store the returned account_id/token in ~/.keprix/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `keprix gateway setup`.")
        return
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_keprix_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `keprix pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `keprix pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "KeprixAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``keprix_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from keprix_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.keprix/plugins/
...
def gateway_setup():
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: keprix gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: keprix gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: keprix gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo keprix gateway install --system"
                        )
                    print_info("  Or run in foreground:  keprix gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: keprix gateway run")
                print_info(
                    "  For persistence:   tmux new -s keprix 'keprix gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: keprix gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup keprix gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'keprix gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`keprix_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from keprix_cli.service_manager import (
    ...
    if profile is None:
        ...
        # KEPRIX_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``keprix gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``keprix gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``keprix gateway run`` are unaffected.
      2. ``KEPRIX_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``keprix gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``KEPRIX_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("KEPRIX_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("KEPRIX_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${KEPRIX_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if KEPRIX_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or KEPRIX_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    #
    # Prefer `sleep infinity` (matches the static main-keprix service's
    # pattern in docker/s6-rc.d/main-keprix/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: keprix gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: keprix gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s keprix 'keprix gateway run'"
                )
                print()
            ...
            print()
            print(
                "  keprix gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s keprix 'keprix gateway run'         # persistent via tmux"
            )
            print(
                "  nohup keprix gateway run > ~/.keprix/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  keprix profile create <name>     # creates the s6 service slot")
                print("  keprix -p <name> gateway start   # bring it up via s6")
                print("  keprix status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: keprix gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: keprix gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: keprix gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  keprix profile delete <name>     # tears down the s6 service slot")
                print("  keprix -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `keprix -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: keprix gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_KEPRIX_GATEWAY") == "1":
            print_error(
                "Refusing to stop the gateway from inside the gateway process.\n"
                "This command was blocked to prevent restart loops.\n"
                "Use `keprix gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_KEPRIX_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `keprix gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: keprix gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  keprix gateway install")
                else:
                    print("To install as a service:")
                    print("  keprix gateway install")
                    print("  sudo keprix gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  keprix gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup keprix gateway run > ~/.keprix/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s keprix 'keprix gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  keprix gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  keprix gateway install  # Install as user service")
                    print(
                        "  sudo keprix gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy Keprix gateway unit files from
        # pre-rename installs (e.g. keprix.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_keprix_units(interactive=not yes, dry_run=dry_run)
