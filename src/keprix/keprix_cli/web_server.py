"""
Keprix — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m keprix_cli.main web          # Start on http://127.0.0.1:9119
    python -m keprix_cli.main web --port 8080
"""
...
try:
    ...
    # running `keprix dashboard` needs fastapi+uvicorn; lazy install keeps
...
from keprix.api.audio_transcribe import AudioTranscriptionRequest, transcribe_audio_upload as shared_transcribe_audio_upload

WEB_DIST = Path(os.environ["KEPRIX_WEB_DIST"]) if "KEPRIX_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``keprix gateway run`` — but the
    desktop app spawns a ``keprix dashboard`` backend, not a gateway, so a cron
    ...
    KEPRIX_HOME — whichever process grabs the lock first wins the tick.
    """
...
async def _lifespan(app: "FastAPI"):
    ...
    # Desktop-spawned backends (KEPRIX_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `keprix
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("KEPRIX_DESKTOP") == "1":
...
app = FastAPI(title="Keprix", version=__version__, lifespan=_lifespan)

# ---------------------------------------------------------------------------
# Session token for protecting sensitive endpoints (reveal).
# The desktop shell mints the token and injects it via
# KEPRIX_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
# ---------------------------------------------------------------------------
_SESSION_TOKEN = os.environ.get("KEPRIX_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
_SESSION_HEADER_NAME = "X-Keprix-Session-Token"
...
#
# This list is defined in ``keprix_cli.dashboard_auth.public_paths`` so the
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Keprix-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
async def _dashboard_auth_gate(request: Request, call_next):
    from keprix_cli.dashboard_auth.middleware import gated_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Keprix (no terminal prompt), "
...
class ModelAssignment(BaseModel):
    ...
    # ``keprix model`` custom flow collects. Honored only on the main slot for
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Keprix provider slug — fine.
    ...
    1. Vendor-name → Keprix-provider mapping: when the provider string is
       not a known Keprix provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    """
    from keprix_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from keprix_cli.model_normalize import normalize_model_for_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and "/" in model_in:
        ...
        try:
            ...
            cur_provider = ""
        from keprix_cli.models import _AGGREGATOR_PROVIDERS
...
_MANAGED_FILES_ROOT_ENV = "KEPRIX_DASHBOARD_FILES_ROOT"
...
def _default_keprix_root_is_opt_data() -> bool:
    ...
    try:
        ...
        root = get_default_keprix_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``keprix update``.
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's Keprix root is actually /opt/data
    # (the container/hosted layout) or when KEPRIX_DASHBOARD_FILES_ROOT is set.
    if _default_keprix_root_is_opt_data():
...
async def get_status():
    ...
    # and which providers are registered so ``keprix status`` and the
    # SPA's StatusPage can show "OAuth gate ON via Nous Research" or
    # "loopback only — no auth gate" with no extra round trips.
    auth_required = bool(getattr(app.state, "auth_required", False))
    auth_providers: list[str] = []
    try:
        from keprix_cli.dashboard_auth import list_providers as _list_providers
    ...
    status = {
        ...
        "can_update_keprix": not _dashboard_local_update_managed_externally(),
    ...
    if not auth_required:
        status.update({
            "keprix_home": str(get_keprix_home()),
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the keprix home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_keprix_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def run_curator():
    ...
    try:
        proc = _spawn_keprix_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
async def get_portal_status():
    ...
    auth: Dict[str, Any] = {}
    try:
        from keprix_cli.auth import get_nous_auth_status
    ...
    features = []
    try:
        from keprix_cli.nous_subscription import get_nous_subscription_features
...
async def run_prompt_size():
    try:
        proc = _spawn_keprix_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_keprix_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_keprix_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from keprix_cli.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.keprix/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_keprix_home() / "logs"
...
_ACTION_LOG_FILES: Dict[str, str] = {
    ...
    "keprix-update": "keprix-update.log",
...
def _spawn_keprix_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``keprix <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``keprix_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "keprix_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "KEPRIX_NONINTERACTIVE": "1"},
    }
...
def _spawn_gateway_restart() -> Tuple[subprocess.Popen, bool]:
    """Spawn ``keprix gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``keprix gateway restart`` children race each other on the
    ...
    return _spawn_keprix_action(["gateway", "restart"], "gateway-restart"), False
...
async def restart_gateway():
    """Kick off a ``keprix gateway restart`` in the background."""
    try:
...
@app.post("/api/keprix/update")
async def update_keprix():
    """Kick off ``keprix update`` in the background."""
    if _dashboard_local_update_managed_externally():
        message = (
            "Keprix updates are managed outside this dashboard in "
            "containerized environments. The built-in local updater is "
            "disabled here."
        )
        _record_completed_action("keprix-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "keprix-update",
    ...
    try:
        proc = _spawn_keprix_action(["update"], "keprix-update")
    except Exception as exc:
        _log.exception("Failed to spawn keprix update")
...
@app.get("/api/keprix/update/check")
async def check_keprix_update(force: bool = False):
    """Report whether a Keprix update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/keprix/update`` actually runs ``keprix update``.

    Returns:
        install_method: 'git' | 'pip' | 'docker' | 'nixos' | 'homebrew' | ...
        current_version: installed Keprix version string
    ...
    try:
        ...
        if force:
            try:
                (get_keprix_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def transcribe_audio_upload(payload: AudioTranscriptionRequest):
    return await shared_transcribe_audio_upload(payload)
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.keprix/config.yaml`` under ``tts.``.
    """
...
async def get_profiles_sessions(
    ...
    from keprix_state import SessionDB
    from keprix_cli import profiles as profiles_mod
...
async def search_sessions(q: str = "", limit: int = 20, profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            ...
            # logs, or another Keprix surface. FTS can't find those unless the
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Keprix supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# in keprix_cli/config.py — listed here for deterministic ordering in the UI.
...
def get_model_options(profile: Optional[str] = None):
    ...
    """
    try:
        from keprix_cli.inventory import build_models_payload, load_picker_context
        ...
        # onboarding picker) sees the SAME full provider universe `keprix model`
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `keprix model` does so GUI onboarding lands on a
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.keprix/config.yaml`` — applies to **new** sessions only.
    ...
    try:
        ...
        if model and not body.confirm_expensive_model:
            try:
                from keprix_cli.model_cost_guard import expensive_model_warning
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # post-model-selection behaviour (keprix_cli/main.py
        ...
        if provider.strip().lower() == "nous":
            try:
                from keprix_cli.nous_subscription import apply_nous_managed_defaults
        ...
        # endpoint, mirroring the ``keprix model`` custom flow
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from keprix_cli.main import _auto_provider_name, _save_custom_provider
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Keprix from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Keprix to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Keprix from Slack via Socket Mode.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Keprix to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Keprix in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Keprix through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Keprix via Home Assistant.",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Keprix to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Keprix inside Feishu / Lark.",
    ...
    "bluebubbles": {
        ...
        "description": "Use Keprix through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Keprix to a QQ Bot from the QQ Open Platform.",
    ...
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Keprix to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in keprix gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in keprix gateway setup",
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Keprix").strip() or "Keprix"
...
def _restart_gateway_after_telegram_onboarding() -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Keprix appears
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # all resolve against the requested profile's KEPRIX_HOME.
...
# the canonical ``keprix auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.keprix/.anthropic_oauth.json`` — Keprix-managed PKCE flow (what
    ...
    2. ``ANTHROPIC_API_KEY`` → ``ANTHROPIC_TOKEN`` → ``CLAUDE_CODE_OAUTH_TOKEN``
       ...
       env during ``load_keprix_dotenv()``, so the same check covers them)
    ...
    try:
        from agent.anthropic_adapter import (
            read_keprix_oauth_credentials,
            _KEPRIX_OAUTH_FILE,
        )
    except ImportError:
        read_keprix_oauth_credentials = None  # type: ignore
        _KEPRIX_OAUTH_FILE = None  # type: ignore

    keprix_creds = None
    if read_keprix_oauth_credentials:
        try:
            keprix_creds = read_keprix_oauth_credentials()
        except Exception:
            keprix_creds = None
    if keprix_creds and keprix_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "keprix_pkce",
            "source_label": f"Keprix PKCE ({_KEPRIX_OAUTH_FILE})",
            "token_preview": _truncate_token(keprix_creds.get("accessToken")),
            "expires_at": keprix_creds.get("expiresAt"),
            "has_refresh_token": bool(keprix_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Keprix even
    when they also have a separate Keprix-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "keprix auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "keprix auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "keprix auth add minimax-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "keprix auth add anthropic",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from keprix_cli import auth as hauth
        if provider_id == "nous":
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("keprix_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        # Anthropic clears only the Keprix-managed PKCE file and auth-store entry.
        ...
        if provider_id == "anthropic":
            cleared = False
            try:
                from agent.anthropic_adapter import _KEPRIX_OAUTH_FILE
                if _KEPRIX_OAUTH_FILE.exists():
                    _KEPRIX_OAUTH_FILE.unlink()
                    cleared = True
            except Exception:
                pass
            # Also clear the credential pool entry if present.
            try:
                from keprix_cli.auth import clear_provider_auth
        ...
        try:
            from keprix_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
#          → persists to ~/.keprix/.anthropic_oauth.json AND credential pool
...
# Guarded so keprix web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Keprix file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``keprix auth add anthropic``.
    """
    from agent.anthropic_adapter import _KEPRIX_OAUTH_FILE
    payload = {
    ...
    _KEPRIX_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = _KEPRIX_OAUTH_FILE.with_name(
        f"{_KEPRIX_OAUTH_FILE.name}.tmp.{os.getpid()}.{secrets.token_hex(8)}"
    )
    try:
        ...
        os.replace(tmp_path, _KEPRIX_OAUTH_FILE)
        try:
            _KEPRIX_OAUTH_FILE.chmod(stat.S_IRUSR | stat.S_IWUSR)
        except OSError:
            pass
    finally:
        try:
...
def _submit_anthropic_pkce(
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "keprix-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
async def _start_device_code_flow(
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("KEPRIX_PORTAL_BASE_URL")
...
# the tokens exactly like `keprix auth add xai-oauth`.
...
def _xai_loopback_worker(session_id: str) -> None:
    ...
    try:
        ...
        base_url = hauth._xai_validate_inference_base_url(
            os.getenv("KEPRIX_XAI_BASE_URL", "").strip().rstrip("/")
...
def _add_xai_oauth_pool_entry(
    ...
    """Mirror `keprix auth add xai-oauth`'s credential-pool insert.
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from keprix_cli.auth import persist_nous_credentials
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from keprix_cli.auth import _save_codex_tokens
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics for the Sessions page (mirrors `keprix sessions stats`).
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions older than N days (mirrors `keprix sessions prune`)."""
    if body.older_than_days < 1:
        raise HTTPException(status_code=400, detail="older_than_days must be >= 1")
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_keprix_home()
    db = _open_session_db_for_profile(body.profile)
    try:
...
async def get_logs(
    ...
    from keprix_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_keprix_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from keprix_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, KEPRIX_HOME)."""
    from keprix_cli import profiles as profiles_mod
...
def _annotate_cron_job(job: Dict[str, Any], profile: str, home: Path) -> Dict[str, Any]:
    ...
    annotated["keprix_home"] = str(home)
...
def _call_cron_for_profile(profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from the process KEPRIX_HOME at import time. The dashboard is a single
...
from keprix_cli.mcp_admin_routes import router as _mcp_admin_router  # noqa: E402
...
#
# Wraps the same JSON store the CLI uses (keprix_cli.webhook); the webhook
...
async def list_webhooks():
    import keprix_cli.webhook as wh
...
# spawn the real `keprix gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway():
    try:
        proc = _spawn_keprix_action(["gateway", "start"], "gateway-start")
    except Exception as exc:
...
async def stop_gateway():
    try:
        proc = _spawn_keprix_action(["gateway", "stop"], "gateway-stop")
    except Exception as exc:
...
async def list_credential_pool():
    ...
    from keprix_cli.auth import read_credential_pool
...
# `keprix memory setup`).  The dashboard covers the common admin actions:
...
async def set_memory_provider(body: MemoryProviderSelect):
    ...
    if provider:
        ...
        if provider not in valid:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown memory provider '{provider}'. Run `keprix memory setup` to configure a new one.",
            )
...
async def run_doctor():
    try:
        proc = _spawn_keprix_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_keprix_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_keprix_action(args, "backup")
    except Exception as exc:
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `keprix import`. The spawned action runs with
...
async def run_import(body: ImportRequest):
    ...
    if body.force:
        args.append("--force")
    try:
        proc = _spawn_keprix_action(args, "import")
    except Exception as exc:
...
async def list_hooks():
    ...
    form can offer them.
    """
    from keprix_cli.config import load_config as _load_config
    from agent import shell_hooks

    try:
        from keprix_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <keprix_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_keprix_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_keprix_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``keprix`` subprocess, and
...
async def preview_skill_hub(identifier: str = ""):
    ...
    def _run():
        from keprix_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`keprix -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the KEPRIX_HOME
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_keprix_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "keprix setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local KEPRIX_HOME override so the write lands in the target
    ...
    """
    from keprix_constants import set_keprix_home_override, reset_keprix_home_override

    token = set_keprix_home_override(str(profile_dir))
    try:
        provider, model = _normalize_main_model_assignment(provider, model)
        cfg = load_config()
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local KEPRIX_HOME override (same mechanism as
    ...
    """
    from keprix_constants import set_keprix_home_override, reset_keprix_home_override
    from keprix_cli.mcp_security import validate_mcp_server_entry
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the KEPRIX_HOME override. Returns the
    number of skills newly disabled.
    """
    from keprix_constants import set_keprix_home_override, reset_keprix_home_override
    from keprix_cli.skills_config import get_disabled_skills, save_disabled_skills
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's KEPRIX_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``keprix profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from KEPRIX_HOME).
    """
    from keprix_cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``keprix profile use``).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the KEPRIX_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``keprix profile describe
    <name> --auto``.
    ...
    _resolve_profile_dir(name)
    try:
        from keprix_cli import profile_describer
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_keprix_home()`` at call
       time — the context-local override from ``set_keprix_home_override``
    ...
    retargeted to the *current* ``get_keprix_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a KEPRIX_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
async def get_skills(profile: Optional[str] = None):
    ...
    from keprix_cli.skills_config import get_disabled_skills
    with _profile_scope(profile):
        config = load_config()
...
async def get_toolsets(profile: Optional[str] = None):
    from keprix_cli.tools_config import (
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    helper the CLI ``keprix tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``keprix tools`` picker shows
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``keprix tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.keprix/.env`` via ``save_env_value`` —
    the same store ``keprix tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``keprix tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``keprix -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same KEPRIX_HOME the rest of the
...
#
# The endpoint spawns the same ``keprix --tui`` binary the CLI uses, behind
...
if sys.platform.startswith("win"):
    try:
        from keprix_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from keprix_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from keprix_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Default: whatever ``keprix --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``KEPRIX_TUI_RESUME`` env var —
    matching what ``keprix_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``KEPRIX_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``KEPRIX_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `profile` (when set) scopes the ENTIRE chat to that profile by pointing
    ``KEPRIX_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_keprix_home()`` from that env var at its own import, so the child
    binds the profile's config, skills, memory, and state.db from the start
    — the same propagation ``keprix -p <name>`` performs. The in-process
    ``KEPRIX_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    """
    from keprix_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("KEPRIX_TUI_DISABLE_MOUSE", "1")
    env.setdefault("KEPRIX_TUI_INLINE", "1")

    if profile_dir is not None:
        env["KEPRIX_HOME"] = str(profile_dir)

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["KEPRIX_TUI_RESUME"] = resume

    if sidecar_url:
        env["KEPRIX_TUI_SIDECAR_URL"] = sidecar_url
    ...
    # inherits the profile KEPRIX_HOME set above.
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["KEPRIX_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from keprix_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Keprix inside WSL2 to use the dashboard's /chat "
...
# KEPRIX_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`keprix_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from keprix_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/keprix/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /keprix`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__KEPRIX_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/keprix``)
        ...
        ``__KEPRIX_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__KEPRIX_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__KEPRIX_BASE_PATH__="{prefix}";'
                f"window.__KEPRIX_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__KEPRIX_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__KEPRIX_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__KEPRIX_BASE_PATH__="{prefix}";'
                f"window.__KEPRIX_AUTH_REQUIRED__={gated_js};"
                f"</script>"
    ...
    # under ``/keprix`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Keprix backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Keprix Teal",         "description": "Classic dark teal — the canonical Keprix look"},
    {"name": "default-large", "label": "Keprix Teal (Large)", "description": "Keprix Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.keprix/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.keprix/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_keprix_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.keprix/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as keprix_cli.plugins):
    1. User plugins:    ~/.keprix/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.keprix/plugins/  (only if KEPRIX_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from keprix_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_keprix_home() / "plugins", "user"),
    ...
    # ``keprix_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("KEPRIX_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".keprix" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from keprix_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_keprix_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"keprix auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from keprix_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from keprix_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from keprix_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from keprix_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.keprix/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.keprix/plugins/ if you trust it)",
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"keprix_dashboard_plugin_{plugin['name']}"
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        from keprix_cli.dashboard_auth import list_providers
        if not list_providers():
            # Surface the *specific* reason any bundled provider declined
            # to register (e.g. missing KEPRIX_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Keprix exposes a
    ...
    # KEPRIX_DASHBOARD_READY, open the browser, *then* serve.
    #
