"""keprix hooks — inspect and manage shell-script hooks.

Usage::

    keprix hooks list
    keprix hooks test <event> [--for-tool X] [--payload-file F]
    keprix hooks revoke <command>
    keprix hooks doctor

Consent records live under ``~/.keprix/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.keprix/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``keprix hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: keprix hooks {list|test|revoke|doctor}")
        print("Run 'keprix hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.keprix/config.yaml.")
        print("See `keprix hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `keprix hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `keprix hooks test` and `keprix hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Keprix wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `keprix hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `keprix hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `keprix hooks doctor`.")
