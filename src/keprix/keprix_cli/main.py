#!/usr/bin/env python3
"""
Keprix CLI - Main entry point.

Usage:
    keprix                     # Interactive chat (default)
    keprix chat                # Interactive chat
    keprix gateway             # Run gateway in foreground
    keprix gateway start       # Start gateway as service
    keprix gateway stop        # Stop gateway service
    keprix gateway status      # Show gateway status
    keprix gateway install     # Install gateway service
    keprix gateway uninstall   # Uninstall gateway service
    keprix setup               # Interactive setup wizard
    keprix logout              # Clear stored authentication
    keprix status              # Show status of all components
    keprix cron                # Manage cron jobs
    keprix cron list           # List cron jobs
    keprix cron status         # Check if cron scheduler is running
    keprix doctor              # Check configuration and dependencies
    keprix honcho setup                    # Configure Honcho AI memory integration
    keprix honcho status                   # Show Honcho config and connection status
    keprix honcho sessions                 # List directory → session name mappings
    keprix honcho map <name>               # Map current directory to a session name
    keprix honcho peer                     # Show peer names and dialectic settings
    keprix honcho peer --user NAME         # Set user peer name
    keprix honcho peer --ai NAME           # Set AI peer name
    keprix honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    keprix honcho mode                     # Show current memory mode
    keprix honcho mode [hybrid|honcho|local]  # Set memory mode
    keprix honcho tokens                   # Show token budget settings
    keprix honcho tokens --context N       # Set session.context() token cap
...
# IMPORTANT: keprix_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``keprix_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``keprix update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``keprix_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, keprix crashes on import and the user can't run
# ``keprix update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import keprix_bootstrap  # noqa: F401
...
def _set_process_title() -> None:
    """Set the process title to 'keprix' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``keprix tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``keprix.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("keprix")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"keprix", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"keprix")
        # Windows: the .exe name is already ``keprix.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* keprix_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("KEPRIX_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".keprix", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    ``--tui``/``KEPRIX_TUI=1``, then ``display.interface`` in config.
    """
    if argv is None:
        argv = sys.argv[1:]
    if "--cli" in argv:
        return False
    if os.environ.get("KEPRIX_TUI") == "1" or "--tui" in argv:
        return True
...
# `entry.tsx`; this is just the earlier cousin. ``KEPRIX_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("KEPRIX_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`keprix --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    ...
    print(f"Keprix v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``keprix --version`` before config/logging imports on Termux."""
    if os.environ.get("KEPRIX_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
from keprix_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from keprix_cli.subcommands.cron import build_cron_parser
from keprix_cli.subcommands.gateway import build_gateway_parser
from keprix_cli.subcommands.profile import build_profile_parser
from keprix_cli.subcommands.model import build_model_parser
from keprix_cli.subcommands.setup import build_setup_parser
from keprix_cli.subcommands.postinstall import build_postinstall_parser
from keprix_cli.subcommands.whatsapp import build_whatsapp_parser
from keprix_cli.subcommands.slack import build_slack_parser
from keprix_cli.subcommands.login import build_login_parser
from keprix_cli.subcommands.logout import build_logout_parser
from keprix_cli.subcommands.auth import build_auth_parser
from keprix_cli.subcommands.status import build_status_parser
...
from keprix_cli.subcommands.update import build_update_parser
from keprix_cli.subcommands.uninstall import build_uninstall_parser
from keprix_cli.subcommands.dashboard import build_dashboard_parser
from keprix_cli.subcommands.gui import build_gui_parser
from keprix_cli.subcommands.logs import build_logs_parser
from keprix_cli.subcommands.prompt_size import build_prompt_size_parser
from keprix_cli.subcommands.memory import build_memory_parser
from keprix_cli.subcommands.acp import build_acp_parser
from keprix_cli.subcommands.tools import build_tools_parser
from keprix_cli.subcommands.insights import build_insights_parser
...
from keprix_cli.subcommands.crm_funnel import build_crm_funnel_parser
from keprix_cli.subcommands.document_vault import build_document_vault_parser
...
from keprix_cli import crm_funnel_commands
from keprix_cli import document_vault_commands
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (keprix tools, keprix setup, keprix model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'keprix {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any keprix module import.
#
# Many modules cache KEPRIX_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("KEPRIX_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.keprix/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set KEPRIX_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `keprix mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Keprix' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo keprix -p <name>` against the invoking user's home.
        ...
        candidate = home / ".keprix" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # after the subcommand (`keprix chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors keprix_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If KEPRIX_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.keprix/profiles/coder or
    # /opt/data/profiles/coder).  If KEPRIX_HOME points to the keprix root
    # instead (e.g. systemd hardcodes KEPRIX_HOME=/root/.keprix), we must
    # still read active_profile — the user may have switched profiles via
    # `keprix profile use` and the gateway should honour that choice.
    # See issue #22502.
    keprix_home_env = os.environ.get("KEPRIX_HOME", "")
    if profile_name is None and keprix_home_env:
        if Path(keprix_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the keprix root.
    #
    # EXCEPTION: a supervised s6 gateway child (exported by the container
    # run-script as KEPRIX_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # reserved ``gateway-default`` slot runs bare ``keprix gateway run`` to mean
    # "the root KEPRIX_HOME profile". If the reserved default child read
    ...
    if profile_name is None and not os.environ.get("KEPRIX_S6_SUPERVISED_CHILD"):
        try:
            from keprix_constants import get_default_keprix_root

            active_path = get_default_keprix_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set KEPRIX_HOME
    if profile_name is not None:
        try:
            from keprix_cli.profiles import resolve_profile_env

            keprix_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            keprix_home = _resolve_sudo_user_profile_env(profile_name)
            if not keprix_home:
            ...
            # A bug in profiles.py must NEVER prevent keprix from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["KEPRIX_HOME"] = keprix_home
...
# Load .env from ~/.keprix/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from keprix_cli.config import get_keprix_home
from keprix_cli.env_loader import load_keprix_dotenv

load_keprix_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → KEPRIX_REDACT_SECRETS env
# var BEFORE keprix_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_keprix_home() / "config.yaml"
    if _cfg_path.exists():
        with open(_cfg_path, encoding="utf-8") as _f:
            _early_cfg_raw = _yaml_early.safe_load(_f) or {}
        if "KEPRIX_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["KEPRIX_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `keprix` subcommands
...
try:
    from keprix_logging import setup_logging as _setup_logging
...
if _FORCE_IPV4_EARLY:
    try:
        from keprix_constants import apply_ipv4_preference as _apply_ipv4

        _apply_ipv4(force=True)
    except Exception:
        pass  # best-effort — don't crash if keprix_constants not importable yet
...
from keprix_cli import __version__, __release_date__

# Provider model-selection wizard flows extracted to keprix_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (keprix_cli.main._model_flow_*) keep resolving unchanged.
from keprix_cli.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_keprix_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("KEPRIX_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("KEPRIX_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from keprix_cli.config import get_env_path, get_keprix_home, load_config
    from keprix_cli.auth import get_auth_status

    # Determine whether Keprix itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from keprix_cli.config import DEFAULT_CONFIG
    ...
    # Only count these if Keprix has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Keprix to use their tokens.
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, keprix_bin
        cli_args: the original CLI arguments (everything after 'keprix')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    keprix_bin = container_info["keprix_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo keprix {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo keprix {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  keprix --tui --resume {target}")
    if title:
        print(f'  keprix --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @keprix/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@keprix" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/keprix-ink/package.json",
    "packages/keprix-ink/index.js",
    "packages/keprix-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``KEPRIX_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("KEPRIX_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``KEPRIX_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("KEPRIX_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    keprix_home = os.environ.get("KEPRIX_HOME") or str(Path.home() / ".keprix")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "KEPRIX_HOME": keprix_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(keprix_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(keprix_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if keprix_cli_dir is None:
        keprix_cli_dir = Path(__file__).parent
    bundled = keprix_cli_dir / "tui_dist" / "entry.js"
    return bundled if bundled.is_file() else None


def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (KEPRIX_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("KEPRIX_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from keprix_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("KEPRIX_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with KEPRIX_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset KEPRIX_TUI_DIR (e.g. `unset KEPRIX_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if (
        ...
        npm = _node_bin("npm")
        if not os.environ.get("KEPRIX_QUIET"):
    ...
    if tui_dev:
        # Keep the local @keprix/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @keprix/ink resolves through
        # packages/keprix-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "keprix-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from keprix_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    env = os.environ.copy()
    try:
        from keprix_cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="keprix-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["KEPRIX_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["KEPRIX_PYTHON_SRC_ROOT"] = os.environ.get(
        "KEPRIX_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("KEPRIX_PYTHON", sys.executable)
    env.setdefault("KEPRIX_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["KEPRIX_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["KEPRIX_MODEL"] = model
        env["KEPRIX_INFERENCE_MODEL"] = model
    if provider:
        env["KEPRIX_TUI_PROVIDER"] = provider
        env["KEPRIX_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["KEPRIX_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["KEPRIX_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["KEPRIX_TUI_SKILLS"] = value
    if query:
        env["KEPRIX_TUI_QUERY"] = query
    if image:
        env["KEPRIX_TUI_IMAGE"] = image
    if checkpoints:
        env["KEPRIX_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["KEPRIX_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["KEPRIX_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["KEPRIX_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["KEPRIX_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["KEPRIX_ACCEPT_HOOKS"] = "1"
    ...
    # KEPRIX_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `keprix --tui` try to
    ...
    # still set KEPRIX_TUI_RESUME themselves.
    env.pop("KEPRIX_TUI_RESUME", None)
    if resume_session_id:
        env["KEPRIX_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `keprix update` so
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``KEPRIX_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``keprix kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``keprix kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("KEPRIX_KANBAN_BOARD"):
        return
    try:
        from keprix_cli.kanban_db import get_current_board

        os.environ["KEPRIX_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.keprix/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Keprix — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Keprix still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    Precedence (highest first):
      ...
      2. ``--tui`` flag / ``KEPRIX_TUI=1`` → always TUI
    ...
    if getattr(args, "cli", False):
        return False
    if getattr(args, "tui", False) or os.environ.get("KEPRIX_TUI") == "1":
        return True
    try:
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'keprix sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from keprix_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from keprix_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'keprix doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Keprix isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  keprix setup")
        print()

        from keprix_cli.setup import (
    ...
    if _termux_should_prefetch_update_check():
        try:
            from keprix_cli.banner import prefetch_update_check
    ...
    # setup (config, rules files, plugins, MCP servers) or from Keprix itself.
    # Implemented as a superset of --ignore-user-config + --ignore-rules plus
    # plugin/MCP discovery suppression (KEPRIX_SAFE_MODE is checked by
    # keprix_cli/plugins.py and tools/mcp_tool.py).
    if getattr(args, "safe_mode", False):
        os.environ["KEPRIX_SAFE_MODE"] = "1"
        os.environ["KEPRIX_IGNORE_USER_CONFIG"] = "1"
        os.environ["KEPRIX_IGNORE_RULES"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.keprix/config.yaml and return built-in defaults. Set BEFORE
    ...
    if getattr(args, "source", None):
        os.environ["KEPRIX_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from keprix_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Keprix?")
        print()
    ...
    # has no creds.json.  Every subsequent `keprix gateway` then paid a 30s
    ...
    session_dir = get_keprix_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: keprix gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `keprix gateway` skips it cleanly instead of paying a 30s
        ...
        print("⚠ Pairing may not have completed. Run 'keprix whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``keprix whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``keprix_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from keprix_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_postinstall(args):
    ...
    from keprix_cli.config import stamp_install_method
    from keprix_cli.dep_ensure import ensure_dependency
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from keprix_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``keprix model``) and the setup wizard
    ...
    from keprix_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from keprix_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("KEPRIX_INFERENCE_PROVIDER") or "auto"
    )
    compatible_custom_providers = get_compatible_custom_providers(config)
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from keprix_cli.config import read_raw_config
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'keprix model' for "
                "available providers, or run 'keprix doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in keprix_cli/models.py). A multi-member group shows one
    ...
    # a leftover OPENAI_BASE_URL in ~/.keprix/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.keprix/.env if the active provider is not 'custom'.
    ...
    """
    from keprix_cli.config import get_env_value, save_env_value, load_config
...
#
# Keprix uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `keprix model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `keprix model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`keprix_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from keprix_cli.plugins import get_plugin_auxiliary_tasks
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``keprix model`` flow, then route aux tasks to them here.
    """
    from keprix_cli.config import load_config
    from keprix_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from keprix_cli.auth import _prompt_model_selection
    from keprix_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    from keprix_cli.secret_prompt import masked_secret_prompt
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from keprix_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from keprix_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Keprix URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _remove_custom_provider(config):
    ...
    try:
        from keprix_cli.curses_ui import curses_radiolist
...
# downstream call sites read `keprix_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `keprix` invocation — including
# fast paths like `keprix --version` and slash-command dispatch that never
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from keprix_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``keprix setup`` / ``keprix model``.
    ...
    recover from a malformed paste without editing ``~/.keprix/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from keprix_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    ...
    from keprix_cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `keprix setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from keprix_constants import display_keprix_home as _dhh_fn

            print(
                f"    Keprix will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def cmd_login(args):
    """Authenticate Keprix CLI with a provider."""
    from keprix_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from keprix_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from keprix_cli.auth_commands import auth_command
...
def cmd_cron(args):
    """Cron job management."""
    from keprix_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from keprix_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``keprix slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: keprix slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `keprix slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from keprix_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from keprix_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from keprix_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from keprix_cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `keprix security <subcmd>`."""
...
def cmd_dump(args):
    ...
    from keprix_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from keprix_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from keprix_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Keprix home directory to a zip file."""
    if getattr(args, "quick", False):
        from keprix_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from keprix_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Keprix backup from a zip file."""
    from keprix_cli.backup import run_import
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    try:
        from keprix_cli.banner import check_for_updates
...
def cmd_uninstall(args):
    """Uninstall Keprix (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from keprix_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from keprix_cli.uninstall import run_gui_uninstall
    ...
    from keprix_cli.uninstall import run_uninstall
...
# Critical files that every ``keprix`` invocation imports at startup. If any
# of these fail to parse after a pull, the CLI is bricked — the user can't
# even run ``keprix update`` again to roll forward. The post-pull syntax
# guard validates these and auto-rolls-back on failure.
_UPDATE_CRITICAL_FILES = (
    "keprix_cli/main.py",
    "keprix_cli/config.py",
    "keprix_cli/__init__.py",
    "cli.py",
    "run_agent.py",
    "model_tools.py",
    "toolsets.py",
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``keprix`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="keprix-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``keprix update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``keprix_cli/web_dist/`` (per vite.config.ts
    outDir: "../keprix_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "keprix_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``keprix`` launcher present but ``keprix_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the keprix venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — keprix venv python3, no nix-shell overhead
    ...
    # a keprix venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``keprix update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``keprix web``).
    ...
    # build path usable on a stock `py -m keprix_cli.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (keprix web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``keprix update`` can unconditionally call ``keprix desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``keprix desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $KEPRIX_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_stamp_path() -> Path:
    """Return the path to the desktop build stamp file under $KEPRIX_HOME."""
    from keprix_constants import get_keprix_home
    return get_keprix_home() / "desktop-build-stamp.json"
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``keprix update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Keprix.app/Contents/MacOS/Keprix"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Keprix.exe",
            release_dir / "win-ia32-unpacked" / "Keprix.exe",
            release_dir / "win-arm64-unpacked" / "Keprix.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "keprix",
            release_dir / "linux-unpacked" / "Keprix",
            release_dir / "linux-arm64-unpacked" / "keprix",
            release_dir / "linux-arm64-unpacked" / "Keprix",
        ]
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Keprix'`` desktop build failure: a corrupt zip in the
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Keprix.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Keprix.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Keprix.exe: Access is
    ...
    an unrelated "Keprix" process is never touched. Best-effort: never raises.
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Keprix is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../Keprix.app/Contents/MacOS/Keprix  ->  app bundle = .../Keprix.app
    app = exe.parents[2]
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Keprix Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Keprix Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def cmd_gui(args: argparse.Namespace):
    ...
    try:
        from keprix_logging import setup_logging as _setup_logging_gui
    ...
    if getattr(args, "fake_boot", False):
        env["KEPRIX_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["KEPRIX_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "keprix_root", None):
        env["KEPRIX_DESKTOP_KEPRIX_ROOT"] = str(Path(args.keprix_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["KEPRIX_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  keprix gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if not source_mode:
                ...
                # holds Keprix.exe locked on Windows, so the pack can't replace
            ...
            if build_result.returncode != 0 and not source_mode:
                ...
                # on the final `electron` -> `Keprix` rename: unpack-electron
                ...
                if purged:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Keprix.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Keprix.exe, close any")
                    print("  running Keprix desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> keprix desktop --force-build")
                sys.exit(build_result.returncode or 1)
            packaged_executable = _desktop_packaged_executable(desktop_dir)
            if not source_mode:
                # Locally-built apps are ad-hoc signed; make them relaunchable after
                # an in-place self-update (otherwise macOS reports "Keprix is
    ...
    if source_mode:
        print("→ Launching Keprix Desktop from source build...")
    ...
    print(f"→ Launching packaged Keprix Desktop: {packaged_executable}")
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``keprix dashboard`` processes other than ourselves.

    ``keprix dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``keprix update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    ...
    backend child process: when the desktop spawns ``keprix dashboard`` as
    ...
    environment variable ``KEPRIX_DESKTOP_CHILD_PID`` on the spawned
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "keprix.*dashboard"`) keeps us consistent
            # with `keprix_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `keprix update`.
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``keprix update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``keprix update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: keprix curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``keprix dashboard`` processes.

    Called at the end of ``keprix update`` (default ``reason``) and also
    from ``keprix dashboard --stop`` (which overrides ``reason``).  The
    ...
    """
    # When the Keprix Desktop Electron app spawns this dashboard as a
    # backend child, it sets KEPRIX_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("KEPRIX_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    if killed:
        ...
        print("    keprix dashboard --port <port>")
...
def _update_via_zip(args):
    """Update Keprix by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `keprix update "
            f"--branch {branch}`, or update against main with `keprix update`."
        )
        sys.exit(1)
    zip_url = (
        f"https://github.com/NousResearch/keprix/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="keprix-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"keprix-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for keprix itself
        ...
        # GitHub ZIPs extract to keprix-<branch>/
        extracted = os.path.join(tmp_dir, f"keprix-{branch}")
    ...
    from keprix_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        from keprix_cli.model_catalog import seed_cache_from_checkout
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "keprix-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Keprix behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes keprix completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Keprix couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Keprix couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Keprix behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Keprix couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Keprix couldn't drop "
...
# =========================================================================
# Fork detection and upstream management for `keprix update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/keprix.git",
    "git@github.com:NousResearch/keprix.git",
    "https://github.com/NousResearch/keprix",
    "git@github.com:NousResearch/keprix",
}
OFFICIAL_REPO_URL = "https://github.com/NousResearch/keprix.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_keprix_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_keprix_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Keprix repository.")
        print("  This means you may miss updates from NousResearch/keprix.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NousResearch/keprix.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NousResearch/keprix.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``keprix update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from keprix_constants import get_default_keprix_root

    default_home = get_default_keprix_root()
...
# Install-scoped breadcrumb dropped right before ``keprix update`` mutates the
...
# survives and the next ``keprix`` launch finishes the install instead of
# limping along on a half-built venv (e.g. pip wiped, a core dep like Pillow
# never landed).  Lives next to the venv (not under $KEPRIX_HOME) because the
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``keprix update`` left half-done.
    ...
    protocol stream (``keprix acp`` speaks JSON-RPC on stdout) must never get
    ...
    try:
        ...
        print(
            "⚠ A previous `keprix update` was interrupted mid-install — "
            "finishing dependency installation now..."
        )

        try:
            from keprix_cli.managed_uv import ensure_uv
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``keprix update`` is still progressing even if pip/uv itself is silent.
    """
...
def _keprix_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "keprix.exe",
        scripts_dir / "keprix-gateway.exe",
    ]


def _detect_concurrent_keprix_instances(
    ...
    The Keprix Desktop Electron app spawns ``keprix.EXE`` as a backend child,
    so during ``keprix update`` the user-invoked process and the desktop's
    ...
    shims (``keprix.exe`` / ``keprix-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``keprix update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``keprix.exe``) is a separate process from the Python
    ...
    for shim in _keprix_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated keprix.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``keprix update``
    ...
    #      keprix.exe sitting *under* a non-Keprix parent (e.g. a Keprix
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "keprix.exe"
    lines = ["✗ Another keprix.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Keprix Desktop, exit any open `keprix` REPLs, and")
    lines.append("  stop the gateway (`keprix gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `keprix update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_keprix_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``keprix.exe``.
    ...
    when ``keprix update`` runs, ``keprix.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``keprix.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next keprix invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Keprix Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Keprix Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _keprix_exe_shims(scripts_dir):
        ...
        print(
            "    Close Keprix Desktop, exit other `keprix` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `keprix update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_keprix_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``keprix.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``keprix.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``keprix`` drops off PATH. This wraps
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_keprix_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``keprix.exe.old.*`` left by prior updates.

    Called early on every keprix invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``keprix update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `keprix update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``keprix.exe`` / ``keprix-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_keprix_exe`` for the rationale.
    """
    ...
    # hours later inside ``keprix update``'s desktop-rebuild or skill-sync
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``keprix update``, not the venv we just wrote
    ...
    #
    # Quarantine the running ``keprix.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``keprix`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `keprix update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `keprix update --force` after closing other keprix processes."
        )
        return
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from keprix_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$KEPRIX_HOME/bin/uv``, but on Termux the official
    installer may not work (glibc vs bionic).  Fall back to ``pip install uv``
    which gets a Termux-compatible binary.
    """
    from keprix_cli.managed_uv import resolve_uv
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `keprix update`, so we install root-only first
...
class _UpdateOutputStream:
    """Stream wrapper used during ``keprix update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.keprix/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``keprix update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``keprix update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.keprix/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``keprix update --gateway``) the update is already
    ...
    try:
        ...
        # keprix_cli.config.get_keprix_home to simulate setup failure.
        from keprix_cli.config import get_keprix_home as _get_keprix_home

        logs_dir = _get_keprix_home() / "logs"
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``keprix update --check``: fetch and report without installing.
    ...
    if method == "docker":
        ...
        # same long-form ``docker pull`` guidance ``keprix update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from keprix_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from keprix_cli.config import recommended_update_command
        from keprix_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``keprix update`` without requiring a reinstall.  The
    ...
    ``keprix`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/keprix.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v keprix'`` already resolves.  Idempotent.
    """
    if sys.platform != "linux":
        return
    try:
    ...
    # /usr/local/bin/keprix, code at /usr/local/lib/keprix).
    fhs_link = Path("/usr/local/bin/keprix")
    ...
    path_comment = (
        "# Keprix — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of KEPRIX_HOME before running the update.
    ...
    KEPRIX_HOME directories.  The ``--backup`` flag on ``keprix update``
    ...
    try:
        from keprix_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_keprix_home so the user sees ~/.keprix/...
    try:
        from keprix_constants import get_keprix_home, display_keprix_home

        home = get_keprix_home()
        try:
            display_path = f"{display_keprix_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  keprix import {out_path}")
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    keprix.exe concurrent-instance guard does not see them. They still import
    ...
    print("→ Stopping Windows gateway process(es) before updating Keprix...")
    try:
    ...
    if unmapped_pids:
        ...
        print("    Restart manually after update: keprix gateway run")
...
def _resume_windows_gateways_after_update(token: dict | None) -> None:
    ...
    try:
        from keprix_cli.gateway import launch_detached_profile_gateway_restart
    except Exception as exc:
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``keprix update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update Keprix to the latest version.
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `keprix update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update Keprix via pip (for PyPI installs)."""
    from keprix_cli import __version__
    from keprix_cli.config import is_uv_tool_install
    ...
    if is_uv_tool_install():
        ...
        cmd = [pipx, "upgrade", "keprix"]
    elif uv:
        cmd = [uv, "pip", "install", "--upgrade", "keprix"]
        if in_venv:
        ...
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "keprix"]
    ...
    print("✓ Update complete! Restart keprix to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_keprix_instances(scripts_dir)
            if concurrent:
    ...
    try:
        ...
        # though `git pull` can't touch $KEPRIX_HOME, this is cheap
        ...
        try:
            from keprix_cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in keprix_cli/config.py bricked
        # every user who ran ``keprix update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``keprix
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``keprix update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_keprix_home added to keprix_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # the marker survives and the next ``keprix`` launch finishes the
        ...
        # build.  ``keprix desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``keprix desktop`` shouldn't be forced into a full
        # Electron build by ``keprix update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        has_desktop_app = _desktop_packaged_executable(desktop_dir) is not None or _desktop_dist_exists(desktop_dir)
        if (desktop_dir / "package.json").exists() and shutil.which("npm") and has_desktop_app:
            print("→ Checking if desktop app needs rebuilding...")
            _desktop_build_cmd = [sys.executable, "-m", "keprix_cli.main", "desktop", "--build-only"]
            ...
            if build_result.returncode != 0:
                build_result = subprocess.run(_desktop_build_cmd, cwd=PROJECT_ROOT, check=False)
            if build_result.returncode != 0:
                print("  ⚠ Desktop build failed (non-fatal; run `keprix desktop` to retry)")
        ...
        # current — so copy it straight over ~/.keprix/cache/model_catalog.json
        ...
        # modules in this process.  Reload keprix_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_keprix_home() added since the last release.
        try:
            import importlib
            import keprix_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit KEPRIX_HOME so
        # it is not affected by sync_skills()'s module-level KEPRIX_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's KEPRIX_HOME env var points at the default or a named profile.
        try:
            from keprix_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        try:
            from keprix_cli.profiles import backfill_profile_envs
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'keprix config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: keprix config migrate")
            else:
                print()
                print("Skipped. Run 'keprix config migrate' later to configure.")
        else:
        ...
        try:
            from keprix_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `keprix curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``keprix update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from keprix_cli.tools_config import install_cua_driver
        ...
        # When running as ``keprix update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_keprix_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... keprix-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... keprix-gateway*)
            ...
            # Discover all keprix-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. keprix-gateway.service or keprix-gateway-coder.service
                            ...
                            if _manage_cmd is None:
                                print(
                                    ...
                                    f"    To let `keprix update` restart it automatically, allow\n"
                            ...
                            # path in `keprix gateway restart`
                        ...
                        print(
                            ...
                            f"Check the gateway with: keprix gateway status"
                        )
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: keprix gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: keprix -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Keprix gateway unit files are still installed.
        # When both keprix.service (from a pre-rename install) and the
        # current keprix-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `keprix update` surfaces the issue until the user migrates.
        try:
            from keprix_cli.gateway import (
                has_legacy_keprix_units,
                _find_legacy_keprix_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_keprix_units():
                print()
                print("⚠ Legacy Keprix gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_keprix_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (keprix.service) fight the current")
                print("  keprix-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    keprix gateway migrate-legacy")
                print()
        ...
        print("  keprix model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``keprix -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `keprix profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_keprix_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → keprix -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.keprix)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  keprix profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     keprix -p {name} chat")
                else:
        ...
        from keprix_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from keprix_constants import get_keprix_home as _hh
        ...
        from keprix_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `keprix profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → keprix -p {name}  ({wrapper})")
        print()
        ...
        from keprix_cli.profiles import profile_exists
        ...
        from keprix_cli.profiles import rename_profile
        ...
        from keprix_cli.profiles import export_profile
        ...
        from keprix_cli.profiles import import_profile
        ...
        import tempfile
        from keprix_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="keprix_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  keprix -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      keprix -p {plan.manifest.name} chat")
        ...
        from keprix_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `keprix profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"keprix -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from keprix_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("keprix_requires"):
            print(f"Requires:     Keprix {data['keprix_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from keprix_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.keprix_requires:
        print(f"  Requires: Keprix {mf.keprix_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``keprix dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``keprix dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No keprix dashboard processes running.")
        return 0

    print(f"{len(pids)} keprix dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No keprix dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `keprix update`.
    ...
    # launches the dashboard (`worker dashboard` → KEPRIX_HOME points into
    ...
    try:
        from keprix_cli.profiles import get_active_profile_name
    ...
    if "KEPRIX_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
    ...
    try:
        from keprix_cli.plugins import discover_plugins
        discover_plugins()
    except Exception as exc:
    ...
    try:
        from keprix_cli.mcp_startup import start_background_mcp_discovery
    ...
    from keprix_cli.web_server import start_server
...
def cmd_completion(args, parser=None):
    ...
    from keprix_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from keprix_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Keprix log files."""
    from keprix_cli.logs import tail_log, list_logs
...
# so that in ``keprix -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``keprix_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``keprix -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``keprix`` or only flags → defaults to ``chat``.
        return False
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("KEPRIX_TUI") == "1")
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from keprix_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from keprix_cli.oneshot import run_oneshot
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["KEPRIX_DEFER_AGENT_STARTUP"] = "1"
            os.environ["KEPRIX_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["KEPRIX_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `keprix --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        mem_dir = get_keprix_home() / "memories"
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_keprix_home()}/memories/\n"
            )
            return
...
def cmd_acp(args):
    """Launch Keprix as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        ...
        from keprix_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from keprix_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from keprix_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from keprix_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from keprix_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from keprix_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from keprix_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from keprix_cli.claw import claw_command

    claw_command(args)


def main():
    """Main entry point for keprix CLI."""
    # Cosmetic: make the process show up as 'keprix' instead of 'python3.11'
    # in ps/top/htop.  Non-fatal — just a nicer UX.
    _set_process_title()

    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from keprix_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``keprix.exe.old.*`` quarantine files left by previous
    # ``keprix update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_keprix_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    # Self-heal a venv left half-built by an interrupted ``keprix update``
    ...
    # ``keprix skills install update``) merely defers recovery one launch;
    # under-matching (missing ``keprix -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in keprix_cli/subcommands/model.py)
    # =========================================================================
    ...
    # =========================================================================
    from keprix_cli.fallback_cmd import cmd_fallback
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `keprix model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    from keprix_cli import secrets_cli as _secrets_cli
    ...
    # migrate command
    # =========================================================================
    from keprix_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in keprix_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in keprix_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in keprix_cli/subcommands/postinstall.py)
    # =========================================================================
    build_postinstall_parser(subparsers, cmd_postinstall=cmd_postinstall)

    # =========================================================================
    # whatsapp command  (parser built in keprix_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `keprix whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in keprix_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from keprix_cli.send_cmd import register_send_subparser
    ...
    # =========================================================================
    # auth command  (parser built in keprix_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in keprix_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in keprix_cli/subcommands/cron.py)
    # =========================================================================
    ...
    # =========================================================================
    from keprix_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    # hooks command  (parser built in keprix_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in keprix_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    build_crm_ingest_parser(
        subparsers,
        cmd_crm_ingest=crm_ingest_commands.cmd_crm_ingest,
    )
    build_crm_migrate_parser(
        subparsers,
        cmd_crm_migrate=crm_migrate_commands.cmd_crm_migrate,
    )
    build_crm_funnel_parser(
        subparsers,
        cmd_crm_funnel=crm_funnel_commands.cmd_crm_funnel,
    )
    build_document_vault_parser(
        subparsers,
        cmd_document_vault=document_vault_commands.cmd_document_vault,
    )
    ...
    # =========================================================================
    # dump command  (parser built in keprix_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in keprix_cli/subcommands/debug.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.keprix/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo keprix uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from keprix_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in keprix_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # config command  (parser built in keprix_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # pairing command  (parser built in keprix_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in keprix_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from keprix_cli.bundles import register_cli as _bundles_register, bundles_command
    ...
    # subcommand — ``keprix --help``, ``keprix version``, ``keprix logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from keprix_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from keprix_cli.curator import register_cli as _register_curator_cli
    ...
    # =========================================================================
    # memory command  (parser built in keprix_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in keprix_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `keprix computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `keprix tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: keprix computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: keprix computer-use install")
            return
    ...
    # =========================================================================
    # mcp command  (parser built in keprix_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch keprix --resume <id> by replacing the current process
    ...
    # =========================================================================
    # insights command  (parser built in keprix_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)

    # =========================================================================
    # claw command  (parser built in keprix_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in keprix_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in keprix_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in keprix_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in keprix_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in keprix_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in keprix_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The Keprix-Setup.exe success screen tells users to
    # run `keprix desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in keprix_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in keprix_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in keprix_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``keprix -c Pokemon Agent Dev`` → ``keprix -c 'Pokemon Agent Dev'``
    ...
    from keprix_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'keprix -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (keprix hooks list, cron
