"""
Keprix Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``keprix desktop`` / ``keprix gui``)
     Built inside the agent checkout under ``$KEPRIX_HOME/keprix/``:
       ...
       - ``$KEPRIX_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Keprix.app`` or ``~/Applications/Keprix.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Keprix`` (NSIS per-user)
...
the app name ("Keprix"), separate from ``$KEPRIX_HOME``:
  - macOS:   ``~/Library/Application Support/Keprix``
  - Windows: ``%APPDATA%\\Keprix``
  - Linux:   ``$XDG_CONFIG_HOME/Keprix`` (default ``~/.config/Keprix``)
...
``keprix uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(keprix_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return keprix_home / "keprix"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Keprix"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Keprix"
...
def source_built_gui_artifacts(keprix_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``keprix desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``keprix/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(keprix_home)
    ...
    return [
        ...
        keprix_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Keprix" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Keprix.app"),
            home / "Applications" / "Keprix.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Keprix).
            local_base / "Programs" / "Keprix",
            # Older / alternate layout some builds used.
            local_base / "keprix-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Keprix")
    else:
        ...
        paths += [
            data_base / "applications" / "keprix.desktop",
            data_base / "applications" / "Keprix.desktop",
        ]
    return paths


def agent_is_installed(keprix_home: Path) -> bool:
    """Return True when a usable Python agent install exists under KEPRIX_HOME.
    ...
    if (agent_root / "keprix_cli").is_dir():
        return True
...
def gui_is_installed(keprix_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(keprix_home):
        if p.exists():
            return True
...
def gui_install_summary(keprix_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = keprix_home if keprix_home is not None else get_keprix_home()
...
def uninstall_gui(keprix_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``keprix/keprix_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$KEPRIX_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove keprix' or "
            "'sudo dnf remove keprix'). AppImage builds are a single file you "
