...
Used by ``keprix kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``keprix_cli/goals.py`` — same aux
...
KEPRIX_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    env_int("KEPRIX_KANBAN_SPECIFY_MAX_TOKENS", 6000),
)

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the Keprix board.
...
def _profile_author() -> str:
    """Mirror of ``keprix_cli.kanban._profile_author``. Kept local to
    avoid a circular import when kanban.py imports this module."""
    return (
        os.environ.get("KEPRIX_PROFILE")
    ...
    *,
    ...
    try:
        resp = client.chat.completions.create(
            ...
            temperature=0.3,
            max_tokens=KEPRIX_KANBAN_SPECIFY_MAX_TOKENS,
