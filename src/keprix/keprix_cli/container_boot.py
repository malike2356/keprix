...
``$KEPRIX_HOME/profiles/<name>/`` live on the persistent VOLUME, and
...
Dockerfile (Phase 4 Task 4.0). Runs as root after 01-keprix-setup
(the stage2 hook) has chowned the volume and seeded $KEPRIX_HOME, but
...
def reconcile_profile_gateways(
    *,
    ...
    (the implicit profile that lives at the top of ``$KEPRIX_HOME``,
    not under ``profiles/``). The dispatcher in ``keprix_cli.gateway``
    maps an empty profile suffix to ``gateway-default``, so this slot
    is what ``keprix gateway start`` (no ``-p``) targets. Without it,
    bare ``keprix gateway start`` inside the container would land on
    ...
    ``$KEPRIX_HOME/gateway_state.json`` (sibling to the profile root,
    not under ``profiles/``); stale runtime files there are swept the
    same way as for named profiles.

    Args:
        keprix_home: The container's KEPRIX_HOME (typically /opt/data).
            Profiles live under ``<keprix_home>/profiles/<name>/``;
            the default profile lives at ``<keprix_home>`` itself.
    ...
    # ``keprix gateway start`` (no ``-p``) has somewhere to land;
    ...
    default_prior_state = legacy_default_state or _read_desired_state(keprix_home)
    default_should_start = default_prior_state in _AUTOSTART_STATES
    if not dry_run:
        _cleanup_stale_runtime_files(keprix_home)
    ...
    profiles_root = keprix_home / "profiles"
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            if not entry.is_dir():
                continue
            # SOUL.md is always seeded by `keprix profile create` (config.yaml
            # is not — that comes later via `keprix setup`). Use it as the
            ...
            # via ``keprix -p default-named gateway start`` if they
    ...
    if not dry_run:
        _write_reconcile_log(keprix_home, actions)
    return actions
...
def _maybe_migrate_legacy_gateway_run_state(
    ...
    *,
    ...
    """
    state_file = keprix_home / "gateway_state.json"
    if state_file.exists():
        return None

    if os.environ.get("KEPRIX_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes"):
        return None
...
def _strip_container_argv_prefix(argv: Sequence[str]) -> list[str]:
    """Strip the s6/wrapper prefix off PID 1 argv, leaving the keprix args.

    The container PID 1 argv looks like
    ``/init /opt/keprix/docker/main-wrapper.sh <subcommand> [args...]`` and
    the wrapper re-execs ``keprix <subcommand>``. Peel ``init`` →
    ``main-wrapper.sh`` → ``keprix`` so callers can match on the bare
    ...
    if args and args[0].endswith("main-wrapper.sh"):
        args = args[1:]
    if args and Path(args[0]).name == "keprix":
        args = args[1:]
    return args
...
def _is_dashboard_container(argv: Sequence[str]) -> bool:
    ...
    A dashboard-only container (``keprix dashboard ...``) never spawns or
    ...
    the gateway and dashboard containers share a bind-mounted KEPRIX_HOME,
...
def _register_service(scandir: Path, profile: str, *, start: bool) -> None:
    ...
    try:
        ...
        # it up explicitly with `keprix -p <profile> gateway start`
        ...
        # spawns will EEXIST our dirs/FIFOs and inherit keprix
        # ownership, so runtime s6-svc / s6-svstat / s6-svwait calls
        # (all dispatched as the keprix user) won't hit EACCES. See
...
def _write_reconcile_log(
    keprix_home: Path, actions: list[ReconcileAction],
) -> None:
    """Append one line per profile to $KEPRIX_HOME/logs/container-boot.log.
...
def main() -> int:
    ...
    # bind-mounted KEPRIX_HOME, both race to flock() the same s6-log lock
    ...
    actions = reconcile_profile_gateways(
        keprix_home=keprix_home, scandir=scandir,
    )
    for a in actions:
        print(
