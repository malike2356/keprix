"""
Interactive setup wizard for Keprix.
...
# display_keprix_home imported lazily at call sites (stale-module safety during keprix update)
...
from keprix_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from keprix_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Keprix Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Keprix using environment variables or config commands:")
    print_info("  keprix config set model.provider custom")
    print_info("  keprix config set model.base_url http://localhost:8080/v1")
    print_info("  keprix config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'keprix setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from keprix_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'keprix setup')")


def _print_setup_summary(config: dict, keprix_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'keprix setup' to configure"))
    ...
    # Video generation — opt-in via `keprix tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from keprix_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'keprix setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'keprix setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'keprix setup terminal'"))
    ...
    # Spotify (OAuth via keprix auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'keprix setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {keprix_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('keprix setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('keprix setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('keprix setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('keprix setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('keprix setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('keprix config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('keprix config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('keprix config set <key> <value>', Colors.GREEN)}")
    ...
    print(f"   {color('keprix doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `keprix tools` and `keprix setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``keprix model``)
    ...
    provider added to ``keprix model`` is automatically available here.
    ...
    # Delegate to the shared keprix model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from keprix_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: keprix model")
    ...
    # on demand via `keprix auth add`, `keprix setup` vision, and
    # `keprix setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``keprix model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from keprix_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "neutts":
        ...
        # via the Keprix auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via keprix setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'keprix setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Keprix runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `keprix setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `keprix setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into KEPRIX_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("KEPRIX_MAX_ITERATIONS")
    ...
    print_info("  Run `keprix setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # KEPRIX_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram_auto_result():
    ...
    try:
        from keprix_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_keprix_home(Path(get_keprix_home()))
    except Exception:
        pass
...
def _profile_name_from_keprix_home(keprix_home) -> str | None:
    """Return the active profile name when KEPRIX_HOME is a profile dir."""
    if keprix_home.parent.name == "profiles":
        return keprix_home.name
    return None
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `keprix update`)",
                True,
            ):
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under KEPRIX_HOME, and print
    ...
    """
    try:
        from keprix_cli.slack_cli import _build_full_manifest
        from keprix_constants import get_keprix_home

        manifest = _build_full_manifest(
            bot_name="Keprix",
            bot_description="Your Keprix agent on Slack",
        )
        target = Path(get_keprix_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `keprix slack manifest --write` anytime to refresh after "
            "Keprix adds new commands."
        )
...
def _setup_bluebubbles():
    ...
    print_info("Connects Keprix to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from keprix_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def setup_gateway(config: dict):
    ...
    if not selected:
        print_info("No platforms selected. Run 'keprix setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     keprix config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_keprix_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo keprix gateway install --system")
                print_info("  Or run in foreground:  keprix gateway")
        else:
            from keprix_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   keprix gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   keprix gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `keprix setup tools` and `keprix tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``keprix_cli.auth`` — lists every supported
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from keprix_cli.config
    ...
    if section_key == "model":
        ...
        from keprix_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_keprix.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_keprix migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_keprix", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Keprix.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Keprix to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Keprix at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Keprix at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Keprix at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Keprix at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Keprix equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Keprix config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Keprix.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Keprix's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(keprix_home: Path) -> bool:
    ...
    print_info("Keprix can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: keprix claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=keprix_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: keprix claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Keprix configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Keprix config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Keprix (use keprix claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    print(color("│     ⚕ Keprix Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `keprix portal` == quick setup's Nous step.
    try:
        ...
        print_info("  You can retry later with `keprix portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `keprix portal`: %s", exc)
        print()
    ...
    print_info("  Run `keprix portal info` to inspect routing.")
    print_info("  Run `keprix` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      keprix setup           — full or quick (auto-detected)
      keprix setup model     — just model/provider
      keprix setup tts       — just text-to-speech
      keprix setup terminal  — just terminal backend
      keprix setup gateway   — just messaging platforms
      keprix setup tools     — just tool configuration
      keprix setup agent     — just agent settings
    """
    from keprix_cli.config import is_managed, managed_error
    if is_managed():
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Keprix Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Keprix Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Keprix installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, keprix_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Keprix configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'keprix setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(keprix_home)
        if migration_ran:
            config = load_config()
    ...
    print_info(f"Data folder:  {keprix_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'keprix config edit'")
    ...
    # Tune later with `keprix setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, keprix_home)
...
def _run_first_time_quick_setup(config: dict, keprix_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'keprix setup gateway'",
        ],
        0,
    )
...
def _run_quick_setup(config: dict, keprix_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'keprix setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Keprix to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'keprix setup gateway'.")
