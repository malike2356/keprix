"""MCP picker — interactive `keprix mcp picker` (also the default `keprix mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``keprix mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `keprix plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from keprix_cli.cli_output import prompt_yes_no
from keprix_cli.curses_ui import curses_single_select
from keprix_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Keprix session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from keprix_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `keprix mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: keprix mcp install <name>    Picker: keprix mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Keprix is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Keprix — run `keprix update` "
...
def show_catalog() -> None:
    """`keprix mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`keprix mcp picker` (and default `keprix mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`keprix mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `keprix mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
