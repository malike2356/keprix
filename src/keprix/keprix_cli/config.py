"""
Configuration management for Keprix.

Config files are stored in ~/.keprix/ for easy access:
- ~/.keprix/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.keprix/.env         - API keys and secrets

This module provides:
- keprix config          - Show current configuration
- keprix config edit     - Open config in editor
- keprix config set      - Set a specific value
- keprix config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``keprix config set`` (which rewrites
    ...
    ``config.yaml`` in place: keprix never silently mutates the user's config,
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.keprix/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``keprix logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  keprix config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Keprix makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Keprix itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``keprix update``, the TUI build.
...
#   on every plugin install / ``keprix update``.
...
# * ``KEPRIX_HOME`` / ``KEPRIX_PROFILE`` / ``KEPRIX_CONFIG`` /
#   ``KEPRIX_ENV`` — Keprix runtime location flags. Writing these into
...
#
# IMPORTANT: ``KEPRIX_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (KEPRIX_GEMINI_CLIENT_ID,
# KEPRIX_LANGFUSE_PUBLIC_KEY, KEPRIX_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Keprix runtime location — never via dashboard env writer.
    # NOT a KEPRIX_* blanket: integration credentials (KEPRIX_GEMINI_*,
    # KEPRIX_LANGFUSE_*, KEPRIX_SPOTIFY_*, ...) ARE allowed.
    "KEPRIX_HOME", "KEPRIX_PROFILE", "KEPRIX_CONFIG", "KEPRIX_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Keprix runtime location "
            "(KEPRIX_HOME, KEPRIX_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "ANTHROPIC_API_KEY", "ANTHROPIC_TOKEN", "ANTHROPIC_WORKSPACE_ID", "ANTHROPIC_WORKSPACE",
    ...
    "KEPRIX_TOOL_PROGRESS", "KEPRIX_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `keprix plugins enable
    # observability/langfuse` or `keprix tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "KEPRIX_LANGFUSE_ENV",
    "KEPRIX_LANGFUSE_RELEASE",
    "KEPRIX_LANGFUSE_SAMPLE_RATE",
    "KEPRIX_LANGFUSE_MAX_CHARS",
    "KEPRIX_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from keprix_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("KEPRIX_MANAGED", "").strip()
    if raw:
...
def is_managed() -> bool:
    """Check if Keprix is running in package-manager-managed mode.

    Two signals: the KEPRIX_MANAGED env var (set by the systemd service),
    or a .managed marker file in KEPRIX_HOME (set by the NixOS activation
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Keprix was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.keprix/.install_method`` file (written by installers)
    2. KEPRIX_MANAGED env / .managed marker (NixOS, Homebrew)
    ...
    container detection here:
      ...
      - the published ``nousresearch/keprix`` image stamps ``docker``
    ...
    so ``keprix update`` bailed with "doesn't apply inside the Docker
    ...
    """
    stamp = get_keprix_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.keprix/.install_method."""
    stamp = get_keprix_home() / ".install_method"
    try:
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Keprix lives in a ``uv tool`` layout.

    ``uv tool install keprix`` places the install at
    ``.../uv/tools/keprix/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``keprix`` happens to be uv-tool-installed on the machine while
    the *active* Keprix is a regular pip/venv install, causing
    ``keprix update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/keprix/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade keprix"
    if method == "docker":
        return "docker pull nousresearch/keprix:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade keprix"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade keprix"
        return "pip install --upgrade keprix"
    return "keprix update"
...
# Long-form text for ``keprix update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# keprix_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Keprix, it doesn't update
...
✗ ``keprix update`` doesn't apply inside the Docker container.

Keprix runs as a published image (nousresearch/keprix), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull nousresearch/keprix:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate keprix
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm nousresearch/keprix:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/keprix/tags
  • Your config and session history live under ``$KEPRIX_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``keprix update`` inside Docker.
...
def format_managed_message(action: str = "modify this Keprix installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("KEPRIX_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Keprix installation is managed by NixOS "
            f"(KEPRIX_MANAGED={env_hint}).\n"
            "Edit services.keprix.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Keprix installation is managed by Homebrew "
            f"(KEPRIX_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade keprix"
        )
    ...
    return (
        f"Cannot {action}: this Keprix installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Keprix."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from KEPRIX_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, keprix_bin
    or None if container mode is not active, we're already inside the
    container, or KEPRIX_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("KEPRIX_DEV") == "1":
        return None
    ...
    container_mode_file = get_keprix_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "keprix")
    exec_user = info.get("exec_user", "keprix")
    keprix_bin = info.get("keprix_bin", "/data/current-package/bin/keprix")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "keprix_bin": keprix_bin,
    }
...
def _resolve_keprix_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the KEPRIX_UID / KEPRIX_GID env vars set by Docker deployments.

    Docker containers running Keprix commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level KEPRIX_HOME once, but
    subdirectories created at runtime by ``ensure_keprix_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("KEPRIX_UID", "").strip()
    gid_str = os.environ.get("KEPRIX_GID", "").strip()
    try:
...
def _chown_to_keprix_uid(path) -> None:
    """Chown ``path`` to ``KEPRIX_UID:KEPRIX_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_keprix_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_keprix_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the keprix group can
    share state with the gateway service.

    The mode can be overridden via the KEPRIX_HOME_MODE environment variable
    (e.g. KEPRIX_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse KEPRIX_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``KEPRIX_UID``/``KEPRIX_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("KEPRIX_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_keprix_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Keprix runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("KEPRIX_CONTAINER") or os.environ.get("KEPRIX_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set KEPRIX_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into KEPRIX_HOME if the user doesn't have one yet."""
...
def ensure_keprix_home():
    """Ensure ~/.keprix directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_keprix_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_keprix_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"KEPRIX_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Keprix-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Keprix
        ...
        # identity slot (SOUL.md). Empty by default. The KEPRIX_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Keprix adds a coding operating brief
    ...
    "terminal": {
        ...
        #             KEPRIX_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force KEPRIX_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Keprix auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Keprix sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Keprix
        ...
        #  "/home/user/.keprix/cache/documents:/output"]
        ...
        # bundled Keprix image, which drops to the `keprix` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Keprix sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Keprix should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``keprix chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.keprix/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: keprix sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Keprix truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``keprix kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``keprix kanban decompose`` and the kanban
        ...
        # ``keprix profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `keprix model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `keprix` (and `keprix chat`) launches by default:
        ...
        # REPL and `--tui` (or KEPRIX_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `keprix --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `keprix -c` muscle memory.  Default off so existing
        # users aren't surprised.  KEPRIX_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `keprix --tui` drops a one-time hint
        ...
        # e.g. `model · 68% · ~/projects/keprix`. Per-platform overrides go under
    ...
    "dashboard": {
        ...
        # ``KEPRIX_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``KEPRIX_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # ``portal_url`` defaults to https://keprixai.com).
        "oauth": {
        ...
        # (``KEPRIX_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # Public URL override (env: ``KEPRIX_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/keprix``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Keprix appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.keprix/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.keprix/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, Keprix feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Keprix auto-pauses the goal and
    ...
    # always goes to ~/.keprix/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${KEPRIX_SKILL_DIR} and ${KEPRIX_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `keprix curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # normally restored on every `keprix update`, so pruning them only
        ...
        # skipped), snapshot ~/.keprix/skills/ into
        # ~/.keprix/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `keprix curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for keprix-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⬡ *Keprix*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (KEPRIX_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.keprix/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or KEPRIX_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `keprix doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``keprix_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Keprix to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via KEPRIX_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `keprix -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `keprix` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `keprix kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     keprix's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Keprix tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.keprix/logs/.
    ...
    # update model picker lists without shipping a keprix release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://keprixai.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `keprix model` invocation; network failures
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.keprix/.env,
        ...
        # behavior: files must live under the Keprix cache, under
        ...
        # user. Bridged to KEPRIX_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Keprix
        # cache (~/.keprix/cache/{documents,images,audio,video,screenshots})
        ...
        # to KEPRIX_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # KEPRIX_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to KEPRIX_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.keprix/state.db.
    ...
    "sessions": {
        ...
        # default of ``keprix sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.keprix/sessions/session_{sid}.json`` on every turn
    ...
    # ``keprix update`` behaviour.
    "updates": {
        # Run a full ``keprix backup``-style zip of KEPRIX_HOME before every
        # ``keprix update``.  Backups land in ``<KEPRIX_HOME>/backups/`` and
        # can be restored with ``keprix import <path>``.  Off by default —
        # on large KEPRIX_HOME directories the zip can add minutes to every
        ...
        "backup_keep": 5,
        # What `keprix update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<KEPRIX_HOME>/lsp/bin/`` on first use.
    ...
    # `keprix tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.keprix/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.keprix/.env (or your shell) and never in config.yaml.
            ...
            # ~/.keprix/bin/ on first use.  When False you must install
            ...
            # `keprix secrets bitwarden setup`.
            "server_url": "",
        },
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'keprix model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Keprix auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @keprix:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. KEPRIX_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #keprix)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: keprix-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'keprix' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Keprix API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Keprix API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Keprix API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # KEPRIX_TOOL_PROGRESS and KEPRIX_TOOL_PROGRESS_MODE are deprecated —
    ...
    "KEPRIX_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.keprix.config`` entries, then checks
    ...
    try:
        ...
        # should never break `keprix update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Keprix versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``keprix_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'keprix setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Keprix won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'keprix doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("KEPRIX_HOME", "~/.keprix")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("KEPRIX_TOOL_PROGRESS")
            old_mode = get_env_value("KEPRIX_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from KEPRIX_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from KEPRIX_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("KEPRIX_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from KEPRIX_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$KEPRIX_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_keprix_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `keprix plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `keprix curator status` has no
    ...
    #   3. Creates `~/.keprix/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_keprix_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_keprix_home() / "logs" / "curator"
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from keprix_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: keprix config set <key> <value>")
    ...
    # metadata.keprix.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _keprix_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.keprix/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.keprix/config.yaml.
    ...
    (which change ``KEPRIX_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
#   openai-codex (OAuth — keprix auth) — OpenAI Codex
#   nous         (OAuth — keprix auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.keprix/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.keprix/.env.
    ...
    interactive menu render (`keprix tools`, `keprix setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `keprix tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Keprix env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.keprix/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.keprix/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.keprix/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.keprix/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Keprix — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Keprix vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.keprix/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⬡ Keprix Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from keprix_cli.auth import get_anthropic_key
    ...
    # Warn on stale KEPRIX_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("KEPRIX_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale KEPRIX_MAX_ITERATIONS={_env_ghost} "
                f"(run 'keprix doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  keprix config edit     # Edit config file", Colors.DIM))
    print(color("  keprix config set <key> <value>", Colors.DIM))
    print(color("  keprix setup           # Run setup wizard", Colors.DIM))
    print()
...
def set_config_value(key: str, value: str):
    ...
    api_keys = [
        'OPENROUTER_API_KEY', 'OPENAI_API_KEY', 'ANTHROPIC_API_KEY', 'ANTHROPIC_WORKSPACE_ID',
        'ANTHROPIC_WORKSPACE', 'VOICE_TOOLS_OPENAI_KEY',
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: keprix config set <key> <value>")
            print()
            print("Examples:")
            print("  keprix config set model anthropic/claude-sonnet-4")
            print("  keprix config set terminal.backend docker")
            print("  keprix config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'keprix config migrate' to add them")
        ...
        print("  keprix config           Show current configuration")
        print("  keprix config edit      Open config in editor")
        print("  keprix config set <key> <value>   Set a config value")
        print("  keprix config check     Check for missing/outdated config")
        print("  keprix config migrate   Update config with new options")
        print("  keprix config path      Show config file path")
        print("  keprix config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``keprix config`` UI so users
