"""``keprix gateway`` and ``keprix proxy`` subcommand parsers.
...
def build_gateway_parser(subparsers, *, cmd_gateway: Callable, cmd_proxy: Callable) -> None:
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if KEPRIX_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy keprix.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Keprix gateway unit files "
            "(e.g. keprix.service) left over from older installs. Profile "
            "units (keprix-gateway-<profile>.service) and unrelated "
