...
on Japanese locales, etc.).  Keprix's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``keprix_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Keprix is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``KEPRIX_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("KEPRIX_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Keprix-managed Git install directories so
    ...
    # already-running shells don't see the change — which means keprix
    ...
    # install without Keprix-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Keprix's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Keprix's
    ``keprix config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Keprix (User env var in Windows Settings, or
    export in a PowerShell profile) and Keprix picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Keprix-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\keprix\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``keprix`` from right after install) retain their old PATH.

    Any subprocess Keprix spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Keprix-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "keprix", "git", "cmd"),
        os.path.join(local_appdata, "keprix", "git", "bin"),
        os.path.join(local_appdata, "keprix", "git", "usr", "bin"),
        # Keprix venv Scripts directory — host of the keprix.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes keprix, but harmless to include.
        os.path.join(local_appdata, "keprix", "keprix", "venv", "Scripts"),
