"""``keprix logs`` — view and filter Keprix log files.
...
Usage examples::
    ...
    keprix logs                    # last 50 lines of agent.log
    keprix logs -f                 # follow agent.log in real time
    keprix logs errors             # last 50 lines of errors.log
    keprix logs gateway -n 100    # last 100 lines of gateway.log
    keprix logs gui -f            # follow gui.log (dashboard/pty/ws)
    keprix logs desktop -f        # follow desktop.log (Electron app boot/backend)
    keprix logs --level WARNING    # only WARNING+ lines
    keprix logs --session abc123   # filter by session ID substring
    keprix logs --component tools  # only tool-related lines
    keprix logs --since 1h         # lines from the last hour
    keprix logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_keprix_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Keprix runs — try 'keprix chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_keprix_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_keprix_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_keprix_home()}/logs/")
        return

    print(f"Log files in {display_keprix_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'keprix chat' to generate logs)")
