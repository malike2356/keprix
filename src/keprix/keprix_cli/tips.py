...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or keprix -c.",
    ...
    "Ctrl+Z suspends Keprix to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "keprix -c resumes your most recent CLI session. keprix -c \"project name\" resumes by title.",
    "keprix -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "keprix -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "keprix chat -t web,terminal enables only specific toolsets for a focused session.",
    "keprix chat -s github-pr-workflow preloads a skill at launch.",
    "keprix chat -q \"query\" runs a single non-interactive query and exits.",
    "keprix chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "keprix chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "keprix --yolo bypasses all dangerous command approval prompts for the entire session.",
    "keprix chat --source telegram tags the session for filtering in keprix sessions list.",
    "keprix -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "keprix doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "keprix dump outputs a compact setup summary — great for bug reports.",
    "keprix config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "keprix config edit opens config.yaml in your default editor.",
    "keprix config check scans for missing or stale configuration options.",
    "keprix sessions browse opens an interactive session picker with search.",
    "keprix sessions stats shows session counts by platform and database size.",
    "keprix sessions prune --older-than 30 cleans up old sessions.",
    "keprix skills search react --source skills-sh searches the skills.sh public directory.",
    "keprix skills check scans installed hub skills for upstream updates.",
    "keprix skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "keprix skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "keprix mcp add github --command npx adds MCP servers from the command line.",
    "keprix mcp serve runs Keprix itself as an MCP server for other agents.",
    "keprix auth add lets you add multiple API keys for credential pool rotation.",
    "keprix completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "keprix logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "keprix backup creates a zip backup of your entire Keprix home directory.",
    ...
    "Save money: keprix tools disables unused tools, keprix skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "keprix models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as keprix -w).",
    ...
    "KEPRIX_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Keprix tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'keprix profile create coder' creates the 'coder' command.",
    "keprix profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Keprix prints a resume command with session ID and stats.",
    "keprix sessions export backup.jsonl exports all sessions for backup or analysis.",
    "keprix -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "keprix skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .keprix/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: keprix cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Keprix runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "keprix gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Keprix via a local macOS server.",
    ...
    "Context files (.keprix.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Keprix loads project context from .keprix.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "Checkpoints use shadow repos in ~/.keprix/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "keprix chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "keprix plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "keprix acp runs Keprix as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "KEPRIX_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "keprix auth add nous or keprix auth add openai-codex sets up OAuth-based providers.",
    ...
    "keprix status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.keprix/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Keprix your own.",
    ...
    "Before a gateway session reset, Keprix auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.keprix/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Profiles scope Keprix state via KEPRIX_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "KEPRIX_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in KEPRIX_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.keprix/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'keprix profile create ls' would be rejected.",
    "keprix profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "keprix claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "KEPRIX_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via keprix tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.keprix/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.keprix/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Keprix auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Keprix config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Keprix is working.',
    ...
    '/reload reloads ~/.keprix/.env into the running session — pick up new API keys without restarting.',
    ...
    "KEPRIX_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.keprix/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.keprix/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'keprix curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "keprix curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'keprix curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'keprix auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'KEPRIX_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "KEPRIX_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'keprix dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'keprix dashboard embeds the full Keprix TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.keprix/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.keprix/dashboard-plugins/ — no npm build required.',
    ...
    'KEPRIX_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'KEPRIX_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'KEPRIX_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'KEPRIX_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.keprix/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.keprix/skills/ so drop-in skills appear without restarting the session.',
    ...
    'keprix -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'keprix chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'keprix chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'keprix chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "keprix chat --source tool tags programmatic chats so they don't clutter keprix sessions list.",
    'keprix dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'keprix sessions rename <ID> "new title" renames any past session; keprix sessions delete <ID> removes one.',
    'keprix import restores a session export or profile archive produced by sessions export or profile export.',
    'keprix fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'keprix pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'keprix setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'keprix status --deep runs the full health sweep across every component; plain keprix status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'KEPRIX_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'KEPRIX_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.keprix/plugins/ — trust-gated by design.',
    "KEPRIX_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'KEPRIX_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'KEPRIX_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'KEPRIX_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'KEPRIX_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'KEPRIX_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'KEPRIX_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'KEPRIX_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'KEPRIX_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'KEPRIX_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'KEPRIX_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Keprix block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from keprix sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.keprix/dashboard-plugins/.',
]
