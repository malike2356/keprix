...
``<root>`` is the **shared Keprix root** (the parent of any active
...
``keprix -p <profile>`` joins the same board as the dispatcher that
...
the profile's Keprix home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``KEPRIX_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``KEPRIX_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``keprix kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.keprix``. In Docker / custom
deployments where ``KEPRIX_HOME`` points outside ``~/.keprix`` (e.g.
``/opt/keprix``), ``<root>`` is ``KEPRIX_HOME``. Legacy env-var
overrides still work:

* ``KEPRIX_KANBAN_DB`` — pin the database file path directly.
* ``KEPRIX_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``KEPRIX_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``KEPRIX_KANBAN_DB``,
``KEPRIX_KANBAN_WORKSPACES_ROOT``, and ``KEPRIX_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/keprix-kanban-v1-spec.pdf`` for the full
...
# ``KEPRIX_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``KEPRIX_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("KEPRIX_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``KEPRIX_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("KEPRIX_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``KEPRIX_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get(
        "KEPRIX_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", ""
    ).strip()
    if raw:
        try:
            parsed = int(raw)
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "keprix_kanban_current_board_override",
    default=None,
)
...
# enough that kebab-case names like ``atm10-server`` or ``keprix``
...
def kanban_home() -> Path:
    """Return the shared Keprix root that anchors the kanban board.

    Resolution order:

    1. ``KEPRIX_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_keprix_root()``, which already returns ``<root>``
       when ``KEPRIX_HOME`` is ``<root>/profiles/<name>``, and returns
       ``KEPRIX_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``KEPRIX_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("KEPRIX_KANBAN_HOME", "").strip()
    if override:
        return Path(override).expanduser()
    from keprix_constants import get_default_keprix_root
    return get_default_keprix_root()
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``keprix kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``KEPRIX_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``keprix kanban boards
    ...
    env = os.environ.get("KEPRIX_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``keprix kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``KEPRIX_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("KEPRIX_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``KEPRIX_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("KEPRIX_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``KEPRIX_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
    ...
    see the kanban docs.
    """
    override = os.environ.get("KEPRIX_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``keprix kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``KEPRIX_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``KEPRIX_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("KEPRIX_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Keprix treats as user-controlled-but-trusted on the
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``KEPRIX_KANBAN_DB`` env → ``KEPRIX_KANBAN_BOARD`` env →
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See keprix_state._WAL_INCOMPAT_MARKERS for detection logic.
                from keprix_state import apply_wal_with_fallback
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``keprix kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``KEPRIX_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from keprix_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``keprix --skills ...`` alongside the built-in
    ...
    # `keprix --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``keprix kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `keprix kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``keprix kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``keprix kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``keprix kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``keprix kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``KEPRIX_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Keprix' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Keprix-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``keprix kanban show <id>`` and the dashboard.
...
# without thrashing. Overridable via ``KEPRIX_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Keprix
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``keprix kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Keprix profile.
    ...
    if not rows:
        return False
    try:
        from keprix_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def dispatch_once(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from keprix_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Keprix profile.
        # `_default_spawn` invokes ``keprix -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Keprix
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `keprix kanban tail` — without
...
def _module_keprix_argv() -> list[str]:
    """Return the interpreter-bound Keprix CLI invocation."""
    # ``keprix_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``keprix`` package — there is no
    # ``keprix`` package to import.
    return [sys.executable, "-m", "keprix_cli.main"]


def _absolute_keprix_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Keprix shim."""
...
def _keprix_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Keprix executable path.
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_keprix_argv()
    return [_absolute_keprix_path(path)]


def _resolve_keprix_argv() -> list[str]:
    """Resolve the ``keprix`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$KEPRIX_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("keprix")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\keprix.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m keprix_cli.main`` — fallback for setups where
       Keprix is launched from a venv and the ``keprix`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_keprix_bin`` for the same reason. Kept
    local (not imported from gateway) because ``keprix_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("KEPRIX_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _keprix_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _keprix_path_argv(resolved_env_bin)
        return _module_keprix_argv()

    keprix_bin = _safe_which_no_cwd("keprix") if _IS_WINDOWS else shutil.which("keprix")
    if keprix_bin:
        return _keprix_path_argv(keprix_bin)
    return _module_keprix_argv()


def _kanban_worker_skill_available(keprix_home: Optional[str]) -> bool:
    """True if the bundled ``kanban-worker`` skill resolves for the home the
    spawned worker will run under.

    The dispatcher injects ``--skills kanban-worker`` into every worker. When
    the worker activates a profile (``keprix -p <name>``), its ``SKILLS_DIR``
    ...
    # An unset KEPRIX_HOME means the worker falls back to the default root
    # home (``~/.keprix``), which ships the bundled skill.
    base = _Path(keprix_home) if keprix_home else (_Path.home() / ".keprix")
...
def _resolve_worker_cli_toolsets(keprix_home: Optional[str]) -> Optional[list[str]]:
    ...
    task-scoped kanban lifecycle tools when ``KEPRIX_KANBAN_TASK`` is set.
    """
    if not keprix_home:
        return None
    try:
        from keprix_constants import reset_keprix_home_override, set_keprix_home_override
        from keprix_cli.config import load_config
        from keprix_cli.tools_config import _get_platform_tools

        token = set_keprix_home_override(keprix_home)
        try:
            cfg = load_config()
        ...
        _log.debug(
            "kanban worker: could not resolve CLI toolsets for KEPRIX_HOME=%r (%s)",
            keprix_home,
            exc,
        )
        return None
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``keprix -p <profile> chat -q ...`` subprocess.
    ...
    ``KEPRIX_KANBAN_DB`` / ``KEPRIX_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject KEPRIX_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `keprix -p <name>` the
    # _apply_profile_override() runs *before* keprix_constants is imported.
    # If KEPRIX_HOME is absent from the child's env, get_keprix_home() falls
    # back to Path.home() / ".keprix" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from keprix_cli.profiles import resolve_profile_env
    try:
        env["KEPRIX_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via KEPRIX_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # KEPRIX_HOME never had profiles created.
        pass
    if task.tenant:
        env["KEPRIX_TENANT"] = task.tenant
    env["KEPRIX_KANBAN_TASK"] = task.id
    env["KEPRIX_KANBAN_WORKSPACE"] = workspace
    if task.branch_name:
        env["KEPRIX_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["KEPRIX_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["KEPRIX_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Goal-loop mode: the worker reads these and wraps its run in the
    # Ralph-style /goal judge loop (see cli.py quiet-mode path). Only set
    # when enabled so non-goal tasks keep a clean env.
    if task.goal_mode:
        env["KEPRIX_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["KEPRIX_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # that even when the worker activates a profile (`keprix -p <name>`
    # rewrites KEPRIX_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_keprix_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["KEPRIX_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["KEPRIX_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["KEPRIX_KANBAN_BOARD"] = resolved_board
    # KEPRIX_PROFILE is the author the kanban_comment tool defaults to.
    # `keprix -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["KEPRIX_PROFILE"] = profile_arg

    cmd = [
        *_resolve_keprix_argv(),
        "-p", profile_arg,
        # Worker subprocesses switch to a profile-scoped KEPRIX_HOME above,
    ...
    if _kanban_worker_skill_available(env.get("KEPRIX_HOME")):
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get("KEPRIX_HOME"))
    if worker_toolsets:
    ...
    # `keprix kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`keprix` executable not found on PATH. "
            "Install Keprix or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``keprix kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled KEPRIX_PROFILE values like "keprix-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Keprix root exists

    Reads profile paths directly so this module has no import dependency on
    ``keprix_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from keprix_constants import get_default_keprix_root
        default_root = get_default_keprix_root()
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``keprix kanban assignees`` for the terminal.
