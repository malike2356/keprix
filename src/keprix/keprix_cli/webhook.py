"""keprix webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    keprix webhook subscribe <name> [options]
    keprix webhook list
    keprix webhook remove <name>
    keprix webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.keprix/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _keprix_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: keprix gateway run
"""
...
def webhook_command(args):
    """Entry point for 'keprix webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: keprix webhook {subscribe|list|remove|test}")
        print("Run 'keprix webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (keprix gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: keprix webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from keprix webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (keprix gateway run)")
