...
the existing module-level functions in keprix_cli.gateway and
keprix_cli.gateway_windows directly. This protocol is a thin facade
...
``keprix gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``keprix gateway start/stop/restart``).
    """
    ...
    # `keprix gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged keprix
    ...
    detection always fails. Since every Keprix runtime call inside the
    container drops to keprix via ``s6-setuidgid``, that silent failure
...
# in ``keprix_cli.gateway`` (systemd/launchd) and ``keprix_cli.gateway_windows``
...
# active (set via the ``keprix -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in keprix_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from keprix_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from keprix_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from keprix_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from keprix_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in keprix_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from keprix_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from keprix_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from keprix_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from keprix_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``keprix_cli.gateway_windows`` (Scheduled Task /
    ...
    def is_running(self, name: str) -> bool:
        from keprix_cli import gateway_windows
        from keprix_cli.gateway import find_gateway_pids
...
#
# Per-profile gateways are registered dynamically when `keprix profile create`
# runs inside the container (Phase 4). Static services (main-keprix, dashboard)
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's KEPRIX_HOME blindly;
    derive the shared profile root from the current KEPRIX_HOME and map the
    ...
    keprix_home = Path(os.environ.get("KEPRIX_HOME", "/opt/data"))
    if keprix_home.parent.name == "profiles":
        root = keprix_home.parent.parent
    else:
        root = keprix_home
...
# UID/GID of the in-image ``keprix`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the keprix user.
    ...
    (the ``keprix`` user — UID 10000 — running every Keprix runtime
    ...
    is dispatched as the keprix user.
    ...
    with keprix ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           keprix:keprix, 0755 (parent must already exist)
    ``svc_dir/event/``                     keprix:keprix, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 keprix:keprix, 0755
    ``svc_dir/supervise/event/``           keprix:keprix, 03730
    ``svc_dir/supervise/control``          keprix:keprix, 0660    (FIFO)
    ...
    slot's supervise/ was keprix-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _KEPRIX_UID, _KEPRIX_GID)
        except PermissionError:
            # Running as the keprix user already — directory is keprix-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _KEPRIX_UID, _KEPRIX_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is keprix-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _KEPRIX_UID, _KEPRIX_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``keprix -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`keprix profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-keprix, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources KEPRIX_HOME (and any extra env) via with-contenv —
             so e.g. ``-e KEPRIX_HOME=/data/keprix`` is honored at run
          ...
          4. Drops to the keprix user and exec's
             ``keprix -p <profile> gateway run`` (or just ``keprix
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``keprix gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root KEPRIX_HOME profile" (the implicit profile that exists at
        the top of $KEPRIX_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``keprix_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$KEPRIX_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``KEPRIX_HOME``,
        ...
        allocator in ``keprix_cli.profiles._allocate_gateway_port``).
        ...
        # guard.
        lines.append("export KEPRIX_S6_SUPERVISED_CHILD=1")
        if profile == "default":
            gateway_cmd = "keprix gateway run"
        else:
            gateway_cmd = f"keprix -p {shlex.quote(profile)} gateway run"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid keprix {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${KEPRIX_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the KEPRIX_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e KEPRIX_HOME=/data/keprix``
        gets its logs under /data/keprix/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``keprix logs`` reads and what
        ...
        return (
            ...
            f': "${{KEPRIX_HOME:=/opt/data}}"\n'
            f'log_dir="$KEPRIX_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            ...
            # later then runs its log service as keprix and crash-loops on
            ...
            f'chown keprix:keprix "$KEPRIX_HOME/logs/gateways" 2>/dev/null || true\n'
            f'chown -R keprix:keprix "$log_dir" 2>/dev/null || true\n'
            f'rm -f "$log_dir/lock"\n'
            # Skip the drop when already non-root (CAP_SETGID).
            f'[ "$(id -u)" = 0 ] || exec s6-log 1 n10 s1000000 T "$log_dir"\n'
            f'exec s6-setuidgid keprix s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``keprix gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``keprix -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with keprix ownership
            ...
            # the keprix user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is keprix-owned (see ``_seed_supervise_skeleton``), and on
