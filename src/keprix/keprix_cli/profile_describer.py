...
Used by ``keprix profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``keprix_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Keprix kanban board.
...
Rules:
  ...
  - Never write "Keprix profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_keprix_home())
        else:
