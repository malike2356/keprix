...
manual BotFather token copy-paste. Keprix talks only to the Nous onboarding
...
DEFAULT_API_URL = "https://setup.keprix.nousresearch.com"
...
DEFAULT_MANAGER_BOT = "KeprixSetupBot"

DEFAULT_BOT_NAME = "Keprix"
...
def generate_username_slug(length: int = 16) -> str:
    ...
    keeping ``keprix_<slug>_bot`` under Telegram's 32-character username limit.
    """
...
def generate_bot_username(profile_name: Optional[str] = None) -> str:
    """Generate a secure suggested bot username like ``keprix_<slug>_bot``.
    ...
    """
    _ = profile_name
    return f"keprix_{generate_username_slug()}_bot"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Keprix Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Keprix Telegram onboarding service.")
