"""Slash command definitions and autocomplete for the Keprix CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Keprix config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Keprix works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Keprix is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.keprix/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Keprix to the latest version", "Info"),
    CommandDef("version", "Show Keprix version", "Info", aliases=("v",)),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`keprix_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/keprix`` subcommand mapping, and
...
Telegram only displays a small BotCommand menu in practice.  The full Keprix
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``keprix skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``keprix skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``keprix skills list`` and
...
# unrelated command landed. These claim their slots right after /keprix,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /keprix <command>).
...
# native slot, the alias spelling stays reachable via /keprix reset).
...
# ``/keprix <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/keprix" decision, not a silent clamp.
#   - credits: the billing/top-up surface; reached via /keprix credits on Slack.
#   - debug: the log/report upload surface; reached via /keprix debug on Slack.
_SLACK_VIA_KEPRIX_ONLY = frozenset({"credits", "debug"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/keprix <verb>`` subcommand.
    ...
    can still reach them via ``/keprix <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/keprix`` is always reserved as the first entry so the
    legacy ``/keprix <subcommand>`` form keeps working for anything that
    ...
    # Reserve /keprix as the catch-all top-level command.
    entries.append(("keprix", "Talk to Keprix or run a subcommand", "[subcommand] [args]"))
    seen.add("keprix")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_KEPRIX_ONLY:
            # Intentionally Slack-via-/keprix only (see _SLACK_VIA_KEPRIX_ONLY).
            return
    ...
    # everything except /keprix, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://keprix.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /keprix handler.

    Maps both canonical names and aliases so /keprix bg do stuff works
    the same as /keprix background do stuff.

    Plugin-registered slash commands are included so ``/keprix <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from keprix_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
