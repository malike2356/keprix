"""Profile distributions — shareable, packaged Keprix profiles via git.

A distribution is a Keprix profile published as a git repository (or
...
* ``keprix profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``keprix skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``keprix profile``, not a parallel tree):

    keprix profile install <source> [--name N] [--alias] [--force] [--yes]
    keprix profile update  <name>  [--force-config] [--yes]
    keprix profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "keprix", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            keprix_requires=str(data.get("keprix_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.keprix_requires:
            out["keprix_requires"] = self.keprix_requires
        if self.author:
...
def check_keprix_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Keprix {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this Keprix distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``keprix profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Keprix profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Keprix distribution."
        )

    # Version check up-front so we fail fast
    check_keprix_requires(manifest.keprix_requires, keprix_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.keprix).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="keprix_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `keprix profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`keprix profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="keprix_dist_update_") as tmp:
        plan = plan_install(
