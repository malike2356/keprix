"""
Cron subcommand for keprix CLI.
...
_GATEWAY_LIFECYCLE_PATTERNS = re.compile(
    r"(?i)"
    r"(keprix\s+gateway\s+(restart|stop|start))"
    r"|(launchctl\s+(kickstart|unload|load|stop|restart)\s+.*keprix)"
    r"|(systemctl\s+(restart|stop|start)\s+.*keprix)"
    r"|(p?kill\s+.*keprix.*gateway)"
)
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'keprix cron create ...' or the /cron command in chat.", Colors.DIM))
        return
    ...
    if not find_gateway_pids():
        ...
        print(color("     Start it with: keprix gateway install", Colors.DIM))
        print(color("                    sudo keprix gateway install --system  # Linux servers", Colors.DIM))
        print()
...
def cron_status():
    ...
    if pids:
        ...
        print("    keprix gateway install    # Install as a user service")
        print("    sudo keprix gateway install --system  # Linux servers: boot-time system service")
        print("    keprix gateway            # Or run in foreground")
...
def cron_create(args):
    ...
    if _contains_gateway_lifecycle_command(combined):
        print(color(
            ...
            "Use `keprix gateway restart` from a shell outside the gateway.",
            Colors.RED,
        ))
        return 1
...
def cron_command(args):
    ...
    print("Usage: keprix cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)
