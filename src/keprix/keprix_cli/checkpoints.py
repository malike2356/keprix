"""`keprix checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.keprix/checkpoints/``.  Actions:

    keprix checkpoints               # same as `status`
    keprix checkpoints status        # total size, project count, breakdown
    keprix checkpoints list          # per-project checkpoint counts + workdir
    keprix checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    keprix checkpoints clear [-f]    # nuke the entire base (asks first)
    keprix checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    keprix checkpoints
    keprix checkpoints prune --retention-days 3 --max-size-mb 200
    keprix checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: keprix checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``keprix checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `keprix checkpoints` → status
