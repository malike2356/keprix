...
def auth_add_command(args) -> None:
    ...
    if provider == "nous":
        raise SystemExit(
            ...
            "Use a BYOK provider: keprix model"
        )
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_keprix_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:keprix_pkce",
    ...
    if provider == "nous":
        ...
        # full device-code flow. This makes `keprix --profile <name>
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``keprix auth add openai-codex`` overwrote
    ...
    raise SystemExit(f"`keprix auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Keprix reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `keprix auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `keprix auth` is called bare."""
...
def _pick_provider(prompt: str = "Provider") -> str:
    ...
    known = sorted(set(list(PROVIDER_REGISTRY.keys()) + ["openrouter"]) - {"nous"})
