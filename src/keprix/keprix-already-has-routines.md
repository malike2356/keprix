# Keprix Has Had "Routines" Since March
...
Keprix equivalent; works today:
```bash
keprix cron create "0 2 * * *" \
...
Keprix equivalent; works today:
```bash
keprix webhook subscribe auth-watch \
  --events "pull_request" \
...
Keprix equivalent; works today:
```bash
keprix webhook subscribe alert-triage \
...
Every use case in their blog post; backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review; has a working Keprix implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Keprix |
|---|---|---|
| **Scheduled tasks** | Done:  Schedule-based | Done:  Any cron expression + human-readable intervals |
| **GitHub triggers** | Done:  PR, issue, push events | Done:  Any GitHub event via webhook subscriptions |
| **API triggers** | Done:  POST to unique endpoint | Done:  POST to webhook routes with HMAC auth |
| **MCP connectors** | Done:  Native connectors | Done:  Full MCP client support |
| **Script pre-processing** | Failed:  | Done:  Python scripts run before agent, inject context |
| **Skill chaining** | Failed:  | Done:  Load multiple skills per automation |
...
## Things Keprix Does That Routines Can't
...
Keprix has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
Keprix is open source and free. The automation infrastructure; cron scheduler, webhook platform, skill system, multi-platform delivery; is built in.
...
```bash
keprix gateway setup    # enable webhooks
keprix webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation blueprints gallery: [keprix.nousresearch.com/docs/reference/automation-blueprints-catalog](https://keprix.nousresearch.com/docs/reference/automation-blueprints-catalog)

Documentation: [keprix.nousresearch.com](https://keprix.nousresearch.com)

GitHub: [github.com/NousResearch/keprix](https://github.com/NousResearch/keprix)

---

*Keprix is built by [Nous Research](https://nousresearch.com). Open source, model-agnostic, runs on your infrastructure.*
