# OpenAI-Compatible API Server for Keprix
...
SSE streaming. By exposing this endpoint, keprix becomes instantly
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.keprix/.env`
2. Run `keprix gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with keprix through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns keprix as a model) |
...
Authorization: Bearer keprix-api-key-here
...
But keprix sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "keprix"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured keprix model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of keprix's core prompt
...
Once implemented, keprix works as a drop-in backend for:
