# Streaming LLM Response Support for Keprix
...
```
KEPRIX_STREAMING_ENABLED=true    # Master switch via env
```
...
try:
    ...
    pass
# Env var override
if os.getenv("KEPRIX_STREAMING_ENABLED", "").lower() in ("true", "1", "yes"):
...
|----------|-------------|-------------|-------------------|
| Telegram | Done:  edit_message_text | ~20 edits/min | Edit every 1.5s |
| Discord | Done:  message.edit | 5 edits/5s per message | Edit every 1.2s |
| Slack | Done:  chat.update | Tier 3 (~50/min) | Edit every 1.5s |
| WhatsApp | Failed:  no edit support | N/A | Skip streaming, use normal path |
| HomeAssistant | Failed:  no edit | N/A | Skip streaming |
| API Server | Done:  SSE native | No limit | Real SSE events |
...
| `keprix_cli/config.py` | 1 | +streaming config defaults |
