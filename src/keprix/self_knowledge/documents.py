...
def _identity_document() -> KnowledgeDocument:
    try:
        ...
        version = installed_keprix_version()
    except Exception:
        version = "unknown"
    content = f"""\
...
License: MIT
Installed version at ingestion: {version}
...
The active provider is deployment-specific. Inspect the provider settings at runtime.
Never infer configuration from a documentation snapshot and never reveal credentials.
...
- KEPRIX_MULTI_USER: deployment-specific boolean
- KEPRIX_DEVELOPER_MODE: deployment-specific boolean
...
This is an indexed orientation guide, not the authority for the running build.
Use the runtime OpenAPI document at `/openapi.json` and interactive API docs for
the exact current paths, methods, schemas, and authorization requirements.
...
## Upgrade System
- GET /api/keprix/upgrade/status - current installed version and available alerts
...
Upgrade wizard UI. Read the current version from the live upgrade status.
...
The API key is deployment-specific and must never be displayed by the copilot.
...
User identities are deployment-specific. Read only authorized user metadata at runtime.
...
2. Ingested into the configured shared RAG store
