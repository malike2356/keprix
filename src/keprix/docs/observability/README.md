# Keprix Observer Hooks

Keprix observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Keprix' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Keprix catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Keprix only builds
