...
Keprix middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Keprix'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Keprix calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Keprix retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Keprix guardrails,
...
- `telemetry_schema_version`: currently `keprix.observer.v1`
- `middleware_schema_version`: currently `keprix.middleware.v1`
...
Keprix stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Keprix runs
them as a nested chain in registration order. Middleware failures are fail-open:
Keprix logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Keprix applies middleware in this order:
...
For each tool call, Keprix applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Keprix pre-execution path against the effective arguments:
...
```bash
keprix plugins enable <plugin-name>
```

For isolated local testing, use one `KEPRIX_HOME` for plugin enablement and the
agent run:

```bash
export KEPRIX_HOME=/tmp/keprix-middleware-test
mkdir -p "$KEPRIX_HOME"
keprix plugins enable <plugin-name>
keprix chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run keprix plugins enable <plugin-name>
uv run keprix chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["keprix_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/keprix-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Keprix expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Keprix treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Keprix preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Keprix does not convert downstream
