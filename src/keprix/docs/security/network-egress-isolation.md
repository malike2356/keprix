...
When running Keprix inside Docker, the default `network_mode: host` gives the
...
The Keprix [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   KEPRIX_UID=$(id -u) KEPRIX_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=keprix,keprix-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://keprix-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
