...
There are two ways to add a platform to the Keprix gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.keprix/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Keprix code**.
...
And add it to the `keprix-gateway` composite:

```python
"keprix-gateway": {
    "includes": [..., "keprix-your-platform"]
}
```
...
## 12. Status Display (`keprix_cli/status.py`)
...
## 13. Gateway Setup Wizard (`keprix_cli/gateway.py`)
