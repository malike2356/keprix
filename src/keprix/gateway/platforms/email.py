"""
Email platform adapter for the Keprix gateway.

Allows users to interact with Keprix by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "keprix" "version" "{_keprix_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Keprix")
        ...
        msg_id = f"<keprix-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
