...
def _resolve_matrix_bang_command(name: str) -> str | None:
    """Resolve a ``!command`` token to a dispatchable Keprix command token.

    Matrix clients often reserve leading ``/`` for local client commands.
    Keprix accepts ``!command`` as a Matrix-friendly alias, but only for
...
def _normalize_matrix_bang_command(text: str) -> str:
    """Convert Matrix ``!command`` aliases to normal Keprix ``/command`` text."""
...
# Uses get_keprix_home() so each profile gets its own Matrix store.
from keprix_constants import get_keprix_dir as _get_keprix_dir

_STORE_DIR = _get_keprix_dir("platforms/matrix/store", "matrix/store")
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    # the adapter accepts "!command" as the alias that always reaches Keprix
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(
            os.getenv("KEPRIX_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6")
        )
        self._text_batch_split_delay_seconds = float(
            os.getenv("KEPRIX_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0")
        )
        ...
        if self._encryption:
            ...
            if self._encryption:
                try:
                    ...
                    _acct_id = self._user_id or "keprix"
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "Keprix" become "Agent".
        """
        if not body:
            return ""
