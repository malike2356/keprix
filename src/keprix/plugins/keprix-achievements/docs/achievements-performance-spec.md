# Keprix Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: keprix-achievements plugin
...
### `GET /api/plugins/keprix-achievements/achievements`
...
### `POST /api/plugins/keprix-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/keprix-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/keprix-achievements/overview`
...
- Persist checkpoint data under:
  - `~/.keprix/plugins/keprix-achievements/scan_checkpoint.json`
...
- `~/.keprix/plugins/keprix-achievements/`
