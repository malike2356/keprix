...
Achievement system for the Keprix Dashboard: collectible, tiered badges generated from real local Keprix session history.

![Keprix Achievements dashboard](docs/assets/achievements-dashboard-hd.png)

The screenshots use temporary demo tier data to show the full visual range. The plugin itself reads real local Keprix session history by default.
...
>
> **Share cards (2026-05-04, vendored in keprix v0.4.0):** Unlocked achievement cards now have a "Share" button that renders a 1200×630 PNG share card (client-side canvas, no backend, no network) with Download + Copy-to-clipboard actions. Fits X/Twitter, Discord, LinkedIn, Bluesky link-preview dimensions.

## What it does

Keprix Achievements scans local Keprix sessions and unlocks badges based on real agent behavior:

- autonomous tool chains
- debugging and recovery patterns
- vibe-coding file edits
- Keprix-native skills, memory, cron, and plugin usage
...
Clone into your Keprix plugins directory:

```bash
git clone https://github.com/PCinkusz/keprix-achievements ~/.keprix/plugins/keprix-achievements
```

For local development, keep the repo elsewhere and symlink it:

```bash
git clone https://github.com/PCinkusz/keprix-achievements ~/keprix-achievements
ln -s ~/keprix-achievements ~/.keprix/plugins/keprix-achievements
```
...
If backend API routes 404, restart `keprix dashboard`; plugin APIs are mounted at dashboard startup.
...
```bash
cd ~/.keprix/plugins/keprix-achievements
git pull --ff-only
curl http://127.0.0.1:9119/api/dashboard/plugins/rescan
```

If the update changes backend routes or `plugin_api.py`, restart `keprix dashboard` after pulling.
...
Achievement unlock state is stored locally in `state.json` and is not overwritten by git updates. New achievements are evaluated from your existing Keprix session history. Achievement IDs are stable and should not be renamed casually because they are the unlock-state keys.
...
```text
/api/plugins/keprix-achievements/
```
