...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-Keprix-Sidecar-Token`.

## First-time setup

```bash
# One-shot setup: device login (opens browser) + project + user + sidecar deps
keprix photon setup --phone +15551234567
...
There is no separate `login` command; like every other Keprix channel,
...
Run `keprix photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.keprix/.env` (the same place every other
...
Management metadata lives in `~/.keprix/auth.json` under `credential_pool`:
