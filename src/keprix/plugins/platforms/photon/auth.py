...
Credential storage mirrors every other Keprix channel:

    * runtime SDK creds  -> ``~/.keprix/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.keprix/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a keprix dependency
...
# registers Keprix as its own client_id.
...
# Default name of the project Keprix provisions for the operator.
DEFAULT_PROJECT_NAME = "Keprix"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of keprix.

def _auth_json_path() -> Path:
    """Resolve ``~/.keprix/auth.json`` honouring the active Keprix profile."""
    try:
        from keprix_constants import get_keprix_home
        return Path(get_keprix_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.keprix")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.keprix/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.keprix/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.keprix/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: keprix_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `keprix photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `keprix photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `keprix photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `keprix photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `keprix photon setup`)"
