...
Small Node helper that bridges Keprix to Photon's Spectrum SDK
(`spectrum-ts`).  Keprix is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Keprix-Sidecar-Token`)
...
The Keprix plugin's `keprix photon setup` command runs `npm install`
here automatically.
