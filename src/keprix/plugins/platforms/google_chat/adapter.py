...
# ``model_tools`` import time, so ``keprix status``, ``keprix chat``, etc.
...
# ``keprix_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Keprix is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a keprix MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${KEPRIX_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _keprix_home = _get_keprix_home()
        except (ModuleNotFoundError, ImportError):
            _keprix_home = _Path.home() / ".keprix"
        self._thread_count_store = _ThreadCountStore(
            _keprix_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Keprix is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("KEPRIX_HOME", str(_Path.home() / ".keprix"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${KEPRIX_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a keprix MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Keprix is thinking…' marker message.
        ...
        "Keprix is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Keprix is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Keprix is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Keprix is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Keprix is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``keprix setup``.

    The setup wizard at ``keprix_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.keprix/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. keprix-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.keprix/.env")
    print_info("Restart the gateway: keprix gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``keprix cron`` running as a
    separate process from ``keprix gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Keprix plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'keprix[google_chat]'",
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Keprix is thinking…' marker message "
