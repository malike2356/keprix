"""SimpleX Chat platform adapter (Keprix plugin).
...
This adapter ships as a Keprix platform plugin under
``plugins/platforms/simplex/``. The Keprix plugin loader scans the
...
Optional environment variables:
    ...
    KEPRIX_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``keprix setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Keprix — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("KEPRIX_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``keprix setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.keprix/.env`` via
    ``keprix_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "keprix_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.keprix/.env"
        )
        return
