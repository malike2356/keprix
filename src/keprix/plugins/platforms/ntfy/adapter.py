"""ntfy platform adapter (Keprix plugin).
...
``plugins/platforms/ntfy/``. The Keprix plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "keprix-in"              # subscribe topic (incoming)
          publish_topic: "keprix-out"     # optional — defaults to topic
...
_ECHO_TAG = "keprix"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``keprix cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Keprix dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `keprix gateway status` without
