"""
Microsoft Teams platform adapter for Keprix.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Keprix"}),
            )
    ...
    async def _fetch_attachment_bytes(self, url: str, timeout: float = 30.0) -> bytes:
        ...
        async with httpx.AsyncClient(
            ...
            response = await client.get(
                url,
                headers={"User-Agent": "Mozilla/5.0 (compatible; KeprixAgent/1.0)"},
            )
            response.raise_for_status()
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        keprix_action = data.get("keprix_action", "")
        session_key = data.get("session_key", "")

        if not keprix_action or not session_key:
        ...
        choice = choice_map.get(keprix_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="keprix_approve",
                    data={**btn_data_base, "keprix_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="keprix_approve",
                    data={**btn_data_base, "keprix_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="keprix_approve",
                    data={**btn_data_base, "keprix_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="keprix_approve",
                    data={**btn_data_base, "keprix_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Keprix\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.keprix/.env")
