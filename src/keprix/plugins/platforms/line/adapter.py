"""
LINE Messaging API platform adapter for Keprix.
...
to Keprix. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``KEPRIX_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            keprix_home = Path.home().joinpath(".keprix").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``keprix status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``keprix status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``keprix setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.keprix/.env`` via ``keprix_cli.config``.
    """
    print()
    ...
    try:
        from keprix_cli.config import get_env_var, set_env_var
    except ImportError:
        print("keprix_cli.config not available; set LINE_* vars manually in ~/.keprix/.env")
        return
