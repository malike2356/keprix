"""
IRC Platform Adapter for Keprix.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Keprix agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "keprix-bot")
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Keprix")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Keprix shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: keprix_, keprix_1, keprix_2...
...
def check_requirements() -> bool:
    ...
    # keprix setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `keprix gateway setup` flow for the IRC platform.

    Lazy-imports ``keprix_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect Keprix to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #keprix — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.keprix/.env")
    print_info("Restart the gateway for changes to take effect: keprix gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "keprix-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "keprix-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Keprix (cron)")
