...
``{title, url, description, position}`` rows every other Keprix web
provider produces.
...
prefers Keprix-managed xAI Grok OAuth (via ``keprix auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.keprix/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``keprix tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `keprix auth` to sign in with "
