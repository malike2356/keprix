...
```bash
keprix memory setup    # select "supermemory"
```

Or manually:

```bash
keprix config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.keprix/.env
```

## Config

Config file: `$KEPRIX_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `keprix` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `keprix-{identity}` → `keprix-coder`). |
...
All Supermemory API calls send `x-sm-source: keprix`, and document writes stamp
`metadata.sm_source: keprix`. This is a **functional routing key, not telemetry**:
it groups Keprix-written memories into a dedicated "Keprix" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Keprix profile:

```json
{
  "container_tag": "keprix-{identity}"
}
```

For a profile named `coder`, this resolves to `keprix-coder`. The default profile resolves to `keprix-default`. Without `{identity}`, all profiles share the same container.
