...
```bash
keprix memory setup    # select "byterover"
```

Or manually:
```bash
keprix config set memory.provider byterover
# Optional cloud sync:
echo "BRV_API_KEY=your-key" >> ~/.keprix/.env
```
...
Working directory: `$KEPRIX_HOME/byterover/` (profile-scoped).
