...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/keprix>
...
keprix memory setup          # generic picker, choose Honcho from the list
```

Or manually:
```bash
keprix config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.keprix/.env
```
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$KEPRIX_HOME/honcho.json` | Profile-local (isolated Keprix instances) |
| 2 | `~/.keprix/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Keprix profile: `keprix` (default) or `keprix_<profile>` (previously `keprix_<profile>`).
...
| 4 | `per-session` strategy | Keprix session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`keprix`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`keprix`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-keprix`.
...
Both profiles see the same user (`yourname`) in the same shared environment (`keprix`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Keprix profile: `keprix` (default) or `keprix_<profile>` (previously `keprix_<profile>`) (e.g. `keprix -p coder` -> host key `keprix_coder`). Older `keprix.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `KEPRIX_HONCHO_HOST` | Host key override |
...
| `keprix honcho mode <mode>` | Change recall or observation mode |
| `keprix honcho peer --user <name>` | Update user peer name |
| `keprix honcho peer --ai <name>` | Update AI peer name |
| `keprix honcho tokens --context <N>` | Set context token budget |
| `keprix honcho tokens --dialectic <N>` | Set dialectic max chars |
| `keprix honcho map <name>` | Map current directory to a session name |
| `keprix honcho sync` | Create host blocks for all Keprix profiles |
