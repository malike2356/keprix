...
```bash
keprix memory setup    # select "holographic"
```

Or manually:
```bash
keprix config set memory.provider holographic
```

## Config

Config in `config.yaml` under `plugins.keprix-memory-store`:

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$KEPRIX_HOME/memory_store.db` | SQLite database path |
