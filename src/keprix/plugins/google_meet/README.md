...
Let the keprix agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where keprix runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `keprix meet node run`)                           │
│     │                                                                │
...
| `cli.py` | `keprix meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$KEPRIX_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `keprix meet node {run,list,approve,remove,status,ping}` |
...
keprix meet install                                      # pip + Chromium
keprix meet setup                                        # preflight
keprix meet auth                                         # optional
keprix meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
keprix meet install --realtime                     # installs pulseaudio-utils
...
keprix meet say "Good morning everyone, I'm the note-taker bot."
```
...
keprix plugins enable google_meet
keprix meet node run --display-name my-mac --host 0.0.0.0 --port 18789
