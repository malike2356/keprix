...
keprix meet install                 # pip deps + Chromium (transcribe only)
keprix meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
keprix meet auth                    # optional; skips guest-lobby wait
keprix meet setup                   # preflight checks
```
...
#   Then set OPENAI_API_KEY or KEPRIX_MEET_REALTIME_KEY in ~/.keprix/.env
```
...
pip install playwright websockets && python -m playwright install chromium
keprix plugins enable google_meet
keprix meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
keprix meet node approve my-mac ws://<mac-ip>:18789 <token>
keprix meet node ping my-mac                   # confirm reachable
```

Run `keprix meet setup` to preflight local prereqs.
...
- Guest mode sits in the lobby until a host admits. Warn the user; `keprix meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `KEPRIX_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `keprix_meet_src`). |
...
Local:
```
$KEPRIX_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
