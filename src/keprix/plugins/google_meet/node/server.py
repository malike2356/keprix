...
Launched by ``keprix meet node run``.

Token handling
--------------
On first boot we mint 32 hex chars of entropy and persist them at
``$KEPRIX_HOME/workspace/meetings/node_token.json``. Subsequent boots
...
via ``keprix meet node approve <name> <url> <token>``.
...
def _default_token_path() -> Path:
    return Path(get_keprix_home()) / "workspace" / "meetings" / "node_token.json"
...
class NodeServer:
    ...
    def __init__(
        ...
        display_name: str = "keprix-meet-node",
    ) -> None:
        self.host = host
        self.port = port
