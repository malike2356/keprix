...
session-scoped state file under ``$KEPRIX_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $KEPRIX_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_keprix_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this keprix install, leave it first —
    ...
    env = os.environ.copy()
    env["KEPRIX_MEET_URL"] = url
    env["KEPRIX_MEET_OUT_DIR"] = str(out)
    env["KEPRIX_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["KEPRIX_MEET_HEADED"] = "1"
    if auth_state:
        env["KEPRIX_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["KEPRIX_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if KEPRIX_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["KEPRIX_MEET_MODE"] = mode
    if realtime_model:
        env["KEPRIX_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["KEPRIX_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["KEPRIX_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["KEPRIX_MEET_REALTIME_KEY"] = realtime_api_key
