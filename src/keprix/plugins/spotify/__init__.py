...
``_check_spotify_available()`` — when the user has not run ``keprix auth
spotify``, the tools remain registered (so they appear in ``keprix
...
The Spotify auth flow (``keprix auth spotify``), CLI plumbing, and docs
