"""disk_cleanup — ephemeral file cleanup for Keprix.
...
Rules:
  ...
  - empty dirs    → always delete (under KEPRIX_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly KEPRIX_HOME and /tmp/keprix-*
Never touches: ~/.keprix/logs/ or any system directory.
"""
...
try:
    ...
    def get_keprix_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("KEPRIX_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".keprix").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$KEPRIX_HOME/logs/``."""
    return get_keprix_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$KEPRIX_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under KEPRIX_HOME or ``/tmp/keprix-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    keprix_home = get_keprix_home()
    try:
        path.resolve().relative_to(keprix_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/keprix-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("keprix-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "keprix", "backups", "profiles", ".worktrees",
})
...
# Paths under $KEPRIX_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    """
    # Lazily build the set once per process so KEPRIX_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        keprix_home = get_keprix_home()
        for parent in ("cron", "cronjobs"):
            base = keprix_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside KEPRIX_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under KEPRIX_HOME, but never recurse into known
    # durable state trees.  Some installs place the Keprix checkout, venv,
    # and desktop build under KEPRIX_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    keprix_home = get_keprix_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in keprix_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    keprix_home = get_keprix_home()
    try:
        rel = path.resolve().relative_to(keprix_home)
        ...
        # Path isn't under KEPRIX_HOME (e.g. /tmp/keprix-*) — fall through.
        pass
