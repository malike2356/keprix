...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `KEPRIX_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under KEPRIX_HOME | always | Never |
...
- `is_safe_path()` rejects anything outside `KEPRIX_HOME` or `/tmp/keprix-*`
