...
Optional Keprix observability plugin that maps Keprix observer hooks to
...
not replace Keprix's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Keprix emit NeMo Relay lifecycle events for
the work Keprix already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Keprix can:

- Preserve Keprix execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
keprix plugins enable observability/nemo_relay
```

The `KEPRIX_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `KEPRIX_HOME` that the
agent run will use:

```bash
env KEPRIX_HOME=/tmp/keprix-nemo-relay-test \
  keprix plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`KEPRIX_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`KEPRIX_HOME` is the Keprix profile/config home used by both
`keprix plugins enable ...` and the later `keprix chat ...` run. If unset,
Keprix uses the user's default home, usually `~/.keprix`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export KEPRIX_HOME=/tmp/keprix-nemo-relay-test
keprix plugins enable observability/nemo_relay
keprix chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `keprix` command you run is built from the
...
uv run keprix plugins enable observability/nemo_relay
uv run keprix chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/keprix-*.whl
...
The plugin can configure exporters directly from `KEPRIX_NEMO_RELAY_*`
...
```bash
export KEPRIX_NEMO_RELAY_ATOF_ENABLED=1
export KEPRIX_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export KEPRIX_NEMO_RELAY_ATIF_ENABLED=1
export KEPRIX_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `KEPRIX_NEMO_RELAY_ATOF_FILENAME`
- `KEPRIX_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `KEPRIX_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `KEPRIX_NEMO_RELAY_ATIF_AGENT_NAME`
- `KEPRIX_NEMO_RELAY_ATIF_AGENT_VERSION`
- `KEPRIX_NEMO_RELAY_ATIF_MODEL_NAME`
- `KEPRIX_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Keprix at it:

```bash
export KEPRIX_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
When `KEPRIX_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`KEPRIX_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`KEPRIX_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Keprix does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Keprix keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Keprix routes LLM and tool
...
For the full generic Keprix middleware contract, see
...
export KEPRIX_HOME=/tmp/keprix-nemo-relay-docs/keprix-home
mkdir -p "$KEPRIX_HOME"

cat > "$KEPRIX_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Keprix session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export KEPRIX_NEMO_RELAY_ATOF_ENABLED=1
export KEPRIX_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/keprix-nemo-relay-docs/subagent/atof
export KEPRIX_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export KEPRIX_NEMO_RELAY_ATOF_MODE=overwrite
export KEPRIX_NEMO_RELAY_ATIF_ENABLED=1
export KEPRIX_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/keprix-nemo-relay-docs/subagent/atif
export KEPRIX_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export KEPRIX_NEMO_RELAY_ATIF_AGENT_NAME='Keprix E2E'
export KEPRIX_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export KEPRIX_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"keprix.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Keprix E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Keprix dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/keprix-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/keprix-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/keprix-nemo-relay-docs/workdir/beta.txt
cd /tmp/keprix-nemo-relay-docs/workdir

export KEPRIX_NEMO_RELAY_ATOF_ENABLED=1
export KEPRIX_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/keprix-nemo-relay-docs/parallel/atof
export KEPRIX_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export KEPRIX_NEMO_RELAY_ATOF_MODE=overwrite
export KEPRIX_NEMO_RELAY_ATIF_ENABLED=1
export KEPRIX_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/keprix-nemo-relay-docs/parallel/atif
export KEPRIX_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export KEPRIX_NEMO_RELAY_ATIF_AGENT_NAME='Keprix E2E'
export KEPRIX_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Keprix sessions map to `agent` scopes.
- Keprix API request hooks map to `llm` scope start/end events.
- Keprix tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Keprix execution middleware to hand
...
Enable it for Keprix:

```bash
export KEPRIX_NEMO_RELAY_PLUGINS_TOML=/tmp/keprix-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Keprix routes execution
through these boundaries:

```text
Keprix provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Keprix provider adapter next_call(...)

Keprix tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Keprix tool dispatcher next_call(...)
```
...
middleware for a local Keprix run. This path requires a NeMo Relay runtime that
...
```bash
export KEPRIX_HOME=/tmp/keprix-middleware-test/keprix-home
mkdir -p "$KEPRIX_HOME" /tmp/keprix-middleware-test/nemo-relay
...
cat > /tmp/keprix-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/keprix-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/keprix-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Keprix Middleware E2E"
agent_version = "local"
...
export KEPRIX_NEMO_RELAY_PLUGINS_TOML=/tmp/keprix-middleware-test/nemo-relay/plugins.toml
