...
_KEPRIX_CORE_TOOLS = [
    ...
    "terminal", "process",
    # Read the desktop GUI's embedded terminal pane (gated on KEPRIX_DESKTOP
    ...
    # spawned as a kanban worker (KEPRIX_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `keprix tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (KEPRIX_KANBAN_TASK env "
    ...
    # Coding posture (base Keprix — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in keprix_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Keprix toolsets (CLI + messaging platforms)
    #
    ...
    "keprix-cli": {
        ...
        "tools": _KEPRIX_CORE_TOOLS,
        "includes": []
    },

    "keprix-cron": {
        # Mirrors keprix-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `keprix tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as keprix-cli; gated by `keprix tools`",
        "tools": _KEPRIX_CORE_TOOLS,
        "includes": []
    },
    ...
    "keprix-email": {
        "description": "Email bot toolset - interact with Keprix via email (IMAP/SMTP)",
        "tools": _KEPRIX_CORE_TOOLS,
        "includes": []
    },
    ...
    "keprix-sms": {
        "description": "SMS bot toolset - interact with Keprix via SMS (Twilio)",
        "tools": _KEPRIX_CORE_TOOLS,
        "includes": []
    },
    ...
    "keprix-webhook": {
        ...
        "tools": _KEPRIX_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "keprix-gateway": {
        ...
        "tools": [],
        "includes": ["keprix-telegram", "keprix-discord", "keprix-whatsapp", "keprix-slack", "keprix-signal", "keprix-bluebubbles", "keprix-homeassistant", "keprix-email", "keprix-sms", "keprix-mattermost", "keprix-matrix", "keprix-dingtalk", "keprix-feishu", "keprix-wecom", "keprix-wecom-callback", "keprix-weixin", "keprix-qqbot", "keprix-webhook", "keprix-yuanbao"]
    }
}
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (keprix-<name>).
        # Gives them _KEPRIX_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("keprix-"):
            platform_name = name[len("keprix-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_KEPRIX_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
