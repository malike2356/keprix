...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``keprix://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``keprix://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"keprix://blueprint/{quote(blueprint.key)}{qs}"
