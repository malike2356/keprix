...
#   - cli.py, keprix_cli/*      -> inline on startup (no event loop)
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if os.environ.get("KEPRIX_KANBAN_TASK") and "kanban" not in effective_enabled_toolsets:
            # Dispatcher-spawned workers are scoped by KEPRIX_KANBAN_TASK and
    ...
    # This ensures that even if a composite toolset (like keprix-cli)
    ...
    # window). Core Keprix tools (toolsets._KEPRIX_CORE_TOOLS) are NEVER
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from keprix_cli.config import load_config as _load
        cfg = _load() or {}
...
def _emit_post_tool_call_hook(
    *,
    ...
    """
    try:
        from keprix_cli.plugins import has_hook, invoke_hook
...
def handle_function_call(
    ...
    if not skip_tool_request_middleware:
        try:
            from keprix_cli.middleware import apply_tool_request_middleware
    ...
    try:
        ...
        if not skip_pre_tool_call_hook:
            ...
            try:
                from keprix_cli.plugins import get_pre_tool_call_block_message
