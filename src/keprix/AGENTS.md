# Keprix - Development Guide

Instructions for AI coding assistants and developers working on the keprix codebase.
...
- **E2E validation, not just green unit mocks.** For anything touching
  ...
  against a temp `KEPRIX_HOME`. Mocks hide integration bugs.
...
- **Outbound telemetry / usage attribution without opt-in gating.** No new
  ...
  generic user-facing opt-in (config gate + setup prompt + `keprix tools`
...
`$HOME/.keprix/keprix/venv` (for worktrees that share a venv with the
main checkout).
...
│   ├── keprix-achievements/  # Gamified achievement tracking
...
Applies to TypeScript across Keprix: desktop, TUI, website, and future TS packages.
...
1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `keprix_cli/commands.py`:
```python
CommandDef("mycommand", "Description of what it does", "Session",
           aliases=("mc",), args_hint="[arg]"),
```
2. Add handler in `KeprixCLI.process_command()` in `cli.py`:
```python
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `keprix --tui` or `KEPRIX_TUI=1`.
...
npm run dev       # watch mode (rebuilds keprix-ink + tsx --watch)
npm start         # production
npm run build     # full build (keprix-ink + tsc)
...
be core tools. For custom or local-only tools, do **not** edit Keprix core.
Use the plugin route instead: create `~/.keprix/plugins/<name>/plugin.yaml`
and `~/.keprix/plugins/<name>/__init__.py`, then register tools with
`ctx.register_tool(...)`. Plugin toolsets are discovered automatically and can be
enabled or disabled without touching `tools/` or `toolsets.py`.

Use the built-in route below only when the user is explicitly contributing a new
core Keprix tool that should ship in the base system.
...
### config.yaml options:
1. Add to `DEFAULT_CONFIG` in `keprix_cli/config.py`
...
```
keprix_cli/skin_engine.py    # SkinConfig dataclass, built-in skins, YAML loader
...
Add to `_BUILTIN_SKINS` dict in `keprix_cli/skin_engine.py`:
...
Users create `~/.keprix/skins/<name>.yaml`:
...
Keprix has two plugin surfaces. Both live under `plugins/` in the repo so
repo-shipped plugins can be discovered alongside user-installed ones in
`~/.keprix/plugins/` and pip-installed entry points.

### General plugins (`keprix_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.keprix/plugins/`, `./.keprix/plugins/`,
...
`post_setup(keprix_home, config)` for setup-wizard integration.

**CLI commands via `plugins/memory/<name>/cli.py`:** if a memory plugin
defines `register_cli(subparser)`, `discover_plugin_cli_commands()` finds
it at argparse setup time and wires it into `keprix <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `keprix --help`.
...
[`keprix-example-plugins`](https://github.com/NousResearch/keprix-example-plugins)
...
2. **Tools referenced in SKILL.md prose must be native Keprix tools or
...
`"messaging"`); `_KEPRIX_CORE_TOOLS` is the default bundle most
...
Enable/disable per platform via `keprix tools` (the curses UI) or the
...
go to `~/.keprix/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `keprix_cli/curator.py` wires `keprix curator <verb>` where
...
schedule jobs via the `cronjob` tool; users via `keprix cron <verb>`
...
collaborate on shared tasks. Users drive it via `keprix kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `keprix_cli/kanban.py` wires `keprix kanban` with verbs
...
- **Plugin assets:** `plugins/kanban/dashboard/` (web UI) +
  `plugins/kanban/systemd/` (`keprix-kanban-dispatcher.service` for
...
Keprix-Agent ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
`KEPRIX_HOME` before any module imports. All `get_keprix_home()` references
automatically scope to the active profile.

### Rules for profile-safe code

1. **Use `get_keprix_home()` for all KEPRIX_HOME paths.** Import from `keprix_constants`.
   NEVER hardcode `~/.keprix` or `Path.home() / ".keprix"` in code that reads/writes state.
   ```python
   # GOOD
...
2. **Use `display_keprix_home()` for user-facing messages.** Import from `keprix_constants`.
   This returns `~/.keprix` for default or `~/.keprix/profiles/<name>` for profiles.
   ```python
   # GOOD
...
4. **Tests that mock `Path.home()` must also set `KEPRIX_HOME`**; since code now uses
   ...
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"KEPRIX_HOME": str(tmp_path / ".keprix")}):
       ...
   ```
...
with actual imports (not mocks) against a temp `KEPRIX_HOME`.

### Tests must not write to `~/.keprix/`
The `_isolate_keprix_home` autouse fixture in `tests/conftest.py` redirects `KEPRIX_HOME` to a temp dir. Never hardcode `~/.keprix/` paths in tests.

**Profile tests**: When testing profile features, also mock `Path.home()` so that
`_get_profiles_root()` and `_get_default_keprix_home()` resolve within the temp dir.
Use the pattern from `tests/keprix_cli/test_profiles.py`:
```python
@pytest.fixture
