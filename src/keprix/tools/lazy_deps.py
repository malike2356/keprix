"""
Lazy dependency installer for opt-in Keprix backends.

Many Keprix features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``keprix[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``keprix tools`` or the manual pip command.
...
def _allow_lazy_installs() -> bool:
    ...
    """
    if os.environ.get("KEPRIX_DISABLE_LAZY_INSTALLS") == "1":
        return False
    try:
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``keprix update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    Mirrors the strategy in ``keprix_cli.tools_config._pip_install`` but
...
def active_features() -> list[str]:
    ...
    Used by ``keprix update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``keprix update``. Never raises; lazy-install failures
