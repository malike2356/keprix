...
def _is_keprix_provider_credential(name: str) -> bool:
    """True if ``name`` is a Keprix-managed provider credential (API key,
    token, or similar) per ``_KEPRIX_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Keprix API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    try:
        ...
        return False
    return name in _KEPRIX_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Keprix-managed provider credentials (from
    ``_KEPRIX_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Keprix-managed
    ...
    Non-Keprix third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_keprix_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Keprix provider "
                "credential %r (blocked by _KEPRIX_PROVIDER_ENV_BLOCKLIST). "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Keprix-managed provider credentials must not be passed
                ...
                if _is_keprix_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register Keprix "
                        "provider credential %r from config.yaml (blocked "
                        "by _KEPRIX_PROVIDER_ENV_BLOCKLIST). Operator "
