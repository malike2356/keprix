...
`mcp_tool.py`, and `keprix_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# KeprixMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_keprix_provider_class() -> Optional[type]:
    ...
    class KeprixMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, server_name: str = "", **kwargs: Any):
            ...
            self._keprix_server_name = server_name
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`KeprixTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, KeprixTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._keprix_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import KeprixTokenStorage
                        if isinstance(storage, KeprixTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._keprix_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import KeprixTokenStorage
            if not isinstance(storage, KeprixTokenStorage):
                return
    ...
    return KeprixMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_KEPRIX_PROVIDER_CLS: Optional[type] = _make_keprix_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`KeprixMCPOAuthProvider` directly using the helpers
        ...
        storage = KeprixTokenStorage(server_name)

        if not _is_interactive() and not storage.has_cached_tokens():
            raise OAuthNonInteractiveError(
                "MCP OAuth for "
                f"'{server_name}': non-interactive environment and no "
                "cached tokens found. Run `keprix mcp login "
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``keprix mcp remove <name>`` and (indirectly) by
        ``keprix mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
