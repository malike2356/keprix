...
from tenancy.home import shared_soul_write_error
...
_keprix_config_resolved: str | None = None
_keprix_config_resolved_loaded = False


def _get_keprix_config_resolved() -> str | None:
    """Return the resolved absolute path of the Keprix config file (cached)."""
    global _keprix_config_resolved, _keprix_config_resolved_loaded
    if _keprix_config_resolved_loaded:
        return _keprix_config_resolved
    _keprix_config_resolved_loaded = True
    try:
        from keprix_cli.config import get_config_path
        _keprix_config_resolved = str(get_config_path().resolve())
    except Exception:
        try:
            _keprix_config_resolved = str(Path("~/.keprix/config.yaml").expanduser().resolve())
        except Exception:
            _keprix_config_resolved = None
    return _keprix_config_resolved
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    if resolved in _SENSITIVE_EXACT_PATHS or normalized in _SENSITIVE_EXACT_PATHS:
        return _err
    # Prevent agents from modifying the Keprix config file directly.
    # approvals.mode and other security settings live here; a malicious or
    # prompt-injected agent could silently disable exec approval by writing to
    # this file.
    keprix_config = _get_keprix_config_resolved()
    if keprix_config and (resolved == keprix_config or normalized == keprix_config):
        return (
            f"Refusing to write to Keprix config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.keprix/config.yaml directly or use 'keprix config' instead."
        )
    return None


def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Keprix mirror prefix for Docker file tools."""
    try:
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    """Return a soft-guard warning when ``filepath`` lands in another Keprix
    profile's scoped area, a host-side sandbox-mirror of authoritative profile
    state, or the Docker container's sandbox mirror of Keprix state.
    ...
    * sandbox-mirror (#32049) — writes that hit the
      ``…/sandboxes/<backend>/<task>/home/.keprix/…`` mirror created by a
      non-local terminal backend (Docker, Daytona, etc.), where the host
      Keprix process never reads the mirror and the authoritative file is
      left untouched.
    * container-mirror (#32049 follow-up) — writes from inside a Docker
      ...
      the agent sees a plain ``/root/.keprix/…`` path.

    Returns ``None`` when the write is in-scope or outside Keprix scope.
    ...
    # in a session that cd'd into ``~/.keprix/profiles/other/`` is
...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        # ── Keprix internal path guard ────────────────────────────────
        # Prevent prompt injection via catalog or hub metadata files,
        # and block credential stores under KEPRIX_HOME.  Pass the
        # already-resolved path so a relative-path read against
        # TERMINAL_CWD == KEPRIX_HOME (e.g. "auth.json") still hits the
...
def write_file_tool(path: str, content: str, task_id: str = "default",
    ...
    ``cross_profile`` opts out of the soft cross-Keprix-profile guard. The
    ...
    soul_err = shared_soul_write_error(path)
    if soul_err:
        return tool_error(soul_err)
...
def patch_tool(mode: str = "replace", path: str = None, old_string: str = None,
    ...
    ``cross_profile`` opts out of the soft cross-Keprix-profile guard for
    ...
    for _p in _paths_to_check:
        ...
        if sensitive_err:
            return tool_error(sensitive_err)
        soul_err = shared_soul_write_error(_p)
        if soul_err:
            return tool_error(soul_err)
        if not cross_profile:
...
WRITE_FILE_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Keprix profile's skills/plugins/cron/memories — by default these writes are blocked with a warning because they affect a different profile than the one this session is running under.",
                "default": False,
            },
        },
...
PATCH_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "cross_profile": {
                "type": "boolean",
                "description": "Opt out of the cross-profile soft guard. Defaults to false. Set true ONLY after explicit user direction to edit another Keprix profile's skills/plugins/cron/memories.",
                "default": False,
            },
        },
        "required": ["mode"],
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with keprix_tools.write_file() for very "
            "large files."
        )
