...
Backend is selected during ``keprix tools`` setup (web.backend in config.yaml).
When available, Keprix can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Keprix config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Keprix' config/.env layer (``keprix config set``, ``keprix tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.keprix/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``keprix tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `keprix tools` repaint.
        try:
        ...
        if provider is None:
            response_data = {
                ...
                "error": (
                    ...
                    "Run `keprix tools` to set one up."
                ),
            }
        else:
            logger.info(
