...
`keprix skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.keprix.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.keprix.blueprint`` is a mapping containing
    ...
    keprix = meta.get("keprix") if isinstance(meta, dict) else None
    blueprint = keprix.get("blueprint") if isinstance(keprix, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.keprix.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.keprix.blueprint`` block) they can hand
    to ``keprix skills publish`` to share. ``body`` is the plain-language
