...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   SANDBOX temp dir (for example /tmp/keprix-results/{tool_use_id}.txt on
   standard Linux, or $TMPDIR/keprix-results/{tool_use_id}.txt on Termux)
...
STORAGE_DIR = "/tmp/keprix-results"
HEREDOC_MARKER = "KEPRIX_PERSIST_EOF"
        ...
        if callable(get_temp_dir):
            try:
                ...
                if temp_dir:
                    ...
                    return f"{temp_dir}/keprix-results"
    return STORAGE_DIR
...
def _heredoc_marker(content: str) -> str:
    ...
    return f"KEPRIX_PERSIST_{uuid.uuid4().hex[:8]}"
