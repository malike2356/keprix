...
def auth_json_path():
    """Return the Keprix auth store path, respecting KEPRIX_HOME overrides."""
    return get_keprix_home() / "auth.json"
...
def peek_nous_access_token() -> Optional[str]:
    ...
    Availability scans (`keprix tools`, banner/status paint, provider
...
def read_nous_access_token() -> Optional[str]:
    ...
    try:
        from keprix_cli.auth import resolve_nous_access_token
