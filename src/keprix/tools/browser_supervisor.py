...
One ``CDPSupervisor`` runs per Keprix ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "keprix-dialog-bridge.invalid"
...
(() => {
  if (window.__keprixDialogBridgeInstalled) return;
  window.__keprixDialogBridgeInstalled = true;
  const ENDPOINT = "http://keprix-dialog-bridge.invalid/";
