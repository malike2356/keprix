...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``keprix tools`` repaint, tool-registration scans,
    ...
    2. ``~/.keprix/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_keprix_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.keprix/.env`` first, then ``os.environ``.

    Wraps :func:`keprix_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from keprix_cli.config import get_env_value as _keprix_get_env_value

        value = _keprix_get_env_value(name)
...
def keprix_xai_user_agent() -> str:
    """Return a stable Keprix-specific User-Agent for xAI HTTP calls."""
    try:
        from keprix_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Keprix-Agent/{__version__}"


def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    """Resolve bearer credentials for direct xAI HTTP endpoints.

    Prefers Keprix-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``keprix_cli.config.get_env_value`` so keys
    stored in ``~/.keprix/.env`` (the standard Keprix location) are honored —
