...
it is automatically downloaded from GitHub releases to $KEPRIX_HOME/bin/tirith.
...
def _get_keprix_home() -> str:
    """Return the Keprix home directory, respecting KEPRIX_HOME env var."""
    return str(get_keprix_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_keprix_home(), ".tirith-install-failed")
...
def _keprix_bin_dir() -> str:
    """Return $KEPRIX_HOME/bin, creating it if needed."""
    d = os.path.join(_get_keprix_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $KEPRIX_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_keprix_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $KEPRIX_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $KEPRIX_HOME/bin/tirith
    ...
    keprix_bin = os.path.join(_keprix_bin_dir(), "tirith")
    if os.path.isfile(keprix_bin) and os.access(keprix_bin, os.X_OK):
        _resolved_path = keprix_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return keprix_bin
