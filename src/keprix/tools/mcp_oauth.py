...
This module provides the glue:
    - ``KeprixTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Keprix"
"""
...
from keprix_constants import secure_parent_dir
...
_USER_SKIPPED_SENTINEL = "__keprix_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses KEPRIX_HOME so each profile gets its own OAuth tokens.
    Layout: ``KEPRIX_HOME/mcp-tokens/``
    """
    try:
        from keprix_constants import get_keprix_home
        base = Path(get_keprix_home())
    except ImportError:
        base = Path(os.environ.get("KEPRIX_HOME", str(Path.home() / ".keprix")))
...
# ---------------------------------------------------------------------------
# KeprixTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class KeprixTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        KEPRIX_HOME/mcp-tokens/<server_name>.json         -- tokens
        KEPRIX_HOME/mcp-tokens/<server_name>.client.json   -- client info
        KEPRIX_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Keprix records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Keprix.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://keprix.nousresearch.com/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `keprix mcp login <server>` later to "
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Keprix")
    scope = cfg.get("scope")
...
def build_oauth_auth(
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`keprix mcp login {server_name}` interactively first to complete "
