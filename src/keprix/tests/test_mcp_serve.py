"""
Tests for mcp_serve — Keprix MCP server.
...
def _isolate_keprix_home(tmp_path, monkeypatch):
    """Redirect KEPRIX_HOME to a temp directory."""
    monkeypatch.setenv("KEPRIX_HOME", str(tmp_path))
    try:
        import keprix_constants
        monkeypatch.setattr(keprix_constants, "get_keprix_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking keprix_state schema."""
