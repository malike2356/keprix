...
class TestToolsetConsistency:
    ...
    def test_keprix_platforms_share_core_tools(self):
        """All keprix-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on keprix-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["keprix-cli", "keprix-telegram", "keprix-discord", "keprix-whatsapp", "keprix-slack", "keprix-signal", "keprix-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_keprix_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("keprix-whatsapp")

    def test_keprix_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("keprix-api-server")
