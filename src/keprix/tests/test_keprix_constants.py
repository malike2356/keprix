"""Tests for keprix_constants module."""
...
class TestGetDefaultKeprixRoot:
    """Tests for get_default_keprix_root() — Docker/custom deployment awareness."""

    def test_no_keprix_home_returns_native(self, tmp_path, monkeypatch):
        """When KEPRIX_HOME is not set, returns ~/.keprix."""
        monkeypatch.delenv("KEPRIX_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_keprix_root() == tmp_path / ".keprix"

    def test_keprix_home_is_native(self, tmp_path, monkeypatch):
        """When KEPRIX_HOME = ~/.keprix, returns ~/.keprix."""
        native = tmp_path / ".keprix"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("KEPRIX_HOME", str(native))
        assert get_default_keprix_root() == native

    def test_keprix_home_is_profile(self, tmp_path, monkeypatch):
        """When KEPRIX_HOME is a profile under ~/.keprix, returns ~/.keprix."""
        native = tmp_path / ".keprix"
    ...
    def test_keprix_home_is_docker(self, tmp_path, monkeypatch):
        """When KEPRIX_HOME points outside ~/.keprix (Docker), returns KEPRIX_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("KEPRIX_HOME", str(docker_home))
        assert get_default_keprix_root() == docker_home

    def test_keprix_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any KEPRIX_HOME outside ~/.keprix is treated as the root."""
        custom = tmp_path / "my-keprix-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("KEPRIX_HOME", str(custom))
        assert get_default_keprix_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (KEPRIX_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("KEPRIX_HOME", str(profile))
        assert get_default_keprix_root() == docker_root

    def test_no_keprix_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\keprix, not ~/.keprix."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("KEPRIX_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(keprix_constants.sys, "platform", "win32")

        assert get_default_keprix_root() == local_appdata / "keprix"

    def test_no_keprix_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        """Windows fallback still uses AppData/Local/keprix without LOCALAPPDATA."""
        home = tmp_path / "Home"
        monkeypatch.delenv("KEPRIX_HOME", raising=False)
        ...
        assert get_default_keprix_root() == home / "AppData" / "Local" / "keprix"
...
class TestGetKeprixHome:
    ...
    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        ...
        assert get_keprix_home() == local_appdata / "keprix"
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(keprix_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(keprix_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".keprix"
