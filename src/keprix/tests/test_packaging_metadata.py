...
def test_every_on_disk_subpackage_is_covered_by_packages_find():
    ...
    top-level package is listed twice — bare (``keprix_cli``) for the package
    itself and ``keprix_cli.*`` for its subpackages — EXCEPT when someone
    forgets the wildcard. v0.15.x listed ``keprix_cli`` without ``keprix_cli.*``,
    so the wheel shipped ``keprix_cli/*.py`` but dropped the ``dashboard_auth``
    and ``proxy`` subpackages. The dashboard then died on every install with
    ``ModuleNotFoundError: No module named 'keprix_cli.dashboard_auth'``.
...
def test_packaging_declared_as_core_dependency():
    ...
    keprix_cli/main.py) yet was undeclared, so it only reached users
...
def test_bundled_plugin_manifests_ship_in_both_wheel_and_sdist():
    ...
    Plugin discovery (keprix_cli/plugins.py) registers each bundled plugin by
...
def test_optional_mcps_manifests_ship_in_both_wheel_and_sdist():
    ...
    keprix_cli/mcp_catalog.py resolves the catalog via get_optional_mcps_dir()
    ...
    `keprix mcp catalog` and the dashboard catalog screen come up empty on
