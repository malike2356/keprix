...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Keprix passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("KEPRIX_HOME", "/users/alice/.keprix/profiles/backend-worker")
        monkeypatch.setenv("KEPRIX_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "KEPRIX_KANBAN_DB",
            "/users/alice/.keprix/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.keprix/kanban/boards/smoke"]'
            in cmd
        )
