...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "Keprix Tests")
...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_and_keprix_paths(tmp_path: Path, monkeypatch):
    from agent.context_references import preprocess_context_references_async

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("KEPRIX_HOME", str(tmp_path / ".keprix"))

    keprix_env = tmp_path / ".keprix" / ".env"
    keprix_env.parent.mkdir(parents=True)
    keprix_env.write_text("API_KEY=super-secret\n", encoding="utf-8")
    ...
    result = await preprocess_context_references_async(
        "read @file:.keprix/.env and @file:.ssh/id_rsa",
        cwd=tmp_path,
