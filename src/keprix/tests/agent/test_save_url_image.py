...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate KEPRIX_HOME under tmp_path."""
    monkeypatch.setenv("KEPRIX_HOME", str(tmp_path / ".keprix"))
    (tmp_path / ".keprix").mkdir()

    # Force the constants/image cache helpers to re-read KEPRIX_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("keprix_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_keprix_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under KEPRIX_HOME — gateway cleanup
