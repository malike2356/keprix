...
def test_keprix_client_tag_includes_current_version():
    """The client tag must reflect keprix_cli.__version__ verbatim."""
    from keprix_cli import __version__
    from agent.portal_tags import keprix_client_tag

    assert keprix_client_tag() == f"client=keprix-client-v{__version__}"


def test_keprix_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import keprix_client_tag

    tag = keprix_client_tag()
    assert tag.startswith("client=keprix-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import keprix_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=keprix" in tags
    assert keprix_client_tag() in tags
    assert len(tags) == 2
