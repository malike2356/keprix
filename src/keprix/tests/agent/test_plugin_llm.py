...
""",
        ...
        from keprix_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        keprix_home = tmp_path / ".keprix"
        keprix_home.mkdir()
        (keprix_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("KEPRIX_HOME", str(keprix_home))
        from keprix_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from keprix_cli.plugins import PluginContext, PluginManifest, PluginManager
