...
def isolated_home(monkeypatch):
    """Temp KEPRIX_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="keprix_test_31179_")
    keprix_home = os.path.join(test_home, ".keprix")
    os.makedirs(keprix_home)
...
def _fresh_modules():
    """Drop cached keprix modules so each test reloads against current env."""
