...
engine plugins (e.g. keprix-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``keprix plugins`` picker."""
    from keprix_cli import plugins_cmd
    ...
    monkeypatch.setattr(
        "keprix_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "keprix_cli.plugins.get_plugin_context_engine",
