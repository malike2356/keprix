...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_keprix_env(self):
        """Keprix' own .env inside KEPRIX_HOME is NOT blocked by this rule
        (it's handled by other mechanisms). Only project-local .env is blocked."""
        # Note: keprix internal .env is in ~/.keprix/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.keprix/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.keprix/.env"))
        assert error is not None
...
class TestCacheFileReadBlocking:
    """Internal Keprix cache files must remain blocked."""

    def test_hub_index_cache_blocked(self, tmp_path):
        """Hub index-cache reads are blocked."""
        keprix_home = tmp_path / ".keprix"
        cache = keprix_home / "skills" / ".hub" / "index-cache" / "data.json"
        cache.parent.mkdir(parents=True)
        cache.write_text("{}")

        with patch("agent.file_safety._keprix_home_path", return_value=keprix_home):
            error = get_read_block_error(str(cache))
            assert error is not None
            assert "internal Keprix cache" in error

    def test_hub_directory_blocked(self, tmp_path):
        """Hub directory reads are blocked."""
        keprix_home = tmp_path / ".keprix"
        hub = keprix_home / "skills" / ".hub" / "metadata.json"
...
class TestCombinedGuards:
    ...
    def test_env_guard_works_regardless_of_keprix_home(self, tmp_path):
        """The env basename guard does not depend on KEPRIX_HOME resolution."""
        keprix_home = tmp_path / ".keprix"
        keprix_home.mkdir()
    ...
    def test_cache_guard_still_works_with_env_guard(self, tmp_path):
        ...
        keprix_home = tmp_path / ".keprix"
        cache = keprix_home / "skills" / ".hub" / "index-cache" / "x"
        cache.parent.mkdir(parents=True)
        cache.write_text("")
