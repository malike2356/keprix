"""Tests for keprix_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Keprix entry point
(keprix, keprix, keprix-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Keprix entry point imports keprix_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported keprix_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("keprix_bootstrap", None)
    import keprix_bootstrap  # noqa: WPS433
    return keprix_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Keprix CLI
...
class TestEntryPointsImportBootstrap:
    """Every Keprix entry point must import keprix_bootstrap as its
    ...
    # Entry points that invoke Keprix as a process.  Each one must
    # import keprix_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "keprix_cli/main.py",   # keprix CLI (console_script)
        "run_agent.py",          # keprix (console_script)
        "acp_adapter/entry.py",  # keprix-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import keprix_bootstrap' and that
        ...
        half-finished ``keprix update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``keprix_bootstrap`` as a top-level module) leaves keprix
        ...
        """
        # Resolve relative to the keprix repo root.  Tests live
        # at tests/test_keprix_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # keprix start even when keprix_bootstrap hasn't been
        ...
        assert first_import_name == "keprix_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'keprix_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import keprix_bootstrap' line to be the first import."
        )
