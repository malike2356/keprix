...
class TestConfigFilePermissions(unittest.TestCase):
    ...
    def test_save_config_sets_0600(self):
        ...
        with patch("keprix_cli.config.get_config_path", return_value=config_path), \
             patch("keprix_cli.config.ensure_keprix_home"):
    ...
    def test_ensure_keprix_home_sets_0700(self):
        home = Path(self.tmpdir) / ".keprix"
        with patch("keprix_cli.config.get_keprix_home", return_value=home):
