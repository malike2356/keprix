...
- Path resolution (absolute, relative to KEPRIX_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp KEPRIX_HOME."""
    keprix_home = tmp_path / ".keprix"
    keprix_home.mkdir()
    (keprix_home / "cron").mkdir()
    (keprix_home / "cron" / "output").mkdir()
    (keprix_home / "scripts").mkdir()
    monkeypatch.setenv("KEPRIX_HOME", str(keprix_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "KEPRIX_DIR", keprix_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", keprix_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", keprix_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", keprix_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.keprix/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("KEPRIX_SESSION_PLATFORM") is None
        assert os.environ.get("KEPRIX_SESSION_CHAT_ID") is None
        assert os.environ.get("KEPRIX_SESSION_CHAT_NAME") is None
