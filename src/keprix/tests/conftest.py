"""Shared fixtures for the keprix test suite.
...
2. **Isolated KEPRIX_HOME.** KEPRIX_HOME points to a per-test tempdir so
   code reading ``~/.keprix/*`` via ``get_keprix_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".keprix"`` instead of the canonical
   ``get_keprix_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No KEPRIX_SESSION_* inheritance** — the agent's current gateway
...
# KEPRIX_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_KEPRIX_BEHAVIORAL_VARS = frozenset({
    "KEPRIX_YOLO_MODE",
    "KEPRIX_INTERACTIVE",
    "KEPRIX_QUIET",
    "KEPRIX_TOOL_PROGRESS",
    "KEPRIX_TOOL_PROGRESS_MODE",
    "KEPRIX_MAX_ITERATIONS",
    "KEPRIX_SESSION_PLATFORM",
    "KEPRIX_SESSION_CHAT_ID",
    "KEPRIX_SESSION_CHAT_NAME",
    "KEPRIX_SESSION_THREAD_ID",
    "KEPRIX_SESSION_SOURCE",
    "KEPRIX_SESSION_KEY",
    "KEPRIX_GATEWAY_SESSION",
    "KEPRIX_CRON_SESSION",
    "_KEPRIX_GATEWAY",
    "KEPRIX_PLATFORM",
    "KEPRIX_MODEL",
    "KEPRIX_INFERENCE_MODEL",
    "KEPRIX_INFERENCE_PROVIDER",
    "KEPRIX_TUI_PROVIDER",
    "KEPRIX_MANAGED",
    "KEPRIX_DEV",
    "KEPRIX_CONTAINER",
    ...
    "KEPRIX_TIMEZONE",
    "KEPRIX_REDACT_SECRETS",
    "KEPRIX_BACKGROUND_NOTIFICATIONS",
    "KEPRIX_EXEC_ASK",
    "KEPRIX_HOME_MODE",
    "KEPRIX_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.keprix/kanban.db instead of the per-test KEPRIX_HOME.
    "KEPRIX_KANBAN_DB",
    "KEPRIX_KANBAN_BOARD",
    "KEPRIX_KANBAN_HOME",
    "KEPRIX_KANBAN_WORKSPACES_ROOT",
    "KEPRIX_KANBAN_LOGS_ROOT",
    "KEPRIX_KANBAN_TASK",
    "KEPRIX_KANBAN_WORKSPACE",
    "KEPRIX_KANBAN_RUN_ID",
    "KEPRIX_KANBAN_CLAIM_LOCK",
    "KEPRIX_KANBAN_DISPATCH_IN_GATEWAY",
    "KEPRIX_TENANT",
    ...
    "KEPRIX_DASHBOARD_OAUTH_CLIENT_ID",
    "KEPRIX_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and KEPRIX_HOME to per-test tempdirs so code that
    reads ``~/.keprix/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral KEPRIX_* vars that could change test semantics.
    for name in _KEPRIX_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect KEPRIX_HOME to a per-test tempdir. Code that reads
    #    ``~/.keprix/*`` via ``get_keprix_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.keprix/*`` via
    #    ``Path.home() / ".keprix"`` instead of ``get_keprix_home()``
    #    is a bug to fix at the callsite.
    fake_keprix_home = tmp_path / "keprix_test"
    fake_keprix_home.mkdir()
    (fake_keprix_home / "sessions").mkdir()
    (fake_keprix_home / "cron").mkdir()
    (fake_keprix_home / "memories").mkdir()
    (fake_keprix_home / "skills").mkdir()
    monkeypatch.setenv("KEPRIX_HOME", str(fake_keprix_home))
    ...
    #    ~/.keprix/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import keprix_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_keprix_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal keprix config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``keprix-gateway`` process
...
# correlated with a ``tests/keprix_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> keprix-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart keprix-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting keprix/python patterns are also blocked.
    """
    ...
    def _matches_keprix_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _KEPRIX_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_keprix_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch keprix-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m keprix_cli.main".
                if (
                    "keprix" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live keprix-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting keprix/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `keprix update` (or the
        # equivalent `python -m keprix_cli.main update`).  These commands
        ...
        if "update" in low and (
            # keprix update / keprix update --gateway / setsid bash -c ... keprix update
            ("keprix" in low and "update" in low.split())
            or
            # python -m keprix_cli.main update --gateway
            ("keprix_cli" in low and "update" in low.split())
            or
            # venv/bin/keprix update  (absolute path variant used in tests)
            (".venv/bin/keprix" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`keprix update` against the real checkout, fetching "
