...
binary download are exercised manually by `keprix secrets bitwarden
...
def keprix_home(tmp_path, monkeypatch):
    """Point Keprix at an isolated home directory."""
    home = tmp_path / ".keprix"
    home.mkdir()
    monkeypatch.setenv("KEPRIX_HOME", str(home))
    # Some modules cache get_keprix_home; clear if needed.
    import keprix_constants
    if hasattr(keprix_constants, "_KEPRIX_HOME_CACHE"):
        keprix_constants._KEPRIX_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(keprix_home, monkeypatch):
...
def test_install_bws_happy_path(keprix_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(keprix_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(keprix_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from keprix_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under keprix_home/cache/."""
    home = tmp_path / ".keprix"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
