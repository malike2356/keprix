...
def _fake_warning():
    return SimpleNamespace(
        message=(
            ...
            "openai/gpt-5.5-pro has known pricing above Keprix' safety threshold.\n"
...
def _setup_isolated_home(tmp_path, monkeypatch, *, warn):
    ...
    monkeypatch.setattr(
        "keprix_cli.model_switch.switch_model",
        lambda **kw: _fake_switch_result(),
    )
    monkeypatch.setattr("keprix_constants.get_keprix_home", lambda: keprix_home)
    monkeypatch.setattr("keprix_cli.config.get_keprix_home", lambda: keprix_home)
    monkeypatch.setattr(
