...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"keprix[termux]"' in text
    assert '"keprix[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"keprix[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
