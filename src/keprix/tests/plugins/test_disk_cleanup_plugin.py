...
def _isolate_env(tmp_path, monkeypatch):
    """Isolate KEPRIX_HOME for each test.

    The global hermetic fixture already redirects KEPRIX_HOME to a tempdir,
    but we want the plugin to work with a predictable subpath. We reset
    KEPRIX_HOME here for clarity.
    """
...
def _load_plugin_init():
    ...
    spec = importlib.util.spec_from_file_location(
        "keprix_plugins.disk_cleanup",
        plugin_dir / "__init__.py",
        submodule_search_locations=[str(plugin_dir)],
    )
    # Ensure parent namespace package exists for the relative `. import disk_cleanup`
    import types
    if "keprix_plugins" not in sys.modules:
        ns = types.ModuleType("keprix_plugins")
        ns.__path__ = []
        sys.modules["keprix_plugins"] = ns
    mod = importlib.util.module_from_spec(spec)
    mod.__package__ = "keprix_plugins.disk_cleanup"
    mod.__path__ = [str(plugin_dir)]
    sys.modules["keprix_plugins.disk_cleanup"] = mod
    spec.loader.exec_module(mod)
    return mod
...
class TestIsSafePath:
    def test_accepts_path_under_keprix_home(self, _isolate_env):
        dg = _load_lib()
    ...
    def test_rejects_outside_keprix_home(self, _isolate_env):
        dg = _load_lib()
        assert dg.is_safe_path(Path("/etc/passwd")) is False

    def test_accepts_tmp_keprix_prefix(self, _isolate_env, tmp_path):
        dg = _load_lib()
        assert dg.is_safe_path(Path("/tmp/keprix-abc/x.log")) is True
...
class TestTrackForgetQuick:
    ...
    def test_quick_does_not_descend_into_protected_top_level_dirs(
        ...
        dg = _load_lib()
        protected_empty = (
            _isolate_env / "keprix" / "node_modules" / "pkg" / "empty"
        )
        ...
        def guarded_iterdir(path):
            if path == _isolate_env / "keprix":
                raise AssertionError("quick() descended into protected keprix/")
...
class TestBundledDiscovery:
    def _write_enabled_config(self, keprix_home, names):
    ...
    def test_disk_cleanup_discovered_but_not_loaded_by_default(self, _isolate_env):
        ...
        from keprix_cli import plugins as pmod
