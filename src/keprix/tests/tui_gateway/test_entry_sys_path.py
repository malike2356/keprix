...
def _reload_entry_with_env(env_overrides: dict) -> None:
    ...
    try:
        with patch.dict(os.environ, env_overrides, clear=False):
            _src_root = os.environ.get("KEPRIX_PYTHON_SRC_ROOT", "")
...
def test_keprix_src_root_inserted_at_front():
    original = sys.path[:]
    try:
        fake_root = "/fake/keprix/src"
        with patch.dict(os.environ, {"KEPRIX_PYTHON_SRC_ROOT": fake_root}):
            _src_root = os.environ.get("KEPRIX_PYTHON_SRC_ROOT", "")
            if _src_root and _src_root not in sys.path:
                sys.path.insert(0, _src_root)
            sys.path = [p for p in sys.path if p not in {"", "."}]
...
def test_no_src_root_env_does_not_crash():
    original = sys.path[:]
    try:
        env = {k: v for k, v in os.environ.items() if k != "KEPRIX_PYTHON_SRC_ROOT"}
