...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._keprix_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(keprix_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(keprix_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_keprix_bin(self):
        """After _INSTALL_FAILED, manual install in KEPRIX_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        keprix_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(keprix_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(keprix_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._keprix_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == keprix_bin
            assert _tirith_mod._resolved_path == keprix_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# KEPRIX_HOME isolation
# ---------------------------------------------------------------------------

class TestKeprixHomeIsolation:
    def test_keprix_bin_dir_respects_keprix_home(self):
        """_keprix_bin_dir must use KEPRIX_HOME, not hardcoded ~/.keprix."""
        from tools.tirith_security import _keprix_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"KEPRIX_HOME": tmpdir}):
            result = _keprix_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_keprix_home(self):
        """_failure_marker_path must use KEPRIX_HOME, not hardcoded ~/.keprix."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"KEPRIX_HOME": "/custom/keprix"}):
            result = _failure_marker_path()
        assert result == "/custom/keprix/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets KEPRIX_HOME; verify it's active."""
        keprix_home = os.getenv("KEPRIX_HOME")
        assert keprix_home is not None, "KEPRIX_HOME should be set by conftest"
        assert "keprix_test" in keprix_home, "Should point to test temp dir"

    def test_get_keprix_home_fallback(self):
        """Without KEPRIX_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_keprix_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove KEPRIX_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".keprix")
            result = _get_keprix_home()
