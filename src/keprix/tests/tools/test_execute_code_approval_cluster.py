...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad KEPRIX_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: KEPRIX_GATEWAY_SESSION set, a bound session
    ...
    monkeypatch.delenv("KEPRIX_CRON_SESSION", raising=False)
...
def test_guard_cron_deny_blocks(monkeypatch):
    monkeypatch.setenv("KEPRIX_CRON_SESSION", "1")
    monkeypatch.delenv("KEPRIX_GATEWAY_SESSION", raising=False)
    monkeypatch.setattr(A, "_get_approval_mode", lambda: "manual")
...
def test_env_scrub_keprix_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "KEPRIX_HOME": "/h", "KEPRIX_PROFILE": "p",
        "KEPRIX_CONFIG": "/c.yaml", "KEPRIX_ENV": "/e",
        # other KEPRIX_* → dropped (broad prefix removed)
        "KEPRIX_BASE_URL": "https://x", "KEPRIX_INTERACTIVE": "1",
        "KEPRIX_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("KEPRIX_HOME", "KEPRIX_PROFILE", "KEPRIX_CONFIG", "KEPRIX_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "KEPRIX_BASE_URL", "KEPRIX_INTERACTIVE", "KEPRIX_KANBAN_DB",
...
def test_env_scrub_logs_dropped_keprix_vars(caplog):
    """Dropping a non-allowlisted, non-secret KEPRIX_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset KEPRIX_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "KEPRIX_HOME": "/h",          # allowlisted → kept, not logged
        "KEPRIX_BASE_URL": "https://x",   # dropped → logged
        "KEPRIX_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "KEPRIX_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "KEPRIX_HOME" in out and "PATH" in out
    assert "KEPRIX_BASE_URL" not in out and "KEPRIX_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "KEPRIX_BASE_URL" in msgs and "KEPRIX_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "KEPRIX_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped KEPRIX_* vars."""
    import logging
