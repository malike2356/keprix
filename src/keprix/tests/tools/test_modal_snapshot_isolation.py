...
def _restore_tool_modules():
    original_keprix_home = os.environ.get("KEPRIX_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_keprix_home is None:
            os.environ.pop("KEPRIX_HOME", None)
        else:
            os.environ["KEPRIX_HOME"] = original_keprix_home
        _reset_modules(("tools", "keprix_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    keprix_cli = types.ModuleType("keprix_cli")
    keprix_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["keprix_cli"] = keprix_cli
    keprix_home = tmp_path / "keprix-home"
    os.environ["KEPRIX_HOME"] = str(keprix_home)
    sys.modules["keprix_cli.config"] = types.SimpleNamespace(
        get_keprix_home=lambda: keprix_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="keprix", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": keprix_home / "modal_snapshots.json",
