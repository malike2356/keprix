...
When KEPRIX_UID is remapped at container boot, ``usermod -u`` only rewrites
files under the keprix user's home directory ($KEPRIX_HOME == /opt/data).
Runtime-writable trees under ``/opt/keprix`` must be explicitly chowned to the
new UID before services drop privileges. ``/opt/keprix/gateway`` is one such
