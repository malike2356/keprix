...
class TestWriteDenyExactPaths:
    ...
    def test_keprix_env(self):
        # ``.env`` under the active KEPRIX_HOME (profile-aware, not just
        # ``~/.keprix``) must be write-denied. The hermetic test conftest
        # points KEPRIX_HOME at a tempdir — resolve via get_keprix_home()
        # to match the denylist.
        from keprix_constants import get_keprix_home
        path = str(get_keprix_home() / ".env")
        assert _is_write_denied(path) is True

    def test_keprix_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.keprix/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "keprix_root"
        ...
        # Sanity check: KEPRIX_HOME does point to the profile dir, not the root.
        from keprix_constants import get_keprix_home, get_default_keprix_root
        assert get_keprix_home() == profile_home
        assert get_default_keprix_root() == root
...
class TestWriteAllowed:
    ...
    def test_keprix_control_files_requested_writable(self):
        from keprix_constants import get_keprix_home
