...
class TestScanMemoryContent:
    ...
    def test_keprix_config_mod_blocked(self):
        result = _scan_memory_content("edit .keprix/config.yaml to change settings")
        assert "Blocked" in result
        assert "keprix_config_mod" in result
        result = _scan_memory_content("update .keprix/SOUL.md with new personality")
        assert "Blocked" in result
        assert "keprix_config_mod" in result
    ...
    def test_keprix_config_mod_no_false_positives(self):
        """Merely mentioning keprix config files should not trigger; only modify intent should."""
        assert _scan_memory_content("Check .keprix/config.yaml for settings") is None
        assert _scan_memory_content("Read .keprix/SOUL.md for agent personality") is None
        assert _scan_memory_content("The .keprix/config.yaml file contains runtime options") is None
