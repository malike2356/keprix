...
def test_keprix_provider_subclass_exists():
    """KeprixMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _KEPRIX_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _KEPRIX_PROVIDER_CLS is not None
    assert issubclass(_KEPRIX_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_keprix_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns KeprixMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _KEPRIX_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("KEPRIX_HOME", str(tmp_path))
    _set_interactive_stdin(monkeypatch)
    ...
    assert _KEPRIX_PROVIDER_CLS is not None
    assert isinstance(provider, _KEPRIX_PROVIDER_CLS)
    assert provider._keprix_server_name == "srv"
