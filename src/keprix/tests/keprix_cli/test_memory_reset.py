"""Tests for the `keprix memory reset` CLI command.
...
- Profile-scoped reset (uses KEPRIX_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake KEPRIX_HOME with memory files."""
    keprix_home = tmp_path / ".keprix"
    memories = keprix_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("KEPRIX_HOME", str(keprix_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nKeprix repo is at ~/.keprix/keprix\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `keprix memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        keprix_home = tmp_path / ".keprix"
        (keprix_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("KEPRIX_HOME", str(keprix_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's KEPRIX_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_keprix_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
