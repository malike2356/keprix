"""Tests for the top-level `./keprix` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./keprix` should use `keprix_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "keprix"
    called = []

    fake_main_module = types.ModuleType("keprix_cli.main")

    def fake_main():
        called.append("keprix_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "keprix_cli.main", fake_main_module)
