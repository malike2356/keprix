...
Covers:
 - Normal path (tokens already in Keprix auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Keprix
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `keprix auth openai-codex`. The old
...
@pytest.fixture()
def keprix_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Keprix auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(keprix_auth_only_env):
    """openai-codex appears when tokens are already in Keprix auth store."""
...
def test_codex_picker_uses_live_codex_catalog(keprix_auth_only_env, tmp_path, monkeypatch):
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Keprix
    auth store."""
    keprix_home = tmp_path / ".keprix"
    keprix_home.mkdir()
