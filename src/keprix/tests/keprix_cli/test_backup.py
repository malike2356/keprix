"""Tests for keprix backup and import commands."""
...
def _make_keprix_tree(root: Path) -> None:
    """Create a realistic ~/.keprix directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    (root / "memory_store.db").write_bytes(b"fake-sqlite")
    (root / "keprix_state.db").write_bytes(b"fake-state")
    ...
    # keprix repo (should be EXCLUDED)
    (root / "keprix").mkdir(exist_ok=True)
    (root / "keprix" / "run_agent.py").write_text("# big file\n")
    (root / "keprix" / ".git").mkdir()
    (root / "keprix" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_keprix(self):
        from keprix_cli.backup import _should_exclude
        assert _should_exclude(Path("keprix/run_agent.py"))
        assert _should_exclude(Path("keprix/.git/HEAD"))
    ...
    def test_includes_nested_keprix_in_skills(self):
        """skills/autonomous-ai-agents/keprix/ must NOT be excluded —
        only the root-level keprix/ repo is skipped."""
        from keprix_cli.backup import _should_exclude
        assert not _should_exclude(Path("skills/autonomous-ai-agents/keprix/SKILL.md"))
        assert not _should_exclude(Path("skills/autonomous-ai-agents/keprix/sub/item.txt"))
...
class TestBackup:
    def test_creates_zip(self, tmp_path, monkeypatch):
        ...
        keprix_home.mkdir()
        _make_keprix_tree(keprix_home)

        monkeypatch.setenv("KEPRIX_HOME", str(keprix_home))
        # get_default_keprix_root needs this
    ...
    def test_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        import keprix_cli.backup as backup_mod
        staged_dirs = []
    ...
    def test_pre_update_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        out_zip = keprix_home / "backups" / "pre-update-test.zip"
        ...
        monkeypatch.setattr(backup_mod.tempfile, "NamedTemporaryFile", _spy)
        result = backup_mod._write_full_zip_backup(out_zip, keprix_home)

        assert result is not None
        assert staged_dirs, "no SQLite snapshot was staged"
        assert all(d == str(out_zip.parent) for d in staged_dirs), staged_dirs

    def test_excludes_keprix(self, tmp_path, monkeypatch):
        """Backup does NOT include keprix/ directory."""
        keprix_home = tmp_path / ".keprix"
        keprix_home.mkdir()
        _make_keprix_tree(keprix_home)
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "keprix" in n]
            assert agent_files == [], f"keprix files leaked into backup: {agent_files}"

    def test_includes_nested_keprix_in_skills(self, tmp_path, monkeypatch):
        """Backup includes skills/.../keprix/ but NOT root keprix/."""
        keprix_home = tmp_path / ".keprix"
        keprix_home.mkdir()
        _make_keprix_tree(keprix_home)

        # Add a nested keprix directory inside skills (like the real layout)
        nested = keprix_home / "skills" / "autonomous-ai-agents" / "keprix"
        nested.mkdir(parents=True)
        (nested / "SKILL.md").write_text("# Keprix Skill\n")
        (nested / "sub").mkdir()
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            # Root keprix must be excluded
            root_agent = [n for n in names if n.startswith("keprix/")]
            assert root_agent == [], f"root keprix leaked: {root_agent}"
            # Nested skill keprix must be included
            assert "skills/autonomous-ai-agents/keprix/SKILL.md" in names
            assert "skills/autonomous-ai-agents/keprix/sub/item.txt" in names
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        """When no output path given, zip goes to ~/keprix-backup-*.zip."""
        keprix_home = tmp_path / ".keprix"
        keprix_home.mkdir()
        (keprix_home / "config.yaml").write_text("model: test\n")
        ...
        zips = list(tmp_path.glob("keprix-backup-*.zip"))
        assert len(zips) == 1

    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside KEPRIX_HOME."""
        keprix_home = tmp_path / ".keprix"
        keprix_home.mkdir()
        _make_keprix_tree(keprix_home)
        outside = tmp_path / "outside-secret.txt"
        outside.write_text("outside secret\n")
        _symlink_file_or_skip(keprix_home / "skills" / "outside-link.txt", outside)
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Keprix backup."""
        from keprix_cli.backup import _validate_backup_zip
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only keprix_state.db (old wrong name) is rejected."""
        from keprix_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["keprix_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        """Import extracts files into keprix home."""
        keprix_home = tmp_path / ".keprix"
        keprix_home.mkdir()
        monkeypatch.setenv("KEPRIX_HOME", str(keprix_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (keprix_home / "config.yaml").read_text() == "model:\n  provider: openrouter\n"
        assert (keprix_home / ".env").read_text() == "OPENROUTER_API_KEY=sk-test\n"
        assert (keprix_home / "skills" / "my-skill" / "SKILL.md").read_text() == "# My Skill\n"
        assert (keprix_home / "profiles" / "coder" / "config.yaml").exists()

    def test_strips_keprix_prefix(self, tmp_path, monkeypatch):
        """Import strips .keprix/ prefix if all entries share it."""
        keprix_home = tmp_path / ".keprix"
        keprix_home.mkdir()
        monkeypatch.setenv("KEPRIX_HOME", str(keprix_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            ".keprix/config.yaml": "model: test\n",
            ".keprix/skills/a/SKILL.md": "# A\n",
        })
        ...
        assert (keprix_home / "config.yaml").read_text() == "model: test\n"
        assert (keprix_home / "skills" / "a" / "SKILL.md").read_text() == "# A\n"
    ...
    def test_rejects_non_keprix_zip(self, tmp_path, monkeypatch):
        """Import rejects a zip that doesn't look like a keprix backup."""
        keprix_home = tmp_path / ".keprix"
        keprix_home.mkdir()
        monkeypatch.setenv("KEPRIX_HOME", str(keprix_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        assert (keprix_home / "config.yaml").exists()
        # traversal file should NOT exist outside keprix home
    ...
    def test_confirmation_prompt_abort(self, tmp_path, monkeypatch):
        ...
        (keprix_home / "config.yaml").write_text("existing: true\n")
        monkeypatch.setenv("KEPRIX_HOME", str(keprix_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (keprix_home / "config.yaml").read_text() == "existing: true\n"
    ...
    def test_force_skips_confirmation(self, tmp_path, monkeypatch):
        ...
        assert (keprix_home / "config.yaml").read_text() == "model: restored\n"
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (keprix_home / rel).stat().st_mode & 0o777
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".keprix"
        src_home.mkdir(parents=True)
        _make_keprix_tree(src_home)

        monkeypatch.setenv("KEPRIX_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from keprix_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".keprix"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("KEPRIX_HOME", str(dst_home))
        ...
        # keprix should NOT be present
        assert not (dst_home / "keprix").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from keprix_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without keprix markers fails validation."""
        import io
        from keprix_cli.backup import _validate_backup_zip
    ...
    def test_detect_prefix_keprix(self):
        """Detects .keprix/ prefix wrapping all entries."""
        import io
        from keprix_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".keprix/config.yaml", "test")
            zf.writestr(".keprix/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".keprix/"
...
class TestBackupEdgeCases:
    def test_nonexistent_keprix_home(self, tmp_path, monkeypatch):
        """Backup exits when keprix home doesn't exist."""
        fake_home = tmp_path / "nonexistent" / ".keprix"
        monkeypatch.setenv("KEPRIX_HOME", str(fake_home))
    ...
    def test_output_is_directory(self, tmp_path, monkeypatch):
        ...
        zips = list(out_dir.glob("keprix-backup-*.zip"))
        assert len(zips) == 1
    ...
    def test_empty_keprix_home(self, tmp_path, monkeypatch):
        """Backup handles empty keprix home (no files to back up)."""
        keprix_home = tmp_path / ".keprix"
        keprix_home.mkdir()
        # Only excluded dirs, no actual files
        (keprix_home / "__pycache__").mkdir()
        (keprix_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_permission_error_during_backup(self, tmp_path, monkeypatch):
        ...
        bad_file = keprix_home / "secret.db"
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        old_file = keprix_home / "ancient.txt"
    ...
    def test_skips_output_zip_inside_keprix(self, tmp_path, monkeypatch):
        """Backup skips its own output zip if it's inside keprix root."""
        keprix_home = tmp_path / ".keprix"
        keprix_home.mkdir()
        (keprix_home / "config.yaml").write_text("model: test\n")
        ...
        # Output inside keprix home
        out_zip = keprix_home / "backup.zip"
        args = Namespace(output=str(out_zip))
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        keprix_home = tmp_path / ".keprix"
        keprix_home.mkdir()
        (keprix_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("KEPRIX_HOME", str(keprix_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_keyboard_interrupt_during_confirmation(self, tmp_path, monkeypatch):
        ...
        keprix_home = tmp_path / ".keprix"
        keprix_home.mkdir()
        (keprix_home / ".env").write_text("KEY=val\n")
        monkeypatch.setenv("KEPRIX_HOME", str(keprix_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (keprix_home / "config.yaml").exists()
        assert (keprix_home / "sessions" / "s0599.json").exists()
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert (keprix_home / "profiles" / "coder" / "config.yaml").exists()
        assert (keprix_home / "profiles" / "researcher" / "config.yaml").exists()
        ...
        assert "keprix -p coder" in coder_wrapper
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from keprix_cli.backup import _safe_copy_db
...
class TestQuickSnapshot:
    @pytest.fixture
    def keprix_home(self, tmp_path):
        """Create a fake KEPRIX_HOME with critical state files."""
    ...
    def test_creates_snapshot(self, keprix_home):
        from keprix_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(keprix_home=keprix_home)
        assert snap_id is not None
        snap_dir = keprix_home / "state-snapshots" / snap_id
        assert snap_dir.is_dir()
        assert (snap_dir / "manifest.json").exists()

    def test_label_in_id(self, keprix_home):
        from keprix_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(label="before-upgrade", keprix_home=keprix_home)
        assert "before-upgrade" in snap_id

    def test_state_db_safely_copied(self, keprix_home):
        from keprix_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(keprix_home=keprix_home)
        db_copy = keprix_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_copies_nested_files(self, keprix_home):
        from keprix_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(keprix_home=keprix_home)
        assert (keprix_home / "state-snapshots" / snap_id / "cron" / "jobs.json").exists()

    def test_copies_channel_aliases(self, keprix_home):
        from keprix_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(keprix_home=keprix_home)
        copied = keprix_home / "state-snapshots" / snap_id / "channel_aliases.json"
        assert copied.exists()
        assert "120363408391911677@g.us" in copied.read_text()

    def test_missing_files_skipped(self, keprix_home):
        from keprix_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(keprix_home=keprix_home)
        with open(keprix_home / "state-snapshots" / snap_id / "manifest.json") as f:
            meta = json.load(f)
    ...
    def test_empty_home_returns_none(self, tmp_path):
        ...
        empty.mkdir()
        assert create_quick_snapshot(keprix_home=empty) is None

    def test_list_snapshots(self, keprix_home):
        from keprix_cli.backup import create_quick_snapshot, list_quick_snapshots
        id1 = create_quick_snapshot(label="first", keprix_home=keprix_home)
        id2 = create_quick_snapshot(label="second", keprix_home=keprix_home)

        snaps = list_quick_snapshots(keprix_home=keprix_home)
        assert len(snaps) == 2
        assert snaps[0]["id"] == id2  # most recent first
        assert snaps[1]["id"] == id1

    def test_list_limit(self, keprix_home):
        from keprix_cli.backup import create_quick_snapshot, list_quick_snapshots
        for i in range(5):
            create_quick_snapshot(label=f"s{i}", keprix_home=keprix_home)
        snaps = list_quick_snapshots(limit=3, keprix_home=keprix_home)
        assert len(snaps) == 3

    def test_restore_config(self, keprix_home):
        from keprix_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(keprix_home=keprix_home)

        (keprix_home / "config.yaml").write_text("model:\n  provider: anthropic\n")
        assert "anthropic" in (keprix_home / "config.yaml").read_text()

        result = restore_quick_snapshot(snap_id, keprix_home=keprix_home)
        assert result is True
        assert "openrouter" in (keprix_home / "config.yaml").read_text()

    def test_restore_state_db(self, keprix_home):
        from keprix_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(keprix_home=keprix_home)

        conn = sqlite3.connect(str(keprix_home / "state.db"))
        conn.execute("INSERT INTO sessions VALUES ('s2', 'new')")
        conn.commit()
        conn.close()

        restore_quick_snapshot(snap_id, keprix_home=keprix_home)
    ...
    def test_restore_nonexistent(self, keprix_home):
        from keprix_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", keprix_home=keprix_home) is False

    def test_auto_prune(self, keprix_home):
        from keprix_cli.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
        for i in range(_QUICK_DEFAULT_KEEP + 5):
            create_quick_snapshot(label=f"snap-{i:03d}", keprix_home=keprix_home)
        snaps = list_quick_snapshots(limit=100, keprix_home=keprix_home)
        assert len(snaps) <= _QUICK_DEFAULT_KEEP

    def test_manual_prune(self, keprix_home):
        from keprix_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", keprix_home=keprix_home)
        deleted = prune_quick_snapshots(keep=3, keprix_home=keprix_home)
        assert deleted == 7
        assert len(list_quick_snapshots(keprix_home=keprix_home)) == 3

    def test_snapshot_includes_pairing_directories(self, keprix_home):
        ...
        (keprix_home / "platforms" / "pairing").mkdir(parents=True)
        (keprix_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (keprix_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (keprix_home / "pairing").mkdir()
        (keprix_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (keprix_home / "feishu_comment_pairing.json").write_text(
    ...
    def test_restore_recovers_pairing_data(self, keprix_home):
        ...
        assert restore_quick_snapshot(snap_id, keprix_home=keprix_home) is True
        assert approved.exists()
        assert '"alice"' in approved.read_text()
        assert feishu.exists()
        assert '"user_xyz"' in feishu.read_text()

    def test_empty_pairing_dir_does_not_fail(self, keprix_home):
...
# ---------------------------------------------------------------------------
# Pre-update backup (keprix update safety net)
# ---------------------------------------------------------------------------

class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``keprix update``
    ...
    def keprix_home(self, tmp_path):
        ...
        root.mkdir()
        _make_keprix_tree(root)
        return root

    def test_creates_backup_under_backups_dir(self, keprix_home):
        from keprix_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(keprix_home=keprix_home)
        assert out is not None
        assert out.exists()
        assert out.parent == keprix_home / "backups"
        assert out.name.startswith("pre-update-")
        assert out.suffix == ".zip"

    def test_backup_contents_match_full_backup(self, keprix_home):
        """Pre-update backup should include the same user data that
        ``keprix backup`` would, and should exclude the same directories."""
        from keprix_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(keprix_home=keprix_home)
        assert out is not None
        ...
        assert "skills/my-skill/SKILL.md" in names
        assert "profiles/coder/config.yaml" in names
        # keprix repo excluded
        assert not any(n.startswith("keprix/") for n in names)
        # __pycache__ excluded
        assert not any("__pycache__" in n for n in names)
        # pid files excluded
        assert "gateway.pid" not in names

    def test_does_not_recurse_into_prior_backups(self, keprix_home):
        """The ``backups/`` directory must be excluded so that each backup
        doesn't grow exponentially by including all prior backups."""
        from keprix_cli.backup import create_pre_update_backup
        # First backup
        out1 = create_pre_update_backup(keprix_home=keprix_home)
        assert out1 is not None
        # Second backup — must not include the first
        out2 = create_pre_update_backup(keprix_home=keprix_home)
    ...
    def test_rotation_keeps_only_n(self, keprix_home):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(keprix_home=keprix_home, keep=3)
            created.append(out)
            _t.sleep(1.05)  # ensure distinct seconds in timestamp

        remaining = sorted(
            p.name for p in (keprix_home / "backups").iterdir()
    ...
    def test_rotation_preserves_manual_files(self, keprix_home):
        """Hand-dropped zips in ``backups/`` must not be touched by
        rotation — it only prunes files matching ``pre-update-*.zip``."""
        import time as _t
        from keprix_cli.backup import create_pre_update_backup

        (keprix_home / "backups").mkdir(exist_ok=True)
        manual = keprix_home / "backups" / "my-manual.zip"
        manual.write_bytes(b"manual backup")

        for _ in range(5):
            create_pre_update_backup(keprix_home=keprix_home, keep=2)
            _t.sleep(1.05)
    ...
    def test_returns_none_if_root_missing(self, tmp_path):
        from keprix_cli.backup import create_pre_update_backup
        assert create_pre_update_backup(keprix_home=tmp_path / "does-not-exist") is None

    def test_keep_zero_does_not_delete_freshly_created_backup(self, keprix_home):
        ...
        """
        from keprix_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(keprix_home=keprix_home, keep=0)
        assert out is not None
        assert out.exists(), (
    ...
    def test_keep_negative_does_not_delete_freshly_created_backup(self, keprix_home):
    ...
    def test_keep_zero_still_prunes_older_backups(self, keprix_home):
        ...
        first = create_pre_update_backup(keprix_home=keprix_home, keep=5)
        _t.sleep(1.05)
        second = create_pre_update_backup(keprix_home=keprix_home, keep=5)
        _t.sleep(1.05)
        third = create_pre_update_backup(keprix_home=keprix_home, keep=0)
    ...
    def test_skips_symlinked_files(self, keprix_home, tmp_path):
        """Pre-update backups must not dereference symlinks outside KEPRIX_HOME."""
        from keprix_cli.backup import create_pre_update_backup
...
class TestRunPreUpdateBackup:
    ...
    @pytest.fixture
    def keprix_home(self, tmp_path, monkeypatch):
        root = tmp_path / ".keprix"
        root.mkdir()
        _make_keprix_tree(root)
        # Point KEPRIX_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("KEPRIX_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Bust caches for keprix_cli.config + keprix_constants so they pick up KEPRIX_HOME
        for mod in list(__import__("sys").modules.keys()):
            if mod.startswith("keprix_cli.config") or mod == "keprix_constants":
                del __import__("sys").modules[mod]
        return root

    def test_backup_flag_creates_backup(self, keprix_home, capsys):
        """--backup forces the pre-update backup for one run even when config is off."""
        from keprix_cli.main import _run_pre_update_backup
        _run_pre_update_backup(Namespace(no_backup=False, backup=True))
        out = capsys.readouterr().out
        assert "Creating pre-update backup" in out
        assert "Saved:" in out
        assert "Restore:" in out
        assert "keprix import" in out
        assert "Disable:" in out
        # Actual backup was created
        backups = list((keprix_home / "backups").glob("pre-update-*.zip"))
        assert len(backups) == 1

    def test_default_disabled_is_silent(self, keprix_home, capsys):
        ...
        out = capsys.readouterr().out
        assert out == ""
        assert not (keprix_home / "backups").exists() or not list(
            (keprix_home / "backups").glob("pre-update-*.zip")
        )

    def test_no_backup_flag_skips(self, keprix_home, capsys):
        from keprix_cli.main import _run_pre_update_backup
    ...
    def test_config_enabled_creates_backup(self, keprix_home, capsys):
        ...
        import yaml
        (keprix_home / "config.yaml").write_text(yaml.safe_dump({
            "_config_version": 22,
            "updates": {"pre_update_backup": True},
        }))
        import sys as _sys
        for mod in list(_sys.modules.keys()):
            if mod.startswith("keprix_cli.config"):
    ...
    def test_config_disabled_is_silent(self, keprix_home, capsys):
        ...
        out = capsys.readouterr().out
        assert out == ""
        assert not list((keprix_home / "backups").glob("pre-update-*.zip")) \
            if (keprix_home / "backups").exists() else True

    def test_cli_flag_overrides_enabled_config(self, keprix_home, capsys):
...
# ---------------------------------------------------------------------------
# Pre-migration backup (keprix claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``keprix claw migrate`` runs before mutating ~/.keprix/."""
    ...
    def test_creates_backup_under_backups_dir(self, keprix_home):
        from keprix_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(keprix_home=keprix_home)
        assert out is not None
        assert out.exists()
        # Shares the backups/ directory with pre-update backups so `keprix
        # import` and the update-backup listing both pick them up.
        assert out.parent == keprix_home / "backups"
        assert out.name.startswith("pre-migration-")
        assert out.suffix == ".zip"

    def test_backup_uses_shared_exclusion_rules(self, keprix_home):
        """Pre-migration backup reuses the same exclusion rules as
        ``keprix backup`` / ``create_pre_update_backup`` — no drift."""
        from keprix_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(keprix_home=keprix_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_restorable_with_keprix_import(self, keprix_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Keprix
        backup — `keprix import` should accept it."""
        from keprix_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(keprix_home=keprix_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_recurse_into_prior_backups(self, keprix_home):
        from keprix_cli.backup import create_pre_migration_backup
        out1 = create_pre_migration_backup(keprix_home=keprix_home)
        assert out1 is not None
        out2 = create_pre_migration_backup(keprix_home=keprix_home)
        assert out2 is not None
        with zipfile.ZipFile(out2) as zf:
            names = zf.namelist()
        assert not any(n.startswith("backups/") for n in names)
    ...
    def test_rotation_keeps_only_n(self, keprix_home):
        ...
        created = []
        for _ in range(7):
            out = create_pre_migration_backup(keprix_home=keprix_home, keep=3)
            if out is not None:
                created.append(out)
            _t.sleep(1.05)  # timestamp resolution

        remaining = sorted((keprix_home / "backups").glob("pre-migration-*.zip"))
        assert len(remaining) <= 3, f"expected <=3 backups retained, got {len(remaining)}"

    def test_missing_keprix_home_returns_none(self, tmp_path):
        """Fresh install with no ~/.keprix yet — nothing to back up."""
        from keprix_cli.backup import create_pre_migration_backup
        missing = tmp_path / "does-not-exist"
        out = create_pre_migration_backup(keprix_home=missing)
        assert out is None

    def test_does_not_touch_pre_update_backups(self, keprix_home):
        """Pre-migration rotation must only prune pre-migration-*.zip files,
        leaving pre-update-*.zip backups untouched."""
        from keprix_cli.backup import create_pre_update_backup, create_pre_migration_backup
        update_backup = create_pre_update_backup(keprix_home=keprix_home, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        import time as _t
        for _ in range(3):
            out = create_pre_migration_backup(keprix_home=keprix_home, keep=1)
            assert out is not None
            _t.sleep(1.05)
...
class TestRestoreCronJobsIfEmptied:
    """`keprix update` config migration can leave cron/jobs.json valid-but-empty,
    ...
    def _make_snapshot(self, keprix_home: Path, label="pre-update"):
        from keprix_cli.backup import create_quick_snapshot
        return create_quick_snapshot(label=label, keprix_home=keprix_home, keep=5)

    def test_restores_when_emptied_after_migration(self, tmp_path):
        from keprix_cli.backup import restore_cron_jobs_if_emptied
        keprix_home = tmp_path / ".keprix"
        jobs_path = keprix_home / "cron" / "jobs.json"
        # Pre-update: 3 real jobs.
        self._seed_jobs(jobs_path, [{"id": "a"}, {"id": "b"}, {"id": "c"}])
        snap_id = self._make_snapshot(keprix_home)
        assert snap_id

        # Migration silently empties the file (valid JSON, zero jobs).
        jobs_path.write_text(json.dumps({"jobs": []}))

        result = restore_cron_jobs_if_emptied(snap_id, keprix_home=keprix_home)
        assert result is not None
    ...
    def test_noop_when_snapshot_id_missing(self, tmp_path):
        ...
        jobs_path = keprix_home / "cron" / "jobs.json"
        self._seed_jobs(jobs_path, [])
        assert restore_cron_jobs_if_emptied(None, keprix_home=keprix_home) is None
        assert restore_cron_jobs_if_emptied("", keprix_home=keprix_home) is None
