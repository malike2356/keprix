...
subparser, placing the flag BEFORE the subcommand (e.g. 'keprix --yolo chat')
...
def _build_parser():
    """Build the keprix argument parser from the real code.
...
class TestChatVerboseArg:
    ...
    def test_cmd_chat_forwards_none_when_verbose_is_absent(self, monkeypatch):
        ...
        fake_banner = types.ModuleType("keprix_cli.banner")
        ...
        monkeypatch.setitem(sys.modules, "keprix_cli.banner", fake_banner)
...
class TestYoloEnvVar:
    """Verify --yolo sets KEPRIX_YOLO_MODE regardless of flag position.
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `keprix gateway run --accept-hooks` failed
    ...
    def test_accepted_at_every_position(self, argv):
        """Invoking `keprix <argv>` must exit 0 (help) rather than
        failing with `unrecognized arguments`."""
        import subprocess
        result = subprocess.run(
            [sys.executable, "-m", "keprix_cli.main", *argv],
            capture_output=True,
            text=True,
            timeout=15,
        )
