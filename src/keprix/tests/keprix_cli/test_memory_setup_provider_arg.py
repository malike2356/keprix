"""Tests for `keprix memory setup [provider]` routing.
...
``keprix memory setup honcho``) without the interactive picker — which matters
because the per-provider ``keprix <provider>`` subcommand is only registered
...
from keprix_cli import memory_setup
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "keprix memory setup" in out
