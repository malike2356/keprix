"""Tests for keprix_cli.web_server and related config utilities."""
...
@pytest.fixture
def _install_example_plugin(_isolate_keprix_home):
    """Drop the example-dashboard fixture into the per-test KEPRIX_HOME
    ...
    isolated ``KEPRIX_HOME``.

    The user-plugin source is preferred over a transient
    ``KEPRIX_BUNDLED_PLUGINS`` override because the bundled dir is
    resolved per-call (other tests in the suite implicitly rely on the
    real bundled plugins — kanban, keprix-achievements, model providers
    ...
    from keprix_cli import web_server
    ...
    #      and start serving requests against a torn-down KEPRIX_HOME.
    app = web_server.app
...
class TestReloadEnv:
    ...
    def test_removes_deleted_known_vars(self, tmp_path):
        """reload_env() removes known Keprix vars not present in .env."""
        env_file = tmp_path / ".env"
    ...
    def test_does_not_remove_unknown_vars(self, tmp_path):
        """reload_env() preserves non-Keprix env vars even when absent from .env."""
        env_file = tmp_path / ".env"
        env_file.write_text("")
        with patch.dict(reload_env.__globals__, {"get_env_path": lambda: env_file}):
...
class TestSessionTokenInjection:
    """The desktop shell mints KEPRIX_DASHBOARD_SESSION_TOKEN and signs its
    ...
    def test_honors_injected_token(self, monkeypatch):
        import importlib
        import keprix_cli.web_server as ws

        monkeypatch.setenv("KEPRIX_DASHBOARD_SESSION_TOKEN", "desktop-seeded-token")
        try:
            importlib.reload(ws)
            assert ws._SESSION_TOKEN == "desktop-seeded-token"
        finally:
            monkeypatch.delenv("KEPRIX_DASHBOARD_SESSION_TOKEN", raising=False)
            importlib.reload(ws)
...
class TestWebServerEndpoints:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_keprix_home):
        """Create a TestClient and isolate the state DB under the test KEPRIX_HOME."""
        try:
        ...
        monkeypatch.setattr(keprix_state, "DEFAULT_DB_PATH", get_keprix_home() / "state.db")
    ...
    def test_get_sessions_forwards_min_messages(self, monkeypatch):
        ...
        monkeypatch.setattr("keprix_state.SessionDB", _FakeDB)
    ...
    def test_update_keprix_returns_docker_guidance_without_spawning(self, monkeypatch):
        import keprix_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("docker update guard should not spawn keprix update")

        monkeypatch.setattr(web_server, "detect_install_method", lambda _root: "docker")
        monkeypatch.setattr(web_server, "_spawn_keprix_action", fail_spawn)
        web_server._ACTION_PROCS.pop("keprix-update", None)
        web_server._ACTION_RESULTS.pop("keprix-update", None)

        resp = self.client.post("/api/keprix/update")

        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is False
        assert data["name"] == "keprix-update"
        assert data["pid"] is None
        assert data["error"] == "docker_update_unsupported"
        assert "docker pull nousresearch/keprix:latest" in data["message"]
        assert spawned is False

        status = self.client.get("/api/actions/keprix-update/status")
        assert status.status_code == 200
        status_data = status.json()
        assert status_data["running"] is False
        assert status_data["exit_code"] == 1
        assert status_data["pid"] is None
        assert any("docker pull nousresearch/keprix:latest" in line for line in status_data["lines"])

    def test_update_keprix_returns_managed_runtime_guidance_without_spawning(self, monkeypatch):
        import keprix_cli.web_server as web_server
        ...
        def fail_spawn(*_args, **_kwargs):
            nonlocal spawned
            spawned = True
            raise AssertionError("managed runtime update guard should not spawn keprix update")
    ...
    def test_update_keprix_spawns_on_non_docker_install(self, monkeypatch):
        import keprix_cli.web_server as web_server
        ...
        monkeypatch.setattr(web_server, "_spawn_keprix_action", fake_spawn)
        web_server._ACTION_PROCS.pop("keprix-update", None)
        web_server._ACTION_RESULTS.pop("keprix-update", None)
        ...
        assert resp.status_code == 200
        assert resp.json() == {"ok": True, "pid": 12345, "name": "keprix-update"}
        assert calls == [(["update"], "keprix-update")]
    ...
    def test_action_status_reaps_completed_process(self, monkeypatch):
        ...
        web_server._ACTION_PROCS.pop("keprix-update", None)
        web_server._ACTION_RESULTS.pop("keprix-update", None)
        web_server._ACTION_PROCS["keprix-update"] = proc

        resp = self.client.get("/api/actions/keprix-update/status")
        assert resp.status_code == 200
        data = resp.json()
        ...
        assert "keprix-update" not in web_server._ACTION_PROCS
        assert web_server._ACTION_RESULTS["keprix-update"] == {
            "exit_code": 0,
            "pid": 42424,
        }
    ...
    def test_get_env_vars_marks_channel_managed_keys(self):
        from keprix_cli.web_server import _channel_managed_env_keys
    ...
    def test_platform_scoped_messaging_env_vars_are_channel_managed(self):
        from keprix_cli.web_server import (
    ...
    def test_model_set_requires_confirmation_for_expensive_model(self, monkeypatch):
        monkeypatch.setattr(
            "keprix_cli.model_cost_guard.expensive_model_warning",
    ...
    def test_model_set_maps_unknown_vendor_to_aggregator(self, monkeypatch):
        ...
        a Keprix provider — keep the user's aggregator instead of writing a
    ...
    def test_ops_import_passes_force_flag(self, tmp_path, monkeypatch):
        ...
        `keprix import` doesn't auto-abort at the overwrite prompt."""
        import keprix_cli.web_server as ws
        ...
        monkeypatch.setattr(ws, "_spawn_keprix_action", fake_spawn)
    ...
    def test_reveal_env_var(self, tmp_path):
        ...
        from keprix_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN
    ...
    def test_reveal_env_var_legacy_authorization_header_still_works(self, tmp_path):
        ...
        from keprix_cli.config import save_env_value
        from keprix_cli.web_server import _SESSION_TOKEN
    ...
    def test_weixin_messaging_metadata_describes_personal_ilink_setup(self):
        ...
        assert weixin["docs_url"] == (
            "https://keprixai.com/docs/user-guide/messaging/weixin/"
        )
    ...
    def test_update_messaging_platform_saves_env_and_enablement(self):
        from keprix_cli.config import load_config, load_env
    ...
    def test_telegram_onboarding_worker_request_uses_httpx(self, monkeypatch):
        ...
        assert kwargs["json"] == {"bot_name": "Keprix"}
        assert kwargs["headers"]["Accept"] == "application/json"
        assert kwargs["headers"]["Authorization"] == "Bearer poll-secret"
        assert kwargs["headers"]["Content-Type"] == "application/json"
        assert kwargs["headers"]["User-Agent"].startswith("KeprixDashboard/")
    ...
    def test_telegram_onboarding_start_strips_poll_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            ...
            return {
                ...
                "suggested_username": "keprix_pair123_bot",
                "deep_link": "https://t.me/newbot/KeprixSetupBot/keprix_pair123_bot",
                "qr_payload": "https://t.me/newbot/KeprixSetupBot/keprix_pair123_bot",
    ...
    def test_telegram_onboarding_ready_and_apply_never_returns_bot_token(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "keprix_pair_ready_bot",
                    "deep_link": "https://t.me/newbot/KeprixSetupBot/keprix_pair_ready_bot",
                    "qr_payload": "https://t.me/newbot/KeprixSetupBot/keprix_pair_ready_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
            ...
            return {
                "status": "ready",
                "bot_username": "keprix_pair_ready_bot",
        ...
        monkeypatch.setattr(ws, "_spawn_keprix_action", fake_spawn_action)
    ...
    def test_telegram_onboarding_apply_reports_restart_failure_after_save(
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "keprix_pair_restart_fails_bot",
                    "deep_link": "https://t.me/newbot/KeprixSetupBot/keprix_pair_restart_fails_bot",
                    "qr_payload": "https://t.me/newbot/KeprixSetupBot/keprix_pair_restart_fails_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            assert method == "GET"
            ...
            return {
                "status": "ready",
                "bot_username": "keprix_pair_restart_fails_bot",
                "owner_user_id": 123456789,
                "token": "123456:SECRET",
            }
        ...
        monkeypatch.setattr(ws, "_spawn_keprix_action", fail_spawn_action)
    ...
    def test_telegram_onboarding_apply_reuses_inflight_gateway_restart(
        ...
        second racing ``keprix gateway restart`` child (e.g. when a stale
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            if method == "POST":
                return {
                    ...
                    "poll_token": "poll-secret",
                    "suggested_username": "keprix_pair_reuse_bot",
                    "deep_link": "https://t.me/newbot/KeprixSetupBot/keprix_pair_reuse_bot",
                    "qr_payload": "https://t.me/newbot/KeprixSetupBot/keprix_pair_reuse_bot",
                    "expires_at": "2027-05-18T00:00:00.000Z",
                }
            return {
                "status": "ready",
                "bot_username": "keprix_pair_reuse_bot",
                "owner_user_id": 123456789,
                "token": "123456:SECRET",
            }
    ...
    def test_telegram_onboarding_apply_requires_ready_pairing(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "keprix_pair_waiting_bot",
                "deep_link": "https://t.me/newbot/KeprixSetupBot/keprix_pair_waiting_bot",
                "qr_payload": "https://t.me/newbot/KeprixSetupBot/keprix_pair_waiting_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_telegram_onboarding_cancel_clears_local_session(self, monkeypatch):
        ...
        def fake_request(method, path, *, body=None, bearer_token=None):
            return {
                ...
                "poll_token": "poll-secret",
                "suggested_username": "keprix_pair_cancel_bot",
                "deep_link": "https://t.me/newbot/KeprixSetupBot/keprix_pair_cancel_bot",
                "qr_payload": "https://t.me/newbot/KeprixSetupBot/keprix_pair_cancel_bot",
                "expires_at": "2027-05-18T00:00:00.000Z",
            }
    ...
    def test_readiness_report_on_dashboard(self, tmp_path, monkeypatch):
        """Admin -> Readiness reads /api/admin/readiness on the dashboard."""
        from starlette.testclient import TestClient
        from keprix.auth.session import AuthManager
        from keprix_cli.web_server import app
        ...
        monkeypatch.setattr("keprix.readiness.routes.build_report", lambda **_kwargs: Report())

        client = TestClient(app)
        headers = {"Authorization": f"Bearer {token}"}
        missing = client.get("/api/admin/readiness", follow_redirects=False)
        assert missing.status_code == 401

        status = client.get("/api/admin/readiness", headers=headers, follow_redirects=False)
        assert status.status_code == 200, status.text[:400]
        assert status.headers.get("location") is None
        body = status.json()
        assert body.get("overall") == "pass"
        assert "checks" in body

        evidence = client.get("/api/admin/readiness/restore-evidence", headers=headers, follow_redirects=False)
        assert evidence.status_code == 200, evidence.text[:400]
        assert "evidence" in evidence.json()

    def test_backup_list_on_dashboard(self, tmp_path, monkeypatch):
        """Admin -> Backup reads /api/admin/backup/list on the dashboard."""
        from starlette.testclient import TestClient
        from keprix.auth.session import AuthManager
        from keprix_cli.web_server import app
        ...
        client = TestClient(app)
        headers = {"Authorization": f"Bearer {token}"}
        missing = client.get("/api/admin/backup/list", follow_redirects=False)
        assert missing.status_code == 401

        listed = client.get("/api/admin/backup/list", headers=headers, follow_redirects=False)
        assert listed.status_code == 200, listed.text[:400]
        assert listed.headers.get("location") is None
        assert listed.json()["backups"] == []

    def test_upstream_queue_on_dashboard(self, tmp_path, monkeypatch):
        """Admin -> Hermes upstream reads /api/admin/upstream on the dashboard."""
        from starlette.testclient import TestClient
        from keprix.auth.session import AuthManager
        from keprix_cli.web_server import app
        ...
        monkeypatch.setattr("keprix.api.upstream_routes._monitor", lambda: Monitor())

        client = TestClient(app)
        headers = {"Authorization": f"Bearer {token}"}
        missing = client.get("/api/admin/upstream", follow_redirects=False)
        assert missing.status_code == 401

        overview = client.get("/api/admin/upstream", headers=headers, follow_redirects=False)
        assert overview.status_code == 200, overview.text[:400]
        assert overview.headers.get("location") is None
        body = overview.json()
    ...
    def test_path_traversal_dotdot_blocked(self):
        ...
        resp = self.client.get("/%2e%2e/keprix_cli/web_server.py")
        assert resp.status_code in {200, 404}
        if resp.status_code == 200:
    ...
    def test_spa_assets_are_read_as_utf8(self, monkeypatch, tmp_path):
        ...
        css_resp = spa_client.get("/assets/app.css", headers={"x-forwarded-prefix": "/keprix"})
    ...
    def test_set_model_main_nous_applies_gateway_defaults(self, monkeypatch):
        ...
        import keprix_cli.nous_subscription as ns
        ...
        resp = self.client.post(
            "/api/model/set",
            json={"scope": "main", "provider": "nous", "model": "keprix-4"},
        )
        assert resp.status_code == 200
        data = resp.json()
        assert data["ok"] is True
    ...
    def test_apply_main_model_assignment_base_url_and_context_reconcile(self):
        ...
        from keprix_cli.web_server import _apply_main_model_assignment
    ...
    def test_parse_model_ids_handles_openai_and_bare_shapes(self):
        ...
        from keprix_cli.web_server import _parse_model_ids
    ...
    def test_set_model_main_custom_persists_api_key_and_registers_provider(self):
        ...
        ``keprix model`` custom flow. Regression for the desktop loop where a
    ...
    def test_set_model_main_reports_stale_auxiliary_pins(self):
        ...
        cfg = load_config()
        cfg["model"] = {"provider": "nous", "default": "keprix-4"}
        cfg["auxiliary"] = {
    ...
    def test_recommended_default_nous_honors_free_tier(self, monkeypatch):
        ...
        model (mirroring `keprix model`), not the first curated paid entry."""
        import keprix_cli.models as models_mod
...
class TestBuildSchemaFromConfig:
    def test_produces_expected_field_count(self):
        from keprix_cli.web_server import CONFIG_SCHEMA
    ...
    def test_empty_prefix_produces_correct_keys(self):
        from keprix_cli.web_server import _build_schema_from_config
...
class TestNewEndpoints:
    ...
    @pytest.fixture(autouse=True)
    def _setup(self, monkeypatch, _isolate_keprix_home):
        try:
            from starlette.testclient import TestClient
        except ImportError:
            pytest.skip("fastapi/starlette not installed")
    ...
    def test_cron_blueprints_list(self):
        ...
        assert first["appUrl"].startswith("keprix://")
    ...
    def test_profiles_list_includes_default(self):
        from keprix_constants import get_keprix_home
        get_keprix_home().mkdir(parents=True, exist_ok=True)
    ...
    def test_profiles_list_falls_back_when_profile_listing_fails(self, monkeypatch):
        from keprix_constants import get_keprix_home
        import keprix_cli.profiles as profiles_mod

        keprix_home = get_keprix_home()
        keprix_home.mkdir(parents=True, exist_ok=True)
        (keprix_home / "config.yaml").write_text(
            "model:\n  provider: openrouter\n  name: anthropic/claude-sonnet-4.6\n",
            encoding="utf-8",
        )
        named = keprix_home / "profiles" / "multi-agent"
    ...
    def test_profile_setup_command_uses_named_profile_wrapper(self):
        ...
        (get_keprix_home() / "profiles" / "coder").mkdir(parents=True)
    ...
    def test_profile_setup_command_uses_keprix_for_default_profile(self):
        from keprix_constants import get_keprix_home
        ...
        assert resp.status_code == 200
        assert resp.json()["command"] == "keprix setup"
    ...
    def test_profiles_create_creates_wrapper_alias_when_safe(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(profiles_mod.shutil, "which", lambda name: "/opt/keprix/bin/keprix")
        ...
        assert wrapper_path.read_text() == '#!/bin/sh\nexec /opt/keprix/bin/keprix -p writer "$@"\n'
    ...
    def test_profiles_create_with_clone_from_copies_source_skills(self, monkeypatch):
        ...
        (get_keprix_home() / "config.yaml").write_text(
            "model:\n  provider: openrouter\n",
            encoding="utf-8",
        )
        default_skill = get_keprix_home() / "skills" / "custom" / "new-skill"
        ...
        assert resp.status_code == 200
        cloned_root = get_keprix_home() / "profiles" / "cloned"
    ...
    def test_profiles_create_with_clone_from_duplicates_source(self, monkeypatch):
        ...
        source_skill = get_keprix_home() / "profiles" / "source-prof" / "skills" / "custom" / "src-skill"
        ...
        assert resp.status_code == 200
        cloned_skill = (
            get_keprix_home() / "profiles" / "source-prof-copy" / "skills" / "custom" / "src-skill" / "SKILL.md"
        )
        assert cloned_skill.exists()
    ...
    def test_profiles_create_clone_all_from_named_source(self, monkeypatch):
        ...
        source_dir = get_keprix_home() / "profiles" / "full-src"
        ...
        assert resp.status_code == 200
        target_dir = get_keprix_home() / "profiles" / "full-copy"
    ...
    def test_profiles_create_without_clone_seeds_bundled_skills(self, monkeypatch):
        ...
        assert resp.status_code == 200
        seeded_skill = get_keprix_home() / "profiles" / "fresh" / "skills" / "software-development" / "plan" / "SKILL.md"
    ...
    def test_profiles_create_builder_fields_model_mcp_and_keep_skills(self, monkeypatch):
        ...
        prof_dir = get_keprix_home() / "profiles" / "builder"
        token = set_keprix_home_override(str(prof_dir))
        try:
            cfg = load_config()
    ...
    def test_profile_model_round_trip(self, monkeypatch):
        ...
        import yaml
        cfg_path = get_keprix_home() / "profiles" / "model-prof" / "config.yaml"
    ...
    def test_skills_list_includes_disabled_skills(self, monkeypatch):
        ...
        import keprix_cli.skills_config as skills_config
        import keprix_cli.web_server as web_server
    ...
    def test_toolsets_list_matches_cli_enabled_state(self, monkeypatch):
        import keprix_cli.tools_config as tools_config
...
class TestModelContextLength:
    ...
    def test_normalize_extracts_context_length_from_dict(self):
        ...
        from keprix_cli.web_server import _normalize_config_for_web
    ...
    def test_denormalize_writes_context_length_into_model_dict(self):
        ...
        from keprix_cli.web_server import _denormalize_config_from_web
        from keprix_cli.config import save_config
...
class TestNormaliseThemeDefinition:
    ...
    def test_rejects_missing_name(self):
        from keprix_cli.web_server import _normalise_theme_definition
...
class TestDiscoverUserThemes:
    """Tests for _discover_user_themes() — scans ~/.keprix/dashboard-themes/."""
...
class TestBulkDeleteSessionsEndpoint:
    ...
    1. Route-ordering: ``/api/sessions/bulk-delete`` must shadow the
       ...
       the block comment in ``keprix_cli/web_server.py``).
    ...
    def test_route_order_not_shadowed_by_session_id(self):
        ...
        assert "deleted" in body, (
            ...
            "registration order in keprix_cli/web_server.py."
        )
...
class TestDeleteEmptySessionsEndpoint:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_keprix_home):
        ...
        # Pin the SessionDB to the isolated KEPRIX_HOME so each test
...
class TestPluginAPIAuth:
    ...
    @pytest.fixture(autouse=True)
    def _setup_test_client(self, monkeypatch, _isolate_keprix_home, _install_example_plugin):
        ...
        installs it into the per-test ``KEPRIX_HOME``.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        test fixture (installed into KEPRIX_HOME by the class-level
    ...
    def test_non_kanban_plugin_route_requires_auth(self):
        ...
        so any plugin's API surface — kanban, keprix-achievements, future
        plugins — must require the session token. Hit a non-kanban plugin
        path to lock that in.
        """
        # Real plugin path (keprix-achievements is loaded by default).
        resp = self.client.get("/api/plugins/keprix-achievements/overview")
        assert resp.status_code == 401
...
# or ``sh -c 'printf …'``) instead of the real ``keprix --tui`` binary.  The
...
class TestPtyWebSocket:
    ...
    def test_resolve_chat_argv_uses_dashboard_scroll_env(self, monkeypatch):
        ...
        import keprix_cli.main as main_mod
        ...
        assert env["KEPRIX_TUI_INLINE"] == "1"
        assert env["KEPRIX_TUI_DISABLE_MOUSE"] == "1"

    def test_resolve_chat_argv_applies_terminal_backend_config(
        self, monkeypatch, _isolate_keprix_home
    ):
        import keprix_cli.main as main_mod

        config_path = Path(os.environ["KEPRIX_HOME"]) / "config.yaml"
        config_path.write_text(
            "\n".join(
                [
                    "terminal:",
                    "  backend: docker",
                    "  docker_image: example/keprix-tools:latest",
                    "  docker_extra_args:",
                    "    - --network=host",
                ]
            ),
        ...
        assert env["TERMINAL_DOCKER_IMAGE"] == "example/keprix-tools:latest"
    ...
    def test_streams_child_stdout_to_client(self, monkeypatch):
        ...
        with self.client.websocket_connect(self._url()) as conn:
            ...
            while time.monotonic() < deadline:
                ...
                if frame:
                    buf += frame
                if b"keprix-ws-ok" in buf:
                    break
            assert b"keprix-ws-ok" in buf
    ...
    def test_unavailable_platform_closes_with_message(self, monkeypatch):
        from keprix_cli.pty_bridge import PtyUnavailableError
        ...
        import keprix_cli.web_server as ws_mod
    ...
    def test_channel_param_propagates_sidecar_url(self, monkeypatch):
        ...
        KEPRIX_TUI_SIDECAR_URL env var pointing back at /api/pub on the
    ...
    def test_pub_broadcasts_to_events_subscribers(self):
        ...
        """
        import asyncio
        from keprix_cli import web_server as ws_mod
...
def test_resolve_chat_argv_injects_gateway_ws_url(monkeypatch):
    import keprix_cli.main as cli_main
    import keprix_cli.web_server as ws
    ...
    gateway_url = env.get("KEPRIX_TUI_GATEWAY_URL", "")
...
class TestDashboardPluginStaticAssetAllowlist:
    ...
    def _setup_test_client(self, monkeypatch, _isolate_keprix_home, _install_example_plugin):
        ...
        the per-test ``KEPRIX_HOME`` user-plugins dir.
        """
        try:
            from starlette.testclient import TestClient
        except ImportError:
...
class TestDesktopCronTicker:
    ...
    def test_ticker_runs_when_desktop(self, monkeypatch, _isolate_keprix_home):
        import threading
        import cron.scheduler as sched

        called = threading.Event()
        monkeypatch.setattr(sched, "tick", lambda *a, **k: called.set())
        monkeypatch.setenv("KEPRIX_DESKTOP", "1")

        with self._client():
            assert called.wait(3.0), "expected cron tick under KEPRIX_DESKTOP=1"

    def test_ticker_skipped_without_desktop(self, monkeypatch, _isolate_keprix_home):
        import threading
        import cron.scheduler as sched

        called = threading.Event()
        monkeypatch.setattr(sched, "tick", lambda *a, **k: called.set())
        monkeypatch.delenv("KEPRIX_DESKTOP", raising=False)
