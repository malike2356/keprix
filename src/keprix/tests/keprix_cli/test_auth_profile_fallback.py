...
When ``KEPRIX_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.keprix/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.keprix            (has its own auth.json fixture)
    * Profile     -> tmp_path/.keprix/profiles/coder   (active, KEPRIX_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Keprix is not
...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from keprix_cli.auth import _load_auth_store, _load_provider_state
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    keprix_home = tmp_path / "classic"
    keprix_home.mkdir()
    monkeypatch.setenv("KEPRIX_HOME", str(keprix_home))

    _write(keprix_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (KEPRIX_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.keprix as the "real home" — which is NOT equal
    # to the KEPRIX_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    keprix_home = tmp_path / "classic"
    ...
    _write(keprix_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from keprix_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: KEPRIX_HOME is set to a custom path that is NOT under
    # ~/.keprix/profiles/ — get_default_keprix_root() returns KEPRIX_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from keprix_cli.auth import read_credential_pool, write_credential_pool
