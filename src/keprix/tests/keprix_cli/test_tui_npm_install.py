...
@pytest.fixture
def main_mod():
    import keprix_cli.main as m

    return m


def _touch_ink(root: Path) -> None:
    ink = root / "node_modules" / "@keprix" / "ink" / "package.json"
...
def test_make_tui_argv_decodes_dev_prebuild_with_utf8_replace(
    ...
    ink_dir = tmp_path / "packages" / "keprix-ink"
    ink_dir.mkdir(parents=True)
...
def test_no_stray_lockfiles_in_workspace_subdirs(main_mod) -> None:
    ...
    # Also sweep ui-tui/packages/* (keprix-ink etc.)
