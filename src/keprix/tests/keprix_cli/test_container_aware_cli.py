...
def container_env(tmp_path, monkeypatch):
    """Set up a fake KEPRIX_HOME with .container-mode file."""
    keprix_home = tmp_path / ".keprix"
    keprix_home.mkdir()
    monkeypatch.setenv("KEPRIX_HOME", str(keprix_home))
    monkeypatch.delenv("KEPRIX_DEV", raising=False)

    container_mode = keprix_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=keprix\n"
        "exec_user=keprix\n"
        "keprix_bin=/data/current-package/bin/keprix\n"
    )
    return keprix_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("keprix_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "keprix"
    assert info["exec_user"] == "keprix"
    assert info["keprix_bin"] == "/data/current-package/bin/keprix"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("keprix_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_keprix_dev(container_env, monkeypatch):
    """Returns None when KEPRIX_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("KEPRIX_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_keprix_dev_zero(container_env, monkeypatch):
    """KEPRIX_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("KEPRIX_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        keprix_home = Path(tmpdir) / ".keprix"
        keprix_home.mkdir()
        (keprix_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("keprix_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_keprix_home": lambda: keprix_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("KEPRIX_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=keprix-custom\n"
        "exec_user=myuser\n"
        "keprix_bin=/opt/keprix/bin/keprix\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "keprix-custom"
    assert info["exec_user"] == "myuser"
    assert info["keprix_bin"] == "/opt/keprix/bin/keprix"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from keprix_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "keprix"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "keprix" in cmd
    assert "/data/current-package/bin/keprix" in cmd
    assert "chat" in cmd
