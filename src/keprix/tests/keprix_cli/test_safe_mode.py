"""Tests for `keprix chat --safe-mode` — pristine troubleshooting runs.
...
_VARS = ("KEPRIX_SAFE_MODE", "KEPRIX_IGNORE_USER_CONFIG", "KEPRIX_IGNORE_RULES")
...
class TestSafeModeEnvWiring:
    ...
    def test_safe_mode_sets_all_gates(self):
        # Mirrors the cmd_chat logic in keprix_cli/main.py.
        class Args:
            safe_mode = True
        ...
        assert os.environ.get("KEPRIX_SAFE_MODE") == "1"
...
class TestSafeModePluginDiscovery:
    """Plugin discovery must be a no-op under KEPRIX_SAFE_MODE=1."""

    def test_discovery_skipped(self, monkeypatch):
        monkeypatch.setenv("KEPRIX_SAFE_MODE", "1")
    ...
    def test_discovery_runs_without_safe_mode(self, monkeypatch):
        monkeypatch.delenv("KEPRIX_SAFE_MODE", raising=False)
        from keprix_cli.plugins import PluginManager
...
class TestSafeModeMCP:
    """_load_mcp_config must return no servers under KEPRIX_SAFE_MODE=1."""
...
class TestSafeModeParser:
    """--safe-mode must parse on both the root parser and `keprix chat`."""
