...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (KEPRIX_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via KEPRIX_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in KEPRIX_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from keprix_cli.providers import KEPRIX_OVERLAYS
        assert "bedrock" in KEPRIX_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from keprix_cli.providers import KEPRIX_OVERLAYS
        assert KEPRIX_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from keprix_cli.providers import KEPRIX_OVERLAYS
        assert KEPRIX_OVERLAYS["bedrock"].auth_type == "aws_sdk"
