...
``keprix_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``keprix --help``, ``keprix version``,
``keprix logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``keprix --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from keprix_cli import main as _main

    argv_backup = sys.argv[:]
    sys.argv = ["keprix", "--help"]
    buf = io.StringIO()
    try:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["keprix", "-w", "--tui"], None),
        (["keprix", "version"], "version"),
        (["keprix", "--tui", "chat"], "chat"),
        (["keprix", "-w", "logs"], "logs"),
        (["keprix", "chat", "hello world"], "chat"),
        (["keprix", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["keprix", "-m", "gpt5", "chat"], "chat"),
        (["keprix", "--model", "gpt5", "chat", "hi"], "chat"),
        (["keprix", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["keprix", "-z", "hello world"], None),
        (["keprix", "-z", "hello", "chat"], "chat"),
        (["keprix", "--model=gpt5", "chat"], "chat"),     # inline form
        (["keprix", "--", "chat"], "chat"),               # -- terminator
        (["keprix", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["keprix", "some-plugin-cmd"], "some-plugin-cmd"),
        (["keprix", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["keprix"],                          # bare → chat
        ["keprix", "--help"],                # top-level help
        ["keprix", "-h"],
        ["keprix", "version"],               # known built-in
        ["keprix", "logs"],
        ["keprix", "gateway", "run"],
        ["keprix", "--tui"],
        ["keprix", "-w", "--tui"],
        ["keprix", "chat", "hi"],
        ["keprix", "help"],                  # accepted built-in-ish
        ["keprix", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["keprix", "meet", "join"],          # potential google_meet plugin
        ["keprix", "honcho", "status"],      # potential memory plugin
        ["keprix", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``keprix help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"keprix_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
