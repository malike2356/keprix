"""Regression test: ``keprix mcp add --command`` must not clobber the
top-level ``args.command`` subparser dest.

The top-level argparse parser uses ``dest="command"`` for its subparsers
(``keprix_cli/_parser.py``).  The dispatcher in ``keprix_cli/main.py``
...
result, ``keprix mcp add foo --url ...`` silently launches chat instead
...
def _build_parser():
    """Minimal replica of the slice of the keprix parser that exhibits
...
class TestMcpAddCommandDest:
    def test_url_invocation_preserves_top_level_command(self):
        """`keprix mcp add foo --url ...` must keep args.command == "mcp".
