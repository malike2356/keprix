"""Tests for keprix_cli.security_advisories.
...
import keprix_cli.security_advisories as adv
...
def isolated_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect KEPRIX_HOME so banner cache and config writes are sandboxed."""
...
class TestAck:
    ...
    def test_ack_advisory_persists_id(self, isolated_home, monkeypatch):
        ...
        # don't depend on the full keprix_cli.config bootstrap.
        store: dict = {"security": {}}
        monkeypatch.setattr(
            "keprix_cli.config.load_config", lambda: store
        )
        monkeypatch.setattr(
            "keprix_cli.config.save_config",
...
class TestRendering:
    def test_short_banner_lines_includes_id_and_version(self, fake_advisory):
        ...
        assert "keprix doctor" in joined
