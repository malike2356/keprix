"""Regression tests for #34107 — Docker UID/GID handling in ensure_keprix_home.

When Keprix runs in Docker with ``KEPRIX_UID=1000`` / ``KEPRIX_GID=911``,
the entrypoint chowns the top-level ``KEPRIX_HOME`` once at startup. But
...
The fix is a ``_chown_to_keprix_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_keprix_uid_gid
# ---------------------------------------------------------------------------


class TestResolveKeprixUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("KEPRIX_UID", "1000")
        monkeypatch.setenv("KEPRIX_GID", "911")
        from keprix_cli.config import _resolve_keprix_uid_gid
        uid, gid = _resolve_keprix_uid_gid()
        assert uid == 1000
        assert gid == 911

    def test_returns_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("KEPRIX_UID", raising=False)
        monkeypatch.delenv("KEPRIX_GID", raising=False)
        from keprix_cli.config import _resolve_keprix_uid_gid
        uid, gid = _resolve_keprix_uid_gid()
        assert uid is None
        assert gid is None
    ...
    def test_invalid_uid_returns_none_for_that_field(self, monkeypatch):
        monkeypatch.setenv("KEPRIX_UID", "not-a-number")
        monkeypatch.setenv("KEPRIX_GID", "911")
        from keprix_cli.config import _resolve_keprix_uid_gid
        uid, gid = _resolve_keprix_uid_gid()
        assert uid is None
    ...
    def test_empty_string_treated_as_unset(self, monkeypatch):
        monkeypatch.setenv("KEPRIX_UID", "")
        monkeypatch.setenv("KEPRIX_GID", "")
        from keprix_cli.config import _resolve_keprix_uid_gid
        uid, gid = _resolve_keprix_uid_gid()
        assert uid is None
        assert gid is None
...
# ---------------------------------------------------------------------------
# _chown_to_keprix_uid
# ---------------------------------------------------------------------------


class TestChownToKeprixUid:
    def test_calls_os_chown_when_both_set(self, tmp_path, monkeypatch):
        monkeypatch.setenv("KEPRIX_UID", "1000")
        monkeypatch.setenv("KEPRIX_GID", "911")
        from keprix_cli import config as cfg

        d = tmp_path / "subdir"
        d.mkdir()

        with patch.object(cfg.os, "chown") as mock_chown:
            cfg._chown_to_keprix_uid(d)
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_keprix_uid(d)  # must not raise
