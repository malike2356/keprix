"""Behavior tests for keprix_cli.inventory.
...
from keprix_cli.inventory import (
    ConfigContext,
...
def test_load_picker_context_full_dict():
    ...
    with patch("keprix_cli.config.load_config", return_value=cfg):
...
def test_build_models_payload_does_not_call_provider_model_ids():
    ...
    """
    rows = [{"slug": "nous", "name": "Nous", "models": ["keprix-4-405b"],
             "total_models": 1, "is_current": False, "is_user_defined": False,
             "source": "built-in"}]
    ctx = _empty_ctx()
    with _list_auth_returning(rows), \
         patch("keprix_cli.models.provider_model_ids") as mock_pm:
...
def test_pricing_uses_cached_nous_tier_by_default():
    ...
    with (
        ...
        patch(
            "keprix_cli.models.get_pricing_for_provider",
            return_value={
                "openai/gpt-5.5": {
                    "prompt": "0.000001",
                    "completion": "0.000002",
        ...
        patch("keprix_cli.models.check_nous_free_tier", return_value=False) as mock_free,
    ):
...
def test_picker_hints_adds_warning_to_skeleton_rows():
    ...
    for row in skeleton_rows:
        ...
        # keprix model fallback.
        assert (
            row["warning"].startswith("paste ")
            or row["warning"].startswith("run `keprix model`")
        )
