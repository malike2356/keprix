...
``$KEPRIX_HOME/plugins/``, runs ``PluginManager().discover_and_load()``,
and asserts the registration result.

Mirrors the structure of
``tests/keprix_cli/test_plugin_scanner_recursion.py::TestRegisterImageGenProvider``.
"""
...
class TestRegisterTTSProvider:
    ...
    def test_accepts_valid_provider(self):
        ...
        keprix_home = Path(os.environ["KEPRIX_HOME"])
        _write_plugin(
            keprix_home / "plugins",
            "my-tts-plugin",
            register_body=(
        ...
        _enable(keprix_home, "my-tts-plugin")
    ...
    def test_rejects_non_provider(self, caplog):
        ...
        _enable(keprix_home, "bad-tts-plugin")
    ...
    def test_rejects_builtin_shadow(self, caplog):
        ...
        _enable(keprix_home, "shadow-tts-plugin")
