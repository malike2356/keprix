...
Covers:
- Defense 1: gateway stop/restart refuse when _KEPRIX_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        "keprix gateway restart",
        "keprix gateway stop",
        "keprix gateway start",
        "keprix  gateway  restart",         # double spaces
        "Hermez Gateway Restart".lower().replace("z", "s"),  # case handled
        "KEPRIX GATEWAY RESTART",           # uppercase
    ])
    def test_keprix_gateway_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "launchctl kickstart gui/501/ai.keprix.gateway",
        "launchctl unload ~/Library/LaunchAgents/ai.keprix.gateway.plist",
        "launchctl stop ai.keprix.gateway",
        "systemctl restart keprix-gateway",
        "systemctl stop keprix-gateway.service",
        "systemctl start keprix-gateway",
    ])
    def test_service_manager_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "kill keprix gateway process",
        "pkill -f keprix.*gateway",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "keprix cron list",
        "keprix update",
        "keprix config set model claude",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_keprix_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade keprix then run keprix gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.keprix.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
...
class TestGatewaySelfTargetingGuard:
    """Verify keprix gateway stop/restart refuse when _KEPRIX_GATEWAY=1."""
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_KEPRIX_GATEWAY", raising=False)
