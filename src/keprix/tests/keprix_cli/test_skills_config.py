"""Tests for keprix_cli/skills_config.py and skills_tool disabled filtering."""
...
class TestSaveDisabledSkills:
    @patch("keprix_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from keprix_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("keprix_cli.config.load_config")
    ...
    @patch("keprix_cli.config.load_config")
    @patch.dict("os.environ", {"KEPRIX_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """KEPRIX_SESSION_PLATFORM should be used when KEPRIX_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_keprix_platform_takes_precedence(self, tmp_path, monkeypatch):
        """KEPRIX_PLATFORM should win over KEPRIX_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
    ...
    def test_explicit_param_overrides_env_vars(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("KEPRIX_HOME", str(tmp_path))
        monkeypatch.setenv("KEPRIX_PLATFORM", "telegram")
        monkeypatch.setenv("KEPRIX_SESSION_PLATFORM", "telegram")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from keprix_cli.skills_config import _get_categories
        skills = [
