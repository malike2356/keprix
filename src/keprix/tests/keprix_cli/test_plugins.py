"""Tests for the Keprix plugin system (keprix_cli.plugins)."""
...
from keprix_cli.middleware import (
    VALID_MIDDLEWARE,
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<keprix_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<keprix_home>/plugins/``; we derive
    ``<keprix_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <KEPRIX_HOME>/config.yaml.
        # Config is always read from KEPRIX_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        keprix_home_str = os.environ.get("KEPRIX_HOME")
        if keprix_home_str:
            keprix_home = Path(keprix_home_str)
        else:
            keprix_home = base.parent
        keprix_home.mkdir(parents=True, exist_ok=True)
        cfg_path = keprix_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.keprix/plugins/ are discovered."""
        plugins_dir = tmp_path / "keprix_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("KEPRIX_HOME", str(tmp_path / "keprix_test"))
    ...
    def test_execution_middleware_does_not_retry_downstream_failure(self, monkeypatch):
        ...
        monkeypatch.setattr("keprix_cli.plugins.get_plugin_manager", lambda: manager)
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("KEPRIX_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".keprix" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_missing_init(self, tmp_path, monkeypatch):
        ...
        # missing-init error.
        keprix_home = tmp_path / "keprix_test"
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under keprix_plugins.<name>."""
        plugins_dir = tmp_path / "keprix_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("KEPRIX_HOME", str(tmp_path / "keprix_test"))

        # Clean up any prior namespace module
        sys.modules.pop("keprix_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "keprix_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="keprix_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPreToolCallBlocking:
    ...
    def test_block_message_returned_for_valid_directive(self, monkeypatch):
        monkeypatch.setattr(
            "keprix_cli.plugins.invoke_hook",
...
class TestPluginManagerList:
    ...
    def test_shared_hook_name_credited_to_every_plugin(self, tmp_path, monkeypatch):
        ...
        `keprix plugins list`. Attribution now counts what each plugin's own
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("keprix_cli.plugins._plugin_manager", mgr):
    ...
    def test_get_plugin_context_engine_discovers_plugins_lazily(self, tmp_path, monkeypatch):
        ...
        keprix_home = tmp_path / "keprix_test"
        plugins_dir = keprix_home / "plugins"
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "keprix_test" / "plugins"
        # Set KEPRIX_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("keprix_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("keprix_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("keprix_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("keprix_cli.plugins.PluginContext.dispatch_tool.__module__", "keprix_cli.plugins"):
...
class TestPluginDebugLogging:
    """KEPRIX_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("KEPRIX_PLUGINS_DEBUG", raising=False)
        from keprix_cli import plugins as plugins_mod
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With KEPRIX_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("KEPRIX_PLUGINS_DEBUG", "1")
        from keprix_cli import plugins as plugins_mod
