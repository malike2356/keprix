"""Tests for `_can_open_graphical_browser()` in keprix_cli.auth.

Guards the fix for the May 2026 report where `keprix auth add xai-oauth`
...
def _force_platform_linux(monkeypatch):
    monkeypatch.setattr("keprix_cli.auth.sys.platform", "linux")
...
def test_non_linux_with_gui_allows(monkeypatch):
    ...
    monkeypatch.setattr("keprix_cli.auth.sys.platform", "darwin")
