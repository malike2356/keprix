...
class TestUnifiedDashboardRouting:
    def test_profile_launch_attaches_to_running_dashboard(self, main_mod, monkeypatch):
        monkeypatch.setattr(
            "keprix_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        # KEPRIX_HOME.  For a standard install (KEPRIX_HOME unset) that root is
        # the platform-native default (~/.keprix), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from keprix_constants import get_default_keprix_root
        assert env.get("KEPRIX_HOME") == str(get_default_keprix_root())

    def test_reexec_pins_docker_machine_root(self, main_mod, monkeypatch):
        """In the Docker layout (KEPRIX_HOME=/opt/data, profiles under
        /opt/data/profiles/<name>) the reroute must pin the child to the
        machine root /opt/data — NOT drop KEPRIX_HOME.

        Dropping it makes the child fall back to $HOME/.keprix
        (= /opt/data/.keprix), an empty auto-seeded home, so the dashboard
        ...
        """
        monkeypatch.setenv("KEPRIX_HOME", "/opt/data/profiles/oracle")
        monkeypatch.setattr(
            "keprix_cli.profiles.get_active_profile_name", lambda: "oracle"
        )
        monkeypatch.setattr(main_mod, "_dashboard_listening", lambda host, port: False)
        execs = []
