"""Tests for IRC gateway configuration via `keprix setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Keprix install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Keprix install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#keprix")
            monkeypatch.setenv("IRC_NICKNAME", "keprix-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `keprix setup gateway` experience with IRC present."""
