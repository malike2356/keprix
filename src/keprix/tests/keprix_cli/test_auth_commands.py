...
def test_auth_add_nous_oauth_is_removed(tmp_path, monkeypatch, capsys):
    monkeypatch.setenv("KEPRIX_HOME", str(tmp_path / "keprix"))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    from keprix_cli.auth_commands import auth_add_command
    ...
    payload = json.loads((tmp_path / "keprix" / "auth.json").read_text())
    assert "nous" not in (payload.get("credential_pool") or {})
    assert "nous" not in (payload.get("providers") or {})
