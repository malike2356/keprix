...
class TestKeprixConstantsFallback:
    """Tests for _keprix_home.py fallback when keprix_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_keprix_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _keprix_home.py with keprix_constants blocked."""
        monkeypatch.setitem(sys.modules, "keprix_constants", None)
        spec = importlib.util.spec_from_file_location("_keprix_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_keprix_home_env_var(self, monkeypatch, tmp_path):
        """When keprix_constants is missing, KEPRIX_HOME comes from env var."""
        monkeypatch.setenv("KEPRIX_HOME", str(tmp_path / "custom-keprix"))
        module = self._load_helper(monkeypatch)
        assert module.get_keprix_home() == tmp_path / "custom-keprix"

    def test_fallback_defaults_to_dot_keprix(self, monkeypatch):
        """When keprix_constants is missing and KEPRIX_HOME unset, default to ~/.keprix."""
        monkeypatch.delenv("KEPRIX_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_keprix_home() == Path.home() / ".keprix"

    def test_fallback_ignores_empty_keprix_home(self, monkeypatch):
        """Empty/whitespace KEPRIX_HOME is treated as unset."""
        monkeypatch.setenv("KEPRIX_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_keprix_home() == Path.home() / ".keprix"

    def test_fallback_display_keprix_home_shortens_path(self, monkeypatch):
        """Fallback display_keprix_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("KEPRIX_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_keprix_home() == "~/.keprix"

    def test_fallback_display_keprix_home_profile_path(self, monkeypatch):
        """Fallback display_keprix_home() handles profile paths under ~/."""
        monkeypatch.setenv("KEPRIX_HOME", str(Path.home() / ".keprix/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_keprix_home() == "~/.keprix/profiles/coder"

    def test_fallback_display_keprix_home_custom_path(self, monkeypatch):
        """Fallback display_keprix_home() returns full path for non-home locations."""
        monkeypatch.setenv("KEPRIX_HOME", "/opt/keprix-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_keprix_home() == "/opt/keprix-custom"

    def test_delegates_to_keprix_constants_when_available(self):
        """When keprix_constants IS importable, _keprix_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_keprix_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import keprix_constants
        assert module.get_keprix_home is keprix_constants.get_keprix_home
        assert module.display_keprix_home is keprix_constants.display_keprix_home
...
class TestInstallDeps:
    ...
    Regression coverage for the Keprix Docker image, whose venv is built with
