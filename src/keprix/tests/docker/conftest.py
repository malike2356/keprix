...
Override the image with ``KEPRIX_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("KEPRIX_TEST_IMAGE", "keprix-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``KEPRIX_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("KEPRIX_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged keprix user
# ---------------------------------------------------------------------------
#
# Background: every Keprix runtime path inside the container drops to UID
# 10000 (the ``keprix`` user) via ``s6-setuidgid keprix``. ``docker exec``
...
# fails (PermissionError swallowed) for keprix, so a test that ran as root
...
# helpers below run every probe under ``-u keprix`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: keprix).
