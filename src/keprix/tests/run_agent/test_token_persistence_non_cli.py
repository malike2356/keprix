...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    keprix_state = ModuleType("keprix_state")
    keprix_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "keprix_state", keprix_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Keprix"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Keprix"
