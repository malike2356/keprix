"""
Tests for timezone support (keprix_time module + integration points).
...
def _reset_keprix_time_cache():
    """Reset the keprix_time module cache (replacement for removed reset_cache)."""
    keprix_time._cached_tz = None
    keprix_time._cached_tz_name = None
    keprix_time._cache_resolved = False


# =========================================================================
# keprix_time.now() — core helper
# =========================================================================

class TestKeprixTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_keprix_time_cache()
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["KEPRIX_TIMEZONE"] = "Asia/Kolkata"
        result = keprix_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["KEPRIX_TIMEZONE"] = "UTC"
        result = keprix_time.now()
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["KEPRIX_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="keprix_time"):
            result = keprix_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["KEPRIX_TIMEZONE"] = "Europe/London"
        tz = keprix_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["KEPRIX_TIMEZONE"] = "Not/A/Timezone"
        tz = keprix_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When KEPRIX_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) KEPRIX_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("KEPRIX_TIMEZONE=" + os.environ.get("KEPRIX_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "KEPRIX_TIMEZONE=NOT_SET" in result["output"], (
            "KEPRIX_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When KEPRIX_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("KEPRIX_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=keprix_tz) which shifted
        absolute time when system-local tz != Keprix tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_keprix_tz(self):
        """Already-aware datetimes should be normalized to Keprix tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Keprix tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Keprix tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Keprix is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Keprix tz
        ...
        # Use a Keprix timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _keprix_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["KEPRIX_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_keprix_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Keprix timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["KEPRIX_TIMEZONE"] = "US/Eastern"
        _reset_keprix_time_cache()
