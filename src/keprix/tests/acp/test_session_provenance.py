...
def test_root_session_no_compression(db):
    ...
    assert prov["currentKeprixSessionId"] == "root1"
    assert prov["rootKeprixSessionId"] == "root1"
    assert prov["parentKeprixSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_keprix_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentKeprixSessionId"] == "old"
    assert prov["rootKeprixSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousKeprixSessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootKeprixSessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootKeprixSessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_keprix_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousKeprixSessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"keprix"}
    assert "sessionProvenance" in meta["keprix"]
    assert meta["keprix"]["sessionProvenance"]["currentKeprixSessionId"] == "root1"
