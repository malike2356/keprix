...
@pytest.fixture
def bare_cli():
    """A KeprixCLI with no __init__ — we only exercise the redraw helper."""
    cli = object.__new__(KeprixCLI)
    return cli
...
class TestForceFullRedraw:
    ...
    def test_missing_app_attr_is_safe(self, bare_cli):
        # Simulate KeprixCLI before the TUI has ever been constructed.
        bare_cli._force_full_redraw()  # must not raise
    ...
    def test_resize_recovery_uses_prompt_toolkit_original_resize_before_reset(self, bare_cli, monkeypatch):
        ...
        erase_down(). If Keprix resets the renderer first, that cursor position
