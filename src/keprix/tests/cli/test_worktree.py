...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("keprix/keprix-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned keprix/* and pr-* branches."""

    def test_prunes_orphaned_keprix_branch(self, git_repo):
        """keprix/keprix-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "keprix/keprix-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "keprix/keprix-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "keprix/keprix-deadbeef" in result.stdout
        ...
        assert "keprix/keprix-deadbeef" in orphaned
        ...
        assert "keprix/keprix-deadbeef" not in result.stdout
