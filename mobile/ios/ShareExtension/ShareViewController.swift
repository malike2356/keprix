...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.verlox.carinakeprix.app", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to Carina", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to Carina gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to Carina.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "CarinaShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "Carina is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "Carina Share",
