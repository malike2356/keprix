...
@MainActor
enum CarinaAppModelRegistry {
    static var appModel: NodeAppModel?
}

@MainActor
final class CarinaAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.verlox.carinakeprix.app", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.verlox.carinakeprix.app", category: "BackgroundWake")
    ...
    private static var appBundleIdentifier: String {
        guard let bundleId = Bundle.main.bundleIdentifier?.trimmingCharacters(in: .whitespacesAndNewlines),
            ...
            return "ai.verlox.carinakeprix.app"
        }
    ...
    private func resolvedAppModel() -> NodeAppModel? {
        self.appModel ?? CarinaAppModelRegistry.appModel
    }
...
enum WatchPromptNotificationBridge {
    ...
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: CarinaWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> CarinaWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return CarinaWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Carina" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [CarinaWatchAction]) -> [UNNotificationAction] {
...
struct CarinaApp: App {
    ...
    @UIApplicationDelegateAdaptor(CarinaAppDelegate.self) private var appDelegate
    ...
    init() {
        ...
        #endif
        CarinaAppModelRegistry.appModel = appModel
...
extension CarinaApp {
    ...
    private static func installUncaughtExceptionLogger() {
        NSLog("Carina: installing uncaught exception handler")
