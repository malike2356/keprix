...
final class LiveActivityManager {
    ...
    private let logger = Logger(subsystem: "ai.verlox.carinakeprix.app", category: "LiveActivity")
    private let connectingStaleSeconds: TimeInterval = 120
    private let hydrationStaleSeconds: TimeInterval = 300
    private var currentActivity: Activity<CarinaActivityAttributes>?
    ...
    func showConnecting(statusText: String = "Connecting...", agentName: String, sessionKey: String) {
        ...
        let attributes = CarinaActivityAttributes(agentName: agentName, sessionKey: sessionKey)
    ...
    private func hydrateCurrentAndPruneDuplicates() {
        let active = Activity<CarinaActivityAttributes>.activities
    ...
    private func updateCurrent(state: CarinaActivityAttributes.ContentState, staleDate: Date? = nil) {
    ...
    private func end(activity: Activity<CarinaActivityAttributes>) {
        Task {
            await activity.end(
                ActivityContent(state: self.disconnectedState(), staleDate: nil),
                dismissalPolicy: .immediate)
    ...
    private func connectingState(statusText: String = "Connecting...") -> CarinaActivityAttributes.ContentState {
        CarinaActivityAttributes.ContentState(
    ...
    private func attentionState(statusText: String) -> CarinaActivityAttributes.ContentState {
        CarinaActivityAttributes.ContentState(
            statusText: statusText,
            isIdle: false,
            isDisconnected: false,
    ...
    private func disconnectedState() -> CarinaActivityAttributes.ContentState {
        CarinaActivityAttributes.ContentState(
