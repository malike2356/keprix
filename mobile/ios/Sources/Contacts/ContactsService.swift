...
final class ContactsService: ContactsServicing {
    ...
    func search(params: CarinaContactsSearchParams) async throws -> CarinaContactsSearchPayload {
        ...
        return CarinaContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: CarinaContactsAddParams) async throws -> CarinaContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return CarinaContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return CarinaContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> CarinaContactPayload {
        CarinaContactPayload(
