...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: CarinaWatchNotifyParams) -> CarinaWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [CarinaWatchAction]?,
        kind: String?,
        promptId: String?) -> [CarinaWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> CarinaWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return CarinaWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                CarinaWatchAction(id: "approve", label: "Approve"),
                CarinaWatchAction(id: "decline", label: "Decline", style: "destructive"),
                CarinaWatchAction(id: "open_phone", label: "Open iPhone"),
                CarinaWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            CarinaWatchAction(id: "done", label: "Done"),
            CarinaWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            CarinaWatchAction(id: "open_phone", label: "Open iPhone"),
            CarinaWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: CarinaWatchRisk?,
        priority: CarinaNotificationPriority?) -> CarinaWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: CarinaNotificationPriority?,
        risk: CarinaWatchRisk?) -> CarinaNotificationPriority?
    {
