...
final class NodeAppModel {
    ...
    private let deepLinkLogger = Logger(subsystem: "ai.verlox.carinakeprix.app", category: "DeepLink")
    private let pushWakeLogger = Logger(subsystem: "ai.verlox.carinakeprix.app", category: "PushWake")
    private let pendingActionLogger = Logger(subsystem: "ai.verlox.carinakeprix.app", category: "PendingAction")
    private let locationWakeLogger = Logger(subsystem: "ai.verlox.carinakeprix.app", category: "LocationWake")
    private let watchReplyLogger = Logger(subsystem: "ai.verlox.carinakeprix.app", category: "WatchReply")
    private let watchExecApprovalLogger = Logger(subsystem: "ai.verlox.carinakeprix.app", category: "WatchExecApproval")
    private let execApprovalNotificationLogger = Logger(
        subsystem: "ai.verlox.carinakeprix.app",
    ...
    func makeChatTransport() -> any CarinaChatTransport {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = CarinaCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = CarinaCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = CarinaCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = CarinaCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = CarinaCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    func requestLocationPermissions(mode: CarinaLocationMode) async -> Bool {
            ...
            check: { [weak self] in
                ...
                do {
                    let data = try await self.operatorGateway.request(
                        ...
                        paramsJSON: nil,
                        timeoutSeconds: 6)
                    guard let decoded = try? JSONDecoder().decode(CarinaGatewayHealthOK.self, from: data) else {
                        return false
                    }
        ...
        do {
            ...
            case .unknownCommand:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: CarinaNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            case .handlerUnavailable:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: CarinaNodeError(code: .unavailable, message: "node handler unavailable"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: CarinaNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(CarinaLocationGetParams.self, from: req.paramsJSON)) ??
            CarinaLocationGetParams()
        ...
        let payload = CarinaLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case CarinaCanvasCommand.present.rawValue:
            // iOS ignores placement hints; canvas always fills the screen.
            let params = (try? Self.decodeParams(CarinaCanvasPresentParams.self, from: req.paramsJSON)) ??
                CarinaCanvasPresentParams()
        ...
        case CarinaCanvasCommand.hide.rawValue:
            self.screen.hideCanvas()
            return BridgeInvokeResponse(id: req.id, ok: true)
        case CarinaCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(CarinaCanvasNavigateParams.self, from: req.paramsJSON)
            let trimmedURL = params.url.trimmingCharacters(in: .whitespacesAndNewlines)
            self.screen.present(urlString: trimmedURL)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case CarinaCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(CarinaCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case CarinaCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(CarinaCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let command = req.command
        switch command {
        case CarinaCanvasA2UICommand.reset.rawValue:
        ...
        case CarinaCanvasA2UICommand.push.rawValue, CarinaCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [CarinaKit.AnyCodable]
            if command == CarinaCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(CarinaCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try CarinaCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(CarinaCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try CarinaCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                const host = globalThis.openclawA2UI;
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case CarinaCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case CarinaCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(CarinaCameraSnapParams.self, from: req.paramsJSON)) ??
                CarinaCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case CarinaCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(CarinaCameraClipParams.self, from: req.paramsJSON)) ??
                CarinaCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(CarinaScreenRecordParams.self, from: req.paramsJSON)) ??
            CarinaScreenRecordParams()
    ...
    private func handleSystemNotify(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(CarinaSystemNotifyParams.self, from: req.paramsJSON)
        ...
        if title.isEmpty, body.isEmpty {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: CarinaNodeError(code: .invalidRequest, message: "INVALID_REQUEST: empty notification"))
        }
        ...
        guard finalStatus == .authorized || finalStatus == .provisional || finalStatus == .ephemeral else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: CarinaNodeError(code: .unavailable, message: "NOT_AUTHORIZED: notifications"))
        }
        ...
        if case let .failure(error) = addResult {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: CarinaNodeError(code: .unavailable, message: "NOTIFICATION_FAILED: \(error.message)"))
        }
        return BridgeInvokeResponse(id: req.id, ok: true)
    }

    private func handleChatPushInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(CarinaChatPushParams.self, from: req.paramsJSON)
        let text = params.text.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !text.isEmpty else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: CarinaNodeError(code: .invalidRequest, message: "INVALID_REQUEST: empty chat.push text"))
        }
        ...
        let payload = CarinaChatPushPayload(messageId: messageId)
        let json = try Self.encodePayload(payload)
        return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
    }
    ...
    private func handleDeviceInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case CarinaDeviceCommand.status.rawValue:
            let payload = try await self.deviceStatusService.status()
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case CarinaDeviceCommand.info.rawValue:
    ...
    private func handlePhotosInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(CarinaPhotosLatestParams.self, from: req.paramsJSON)) ??
            CarinaPhotosLatestParams()
    ...
    private func handleContactsInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case CarinaContactsCommand.search.rawValue:
            let params = (try? Self.decodeParams(CarinaContactsSearchParams.self, from: req.paramsJSON)) ??
                CarinaContactsSearchParams()
            let payload = try await self.contactsService.search(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case CarinaContactsCommand.add.rawValue:
            let params = try Self.decodeParams(CarinaContactsAddParams.self, from: req.paramsJSON)
    ...
    private func handleCalendarInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case CarinaCalendarCommand.events.rawValue:
            let params = (try? Self.decodeParams(CarinaCalendarEventsParams.self, from: req.paramsJSON)) ??
                CarinaCalendarEventsParams()
            let payload = try await self.calendarService.events(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case CarinaCalendarCommand.add.rawValue:
            let params = try Self.decodeParams(CarinaCalendarAddParams.self, from: req.paramsJSON)
    ...
    private func handleRemindersInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case CarinaRemindersCommand.list.rawValue:
            let params = (try? Self.decodeParams(CarinaRemindersListParams.self, from: req.paramsJSON)) ??
                CarinaRemindersListParams()
            let payload = try await self.remindersService.list(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case CarinaRemindersCommand.add.rawValue:
            let params = try Self.decodeParams(CarinaRemindersAddParams.self, from: req.paramsJSON)
    ...
    private func handleMotionInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case CarinaMotionCommand.activity.rawValue:
            let params = (try? Self.decodeParams(CarinaMotionActivityParams.self, from: req.paramsJSON)) ??
                CarinaMotionActivityParams()
            let payload = try await self.motionService.activities(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case CarinaMotionCommand.pedometer.rawValue:
            let params = (try? Self.decodeParams(CarinaPedometerParams.self, from: req.paramsJSON)) ??
                CarinaPedometerParams()
    ...
    private func handleTalkInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case CarinaTalkCommand.pttStart.rawValue:
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case CarinaTalkCommand.pttStop.rawValue:
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case CarinaTalkCommand.pttCancel.rawValue:
            let payload = await self.talkMode.cancelPushToTalk()
            self.voiceWake.resumeAfterExternalAudioCapture(wasSuspended: self.pttVoiceWakeSuspended)
            self.pttVoiceWakeSuspended = false
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case CarinaTalkCommand.pttOnce.rawValue:
            self.pttVoiceWakeSuspended = self.voiceWake.suspendForExternalAudioCapture()
            defer {
                self.voiceWake.resumeAfterExternalAudioCapture(wasSuspended: self.pttVoiceWakeSuspended)
                self.pttVoiceWakeSuspended = false
...
extension NodeAppModel {
    ...
    private func buildCapabilityRouter() -> NodeCapabilityRouter {
        ...
        register([CarinaLocationCommand.get.rawValue]) { [weak self] req in
        ...
        register([
            ...
            CarinaCanvasCommand.hide.rawValue,
            CarinaCanvasCommand.navigate.rawValue,
            CarinaCanvasCommand.evalJS.rawValue,
        ...
        register([
            ...
            CarinaCanvasA2UICommand.push.rawValue,
            CarinaCanvasA2UICommand.pushJSONL.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([CarinaScreenCommand.record.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleScreenRecordInvoke(req)
        }

        register([CarinaSystemCommand.notify.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleSystemNotify(req)
        }

        register([CarinaChatCommand.push.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            CarinaWatchCommand.status.rawValue,
            CarinaWatchCommand.notify.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleWatchInvoke(req)
        }

        register([CarinaPhotosCommand.latest.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            CarinaRemindersCommand.list.rawValue,
            CarinaRemindersCommand.add.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
    ...
    private func handleWatchInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case CarinaWatchCommand.status.rawValue:
            let status = await self.watchMessagingService.status()
            let payload = CarinaWatchStatusPayload(
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case CarinaWatchCommand.notify.rawValue:
            let params = try Self.decodeParams(CarinaWatchNotifyParams.self, from: req.paramsJSON)
            ...
            do {
                ...
                let payload = CarinaWatchNotifyPayload(
    ...
    private func locationMode() -> CarinaLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return CarinaLocationMode(rawValue: raw) ?? .off
    }
...
extension NodeAppModel {
    ...
    func runSharePipelineSelfTest() async {
        ...
        let payload = SharedContentPayload(
            title: "Carina Share Self-Test",
            url: URL(string: "https://keprix.local/share-self-test"),
    ...
    private static func makeWatchExecApprovalItem(from prompt: ExecApprovalPrompt) -> CarinaWatchExecApprovalItem {
        let decisions = prompt.allowedDecisions.compactMap { decision in
            ...
            return CarinaWatchExecApprovalDecision(rawValue: normalizedDecision)
        }
        let preview = Self.trimmedOrNil(prompt.commandPreview) ?? Self.trimmedOrNil(prompt.commandText)
        return CarinaWatchExecApprovalItem(
            id: prompt.id,
    ...
    private func publishWatchExecApprovalPrompt(_ prompt: ExecApprovalPrompt, reason: String) async {
        let message = CarinaWatchExecApprovalPromptMessage(
        ...
        source: String) async
    {
        ...
        let message = CarinaWatchExecApprovalResolvedMessage(
    ...
    private func publishWatchExecApprovalExpired(
        approvalId: String,
        reason: CarinaWatchExecApprovalCloseReason) async
    {
        let normalizedApprovalID = approvalId.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !normalizedApprovalID.isEmpty else { return }
        self.removeWatchExecApprovalPrompt(normalizedApprovalID)
        let message = CarinaWatchExecApprovalExpiredMessage(
            approvalId: normalizedApprovalID,
            reason: reason,
    ...
    private func syncWatchExecApprovalSnapshot(reason: String) async {
        ...
        let message = CarinaWatchExecApprovalSnapshotMessage(
            approvals: approvals,
            sentAtMs: Int(Date().timeIntervalSince1970 * 1000),
    ...
    private func makeWatchChatPreview() async -> WatchChatPreview {
        do {
            let payload: CarinaChatHistoryPayload
            if self.isAppleReviewDemoModeEnabled {
    ...
    private nonisolated static func decodeWatchChatMessage(
        _ raw: CarinaKit.AnyCodable) -> CarinaChatMessage?
    {
        guard let data = try? JSONEncoder().encode(raw) else { return nil }
        return try? JSONDecoder().decode(CarinaChatMessage.self, from: data)
    }

    private nonisolated static func makeWatchChatItems(
        from raw: [CarinaKit.AnyCodable]) -> [CarinaWatchChatItem]
    {
        var readableMessages: [(CarinaChatMessage, String)] = []
    ...
    private nonisolated static func watchChatText(from message: CarinaChatMessage) -> String {
        let parts = message.content.compactMap { content -> String? in
            ...
            if let dict = content.content?.value as? [String: CarinaKit.AnyCodable],
    ...
    private func makeWatchAppSnapshot(
        chatPreview: WatchChatPreview? = nil) -> CarinaWatchAppSnapshotMessage
    {
        self.pruneExpiredWatchExecApprovalPrompts()
        ...
        return CarinaWatchAppSnapshotMessage(
        ...
        sourceReason: String? = nil) async -> ExecApprovalResolutionOutcome
    {
        ...
        guard connected else {
            ...
            return .failed(message: "Carina couldn't connect to the gateway operator session.")
        }
        ...
        do {
            ...
            await self.publishWatchExecApprovalResolved(
                approvalId: normalizedApprovalID,
                decision: CarinaWatchExecApprovalDecision(rawValue: normalizedDecision),
                source: "iphone")
            return .resolved
        } catch {
            if Self.isApprovalNotificationStaleError(error) {
            ...
            return .failed(
                message: "Carina couldn't resolve this approval right now. Try again.")
        }
    }
...
extension NodeAppModel {
    ...
    nonisolated static func _test_makeWatchChatItems(from raw: [CarinaKit.AnyCodable]) -> [CarinaWatchChatItem] {
