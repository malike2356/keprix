...
struct IOSGatewayChatTransport: CarinaChatTransport {
    static let logger = Logger(subsystem: "ai.verlox.carinakeprix.app", category: "ios.chat.transport")
    ...
    private struct ChatSendParams: Codable {
        ...
        var message: String
        var thinking: String
        var attachments: [CarinaChatAttachmentPayload]?
        var timeoutMs: Int
    ...
    static func makeChatSendParamsJSON(
        ...
        attachments: [CarinaChatAttachmentPayload]) throws -> String
    {
        ...
        parentSessionKey: String?) async throws -> CarinaChatCreateSessionResponse
    {
        ...
        return try JSONDecoder().decode(CarinaChatCreateSessionResponse.self, from: res)
    }
    ...
    func listSessions(limit: Int?) async throws -> CarinaChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(CarinaChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> CarinaChatHistoryPayload {
        ...
        return try JSONDecoder().decode(CarinaChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [CarinaChatAttachmentPayload]) async throws -> CarinaChatSendResponse
    {
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(CarinaChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(CarinaGatewayHealthOK.self, from: res))?.ok ?? true
    }
    ...
    static func mapEventFrame(_ evt: EventFrame) -> CarinaChatTransportEvent? {
        switch evt.event {
        case "tick":
            return .tick
        case "seqGap":
        ...
        case "health":
            ...
            let ok = (try? GatewayPayloadDecoding.decode(
                payload,
                as: CarinaGatewayHealthOK.self))?.ok ?? true
            return .health(ok: ok)
        case "chat":
            guard let payload = evt.payload else { return nil }
        ...
        case "session.message":
            ...
            guard let message = try? GatewayPayloadDecoding.decode(
                payload,
                as: CarinaSessionMessageEventPayload.self)
            else {
                return nil
            }
        ...
        case "agent":
            ...
            guard let agentPayload = try? GatewayPayloadDecoding.decode(
                payload,
                as: CarinaAgentEventPayload.self)
            else {
                return nil
            }
