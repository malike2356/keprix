...
final class TalkModeManager: NSObject {
    ...
    private var pttCompletion: CheckedContinuation<CarinaTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.verlox.carinakeprix.app", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = CarinaTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> CarinaTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = activePTTCaptureId {
            return CarinaTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> CarinaTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> CarinaTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return CarinaTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> CarinaTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = CarinaTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: CarinaTalkPTTStopPayload) {
            ...
            group.addTask { [runId] in
                ...
                for await evt in stream {
                    ...
                    if evt.event == "chat" {
                        ...
                        if let text = CarinaChatEventText.assistantText(from: chatEvent) {
