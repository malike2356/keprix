...
final class RealtimeTalkRelaySession {
    ...
    private let logger = Logger(subsystem: "ai.verlox.carinakeprix.app", category: "RealtimeTalkRelay")
    ...
    private func handleToolCall(_ payload: [String: AnyCodable]) async {
        ...
        do {
            ...
            let result: [String: Any] = completion.failed
                ? ["error": "Carina tool call failed"]
                : ["text": completion.text ?? "Carina finished with no text."]
                ...
                for await event in stream {
                    ...
                    if chatEvent.state == "final" {
                        return ChatCompletionResult(
                            text: CarinaChatEventText.assistantText(from: chatEvent),
                            failed: false)
                    }
