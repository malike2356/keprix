...
extension GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [
            ...
            CarinaCapability.screen.rawValue,
        ]
        ...
        if cameraEnabled { caps.append(CarinaCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(CarinaCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = CarinaLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(CarinaCapability.location.rawValue) }

        caps.append(CarinaCapability.device.rawValue)
        caps.append(CarinaCapability.talk.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(CarinaCapability.watch.rawValue)
        }
        caps.append(CarinaCapability.photos.rawValue)
        caps.append(CarinaCapability.contacts.rawValue)
        caps.append(CarinaCapability.calendar.rawValue)
        caps.append(CarinaCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(CarinaCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            CarinaScreenCommand.record.rawValue,
            CarinaSystemCommand.notify.rawValue,
            CarinaChatCommand.push.rawValue,
        ...
        if caps.contains(CarinaCapability.camera.rawValue) {
            commands.append(CarinaCameraCommand.list.rawValue)
            commands.append(CarinaCameraCommand.snap.rawValue)
            commands.append(CarinaCameraCommand.clip.rawValue)
        }
        if caps.contains(CarinaCapability.location.rawValue) {
            commands.append(CarinaLocationCommand.get.rawValue)
        }
        if caps.contains(CarinaCapability.device.rawValue) {
            commands.append(CarinaDeviceCommand.status.rawValue)
            commands.append(CarinaDeviceCommand.info.rawValue)
        }
        if caps.contains(CarinaCapability.watch.rawValue) {
            commands.append(CarinaWatchCommand.status.rawValue)
            commands.append(CarinaWatchCommand.notify.rawValue)
        }
        if caps.contains(CarinaCapability.photos.rawValue) {
            commands.append(CarinaPhotosCommand.latest.rawValue)
        }
        if caps.contains(CarinaCapability.contacts.rawValue) {
            commands.append(CarinaContactsCommand.search.rawValue)
            commands.append(CarinaContactsCommand.add.rawValue)
        }
        if caps.contains(CarinaCapability.calendar.rawValue) {
            commands.append(CarinaCalendarCommand.events.rawValue)
            commands.append(CarinaCalendarCommand.add.rawValue)
        }
        if caps.contains(CarinaCapability.reminders.rawValue) {
            commands.append(CarinaRemindersCommand.list.rawValue)
            commands.append(CarinaRemindersCommand.add.rawValue)
        }
        if caps.contains(CarinaCapability.motion.rawValue) {
            commands.append(CarinaMotionCommand.activity.rawValue)
            commands.append(CarinaMotionCommand.pedometer.rawValue)
        }
