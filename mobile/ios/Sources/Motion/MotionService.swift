...
final class MotionService: MotionServicing {
    func activities(params: CarinaMotionActivityParams) async throws -> CarinaMotionActivityPayload {
        ...
        let mapped: [CarinaMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return CarinaMotionActivityPayload(activities: mapped)
    }
        ...
        return try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    ...
                    let formatter = ISO8601DateFormatter()
                    let payload = CarinaPedometerPayload(
