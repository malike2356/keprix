...
struct RootTabs: View {
    ...
    private func sidebarNavigationSplitContent(sidebarWidth: CGFloat) -> some View {
        ...
        .background(CarinaProBackground())
    }
    ...
    private var sidebarIdentityHeader: some View {
        HStack(spacing: 10) {
            CarinaProMark(size: 30, shadowRadius: 3)
        ...
        .accessibilityLabel("Carina \(self.sidebarGatewayStatusTitle)")
    }
        ...
        title: String? = nil) -> some View
    {
        ...
        .buttonStyle(.plain)
        .foregroundStyle(destination == self.selectedSidebarDestination ? CarinaBrand.accent : .primary)
        .listRowBackground(
            destination == self.selectedSidebarDestination
                ? CarinaBrand.accent.opacity(0.12)
                : Color.clear)
    ...
    private var sidebarHeaderLeadingAction: CarinaSidebarHeaderAction? {
        ...
        if self.isSidebarVisible {
            return CarinaSidebarHeaderAction(
    ...
    private func makeHomeCanvasPayload() -> RootTabsHomeCanvasPayload {
        ...
        case .connected:
            return RootTabsHomeCanvasPayload(
                ...
                footer: "Carina only runs phone-side capabilities while the app is connected and permitted.")
        case .connecting:
            return RootTabsHomeCanvasPayload(
        ...
        case .error, .disconnected:
            return RootTabsHomeCanvasPayload(
                ...
                eyebrow: self.gatewayStatus == .error ? "Gateway needs attention" : "Carina iOS",
                ...
                agents: Array(agents.prefix(4)),
                footer:
                "Use Settings to scan a pairing QR code or paste a setup code from your Carina gateway.")
        }
    }
