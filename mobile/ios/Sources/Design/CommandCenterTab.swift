...
struct CommandCenterTab: View {
    ...
    @State private var defaultChatSessionEntry: CarinaChatSessionEntry?
    @State private var recentChatSessions: [CarinaChatSessionEntry] = []
    var headerTitle: String = "Carina"
    var headerLeadingAction: CarinaSidebarHeaderAction?
        ...
        NavigationStack {
            GeometryReader { geometry in
                ZStack {
                    ...
                    .safeAreaPadding(.bottom, CarinaProMetric.bottomScrollInset)
                }
            }
    ...
    private var header: some View {
            ...
            subtitleLineLimit: 1)
        {
            ...
            {
                CarinaProMark(size: 28, shadowRadius: 5)
            }
        } accessory: {
        ...
        HStack(spacing: 8) {
            ...
            if let badgeValue {
                Text(badgeValue)
                    ...
                    .padding(.vertical, 3)
                    .background(CarinaBrand.accentHot, in: Capsule())
            }
            Spacer(minLength: 8)
            if let value {
                if let action {
    ...
    private var gatewayStatusColor: Color {
        self.gatewayConnected ? CarinaBrand.ok : .secondary
    }
    ...
    private var defaultChatWorkItem: WorkItem {
        ...
        return WorkItem(
            ...
            trailing: "chat",
            color: isOpen ? CarinaBrand.accent : CarinaBrand.ok,
            progress: nil,
            route: .chat(nil))
    }
    ...
    private static func sessionChoices(
        _ sessions: [CarinaChatSessionEntry],
        currentSessionKey: String,
        defaultSessionKey: String) -> [CarinaChatSessionEntry]
    {
    ...
    static func sessionWorkItem(
        for session: CarinaChatSessionEntry,
        currentSessionKey: String) -> WorkItem
    {
        ...
        return WorkItem(
            ...
            trailing: "chat",
            color: isCurrent ? CarinaBrand.accent : CarinaBrand.ok,
            progress: nil,
            route: .chat(session.key))
    }

    fileprivate static func sessionTitle(_ session: CarinaChatSessionEntry) -> String {
    ...
    fileprivate static func sessionDetail(_ session: CarinaChatSessionEntry) -> String {
...
struct CommandSessionsScreen: View {
    ...
    @State private var sessions: [CarinaChatSessionEntry] = []
    @State private var isLoading = false
    @State private var loadErrorText: String?
    let headerLeadingAction: CarinaSidebarHeaderAction?
    let openChat: () -> Void

    init(headerLeadingAction: CarinaSidebarHeaderAction? = nil, openChat: @escaping () -> Void) {
