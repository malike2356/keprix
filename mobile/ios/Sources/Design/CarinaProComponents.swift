...
struct CarinaProBackground: View {
            ...
            if let actionTitle {
                if let action {
                    Button(actionTitle, action: action)
                        .font(.caption.weight(.medium))
                        .foregroundStyle(CarinaBrand.accent)
                } else {
                    Text(actionTitle)
                        .font(.caption.weight(.medium))
                        .foregroundStyle(.secondary)
...
struct ProCard<Content: View>: View {
    ...
    var radius: CGFloat = CarinaProMetric.cardRadius
    ...
    var body: some View {
        ...
        shape
            ...
            .overlay {
                if self.isProminent {
                    shape.strokeBorder(
                        CarinaBrand.accent.opacity(self.colorScheme == .dark ? 0.12 : 0.07),
                        lineWidth: 1)
                        .padding(1)
                }
            }
...
extension View {
    func proPanelSurface(
        tint: Color? = nil,
        radius: CGFloat = CarinaProMetric.cardRadius,
...
struct CarinaSidebarHeaderAction {
    let systemName: String
...
struct CarinaSidebarRevealButton: View {
    let headerAction: CarinaSidebarHeaderAction

    init(action: CarinaSidebarHeaderAction) {
...
struct CarinaSidebarHeaderLeadingSlot: View {
    let action: CarinaSidebarHeaderAction

    var body: some View {
        CarinaSidebarRevealButton(action: self.action)
            .frame(width: 44, height: 44, alignment: .center)
    }
}

struct CarinaAdaptiveHeaderRow<Leading: View, Accessory: View>: View {
    let title: String
    let subtitle: String
...
struct CarinaProMark: View {
    ...
    var body: some View {
        Image("CarinaIcon")
            ...
            .shadow(color: CarinaBrand.accent.opacity(0.18), radius: self.shadowRadius, y: self.shadowRadius / 3)
            .accessibilityLabel("Carina")
    }
}

struct ProProgressBar: View {
    let progress: Double
    var color: Color = CarinaBrand.accentHot
...
struct CarinaGatewayCompactPill: View {
