...
extension SettingsProTab {
    ...
    var settingsListSection: some View {
        VStack(spacing: 10) {
            self.settingsListRow(
                ...
                route: .approvals,
                color: self.pendingApproval == nil ? .secondary : CarinaBrand.warn,
    ...
    func destination(for route: SettingsRoute) -> some View {
        ZStack {
            ...
            ScrollView {
                ...
                .padding(.top, 18)
                .padding(.bottom, CarinaProMetric.bottomScrollInset)
            }
        }
    ...
    var approvalsDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                ...
                color: self.pendingApproval == nil ? CarinaBrand.ok : CarinaBrand.warn)
                ...
                if let pendingApproval {
                    ...
                    if let errorText = self.appModel.pendingExecApprovalPromptErrorText {
                        Text(errorText)
                            ...
                            .foregroundStyle(CarinaBrand.danger)
                    }
                    ...
                    HStack(spacing: 12) {
                        ProIconBadge(systemName: "checkmark.shield.fill", color: CarinaBrand.ok)
                        VStack(alignment: .leading, spacing: 3) {
    ...
    var permissionsDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.toggleCard(
                icon: "camera",
                title: "Camera",
                detail: "Allow the gateway to request photos or video while Carina is foregrounded.",
            ...
            self.toggleCard(
                icon: "lock.display",
                title: "Keep Awake",
                detail: "Keep the screen awake while Carina is open.",
    ...
    var voiceDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                ...
                color: self.talkEnabled || self.voiceWakeEnabled ? CarinaBrand.accent : .secondary)
    ...
    var diagnosticsDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                ...
                color: self.gatewayDiagnosticConnected ? CarinaBrand.ok : CarinaBrand.warn)
    ...
    var privacyDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                icon: "hand.raised",
                title: "Privacy",
                detail: "Control what device context Carina can expose to the gateway.",
    ...
    var notificationsDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                icon: "bell",
                title: "Notifications",
                detail: "Approvals and event alerts from Carina.",
                value: self.notificationStatusText,
                color: self.notificationStatusText == "Allowed" ? CarinaBrand.ok : .secondary)
            ...
            ProCard(radius: SettingsLayout.cardRadius) {
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    Text("Carina uses notifications for approval prompts and mirrored event alerts.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            }
        ...
        isOn: Binding<Bool>) -> some View
    {
        ProCard(radius: SettingsLayout.cardRadius) {
            Toggle(isOn: isOn) {
                HStack(spacing: 12) {
                    ProIconBadge(systemName: icon, color: isOn.wrappedValue ? CarinaBrand.accent : .secondary)
                    VStack(alignment: .leading, spacing: 3) {
                        Text(title)
                            .font(.subheadline.weight(.semibold))
                        Text(detail)
        ...
        ProCard(radius: SettingsLayout.cardRadius) {
            VStack(alignment: .leading, spacing: 12) {
                HStack(spacing: 12) {
                    ProIconBadge(
                        systemName: "location",
                        color: self.locationModeRaw == CarinaLocationMode.off.rawValue ? .secondary : CarinaBrand
                            .accent)
                    VStack(alignment: .leading, spacing: 3) {
                        Text("Location")
                            .font(.subheadline.weight(.semibold))
                ...
                Picker("Location", selection: self.$locationModeRaw) {
                    Text("Off").tag(CarinaLocationMode.off.rawValue)
                    Text("While Using").tag(CarinaLocationMode.whileUsing.rawValue)
                    Text("Always").tag(CarinaLocationMode.always.rawValue)
                }
                .pickerStyle(.segmented)
                ...
                if let locationStatusText {
                    Text(locationStatusText)
                        .font(.caption2)
                        .foregroundStyle(CarinaBrand.warn)
                }
            }
        }
        .padding(.horizontal, CarinaProMetric.pagePadding)
