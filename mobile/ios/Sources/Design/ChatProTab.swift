...
struct ChatProTab: View {
                ...
                VStack(spacing: 0) {
                    ...
                    if let viewModel {
                        CarinaChatView(
                            ...
                            assistantAvatarTint: CarinaBrand.accent,
                    ...
                    Circle()
                        .fill(
                            LinearGradient(
                                colors: [
                                    CarinaBrand.accent,
                                    CarinaBrand.accentHot,
                                ],
                                startPoint: .topLeading,
                                endPoint: .bottomTrailing)))
                .overlay(Circle().strokeBorder(.white.opacity(0.18), lineWidth: 1))
                .shadow(color: CarinaBrand.accent.opacity(0.18), radius: 10, y: 5)
        } else {
            ProIconBadge(systemName: "bubble.left", color: CarinaBrand.accent)
        }
    }
    ...
    private func syncChatViewModel() {
        ...
        guard let viewModel else {
            ...
            self.viewModel = CarinaChatViewModel(
    ...
    private var talkControl: CarinaChatTalkControl {
        CarinaChatTalkControl(
    ...
    private var chatUserAccent: Color {
        self.colorScheme == .light ? Color(red: 0 / 255.0, green: 122 / 255.0, blue: 255 / 255.0) : CarinaBrand.accent
    }
