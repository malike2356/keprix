...
struct TalkProTab: View {
    ...
    private var header: some View {
        HStack(alignment: .center, spacing: 11) {
            ...
            CarinaProMark(size: 31, shadowRadius: 9)
...
struct TalkProState: Equatable {
    ...
    var title: String {
        ...
        if self.normalizedStatus.contains("thinking") { return "Asking Carina" }
    ...
    var color: Color {
        ...
        case .requestFailed, .loadFailed:
            return CarinaBrand.danger
        case .missingScope, .requestingUpgrade, .upgradeRequested, .apiKeyMissing:
            return CarinaBrand.warn
        default:
            if !self.isConfigLoaded { return CarinaBrand.warn }
            return self.isEnabled ? CarinaBrand.ok : CarinaBrand.accentHot
        }
    }
    ...
    var primaryButtonFill: AnyShapeStyle {
        switch self.primaryAction {
        case .stop:
            AnyShapeStyle(CarinaBrand.danger)
        case .waiting:
            AnyShapeStyle(CarinaBrand.warn.opacity(0.72))
        default:
            AnyShapeStyle(LinearGradient(
                colors: [self.color.opacity(0.95), CarinaBrand.accent],
