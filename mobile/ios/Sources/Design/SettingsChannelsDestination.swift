...
struct SettingsChannelsDestination: View {
    ...
    private var summaryColor: Color {
        ...
        if self.errorText != nil { return CarinaBrand.warn }
        return self.channelEntries.contains(where: { $0.running || $0.connected }) ? CarinaBrand.ok : CarinaBrand
            .accent
    }
...
private struct SettingsChannelEntry: Identifiable {
    ...
    var color: Color {
        if self.connected || self.running { return CarinaBrand.ok }
        if self.lastError != nil { return CarinaBrand.warn }
        return self.configured ? CarinaBrand.accent : .secondary
    }
