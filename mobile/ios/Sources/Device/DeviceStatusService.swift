...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> CarinaDeviceStatusPayload {
        ...
        return CarinaDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> CarinaDeviceInfoPayload {
        ...
        return CarinaDeviceInfoPayload(
    ...
    private func batteryStatus() -> CarinaBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: CarinaBatteryState = switch device.batteryState {
        ...
        return CarinaBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> CarinaThermalStatusPayload {
        let state: CarinaThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return CarinaThermalStatusPayload(state: state)
    }

    private func storageStatus() -> CarinaStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return CarinaStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
