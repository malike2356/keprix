...
final class RemindersService: RemindersServicing {
    func list(params: CarinaRemindersListParams) async throws -> CarinaRemindersListPayload {
        ...
        let payload: [CarinaReminderPayload] = try await withCheckedThrowingContinuation { cont in
        ...
        return CarinaRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: CarinaRemindersAddParams) async throws -> CarinaRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = CarinaReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return CarinaRemindersAddPayload(reminder: payload)
    }
