...
@Suite(.serialized) struct GatewayConnectionControllerTests {
    ...
    @Test @MainActor func currentCapsReflectToggles() {
        withUserDefaults([
            ...
            "camera.enabled": true,
            "location.enabledMode": CarinaLocationMode.always.rawValue,
            ...
            #expect(caps.contains(CarinaCapability.canvas.rawValue))
            #expect(caps.contains(CarinaCapability.screen.rawValue))
            #expect(caps.contains(CarinaCapability.camera.rawValue))
            #expect(caps.contains(CarinaCapability.location.rawValue))
            #expect(caps.contains(CarinaCapability.voiceWake.rawValue))
            #expect(caps.contains(CarinaCapability.talk.rawValue))
        }
    }
    ...
    @Test @MainActor func currentCommandsIncludeLocationWhenEnabled() {
        withUserDefaults([
            "node.instanceId": "ios-test",
            "location.enabledMode": CarinaLocationMode.whileUsing.rawValue,
        ]) {
            let appModel = NodeAppModel()
            let controller = GatewayConnectionController(appModel: appModel, startDiscovery: false)
            let commands = Set(controller._test_currentCommands())

            #expect(commands.contains(CarinaLocationCommand.get.rawValue))
        }
    }
    ...
    @Test @MainActor func currentCommandsExcludeDangerousSystemExecCommands() {
        withUserDefaults([
            ...
            #expect(commands.contains(CarinaSystemCommand.notify.rawValue))
            #expect(!commands.contains(CarinaSystemCommand.run.rawValue))
            #expect(!commands.contains(CarinaSystemCommand.which.rawValue))
            #expect(!commands.contains(CarinaSystemCommand.execApprovalsGet.rawValue))
            #expect(!commands.contains(CarinaSystemCommand.execApprovalsSet.rawValue))
        }
    }
    ...
    @Test @MainActor func loadLastConnectionReadsSavedValues() {
        let prior = KeychainStore.loadString(service: "ai.verlox.carinakeprix.app.gateway", account: "lastConnection")
        defer {
            if let prior {
                _ = KeychainStore.saveString(
                    prior,
                    service: "ai.verlox.carinakeprix.app.gateway",
                    account: "lastConnection")
            } else {
                _ = KeychainStore.delete(service: "ai.verlox.carinakeprix.app.gateway", account: "lastConnection")
            }
        }
        _ = KeychainStore.delete(service: "ai.verlox.carinakeprix.app.gateway", account: "lastConnection")
