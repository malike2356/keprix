# Carina iOS (Super Alpha)

This iOS app is super-alpha and internal-use only. The first public App Store release targets iPhone and connects to an Carina Gateway as a `role: node`.
...
- `xcodegen`
- `fastlane`
- Apple account signed into Xcode for the canonical Carina team (`FWJYW4S8P8`)
- `asc` CLI authenticated for the canonical Carina team
- Release-owner access to the encrypted signing repo password (`ASC_MATCH_PASSWORD`)
- App Store Connect app already created for `ai.verlox.carinakeprix.app`
...
- Local development uses the canonical `ai.verlox.carinakeprix.app*` bundle IDs when the Carina team is available, and unique `ai.verlox.carinakeprix.app.test.*` bundle IDs only for non-canonical fallback teams.
- App Store release uses canonical `ai.verlox.carinakeprix.app*` bundle IDs through a temporary generated xcconfig in `apps/ios/build/AppStoreRelease.xcconfig`.
...
- App Store release also switches the app to `CarinaPushTransport=relay`, `CarinaPushDistribution=official`, `CarinaPushAPNsEnvironment=production`, and a production `aps-environment` entitlement.
...
- Release builds default to `{serverURL}/api/notifications/push`.
...
3. Confirm the App Store Connect app and Apple Developer identifiers/capabilities exist for:
   - `ai.verlox.carinakeprix.app`
   - `ai.verlox.carinakeprix.app.share`
   - `ai.verlox.carinakeprix.app.activitywidget`
   - `ai.verlox.carinakeprix.app.watchkitapp`
   - `ai.verlox.carinakeprix.app.watchkitapp.extension`
...
4. Optional: set a custom official relay URL for the build. If unset, the release flow uses `{serverURL}/api/notifications/push`.
...
8. Expected outputs after a successful run:
   - `apps/ios/build/app-store/Carina-<version>.ipa`
   - `apps/ios/build/app-store/Carina-<version>.app.dSYM.zip`
...
- `apps/ios/Sources/Carina.entitlements` derives `aps-environment` from the active build configuration/signing override.
- APNs token registration to gateway happens only after gateway connection (`push.apns.register`).
- Local/manual builds default to `CarinaPushTransport=direct`, `CarinaPushDistribution=local`, and a development `aps-environment` entitlement.
...
- Debug builds default to `CarinaPushAPNsEnvironment=sandbox`; Release builds default to `production`.
...
This exists to keep the hosted relay limited to genuine Carina official builds and to ensure a
...
The iOS app is not a Codex Computer Use backend. Computer Use and `cua-driver mcp` are macOS desktop-control paths; iOS exposes device capabilities as Carina node commands through the gateway. Agents can drive the iPhone canvas, camera, screen, location, voice, and other node capabilities with `node.invoke`, subject to iOS foreground/background limits.
