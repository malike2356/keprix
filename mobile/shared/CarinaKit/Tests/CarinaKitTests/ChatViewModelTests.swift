...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> CarinaChatHistoryPayload
{
    CarinaChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> CarinaChatSessionEntry {
...
private func thinkingOption(_ id: String, label: String? = nil) -> CarinaChatThinkingLevelOption {
    CarinaChatThinkingLevelOption(id: id, label: label ?? id)
}
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> CarinaChatSessionEntry
{
    CarinaChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> CarinaChatModelChoice {
    CarinaChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [CarinaChatHistoryPayload],
    sessionsResponses: [CarinaChatSessionsListResponse] = [],
    modelResponses: [[CarinaChatModelChoice]] = [],
    ...
    -> (TestChatTransport, CarinaChatViewModel)
{
...
private func sendUserMessage(_ vm: CarinaChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, CarinaChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [CarinaChatHistoryPayload]
    private let sessionsResponses: [CarinaChatSessionsListResponse]
    private let modelResponses: [[CarinaChatModelChoice]]
    ...
    private let stream: AsyncStream<CarinaChatTransportEvent>
    private let continuation: AsyncStream<CarinaChatTransportEvent>.Continuation
        ...
        healthResponses: [Bool] = [true])
    {
        ...
        var cont: AsyncStream<CarinaChatTransportEvent>.Continuation!
        ...
        parentSessionKey: String?) async throws -> CarinaChatCreateSessionResponse
    {
        ...
        return CarinaChatCreateSessionResponse(ok: true, key: key, sessionId: "created-\(key)")
    }
    ...
    func requestHistory(sessionKey: String) async throws -> CarinaChatHistoryPayload {
        ...
        return self.historyResponses.last ?? CarinaChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
        ...
        attachments _: [CarinaChatAttachmentPayload]) async throws -> CarinaChatSendResponse
    {
        ...
        return CarinaChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> CarinaChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? CarinaChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: CarinaChatTransportEvent) {
...
struct ChatViewModelTests {
    @Test func `displays error message fallback only for assistant error turns`() throws {
        func decodeMessage(role: String, stopReason: String, contentText: String? = nil) throws -> CarinaChatMessage {
        ...
        #expect(
            CarinaChatMessage.errorDisplayText(
    ...
    @Test @MainActor func `bounded repeated same text reply invalidates older stale refresh`() async throws {
        ...
        transport.emit(CarinaChatTransportEvent.seqGap)
    ...
    @Test func `appends external session user message for active session`() async throws {
        ...
        transport.emit(
            .sessionMessage(
                CarinaSessionMessageEventPayload(
                    sessionKey: "agent:aiden:main",
                    message: CarinaChatMessage(
                        role: "user",
                        content: [
    ...
    @Test func `session choices prefer main and recent`() async throws {
        ...
        let sessions = CarinaChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func `bootstraps model selection from session and defaults`() async throws {
        ...
        let sessions = CarinaChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: CarinaChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func `selecting default model patches nil and updates selection`() async throws {
        ...
        await MainActor.run { vm.selectModel(CarinaChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == CarinaChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func `selecting provider qualified model disambiguates duplicate model I ds`() async throws {
        ...
        let sessions = CarinaChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: CarinaChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func `late model completion does not replay current session selection into previous session`() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = CarinaChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = CarinaChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func `explicit thinking level wins over history and persists changes`() async throws {
        let history = CarinaChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func `decodes gateway thinking metadata from session list`() throws {
        ...
        let decoded = try JSONDecoder().decode(
            CarinaChatSessionsListResponse.self,
            from: Data(json.utf8))
    ...
    @Test func `strips inbound metadata from history messages`() async throws {
        ...
        let vm = await MainActor.run { CarinaChatViewModel(sessionKey: "main", transport: transport) }
