...
public enum CarinaNodeStorage {
    public static func appSupportDir() throws -> URL {
        let base = FileManager().urls(for: .applicationSupportDirectory, in: .userDomainMask).first
        guard let base else {
            throw NSError(domain: "CarinaNodeStorage", code: 1, userInfo: [
                NSLocalizedDescriptionKey: "Application Support directory unavailable",
            ])
        }
        return base.appendingPathComponent("Carina", isDirectory: true)
    }
    ...
    public static func cachesDir() throws -> URL {
        ...
        guard let base else {
            throw NSError(domain: "CarinaNodeStorage", code: 2, userInfo: [
