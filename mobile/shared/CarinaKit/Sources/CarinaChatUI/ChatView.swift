...
@MainActor
public struct CarinaChatView: View {
    public enum Style {
        case standard
        case onboarding
    }
    ...
    @State private var viewModel: CarinaChatViewModel
    ...
    private let talkControl: CarinaChatTalkControl?
    ...
    public init(
        ...
        talkControl: CarinaChatTalkControl? = nil)
    {
    ...
    private var visibleMessages: [CarinaChatMessage] {
        let base: [CarinaChatMessage]
    ...
    private func mergeToolResults(in messages: [CarinaChatMessage]) -> [CarinaChatMessage] {
    ...
    private func isToolResultMessage(_ message: CarinaChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: CarinaChatMessage) -> Bool {
    ...
    private func primaryText(in message: CarinaChatMessage) -> String {
        let parts = message.content.compactMap { content -> String? in
            let kind = (content.type ?? "text").lowercased()
            guard kind == "text" || kind.isEmpty else { return nil }
            return content.text
        }
        return CarinaChatMessage.displayText(
    ...
    private func hasInlineAttachments(in message: CarinaChatMessage) -> Bool {
    ...
    private func toolCalls(in message: CarinaChatMessage) -> [CarinaChatMessageContent] {
    ...
    private func inlineToolResults(in message: CarinaChatMessage) -> [CarinaChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: CarinaChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: CarinaChatMessage) -> String {
...
private struct ChatAssistantIntroCard: View {
    ...
    var body: some View {
        Text(self.text)
            .font(.system(size: 15))
            .lineSpacing(4)
            .foregroundStyle(CarinaChatTheme.assistantText)
            .multilineTextAlignment(.leading)
            .padding(.vertical, 12)
            .padding(.horizontal, 14)
            .background(
                RoundedRectangle(cornerRadius: 16, style: .continuous)
                    .fill(CarinaChatTheme.assistantBubble)
                    .overlay(
                        RoundedRectangle(cornerRadius: 16, style: .continuous)
