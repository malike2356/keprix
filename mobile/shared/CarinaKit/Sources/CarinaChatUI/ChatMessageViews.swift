...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? CarinaChatTheme.userText : CarinaChatTheme.assistantText
    ...
    private var inlineAttachments: [CarinaChatMessageContent] {
    ...
    private var toolCalls: [CarinaChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [CarinaChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? CarinaChatTheme.userBubble
        }
        if self.style == .onboarding {
            return CarinaChatTheme.onboardingAssistantBubble
        }
        return CarinaChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return CarinaChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: CarinaChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? CarinaChatTheme.userText : CarinaChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: CarinaChatMessageContent
    let isUser: Bool
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [CarinaChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: CarinaChatTheme.assistantText)
            }
        }
    }
}
