...
public struct CarinaChatTalkControl {
...
@MainActor
struct CarinaChatComposer: View {
    @Bindable var viewModel: CarinaChatViewModel
    let style: CarinaChatView.Style
    let showsSessionSwitcher: Bool
    let userAccent: Color?
    let assistantName: String?
    let assistantAvatarText: String?
    let assistantAvatarTint: Color?
    let composerChrome: CarinaChatView.ComposerChrome
    let isComposerEnabled: Bool
    let messagePlaceholder: String?
    let talkControl: CarinaChatTalkControl?
            ...
            if self.composerChrome == .full {
                ...
                if self.style == .standard {
                    let shape = UnevenRoundedRectangle(
                        ...
                        style: .continuous)
                    shape
                        .fill(CarinaChatTheme.composerBackground)
                        .overlay(shape.strokeBorder(CarinaChatTheme.composerBorder, lineWidth: 1))
    ...
    private var modelPicker: some View {
        ...
        {
            Text(self.viewModel.defaultModelLabel).tag(CarinaChatViewModel.defaultModelSelectionID)
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \CarinaPendingAttachment.id)
                { (att: CarinaPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            CarinaPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var fullEditor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 14, style: .continuous)
                .fill(CarinaChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 14, style: .continuous)
                        .strokeBorder(CarinaChatTheme.composerBorder)))
    ...
    private func talkButton(_ talkControl: CarinaChatTalkControl) -> some View {
        Button {
    ...
    private func compactTalkButton(_ talkControl: CarinaChatTalkControl) -> some View {
        Button {
            talkControl.toggle(self.viewModel.sessionKey)
        } label: {
            Image(systemName: talkControl.isEnabled ? "stop.fill" : "waveform")
    ...
    private func talkButtonFill(_ talkControl: CarinaChatTalkControl) -> AnyShapeStyle {
        if talkControl.isEnabled {
            return AnyShapeStyle(CarinaChatTheme.userBubble)
        }
        if !talkControl.isGatewayConnected {
            return AnyShapeStyle(Color.secondary.opacity(0.12))
        }
        return CarinaChatTheme.subtleCard
    }

    private func talkButtonStroke(_ talkControl: CarinaChatTalkControl) -> Color {
        if talkControl.isEnabled {
            return Color.white.opacity(0.18)
        }
        return CarinaChatTheme.composerBorder
    }

    private func talkAccessibilityValue(_ talkControl: CarinaChatTalkControl) -> String {
    ...
    private func talkHelpText(_ talkControl: CarinaChatTalkControl) -> String {
    ...
    private var connectionPill: some View {
        ...
        .background {
            if self.composerChrome == .full {
                Capsule()
                    .fill(CarinaChatTheme.subtleCard)
            }
        }
    }
    ...
    private var sendButtonFill: Color {
        self.userAccent ?? CarinaChatTheme.userBubble
    }
