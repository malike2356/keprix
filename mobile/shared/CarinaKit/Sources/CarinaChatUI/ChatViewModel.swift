...
private let chatUILogger = Logger(subsystem: "com.verlox.carinakeprix", category: "CarinaChatUI")

@MainActor
@Observable
// swiftlint:disable:next type_body_length
public final class CarinaChatViewModel {
    public static let defaultModelSelectionID = "__default__"
    static let maxAttachmentBytes = 5_000_000

    public private(set) var messages: [CarinaChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var thinkingLevelOptions: [CarinaChatThinkingLevelOption]
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [CarinaChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [CarinaPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [CarinaChatPendingToolCall] = []
    public private(set) var sessions: [CarinaChatSessionEntry] = []
    private let transport: any CarinaChatTransport
    private var sessionDefaults: CarinaChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: CarinaChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any CarinaChatTransport,
    ...
    public var sessionChoices: [CarinaChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [CarinaChatSessionEntry] = []
    ...
    private static let baseThinkingLevelOptions: [CarinaChatThinkingLevelOption] = [
        CarinaChatThinkingLevelOption(id: "off", label: "off"),
        CarinaChatThinkingLevelOption(id: "minimal", label: "minimal"),
        CarinaChatThinkingLevelOption(id: "low", label: "low"),
    ...
    public func removeAttachment(_ id: CarinaPendingAttachment.ID) {
    ...
    private func applyHistoryPayload(
        _ payload: CarinaChatHistoryPayload,
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [CarinaChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: CarinaChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: CarinaChatMessage) -> CarinaChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> CarinaChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return CarinaChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageContentFingerprint(for message: CarinaChatMessage) -> String {
    ...
    private static func messageIdentityKey(for message: CarinaChatMessage) -> String? {
    ...
    private static func userRefreshIdentityKey(for message: CarinaChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private func adoptPendingLocalUserEcho(incoming: CarinaChatMessage) -> Bool {
        ...
        updated[matchIndex] = CarinaChatMessage(
            id: existing.id,
            role: incoming.role,
    ...
    private static func reconcileMessageIDs(
        previous: [CarinaChatMessage],
        incoming: [CarinaChatMessage]) -> [CarinaChatMessage]
    {
    ...
    private static func reconcileRunRefreshMessages(
        previous: [CarinaChatMessage],
        incoming: [CarinaChatMessage],
        pendingLocalUserEchoIDs: Set<UUID>) -> [CarinaChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [CarinaChatMessage]) -> [CarinaChatMessage] {
        var result: [CarinaChatMessage] = []
    ...
    private static func dedupeKey(for message: CarinaChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [CarinaChatMessageContent] = [
            CarinaChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> CarinaChatAttachmentPayload in
            CarinaChatAttachmentPayload(
                type: att.type,
    ...
    private static func isUnsupportedCreateSessionError(_ error: Error) -> Bool {
        ...
        return nsError.domain == "CarinaChatTransport"
    ...
    private func resolvedThinkingLevelOptions(
        for currentSession: CarinaChatSessionEntry?) -> [CarinaChatThinkingLevelOption]
    {
    ...
    private static func sessionModelMatchesDefaults(
        _ session: CarinaChatSessionEntry,
        defaults: CarinaChatSessionsDefaults?) -> Bool
    {
    ...
    private static func normalizedThinkingLevelOptions(
        _ levels: [CarinaChatThinkingLevelOption]?) -> [CarinaChatThinkingLevelOption]?
    {
        ...
        return Self.dedupedThinkingOptions(
            levels.compactMap { level in
                ...
                return CarinaChatThinkingLevelOption(id: id, label: label.isEmpty ? id : label)
            })
    }

    private static func thinkingOptions(from labels: [String]?) -> [CarinaChatThinkingLevelOption]? {
        guard let labels else { return nil }
        return Self.dedupedThinkingOptions(
            labels.compactMap { label in
                guard let id = Self.normalizedThinkingLevel(label) else { return nil }
                let trimmed = label.trimmingCharacters(in: .whitespacesAndNewlines)
                return CarinaChatThinkingLevelOption(id: id, label: trimmed.isEmpty ? id : trimmed)
            })
    }

    private static func withCurrentThinkingOption(
        _ options: [CarinaChatThinkingLevelOption],
        current: String) -> [CarinaChatThinkingLevelOption]
    {
        guard !options.contains(where: { $0.id == current }) else { return options }
        return options + [CarinaChatThinkingLevelOption(id: current, label: current)]
    }

    private static func dedupedThinkingOptions(
        _ options: [CarinaChatThinkingLevelOption]) -> [CarinaChatThinkingLevelOption]
    {
        var result: [CarinaChatThinkingLevelOption] = []
        var seen = Set<String>()
    ...
    private func placeholderSession(key: String) -> CarinaChatSessionEntry {
    ...
    private func updateCurrentSessionThinkingLevel(_ thinkingLevel: String?, sessionKey: String) {
        ...
        self.sessions[index] = CarinaChatSessionEntry(
            key: current.key,
            kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: CarinaChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleSessionMessageEvent(_ payload: CarinaSessionMessageEventPayload) {
    ...
    private func handleChatEvent(_ chat: CarinaChatEventPayload) {
    ...
    private func appendFinalChatMessageIfPresent(_ chat: CarinaChatEventPayload) {
        guard chat.state == "final" else { return }
        guard let text = CarinaChatEventText.assistantText(from: chat) else { return }

        let decoded = chat.message.flatMap {
            try? ChatPayloadDecoding.decode($0, as: CarinaChatMessage.self)
        }
    ...
    private static func isAssistantMessage(_ message: CarinaChatMessage) -> Bool {
        message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() == "assistant"
    }

    private static func messageWithTimestampIfNeeded(_ message: CarinaChatMessage) -> CarinaChatMessage {
    ...
    private func handleAgentEvent(_ evt: CarinaAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = CarinaChatPendingToolCall(
    ...
    private func handleAgentLifecycleEvent(_ evt: CarinaAgentEventPayload, isPendingRun: Bool) {
    ...
    private static func agentLifecycleErrorMessage(_ evt: CarinaAgentEventPayload, aborted: Bool) -> String {
        if aborted {
            return "Run aborted"
        }
    ...
    private static func hasUnansweredLatestUser(in messages: [CarinaChatMessage]) -> Bool {
        self.latestUserTurn(in: messages) != nil && !self.hasAssistantMessageAfterLatestUser(in: messages)
    }

    private static func latestUserTurn(in messages: [CarinaChatMessage]) -> LatestUserTurn? {
    ...
    private static func hasAnsweredUser(
        ...
        in messages: [CarinaChatMessage])
        -> Bool
    {
        guard let refreshKey = user.refreshKey else { return false }
        var occurrence = 0
        var latestMatchingUserIndex: [CarinaChatMessage].Index?
    ...
    private static func hasAssistantMessageAfterLatestUser(in messages: [CarinaChatMessage]) -> Bool {
        guard let lastUserIndex = messages.lastIndex(where: { $0.role.lowercased() == "user" }) else {
            return false
        }
