...
struct WebChatSwiftUISmokeTests {
    private struct TestTransport: CarinaChatTransport {
        func requestHistory(sessionKey: String) async throws -> CarinaChatHistoryPayload {
            let json = """
            {"sessionKey":"\(sessionKey)","sessionId":null,"messages":[],"thinkingLevel":"off"}
            """
            return try JSONDecoder().decode(CarinaChatHistoryPayload.self, from: Data(json.utf8))
        }
            ...
            attachments _: [CarinaChatAttachmentPayload]) async throws -> CarinaChatSendResponse
        {
            ...
            """
            return try JSONDecoder().decode(CarinaChatSendResponse.self, from: Data(json.utf8))
        }
