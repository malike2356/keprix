...
struct CarinaConfigFileTests {
    ...
    func `config path respects env override`() async {
        ...
        await TestIsolation.withEnvValues(["OPENCLAW_CONFIG_PATH": override]) {
            #expect(CarinaConfigFile.url().path == override)
        }
    }
    ...
    func `remote gateway port parses and matches host`() async {
        ...
        await TestIsolation.withEnvValues(["OPENCLAW_CONFIG_PATH": override]) {
            ...
            #expect(CarinaConfigFile.remoteGatewayPort() == 19999)
            #expect(CarinaConfigFile.remoteGatewayPort(matchingHost: "gateway.ts.net") == 19999)
            #expect(CarinaConfigFile.remoteGatewayPort(matchingHost: "GATEWAY.ts.net.") == 19999)
            #expect(CarinaConfigFile.remoteGatewayPort(matchingHost: "gateway") == nil)
            #expect(CarinaConfigFile.remoteGatewayPort(matchingHost: "other.ts.net") == nil)
            #expect(CarinaConfigFile.remoteGatewayPort(matchingHost: "gateway.attacker.tld") == nil)
        }
    }
    ...
    func `set remote gateway url string replaces scheme`() async {
        ...
        await TestIsolation.withEnvValues(["OPENCLAW_CONFIG_PATH": override]) {
            ...
            CarinaConfigFile.setRemoteGatewayUrlString("ws://127.0.0.1:18789")
            let root = CarinaConfigFile.loadDict()
    ...
    func `set remote gateway url preserves scheme`() async {
        ...
        await TestIsolation.withEnvValues(["OPENCLAW_CONFIG_PATH": override]) {
            ...
            CarinaConfigFile.setRemoteGatewayUrl(host: "new-host", port: 2222)
            let root = CarinaConfigFile.loadDict()
            let url = ((root["gateway"] as? [String: Any])?["remote"] as? [String: Any])?["url"] as? String
    ...
    func `clear remote gateway url removes only url field`() async {
        ...
        await TestIsolation.withEnvValues(["OPENCLAW_CONFIG_PATH": override]) {
            ...
            CarinaConfigFile.clearRemoteGatewayUrl()
            let root = CarinaConfigFile.loadDict()
    ...
    func `state dir override sets config path`() async {
        ...
        await TestIsolation.withEnvValues([
            ...
            #expect(CarinaConfigFile.stateDirURL().path == dir)
            #expect(CarinaConfigFile.url().path == "\(dir)/openclaw.json")
        }
    }
    ...
    func `save dict preserves gateway auth unless explicitly allowed`() async throws {
        ...
        await TestIsolation.withEnvValues([
            ...
            let allowedRoot = CarinaConfigFile.loadDict()
    ...
    func `load dict audits suspicious out-of-band clobbers`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            try CarinaConfigFile.withTestingFileLock {
                CarinaConfigFile.saveDict([
                    "update": ["channel": "beta"],
                ...
                let loaded = CarinaConfigFile.loadDict()
    ...
    func `save dict records preserved gateway auth in audit`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            let saved = CarinaConfigFile.saveDict([
                "gateway": [
                    "mode": "local",
                ],
                "browser": [
