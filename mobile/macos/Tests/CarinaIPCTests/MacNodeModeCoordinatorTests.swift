...
struct MacNodeModeCoordinatorTests {
    @Test func `remote mode does not advertise browser proxy`() {
        ...
        #expect(!caps.contains(CarinaCapability.browser.rawValue))
        #expect(!commands.contains(CarinaBrowserCommand.proxy.rawValue))
        #expect(commands.contains(CarinaCanvasCommand.present.rawValue))
