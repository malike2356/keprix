...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: CarinaProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: CarinaProtocol.AnyCodable] = [
            "presence": .init([CarinaProtocol.AnyCodable(entry)]),
        ]
        let payload = CarinaProtocol.AnyCodable(payloadMap)
