...
final class AppState {
    private static let logger = Logger(subsystem: "com.verlox.carinakeprix", category: "app-state")
    ...
    init(preview: Bool = false) {
        ...
        let configRoot = CarinaConfigFile.loadDict()
    ...
    private static func sshTunnelGatewayUrl(existingUrl: String?, expectedRemoteHost: String?) -> String {
        ...
        let preservePort: Bool = if LoopbackHost.isLoopbackHost(host) {
            ...
            CarinaConfigFile.canonicalHostForComparison(host) ==
                CarinaConfigFile.canonicalHostForComparison(expectedRemoteHost)
        } else {
            false
        }
    ...
    private func startConfigWatcher() {
        let configUrl = CarinaConfigFile.url()
    ...
    func syncGatewayConfigNow() {
        ...
        let synced = Self.syncedGatewayRoot(
            currentRoot: CarinaConfigFile.loadDict(),
            draft: .init(
        ...
        guard CarinaConfigFile.saveDict(synced.root) else {
