...
enum CommandResolver {
    ...
    static func projectCarinaExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func hasAnyCarinaInvoker(searchPaths: [String]? = nil) -> Bool {
        ...
        projectRoot: URL? = nil) -> [String]
    {
        ...
        let root = projectRoot ?? self.projectRoot()
        if let openclawPath = self.projectCarinaExecutable(projectRoot: root) {
    ...
    {
        let root = configRoot ?? CarinaConfigFile.loadDict()
