...
enum ExecApprovalsStore {
    private static let logger = Logger(subsystem: "com.verlox.carinakeprix", category: "exec-approvals")
    ...
    static func fileURL() -> URL {
        CarinaPaths.stateDirURL.appendingPathComponent("exec-approvals.json")
    }

    static func socketPath() -> String {
        CarinaPaths.stateDirURL.appendingPathComponent("exec-approvals.sock").path
    }

    private static func legacyStateDirURLs() -> [URL] {
        if let home = CarinaEnv.path("OPENCLAW_HOME") {
            var urls = [
    ...
    private static func legacyFileURLIfPending() -> URL? {
        guard CarinaEnv.path("OPENCLAW_STATE_DIR") != nil else { return nil }
    ...
    private static func ensureSecureStateDirectory() {
        let url = CarinaPaths.stateDirURL
        do {
