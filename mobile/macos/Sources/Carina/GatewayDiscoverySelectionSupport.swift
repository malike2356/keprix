...
enum GatewayDiscoverySelectionSupport {
        ...
        state: AppState)
    {
        ...
        if preferredTransport == .direct {
            CarinaConfigFile.setRemoteGatewayTransport(AppState.RemoteTransport.direct.rawValue)
            if !state.remoteUrl.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                CarinaConfigFile.setRemoteGatewayUrlString(state.remoteUrl)
            } else {
                CarinaConfigFile.clearRemoteGatewayUrl()
            }
        } else {
            CarinaConfigFile.setRemoteGatewayTransport(AppState.RemoteTransport.ssh.rawValue)
            CarinaConfigFile.setRemoteGatewayUrlString(state.remoteUrl)
        }
    }
