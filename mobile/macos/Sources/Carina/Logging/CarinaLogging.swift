...
enum CarinaLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = CarinaOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = CarinaFileLogHandler(label: label)
    ...
    static func parseLabel(_ label: String) -> (String, String) {
        guard let range = label.range(of: labelSeparator) else {
            return ("com.verlox.carinakeprix", label)
        }
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        CarinaLogging.bootstrapIfNeeded()
        let label = CarinaLogging.makeLabel(subsystem: subsystem, category: category)
...
struct CarinaOSLogHandler: AppLogLevelBackedHandler {
...
struct CarinaFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = CarinaLogging.parseLabel(self.label)
