...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: CarinaProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "com.verlox.carinakeprix", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: CarinaKit.AnyCodable]()) {
                $0[$1.key] = CarinaKit.AnyCodable($1.value.base)
            }
        ...
        if let urlError = error as? URLError {
            ...
            case .notConnectedToInternet:
                if Self.isLikelyLocalNetworkPermissionBlock() {
                    return """
                    macOS is blocking Carina Local Network access.
                    Allow Carina in System Settings → Privacy & Security → Local Network, then relaunch the app.
                    """
                }
    ...
    private static func bridgeToProtocolArgs(
        _ value: CarinaProtocol.AnyCodable?) -> [String: CarinaProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: CarinaProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: CarinaKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: CarinaProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
