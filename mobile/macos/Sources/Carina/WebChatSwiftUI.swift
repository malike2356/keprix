...
private let webChatSwiftLogger = Logger(subsystem: "com.verlox.carinakeprix", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: CarinaChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> CarinaChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(CarinaChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            CarinaChatSessionsDefaults(
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> CarinaChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                CarinaGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "session.message":
                ...
                guard let message = try? JSONDecoder().decode(
                    CarinaSessionMessageEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
    ...
    private static func mapModelChoice(_ model: CarinaProtocol.ModelChoice) -> CarinaChatModelChoice {
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<CarinaChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any CarinaChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = CarinaChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: CarinaChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<CarinaChatView>) -> NSViewController
    {
