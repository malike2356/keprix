...
actor GatewayEndpointStore {
    ...
    private static let staticLogger = Logger(subsystem: "com.verlox.carinakeprix", category: "gateway-endpoint")
    ...
    private let logger = Logger(subsystem: "com.verlox.carinakeprix", category: "gateway-endpoint")
    ...
    init(deps: Deps = .live) {
        ...
        let bind = GatewayEndpointStore.resolveGatewayBindMode(
            ...
            env: ProcessInfo.processInfo.environment)
        let customBindHost = GatewayEndpointStore.resolveGatewayCustomBindHost(root: CarinaConfigFile.loadDict())
