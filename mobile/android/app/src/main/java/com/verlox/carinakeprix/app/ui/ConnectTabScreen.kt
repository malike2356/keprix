    ...
    if (showDiagnostics) {
      Surface(
        ...
        Column(
          ...
          Text("Carina Android ${openClawAndroidVersionLabel()}", style = mobileCaption1, color = mobileTextSecondary)
          Button(
            onClick = {
...
private fun gatewayPairingInstruction(problem: GatewayConnectionProblem?): String =
  if (problem?.canAutoRetry == true) {
    "Approve this phone on the gateway. Carina will reconnect automatically."
  } else {
