...
import com.verlox.carinakeprix.app.GatewayConnectionProblem
import com.verlox.carinakeprix.app.LocationMode
import com.verlox.carinakeprix.app.MainViewModel
import com.verlox.carinakeprix.app.R
import com.verlox.carinakeprix.app.SensitiveFeatureConfig
...
import com.verlox.carinakeprix.app.ui.design.ClawErrorState
import com.verlox.carinakeprix.app.ui.design.ClawListItem
...
fun OnboardingFlow(
  ...
  ClawDesignTheme(dark = onboardingDark) {
    ...
    CarinaSystemBarAppearance(lightAppearance = !onboardingDark && step != OnboardingStep.Welcome)
...
private fun WelcomeLogo() {
  Surface(
    ...
    Box(modifier = Modifier.fillMaxSize().padding(12.dp), contentAlignment = Alignment.Center) {
      Image(painter = painterResource(id = R.drawable.openclaw_logo), contentDescription = "Carina logo", modifier = Modifier.fillMaxSize())
    }
  }
}
      ...
      LazyColumn(
        ...
        item {
          Column(modifier = Modifier.padding(top = 10.dp, bottom = 18.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
            ...
            Text(
              text = "These permissions keep Carina secure\nand useful.",
              style = ClawTheme.type.body,
              color = ClawTheme.colors.textMuted,
            )
          }
...
private fun PermissionTopBar(onBack: () -> Unit) {
  ...
  if (showHelp) {
    AlertDialog(
      ...
      text = {
        Text(
          "Choose what this phone can share with Carina. You can change these later in Settings.",
          style = ClawTheme.type.body,
          color = ClawTheme.colors.textMuted,
        )
      },
...
internal enum class GatewayRecoveryUiState(
  ...
  Pairing(
    title = "Pairing Gateway",
    message = "Approval is in progress.\nCarina will reconnect automatically.",
  ),
  Finishing(
    title = "Connecting Gateway",
    message = "Carina is checking gateway and node access.",
  ),
  Failed(
...
private fun recoveryGatewayDetail(
  ...
  remoteAddress
    ...
    ?: if (ready) {
      ...
      "Gateway approval is in progress. Carina will retry automatically."
    } else {
      "Gateway unreachable"
    }
...
private fun copyApprovalCommand(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("Carina pairing approval command", command))
...
private fun copyGatewayDiagnostic(
  ...
  val diagnostic =
    listOfNotNull(
      "Carina Android gateway diagnostic",
      "Status: $statusText",
  ...
  val clipboard = context.getSystemService(Context.CLIPBOARD_SERVICE) as ClipboardManager
  clipboard.setPrimaryClip(ClipData.newPlainText("Carina gateway diagnostic", diagnostic))
